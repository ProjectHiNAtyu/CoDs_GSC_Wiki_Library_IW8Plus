...
init()
{
    ...
    registerscriptperk( "specialty_reduce_regen_delay_on_kill" );
...
initperktable()
{
    ...
    for (;;)
    {
        ...
        var_5.ref = var_2;
        var_5.id = int( var_1 );
        var_5.specialist = int( var_3 );

        if ( var_4 != "" )
            game["dialog"][var_2] = var_4;

        if ( !isdefined( level.perktable[var_2] ) )
            level.perktable[var_2] = var_5;

        level.perksbyid[var_5.id] = var_5.ref;
        var_0++;
    }
}
...
onspecialistbonusavailable( var_0 )
{
    foreach ( var_2 in level.perktable )
    {
        if ( istrue( var_2.specialist ) && !perkisrestricted( var_2.ref ) )
            scripts\mp\utility\perk::giveperk( var_2.ref );
    }
...
giveperksafterspawn()
{
    ...
    if ( self.avoidkillstreakonspawntimer > 0 )
    {
        ...
        if ( istrue( self.inspawncamera ) )
            var_0 = var_0 + scripts\mp\spawncamera::getspawncamerablendtime();
