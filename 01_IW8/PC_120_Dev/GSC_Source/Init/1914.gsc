...
getspawnpoint()
{
    ...
    if ( isdefined( self.selectedspawnarea ) )
    {
        if ( issubstr( self.selectedspawnarea, "squad" ) )
        {
            ...
            if ( isdefined( var_3.vehicle ) )
            {
                ...
                self.vehiclespawninginto = var_3.vehicle.vehiclename;
            }
    ...
    if ( istrue( level.usesquadspawn ) && istrue( self.squadspawnconfirmed ) )
    {
        ...
        if ( isdefined( var_14 ) && isdefined( self.squadindex ) && self.team == var_14.team && self.squadindex == var_14.squadindex )
        {
            ...
            var_14 thread scripts\mp\utility\points::givexpwithtext( "squad_spawn" );
            thread scripts\mp\utility\points::givexpwithtext( "squad_spawn_self" );
            self.wassquadspawned = 1;
        }
    }
    ...
    for (;;)
    {
        ...
        foreach ( var_3 in level.availablespawnlocations[self.team] )
        {
            ...
            else if ( issubstr( var_3, "vehicle" ) )
            {
                ...
                {
                    var_8 = istrue( var_11.indanger );
                    var_9 = hassquadmatepassengers( var_11, self );
...
refreshsquadspawnareaomnvars( var_0, var_1 )
{
...
{
    return level.objectives[var_0].ownerteam != var_1 || level.objectives[var_0].stalemate || level.objectives[var_0].captureblocked || istrue( level.objectives[var_0].spawnpointdanger ) || level.objectives[var_0].claimteam != "none" && level.objectives[var_0].claimteam != var_1;
}

isvehicleindanger( var_0 )
{
    var_1 = gettime();
...
vehicleindangertracking( var_0 )
{
    ...
    {
        var_1 = gettime();
        var_2 = istrue( var_0.burningdown );
        var_3 = scripts\cp_mp\utility\weapon_utility::islockedonto( var_0 );
        var_4 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_canspawninto( var_0 );
        var_5 = isdefined( var_0 ) && ( isdefined( var_0.lasttimedamaged ) && var_0.lasttimedamaged + 5000 > var_1 );
        var_6 = isdefined( var_0 ) && var_0 scripts\mp\outofbounds::istouchingoobtrigger();
        var_7 = 0;

        if ( isdefined( var_0.team ) )
        {
            var_8 = scripts\common\utility::playersnear( var_0.origin, 384 );

            foreach ( var_10 in var_8 )
            {
                if ( isdefined( var_10 ) && var_10.team != var_0.team && isalive( var_10 ) && !isdefined( var_10.fauxdead ) )
                {
                    if ( abs( var_0.origin[2] - var_10.origin[2] ) < 100 )
                    {
                        var_7 = 1;
                        break;
                    }
        ...
        var_12 = var_0.indanger;

        if ( var_5 || var_6 || var_7 || var_2 || var_3 || !var_4 )
            var_0.indanger = 1;
        else
            var_0.indanger = 0;
...
hassquadmatepassengers( var_0, var_1 )
{
...
spawnselectionthink( var_0 )
{
    ...
    if ( istrue( level.showteamtanks ) )
        thread scripts\mp\gametypes\arm::vehicle_showteamtanks();

    if ( istrue( level.showteamlittlebirds ) )
        thread scripts\mp\gametypes\arm::vehicle_showvalidlittlebirds();
...
selectionmade()
{
    ...
    for (;;)
    {
        ...
        {
            var_1 = updateplayerspawninputtype( var_1 );
            var_2 = undefined;
            ...
            foreach ( var_4 in level.spawnselectionlocations )
            {
                ...
                {
                    updatespecificfobindanger( var_4[self.team].typeref, self, self.team );

                    if ( isdefined( var_4[self.team].objectivekey ) && isobjectiveindanger( var_4[self.team].objectivekey, self.team ) )
                        break;

                    if ( isdefined( var_4[self.team].typeref ) && issubstr( var_4[self.team].typeref, "vehicle" ) && istrue( var_4[self.team].dynamicent.indanger ) )
                        break;
            ...
            if ( !isdefined( var_2 ) )
            {
                var_6 = getsquadspawnlocations( self.team, self.squadindex );
                ...
                foreach ( var_4 in var_6 )
                {
                    if ( isdefined( var_4.dynamicent ) && var_4.dynamicent getentitynumber() == var_1 )
                    {
                        ...
                        var_9 = spawnstruct();
                        var_9 = scripts\mp\spawnscoring::capsulepass( var_9, var_4.dynamicent );

                        if ( !isdefined( var_9.spawnposition ) )
                        {
                            var_4.dynamicent.nogroundfoundtime = gettime();
                            break;
                        }
...
updateplayerspawninputtype( var_0 )
{
    ...
    var_3 = 262144;
    var_4 = 524288;
    self.gwinputtypesused["rightmouseup"] = istrue( self.gwinputtypesused["rightmouseup"] ) || ( var_0 & var_1 ) > 0;
    self.gwinputtypesused["leftmouseup"] = istrue( self.gwinputtypesused["leftmouseup"] ) || ( var_0 & var_2 ) > 0;
    self.gwinputtypesused["activate"] = istrue( self.gwinputtypesused["activate"] ) || ( var_0 & var_3 ) > 0;
    self.gwinputtypesused["gostand"] = istrue( self.gwinputtypesused["gostand"] ) || ( var_0 & var_4 ) > 0;
...
watchendconditions()
{
    ...
    var_0 = scripts\engine\utility::waittill_any_return_no_endon_death_6( "tac_ops_map_selection_valid", "tac_ops_map_changed", "tac_ops_map_cleared", "spawned_player", "tac_ops_map_game_ended", "tac_ops_map_nuke" );
...
endcondition_focuschanged( var_0 )
{
    ...
    foreach ( var_3 in level.spawnselectionlocations )
    {
        ...
        {
            thread keepstreamposfresh( var_3[self.team].anchorentity.origin );
    ...
    if ( !isdefined( var_1 ) )
    {
        var_5 = getsquadspawnlocations( self.team, self.squadindex );

        foreach ( var_3 in var_5 )
        {
            if ( isdefined( var_3.dynamicent ) && var_3.dynamicent getentitynumber() == var_0 )
            {
                thread keepstreamposfresh( var_3.dynamicent.origin );
                var_1 = var_3.typeref;
                break;
            }
        }
    ...
    if ( issubstr( var_1, "vehicle" ) )
    {
        ...
        thread managevehiclehealthui( var_1 );
        var_8 = tablelookuprownum( "mp/spawnSelectionMapData.csv", 0, var_1 );
        self setclientomnvar( "ui_tom_previous_selection", var_8 );
        return;
    }
...
keepstreamposfresh( var_0 )
{
    ...
    if ( !isdefined( level.worldstreamingkeepalive ) )
        level.worldstreamingkeepalive = max( getdvarint( "NOLPSOTORP", 20000 ) * 0.5 / 1000, 1.0 );
...
getsquadspawnlocations( var_0, var_1 )
{
    var_2 = [];
...
evaluatesquadspawn()
{
    ...
    for (;;)
    {
        ...
        var_11 = getsquadspawnlocations( self.team, self.squadindex );
        ...
        for ( var_4 = 0; var_4 < 4; var_4++ )
        {
            ...
            if ( isdefined( var_9 ) && self != var_9 && issquadspawnable( var_9 ) && !istrue( var_14 ) )
            {
                ...
                if ( isdefined( var_9.vehicle ) )
                {
                    ...
                    if ( var_9.vehicle.vehiclename == "little_bird" )
                        var_16 = var_16 + 4096;
                }
                ...
                if ( !isdefined( var_3[var_4].idmask ) || var_3[var_4].idmask != var_16 )
                {
                    var_3[var_4].idmask = var_16;
            ...
            if ( isdefined( var_3[var_4].idmask ) && var_3[var_4].idmask != -1 )
            {
                var_3[var_4].idmask = -1;
...
handlesquadspawncycle()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::waittill_any_return_2( "cyclePos", "cycleNeg" );
        ...
        if ( isdefined( var_0 ) )
        {
            ...
            if ( var_0 == "cyclePos" )
                thread cyclevalidsquadspectate( var_1.squadmemberindex, 1 );
            else if ( var_0 == "cycleNeg" )
                thread cyclevalidsquadspectate( var_1.squadmemberindex, 0 );
        }
    }
}
...
issquadmateindanger( var_0 )
{
    ...
    if ( isdefined( var_0.lastweaponfiretimestart ) && var_0.lastweaponfiretimestart + 3000 > var_1 )
        return 1;

    if ( isdefined( var_0.lastweaponfiretimestart ) && isdefined( var_0.lastweaponfiretimeend ) && var_0.lastweaponfiretimestart > var_0.lastweaponfiretimeend || isdefined( var_0.lastweaponfiretimestart ) && !isdefined( var_0.lastweaponfiretimeend ) )
        return 1;
    ...
    {
        if ( istrue( var_0.vehicle.burningdown ) )
            return 1;
    ...
    if ( isdefined( var_0 ) && isdefined( var_0.nogroundfoundtime ) && var_0.nogroundfoundtime + 1000 > var_1 )
        return 1;
...
timeuntilspawnmessaging( var_0, var_1 )
{
    ...
    if ( var_0 > 0 )
    {
        ...
        scripts\engine\utility::waittill_any_timeout_no_endon_death_1( var_0, "force_spawn" );
    }
}
...
getclosestavailablespawnlocation()
{
    ...
    else
    {
        ...
        {
            var_7 = getsquadspawnlocations( self.team, self.squadindex );
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            foreach ( var_3 in level.availablespawnlocations[var_1] )
            {
                ...
                if ( isdefined( var_4.objectivekey ) )
                    var_4.indanger = isobjectiveindanger( var_4.objectivekey, var_1 );
            }
        }
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            ...
            foreach ( var_4 in level.availablespawnlocations[var_1] )
            {
                if ( !issubstr( var_4, "HQ" ) && issubstr( var_4, "fob" ) )
                {
                    ...
                    level.objectives[var_17].spawnpointdanger = var_14;
                    level.objectives[var_17].spawnpointdangertime = gettime();
                }
            }
        }
...
updatespecificfobindanger( var_0, var_1, var_2 )
{
    var_3 = level.spawnselectionlocations[var_0][var_2].objectivekey;

    if ( !isdefined( var_3 ) || !isdefined( level.objectives[var_3] ) || !isdefined( level.objectives[var_3].spawnpointdangertime ) || level.objectives[var_3].spawnpointdangertime == gettime() )
        return;
    ...
    level.objectives[var_3].spawnpointdanger = var_13;
    level.objectives[var_3].spawnpointdangertime = gettime();
}
