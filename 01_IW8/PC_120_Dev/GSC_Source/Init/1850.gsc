...
onforfeit( var_0 )
{
    ...
    {
        logstring( "forfeit, win: " + var_3 );

        foreach ( var_5 in level.teamnamelist )
            logstring( var_5 + ":" + game["teamScores"][var_5] );
    }
...
default_ondeadevent( var_0 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        foreach ( var_2 in scripts\mp\utility\teams::getteamdata( var_0, "players" ) )
            var_2 scripts\cp_mp\utility\game_utility::stopkeyearning();
    }
    ...
    {
        var_7 = scripts\mp\utility\teams::getenemyteams( var_0 );
        var_8 = [];
        ...
        if ( !istrue( level.disablespawning ) )
        {
            foreach ( var_5 in var_7 )
            {
                foreach ( var_2 in scripts\mp\utility\teams::getteamdata( var_5, "players" ) )
                {
        ...
        foreach ( var_5 in var_7 )
        {
            if ( scripts\mp\utility\teams::getteamdata( var_5, "aliveCount" ) || var_8[var_5] )
                var_15[var_15.size] = var_5;
        }

        if ( var_15.size == 1 )
            thread endgame( var_15[0], game["end_reason"]["enemies_eliminated"] );
    }
}
...
default_ontimelimit()
{
    ...
    if ( level.teambased )
    {
        ...
        logstring( "time limit, win: " + var_0 );

        foreach ( var_3 in level.teamnamelist )
            logstring( var_3 + ":" + game["teamScores"][var_3] );
    }
    else
    {
...
forceend( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        logstring( "host ended game, win: " + var_4 );

        foreach ( var_2 in level.teamnamelist )
            logstring( var_2 + ":" + game["teamScores"][var_2] );
    }
    else
    {
        ...
        if ( isdefined( var_4 ) )
            logstring( "host ended game, win: " + var_4.name );
        else
...
onscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        var_2 = scripts\mp\gamescore::calculatewinningteam();

        if ( var_2 == "none" )
            var_2 = "tie";

        logstring( "scorelimit, win: " + var_2 );

        foreach ( var_4 in level.teamnamelist )
            logstring( var_4 + ":" + game["teamScores"][var_4] );
    }
    else
    {
...
waitforplayers( var_0 )
{
    ...
    for (;;)
    {
        ...
        var_6 = getstreamedinplayercount();
        ...
        if ( var_9 )
        {
            ...
            {
                if ( !checkrequiredteamcount( var_11 ) )
                {
                    var_8 = 0;
                    break;
                }
        ...
        foreach ( var_11 in level.teamnamelist )
        {
            if ( !checkrequiredteamstreamcount( var_11 ) )
            {
                var_9 = 0;
                break;
            }
        ...
        var_15 = var_1 + level.prematchperiod * 1000 - 200;
...
updatematchbonusscores( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        foreach ( var_3 in level.players )
        {
            ...
            var_3 calculateweaponmatchbonus( var_1 );
        }
    }
    else
    {
...
calculateweaponmatchbonus( var_0 )
{
    ...
    var_1 = scripts\mp\weaponrank::getgametypekillsperhouravg() / 60;
    var_2 = var_0 / 60;
...
setxenonranks( var_0 )
{
    ...
    {
        var_3 = var_1[var_2];
        _setplayerteamrank( var_3 );
    }
}

_setplayerteamrank( var_0, var_1 )
{
    if ( !isdefined( var_0 ) )
        return;
    ...
    else if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::getsubgametype() == "" )
    {
        var_3 = 1000 - var_1;
...
checktimelimit( var_0 )
{
    ...
    if ( var_1 > 0 && !isdefined( level.forceotlogictorun ) )
    {
        if ( var_2 )
        {
...
checkteamscorelimitsoon( var_0 )
{
    ...
    if ( !level.playedmatchendingsoon && scripts\mp\utility\game::canplayhalfwayvo() )
    {
        if ( getteamscore( var_0 ) >= int( level.scorelimit * level.currentround - level.scorelimit / 2 ) )
        {
            ...
            thread scripts\mp\music_and_dialog::midpoint_music( var_0 );
...
matchstarttimerperplayer_internal( var_0 )
{
    waittillframeend;
    ...
    {
        var_4 = int( var_0 );
        childthread teamstarttimer( var_3, var_4 );
    ...
    childthread clearvisionsettimer( var_1 );
    wait( var_1 );
}

teamstarttimer( var_0, var_1 )
{
    level endon( "match_start_timer_beginning" );
    ...
    if ( !level.gameended )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            thread scripts\mp\gametypes\br_public::prematchmusic( var_0 );
            thread scripts\mp\gametypes\br_public::prematchdialog( var_0 );
            thread scripts\mp\gametypes\br_public::applyprematchplotarmor();
        }
        ...
        while ( var_1 > 0 && !level.gameended )
        {
            ...
            {
                if ( var_1 < 4 )
                    var_2 playsound( "match_start_tick_in3" );
                else
                    var_2 playsound( "match_start_tick_in5" );
            }
        ...
        foreach ( var_5 in var_3 )
        {
            scripts\mp\playerlogic::clearprematchlook( var_5 );
...
checkroundswitch( var_0 )
{
    ...
    if ( game["roundsPlayed"] % level.roundswitch == 0 || var_0 )
        return 1;
...
callback_startgametype()
{
    ...
    level.cdlgametuning = getdvarint( "scr_game_cdltuning", 0 );
    ...
    level.matchrules_vampirism = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_vampirism", "scr_game_vampirism" );

    if ( level.matchrules_vampirism )
        level.modifyplayerdamage = scripts\mp\damage::gamemodemodifyplayerdamage;

    level.finalkillcamtype = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_killcamType", "scr_game_killcamType" );
    level.allowkillstreaks = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_allowKillstreaks", "scr_game_allowKillstreaks" );
    level.roundretainstreaks = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreaks", "scr_game_roundRetainStreaks" );
    level.roundretainstreakprog = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreakProg", "scr_game_roundRetainStreakProg" );
    level.deathretainstreaks = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_deathRetainStreaks", "scr_game_deathRetainStreaks" );
    level.allowperks = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_allowPerks", "scr_game_allowPerks" );
    level.allowsupers = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_allowSupers", "scr_game_allowSupers" );
    level.superfastchargerate = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_superFastChargeRate", "scr_game_superFastChargeRate" );
    level.superpointsmod = scripts\mp\utility\dvars::getoverridedvarfloatexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_superPointsMod", "scr_game_superPointsMod" );
    level.spawnprotectiontimer = scripts\mp\utility\dvars::getoverridedvarfloatexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_spawnProtectionTimer", "scr_game_spawnProtectionTimer" );
    level.lethaldelay = scripts\mp\utility\dvars::getoverridedvarfloatexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_lethalDelay", "scr_game_lethalDelay" );
    level.equipmentmatchstartshieldms = scripts\mp\utility\dvars::getoverridedvarfloatexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_equipmentMSProtect", "scr_game_equipmentMSProtect" ) * 1000;
    level.magcount = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_magcount", "scr_game_magcount" );

    if ( !isdefined( level.practiceround ) )
        level.practiceround = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_practiceRound", "scr_game_practiceRound" );

    level.postgameexfil = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_postGameExfil", "scr_game_postGameExfil" );
    level.exfilactivetimer = scripts\mp\utility\dvars::getoverridedvarfloatexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilActiveTimer", "scr_game_exfilActiveTimer" );
    level.exfilextracttimer = scripts\mp\utility\dvars::getoverridedvarfloatexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilExtractTimer", "scr_game_exfilExtractTimer" );
    level.useammorestocklocs = scripts\mp\utility\dvars::getoverridedvarfloatexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_useAmmoRestockLocs", "scr_game_useAmmoRestockLocs" );
    level.spectate3rdallowed = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_allow3rdspectate", "scr_game_allow3rdspectate" );
    setdvar( "LKNNQKNTS", level.spectate3rdallowed );
    ...
    level.radarhideshots = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + var_0 + "_radarHidePings", "scr_game_radarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_minimap", level.radarhideshots );
    level.navbarhideshots = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + var_0 + "_navBarHidePings", "scr_game_navBarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_navbar", level.navbarhideshots );
    level.navbarhideenemies = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + var_0 + "_navBarHideEnemy", "scr_game_navBarHideEnemy" );
    ...
    level.disablemount = getdvarint( "scr_player_disableMount" );
    level.disablebattlechatter = getdvarint( "scr_game_disableBattleChatter" );
    level.disableannouncer = getdvarint( "scr_game_disableAnnouncer" );
    level.scoremod = [];
    ...
    if ( scripts\cp_mp\utility\game_utility::isarenamap() && scripts\mp\utility\game_utility_mp::moderestrictsarenakillstreaks() )
    {
    ...
    level.maxallowedteamkills = scripts\mp\utility\dvars::getoverridedvarintexceptmatchrulesvalues( "scr_" + scripts\mp\utility\game::getgametype() + "_ffPunishLimit", "scr_game_ffPunishLimit" );
    ...
    if ( !istrue( game["isLaunchChunk"] ) )
    {
        if ( istrue( level.postgameexfil ) || istrue( level.useexfilchoppers ) )
            thread scripts\mp\gametypes\exfil::exfilinit();
    }
    ...
    while ( game["state"] == "playing" )
    {
        if ( !level.timerstopped && scripts\mp\utility\game::gettimelimit() && !istrue( level.bombsplanted ) )
        {
            ...
            if ( var_6 <= 10 || var_6 <= 30 && var_6 % var_4 == var_7 )
            {
                ...
                if ( var_8 )
                {
                    var_9 = getclocksoundaliasfortimeleft( var_5 );
...
gametimer()
{
    ...
    if ( !isdefined( game["startTimeFromMatchStart"] ) )
    {
        ...
        scripts\common\utility::trycall( level.matchdata_onmatchstart );
    }
...
displayroundswitch()
{
    ...
    var_0 = getintermissionspawnpointoverride();
...
freezeallplayers( var_0, var_1, var_2, var_3 )
{
    ...
    foreach ( var_5 in level.players )
    {
        ...
        var_5 setclientdvars( "LQKPQMPRQN", 1, "cg_drawSpectatorMessages", 0 );

        if ( isdefined( var_1 ) && isdefined( var_2 ) )
        {
            if ( var_1 == "NSSLSNKPN" && var_5 issplitscreenplayer() )
...
endgame_regularmp( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( var_0 == "tie" && shouldmodescoreonties() )
            {
                foreach ( var_7 in level.teamnamelist )
    ...
    level.ctfnukeended = istrue( var_3 ) && scripts\mp\utility\game::getgametype() == "ctf";

    if ( level.ctfnukeended )
    {
    ...
    {
        if ( !isbot( var_14 ) && var_14.team != "spectator" && var_14.team != "follower" )
            logendofround( var_14 );
    }
...
postroundfadenokillcam()
{
    foreach ( var_1 in level.players )
    {
        ...
        var_1 thread predictandclearintermissionstreaming();
    }
...
setgameendflagsandnotifies( var_0 )
{
    game["state"] = "postgame";
...
endgame_endround( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() )
    {
        ...
        if ( istrue( level.postgameexfil ) && !level.forcedend )
        {
            if ( var_0 != "tie" )
            {
                level thread scripts\mp\gametypes\exfil::onexfilstart( var_0 );
    ...
    if ( level.doeomcombat )
    {
        if ( istrue( level.docmdoutro ) )
        {
            ...
            foreach ( var_5 in level.players )
            {
                if ( level.teambased )
                {
                    var_5 thread scripts\mp\hud_message::teamoutcomenotify( var_0, 0, var_1, var_2 );
                    continue;
                }

                var_5 thread scripts\mp\hud_message::outcomenotify( var_0, var_1, var_2 );
            }
        ...
        foreach ( var_5 in level.players )
        {
            var_5 setclientdvar( "MQNNLTKNTS", 1 );
            ...
            if ( scripts\mp\utility\game::wasonlyround() || scripts\mp\utility\game::waslastround() )
            {
                ...
                var_5 scripts\mp\bounty::playerresetbountystreak();
            }
    ...
    {
        if ( !( level.gametype == "tac_ops" && isdefined( level.tacopssublevel ) ) )
        {
            thread waitforhitmarkerspostgame();
            setgameendflagsandnotifies( var_0 );
        }
    ...
    foreach ( var_5 in level.players )
        var_5 scripts\mp\class::loadout_clearperks();
    ...
    if ( istrue( game["practiceRound"] ) && !istrue( level.forcedend ) )
    {
        ...
        if ( level.allowsupers && !scripts\mp\utility\game::runleanthreadmode() )
        {
            foreach ( var_5 in level.players )
            {
                var_5 scripts\mp\supers::clearsuper();
                var_5 scripts\mp\perks\perkpackage::perkpackage_reset();
            }
        }
    ...
    else
    {
        if ( level.teambased )
        {
            foreach ( var_19 in level.teamnamelist )
                scripts\mp\gamescore::updatetotalteamscore( var_19 );
        }
        ...
        if ( !scripts\mp\utility\game::wasonlyround() )
        {
            if ( !scripts\mp\utility\game::waslastround() )
            {
                ...
                scripts\mp\flags::levelflagclear( "block_notifies" );
                var_21 = checkroundswitch( level.playovertime );

                if ( var_21 )
                    onroundswitch( level.playovertime );

                if ( var_21 || game["finalRound"] == 1 || game["matchPoint"] == 1 )
                {
                    var_22 = 1;
                ...
                foreach ( var_5 in level.players )
                {
                    ...
                    if ( isalive( var_5 ) && isdefined( var_5.matchdatalifeindex ) )
                        var_5 scripts\common\utility::trycall( level.matchdata_logplayerdeath, var_5.matchdatalifeindex, undefined, undefined, "MOD_ROUND_ENDED", "none", undefined, undefined );
                }
...
endgame_endgame( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\gametypes\br_public::isbotpracticematch() && !level.forcedend )
    {
        level._waitforlui = 1;
    ...
    scripts\cp_mp\challenges::roundend( var_0 );
    ...
    {
        level notify( "spawning_intermission" );
        var_4 = getintermissionspawnpointoverride();

        foreach ( var_6 in level.players )
        {
            var_6 thread scripts\mp\utility\game::setuipostgamefade( 0.0 );

            if ( !isdefined( level.modespawnendofgame ) || !var_6 [[ level.modespawnendofgame ]]() )
                var_6 thread scripts\mp\playerlogic::spawnintermission( var_4 );
        }
    }
    ...
    if ( scripts\mp\utility\game::getgametype() != "br" || scripts\mp\utility\game::getsubgametype() == "dmz" || scripts\mp\utility\game::isprophuntgametype() )
        processlobbydata();
    ...
    if ( level.teambased )
    {
        if ( scripts\mp\utility\teams::isgameplayteam( var_0 ) )
        {
            ...
            var_8 = scripts\engine\utility::array_sort_with_func( scripts\mp\utility\teams::getteamdata( var_0, "players" ), ::compare_player_score );
        }
        else
        {
            setmatchdata( "victor", "none" );
            var_8 = level.placement["all"];
        }
    ...
    foreach ( var_6 in level.players )
    {
        var_6 setplayerdata( "common", "round", "endReasonTextIndex", var_1 );
        var_6 scripts\cp_mp\utility\game_utility::stopkeyearning( var_0 );

        if ( isalive( var_6 ) && isdefined( var_6.matchdatalifeindex ) )
            var_6 scripts\common\utility::trycall( level.matchdata_logplayerdeath, var_6.matchdatalifeindex, undefined, undefined, "MOD_GAME_ENDED", "none", undefined, undefined );

        var_10 = int( min( 3, var_8.size ) );
        var_11 = 0;
        ...
        if ( var_11 )
            var_6 scripts\cp_mp\pet_watch::addwatchchargewintop3();
        ...
        if ( level.teambased )
        {
            if ( scripts\mp\utility\teams::isgameplayteam( var_0 ) )
            {
                if ( isdefined( var_6.team ) )
                {
                    if ( var_6.team == var_0 )
                    {
                        var_6 logplayerendmatchdatamatchresult( var_6.clientid, "win" );
                        var_6 scripts\cp_mp\pet_watch::addwatchchargewin();
                    }
                    else
                        var_6 logplayerendmatchdatamatchresult( var_6.clientid, "loss" );
                }
                else
                    var_6 logplayerendmatchdatamatchresult( var_6.clientid, "none" );
            }
            else if ( scripts\mp\utility\game::allteamstied() )
                var_6 logplayerendmatchdatamatchresult( var_6.clientid, "draw" );
            else
                var_6 logplayerendmatchdatamatchresult( var_6.clientid, "none" );
        }
        else if ( isplayer( var_0 ) && var_0.clientid == var_6.clientid )
        {
            var_6 logplayerendmatchdatamatchresult( var_6.clientid, "win" );
            var_6 scripts\cp_mp\pet_watch::addwatchchargewin();
        }
        ...
        var_6 scripts\common\utility::trycall( level.matchdata_logplayerdata );
    }
    ...
    if ( getdvarint( "TLRPKRKMS" ) != 0 )
    {
        if ( isgamebattlematch() )
        {
            foreach ( var_6 in level.players )
            {
                var_15 = var_6 getxuid();
                var_16 = var_6.team;
                var_17 = var_6 scripts\mp\utility\stats::getpersstat( "score" );
                setgamebattleplayerstats( var_15, var_16, var_17 );
            }

            var_19 = level.mapname;
            var_20 = scripts\mp\utility\game::getgametype();
            var_21 = getmatchdata( "victor" );
            var_22 = getteamscore( "allies" );
            var_23 = getteamscore( "axis" );
            var_24 = scripts\mp\matchdata::getmatchstarttimeutc();
            var_25 = scripts\mp\matchdata::getmatchendtimeutc();
            setgamebattlematchstats( var_19, var_20, var_21, var_22, var_23, var_24, var_25 );
        }
    }
    ...
    if ( istrue( game["isLaunchChunk"] ) )
    {
        ...
        {
            game["roundsWon"][var_16] = 0;
            game["teamScores"][var_16] = 0;
            setteamscore( var_16, int( 0 ) );
        }

        if ( game["launchChunkRuleSet"] == 3 )
        {
            scripts\mp\gametypes\arena_alt::droplaunchchunkbots();
            wait 1;
        }
    ...
    if ( scripts\mp\gametypes\br_public::isbotpracticematch() && isdefined( level._luidecision ) && level._luidecision == 2 )
    {
...
sendendofmatchdata()
{
    ...
    {
        dlog_recordevent( "dlog_event_server_match_end", [ "utc_start_time_s", scripts\mp\matchdata::getmatchstarttimeutc(), "utc_end_time_s", scripts\mp\matchdata::getmatchendtimeutc() ] );
...
processlobbydata()
{
    foreach ( var_1 in level.players )
    {
        ...
        processlobbydataforclient( var_1 );
    }
...
trackattackerleaderboarddeathstats( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( var_0 ) && isplayer( var_0 ) )
    {
        if ( var_3 != "MOD_FALLING" )
        {
            ...
            {
                var_0 scripts\common\utility::trycall( level.matchdata_logattachmentstat, "tactical", "kills", 1, var_2 );
                var_0 scripts\common\utility::trycall( level.matchdata_logattachmentstat, "tactical", "hits", 1, var_2 );
            ...
            if ( var_3 == "MOD_MELEE" && !scripts\mp\riotshield::isriotshield( var_2 ) && !scripts\mp\utility\weapon::isknifeonly( var_2 ) && !scripts\mp\utility\weapon::isaxeweapon( var_2 ) )
            {
                var_0 scripts\common\utility::trycall( level.matchdata_logattachmentstat, "none", "kills", 1, var_2 );
                var_0 scripts\common\utility::trycall( level.matchdata_logattachmentstat, "none", "hits", 1, var_2 );
...
setweaponstat( var_0, var_1, var_2 )
{
    ...
    if ( scripts\mp\utility\weapon::ispickedupweapon( var_3 ) )
        return;
    ...
    if ( var_5 == "weapon_grenade" || var_5 == "weapon_explosive" || var_4 == "trophy_mp" || var_4 == "forcepush_mp" )
    {
        ...
        scripts\common\utility::trycall( level.matchdata_logweaponstat, var_4, var_2, var_1, var_6 );
        return;
    }
    ...
    if ( var_2 == "deaths" )
    {
        ...
        scripts\common\utility::trycall( level.matchdata_logweaponstat, var_9, "deaths", var_1, var_6 );

        foreach ( var_12 in var_10 )
        {
            scripts\mp\persistence::incrementattachmentstat( var_12, var_2, var_1, var_3 );
            scripts\common\utility::trycall( level.matchdata_logattachmentstat, var_12, var_2, var_1, var_3 );
        }
    }
}
...
updateleaderboardstatscontinuous()
{
    ...
    for (;;)
    {
        ...
        {
            var_1 updateplayerleaderboardstatsinternal();
            wait 0.1;
        }
...
updateplayerleaderboardstatsinternal()
{
    ...
    if ( isdefined( level.brgametype ) && isdefined( level.brgametype.name ) )
    {
        switch ( level.brgametype.name )
        {
            ...
            case "extract":
            case "dmz":
                var_0 = var_0 + ( "_" + level.brgametype.name );
                break;
            case "":
                break;
            default:
    ...
    var_1 = scripts\engine\utility::ter_op( level.teambased, self.score, self.leaderboarddata["gamemodeScore"] );
    ...
    incrementleaderboardstat( "kills", var_0, self.leaderboarddata["kills"] );
    incrementleaderboardstat( "deaths", var_0, self.leaderboarddata["deaths"] );

    switch ( scripts\mp\utility\game::getgametype() )
    {
        case "war":
            incrementleaderboardstat( "assists", var_0, self.leaderboarddata["assists"] );
            break;
        case "arena":
            incrementleaderboardstat( "damage", var_0, self.leaderboarddata["damage"] );
            incrementleaderboardstat( "assists", var_0, self.leaderboarddata["assists"] );
            break;
        case "arm":
            incrementleaderboardstat( "captures", var_0, self.leaderboarddata["captures"] );
            incrementleaderboardstat( "defends", var_0, self.leaderboarddata["defends"] );
            break;
        case "cyber":
            incrementleaderboardstat( "revives", var_0, self.leaderboarddata["rescues"] );
            incrementleaderboardstat( "plants", var_0, self.leaderboarddata["plants"] );
            break;
        case "dm":
            updateleaderboardstatmaximum( "streak", var_0, self.leaderboarddata["killChains"] );
            break;
        case "dom":
            incrementleaderboardstat( "captures", var_0, self.leaderboarddata["captures"] );
            incrementleaderboardstat( "defends", var_0, self.leaderboarddata["defends"] );
        ...
        case "sd":
            incrementleaderboardstat( "plants", var_0, self.leaderboarddata["plants"] );
            incrementleaderboardstat( "defuses", var_0, self.leaderboarddata["defuses"] );
            break;
        case "conf":
            incrementleaderboardstat( "confirms", var_0, self.leaderboarddata["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.leaderboarddata["denied"] );
            break;
        case "hq":
            incrementleaderboardstat( "captures", var_0, self.leaderboarddata["captures"] );
            incrementleaderboardstat( "defends", var_0, self.leaderboarddata["defends"] );
            break;
        case "koth":
            incrementleaderboardstat( "objTime", var_0, self.leaderboarddata["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.leaderboarddata["defends"] );
            break;
        case "ctf":
            incrementleaderboardstat( "captures", var_0, self.leaderboarddata["captures"] );
            incrementleaderboardstat( "returns", var_0, self.leaderboarddata["returns"] );
            break;
        case "sr":
            incrementleaderboardstat( "plants", var_0, self.leaderboarddata["plants"] );
            incrementleaderboardstat( "rescues", var_0, self.leaderboarddata["rescues"] );
            break;
        case "siege":
            incrementleaderboardstat( "captures", var_0, self.leaderboarddata["captures"] );
            incrementleaderboardstat( "revives", var_0, self.leaderboarddata["rescues"] );
            break;
        case "grind":
            incrementleaderboardstat( "banks", var_0, self.leaderboarddata["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.leaderboarddata["denied"] );
            break;
        case "infect":
            incrementleaderboardstat( "time", var_0, scripts\mp\utility\stats::getpersstat( "extrascore0" ) );
            incrementleaderboardstat( "infected", var_0, self.leaderboarddata["killsAsInfected"] );
            break;
        case "gun":
            incrementleaderboardstat( "stabs", var_0, self.leaderboarddata["stabs"] );
            incrementleaderboardstat( "setBacks", var_0, self.leaderboarddata["setbacks"] );
            break;
        case "grnd":
            incrementleaderboardstat( "objTime", var_0, self.leaderboarddata["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.leaderboarddata["defends"] );
        ...
        case "pill":
            ...
            break;
        case "br":
            incrementleaderboardstat( "downs", var_0, self.leaderboarddata["downs"] );
            incrementleaderboardstat( "contracts", var_0, self.leaderboarddata["contracts"] );
            incrementleaderboardstat( "wins", var_0, self.leaderboarddata["wins"] );
            incrementleaderboardstat( "topFive", var_0, self.leaderboarddata["topFive"] );
            incrementleaderboardstat( "topTen", var_0, self.leaderboarddata["topTen"] );
            incrementleaderboardstat( "topTwentyFive", var_0, self.leaderboarddata["topTwentyFive"] );
            incrementleaderboardstat( "gamesPlayed", var_0, self.leaderboarddata["gamesPlayed"] );
            incrementleaderboardstat( "revives", var_0, self.leaderboarddata["rescues"] );
            incrementleaderboardstat( "cash", var_0, self.leaderboarddata["cash"] );
            break;
    }
}
...
checkforbestweapon()
{
    var_0 = scripts\common\utility::trycall( level.matchdata_buildweaponrootlist );
    var_1 = "";
    var_2 = -1;
...
watchforlowpopmatchstart( var_0 )
{
    ...
    if ( var_6 <= var_8 )
    {
        ...
        dlog_recordevent( "dlog_event_low_pop_start", [ "start_players", var_6, "wait_time_ms", var_9, "active_client_count", var_0, "lowpop_min", var_4, "lowpop_max", var_8, "lowpop_time", var_10 ] );
...
removepatchablecollision_delayed()
{
    ...
    foreach ( var_2 in var_0 )
        scripts\cp_mp\utility\game_utility::removematchingents_bykey( var_2, "targetname" );
}
...
checkforsubgametypeoverrides()
{
    level.allowsupers = getdvarint( "scr_" + scripts\mp\utility\game::getsubgametype() + "_allowSupers", level.allowsupers );
}
...
logendofround( var_0 )
{
    ...
    if ( isdefined( var_0.damagethisround ) )
        var_6 = var_0.damagethisround;
