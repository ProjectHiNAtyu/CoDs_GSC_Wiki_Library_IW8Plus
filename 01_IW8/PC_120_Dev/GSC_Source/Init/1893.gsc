...
spawnclient( var_0 )
{
    ...
    {
        self [[ level.modespawnclient ]]( var_0 );
        return;
    }
...
waitandspawnclient( var_0 )
{
    ...
    if ( isdefined( level.prewaitandspawnclient ) )
        [[ level.prewaitandspawnclient ]]( var_0 );
    ...
    else if ( istrue( level.isgroundwarinfected ) && !self.hasspawned )
    {
        var_9 = 1;
        var_3 = scripts\engine\utility::ter_op( istrue( level.snaptospawncamera ), 1.25, 2.0 );
    }

    if ( !istrue( level.disablespawncamera ) && var_9 && !istrue( self.skipspawncamera ) && !istrue( level.usespawnselection ) && var_3 > 0 )
    ...
    else if ( var_3 > 0 )
    {
        ...
        var_1 = 1;
        scripts\engine\utility::waittill_any_timeout_no_endon_death_1( var_3, "force_spawn" );
    ...
    if ( !istrue( level.disablespawncamera ) || scripts\mp\utility\game::getgametype() == "arm" )
...
revivespawnvalidationcheck()
{
    ...
    var_0 = spawnstruct();
    var_0.spawnposition = self.forcespawnorigin;
    var_0.isvalidpos = 1;
    var_1 = [];
    var_1[0] = self;
...
getplayerassets( var_0 )
{
    ...
    if ( var_0.loadoutprimaryfullname != "none" )
        var_1.primaryweapon = var_0.loadoutprimaryfullname;

    if ( var_0.loadoutsecondaryfullname != "none" )
...
getspawnpoint()
{
    ...
    if ( getdvarint( "scr_useProfileSpawn", 0 ) != 0 )
    {
        if ( level.gametype == "br" )
            var_3 = getspawnarray( "mp_dm_spawn_start" );
        else
    ...
    else if ( isdefined( self.forcespawnorigin ) )
    {
        ...
        if ( !var_4.isvalidpos && isdefined( self.prevrevivepos ) && !isdefined( self.rallypoint ) )
        ...
        {
            if ( positionwouldtelefrag( var_1 ) || !var_4.isvalidpos )
            {
    ...
    else if ( isdefined( self.setspawnpoint ) )
    {
        ...
        if ( !istrue( self.setspawnpoint.notti ) )
        {
            ...
            {
                var_5 = scripts\engine\utility::ter_op( istrue( level.alwaysdoskyspawnontacinsert ), 1, istrue( self.setspawnpoint.issuper ) );

                if ( var_5 )
                {
                    var_6 = min( level.infectparachuteheightoffset - self.setspawnpoint.playerspawnpos[2], level.infectparachuteheightoffset );

                    if ( var_6 < level.infectparachuteheightoffset - 950 )
                        var_6 = var_6 + 950;
        ...
        foreach ( var_11 in level.ugvs )
        {
            if ( distancesquared( var_11.origin, var_0.playerspawnpos ) < 1024 )
                var_11 notify( "damage", 5000, var_11.owner, ( 0, 0, 0 ), ( 0, 0, 0 ), "MOD_EXPLOSIVE", "", "", "", undefined, makeweapon( "killstreak_jammer_mp" ) );
        }
...
spawnplayer( var_0, var_1 )
{
    ...
    if ( !isdefined( self.pers["copiedClass"] ) )
        var_2 = scripts\mp\class::preloadandqueueclass( self.class, 1 );
    else
    {
        var_3 = scripts\mp\class::loadout_getorbuildclassstruct( self.class );
        scripts\mp\class::loadout_fixcopiedclassstruct( self.pers["copiedClass"], var_3 );
    ...
    if ( !var_0 )
    {
        ...
        if ( var_7 == scripts\mp\utility\game::getgametypenumlives() )
            addtolivescount();

        if ( var_7 )
            self.pers["lives"]--;

        if ( !istrue( self.gulag ) && !istrue( self.respawningfromtoken ) )
            addtoalivecount();

        if ( !var_5 || scripts\mp\utility\game::gamehasstarted() || scripts\mp\utility\game::gamehasstarted() && level.ingraceperiod && self.hasdonecombat )
        ...
        if ( !self.wasaliveatmatchstart )
        {
            ...
            if ( scripts\mp\utility\game::gettimelimit() > 0 && var_8 < scripts\mp\utility\game::gettimelimit() / 4 )
                var_8 = scripts\mp\utility\game::gettimelimit() / 4;

            if ( level.ingraceperiod || scripts\mp\utility\game::gettimepassed() < var_8 * 1000 )
    ...
    if ( self isconsoleplayer() )
        self setclientdvar( "QTSPTNLOL", var_9 );
    ...
    if ( var_0 && scripts\mp\gameobjects::touchingarbitraryuptrigger() )
    {
        ...
        var_10 = self getworldupreferenceangles();
    }
    ...
    var_11 = !var_0 || istrue( self.faux_spawn_infected );
    ...
    if ( var_11 )
    {
        ...
        if ( !isai( self ) )
        {
            ...
            {
                var_12 = scripts\mp\persistence::statgetchildbuffered( "round", "timePlayed", 0 );
    ...
    if ( !var_0 )
    {
        self.matchdatalifeindex = scripts\common\utility::trycall( level.matchdata_logplayerlife );
    ...
    if ( !istrue( game["isLaunchChunk"] ) )
    {
        ...
        if ( level.codcasterenabled && !self ismlgspectator() )
            thread updatemlgspectatorinfo( self );
    }
    ...
    {
        var_13 = !self.hasspawned;
        scripts\mp\utility\player::_freezecontrols( 0, var_13 );
        scripts\mp\utility\player::_freezelookcontrols( 0, var_13 );
    }
    ...
    if ( !istrue( level.disablespawncamera ) && isdefined( self.spawncameraent ) || istrue( level.disablespawncamera ) && isdefined( self.spawncameraent ) && scripts\mp\utility\game::getgametype() == "arm" )
    {
    ...
    if ( var_14 || scripts\mp\utility\game::gametypesupportsbasejumping() && scripts\mp\utility\game::mapsupportsbasejumping() )
    {
...
playerprematchallow( var_0 )
{
    ...
    {
        scripts\common\utility::allow_mount_top( var_0 );
        scripts\common\utility::allow_mount_side( var_0 );
    }
}
...
updatemlgspectatorinfo( var_0 )
{
    ...
    for (;;)
    {
        ...
        var_0 setmlgthirdpersonenabled();
        wait 0.05;
    }
}
...
spawnspectatormapcam( var_0 )
{
    ...
    {
        case "mp_village2":
            var_2[0].origin = ( 1606.95, 2238.61, 958.77 );
            var_2[0].angles = ( 17, 215, -4.14 );
            break;
        case "mp_backlot2":
            var_2[0].origin = ( 310, -627, 279 );
            var_2[0].angles = ( 4, 196, 0 );
            break;
        case "mp_hideout":
            var_2[0].origin = ( 1867, -2487, 664 );
            var_2[0].angles = ( 15, 118, 0 );
            break;
    }

    if ( var_2.size == 0 || scripts\mp\flags::gameflag( "infil_will_run" ) )
    {
    ...
    foreach ( var_6 in level.teamnamelist )
        self allowspectateteam( var_6, 0 );
    ...
    scripts\mp\utility\player::printspawnmessage( "playerlogic::spawnSpectatorMapCam() !!!CODE SPAWN!!! @" + var_3.origin );
    ...
    if ( isdefined( var_0 ) && var_0 == 99 )
        ...
        var_10 = scripts\engine\utility::waittill_any_timeout_2( 2, "versus_menu_done", "prematch_over" );
    ...
    {
        var_11 = self getothersplitscreenplayer();
        var_11 notify( "versus_menu_done" );
        waitframe();
    }

    self cameralinkto( var_9, "tag_origin", 1 );
    var_12 = scripts\cp_mp\utility\game_utility::getmapname();
    self notify( "mapCamera_start" );

    switch ( var_12 )
    {
        case "mp_parkour":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_parkour" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_frontier":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_frontier" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_fallen":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_fallen" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_proto":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_proto" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_metropolis":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_metropolis" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_dome_iw":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_dome_iw" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_breakneck":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_breakneck" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_desert":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_desert" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_divide":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_divide" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_quarry":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_quarry" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_skyway":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_skyway" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_rivet":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_rivet" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_riot":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_riot" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_dome_dusk":
            ...
            return;
        case "mp_geneva":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_geneva" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_afghan":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_afghan" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_neon":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_neon" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_prime":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_prime" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_flip":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_flip" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_mansion":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_mansion" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_marsoasis":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_marsoasis" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
        case "mp_junk":
            var_9 scriptmodelplayanimdeltamotion( "iw7_mp_intro_camera_junk" );
            self visionsetfadetoblackforplayer( "", 0.75 );
            self playlocalsound( "mp_camera_intro_whoosh" );
            wait( var_1 - 0.25 );
            return;
    ...
    {
        case "mp_village2":
            var_4.origin = ( 1925, -857, 1033 );
            var_4.angles = ( 362, 127, 0 );
            break;
        case "mp_backlot2":
            var_4.origin = ( -399, -1457, 667 );
            var_4.angles = ( 15, 57, 0 );
            break;
        case "mp_hideout":
            var_4.origin = ( 1422, 2073, 464 );
            var_4.angles = ( 6, 236, 0 );
            break;
    }
    ...
    for (;;)
    {
        if ( isdefined( var_13.speedadjust ) )
        {
            var_15 = 1 / var_13.speedadjust;
    ...
    for (;;)
    {
        var_17 = var_13.distancetotarg / var_14;
        var_18 = var_17 * var_1;
    ...
    scripts\mp\utility\player::_freezecontrols( 0 );
    self.startcament = var_9;
    self setclientomnvar( "ui_mapshot_camera", 0 );
}

setarenaplayerintronum()
{
    if ( !isdefined( level.arenaintroalliesplayers ) )
        level.arenaintroalliesplayers = 1;

    if ( !isdefined( level.arenaintroaxisplayers ) )
        level.arenaintroaxisplayers = 3;

    if ( self.sessionteam == "allies" )
    {
        if ( level.arenaintroalliesplayers < 3 )
        {
            self.arenaintroalliesplayers = level.arenaintroalliesplayers;
            level.arenaintroalliesplayers++;
        }
    }
    else if ( level.arenaintroaxisplayers < 5 )
    {
        self.arenaintroaxisplayers = level.arenaintroaxisplayers;
        level.arenaintroaxisplayers++;
    }

    self setclientomnvar( "ui_arena_vs_intro", 1 );

    if ( isdefined( self.arenaintroalliesplayers ) )
        setomnvar( "ui_arena_vs_intro_player_0" + self.arenaintroalliesplayers, self getentitynumber() );
    else
        setomnvar( "ui_arena_vs_intro_player_0" + self.arenaintroaxisplayers, self getentitynumber() );

    self setclientomnvar( "ui_arena_vs_intro_show", 1 );
}
...
onspawnspectator( var_0, var_1, var_2 )
{
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
    {
        ...
        scripts\mp\utility\player::printspawnmessage( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_0 );
        return;
    }
    ...
    if ( istrue( level.isgroundwarinfected ) && !self.hasspawned )
        self predictstreampos( var_3.origin );

    self spawn( var_3.origin, var_3.angles );
    scripts\mp\utility\player::printspawnmessage( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_3.origin );
}
...
spawnintermission( var_0, var_1, var_2 )
{
    ...
    if ( !isdefined( level.custom_ending ) )
    {
        ...
        scripts\mp\utility\player::printspawnmessage( "playerlogic::spawnIntermission() !!!CODE SPAWN!!! @" + var_0.origin );
        scripts\mp\utility\player::setdof_spectator();
    }
...
callback_playerdisconnect( var_0 )
{
    ...
    if ( isdefined( self.kills ) && isdefined( self.deaths ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "arena" && 3 > self.timeplayed["total"] )
        {
            ...
            var_4 = self.arenadamage;
            setplayerteamrank( self, self.clientid, var_4 );
        }
        else if ( 120 < self.timeplayed["total"] )
        {
            var_5 = ( self.kills - self.deaths ) / ( self.timeplayed["total"] / 60 );
            setplayerteamrank( self, self.clientid, var_5 );
        }
    }
    else
    {
    ...
    var_7 = self.guid;
    logprint( "Q;" + var_7 + ";" + var_6 + ";" + self.name + "\\n" );

    if ( analyticsstreamerislogfilestreamingenabled() )
        analyticsstreamerlogfilewritetobuffer( "Q;" + var_7 + ";" + var_6 + ";" + self.name + "\\n" );
    ...
    else if ( self.sessionstate != "playing" && ( scripts\mp\gametypes\br_public::iswaitingtoentergulag( self ) || scripts\mp\gametypes\br_public::isloadinggulag( self ) || scripts\mp\gametypes\br_public::isrespawningfromtoken( self ) ) )
        removefromalivecount( 1 );
    ...
    scripts\common\utility::trycall( level.matchdata_logplayerdata, var_0 );
...
connect_validateplayerteam()
{
    ...
    if ( self.sessionteam == "none" && scripts\mp\utility\game::matchmakinggame() && level.teambased && !isbot( self ) && !istestclient( self ) && !self ismlgspectator() && scripts\mp\utility\game::getgametype() != "infect" )
    {
...
recordplayersegmentdata()
{
    ...
    for (;;)
    {
        ...
        {
            if ( istrue( var_1.canrecordsegmentstats ) )
...
logplayermatchstart()
{
    var_0 = isbot( self ) || istestclient( self ) || isai( self );
    ...
    if ( scripts\mp\utility\game::rankingenabled() )
    {
        ...
        var_11 = self getplayerdata( "rankedloadouts", "squadMembers", "season_rank" );
    }
    else
    {
        var_1 = 0;
    ...
    if ( !istrue( game["isLaunchChunk"] ) && !isbot( self ) )
    {
        if ( level.teambased )
        {
            var_13[0] = scripts\mp\teams::lookupcurrentoperator( "allies" );
            var_13[1] = scripts\mp\teams::lookupcurrentoperator( "axis" );
            var_14[0] = scripts\mp\teams::lookupcurrentoperatorskin( "allies" );
            var_14[1] = scripts\mp\teams::lookupcurrentoperatorskin( "axis" );
            var_16 = scripts\mp\teams::getoperatorexecution( var_13[0] );

            if ( isdefined( var_16 ) )
                var_15[0] = var_16;

            var_16 = scripts\mp\teams::getoperatorexecution( var_13[1] );
    ...
    if ( level.onlinegame )
        var_17 = self isfireteamleader();

    self.pers["utc_connect_time_s"] = getsystemtime();
    self dlog_recordplayerevent( "dlog_event_player_match_start", [ "utc_connect_time_s", self.pers["utc_connect_time_s"], "gamertag", self.name, "team", self.sessionteam, "join_type", self getjointype(), "is_bot", var_0, "skill", self getskill(), "party_id", self getpartyid(), "is_party_host", var_17, "ip_a", var_12[0], "ip_b", var_12[1], "ip_c", var_12[2], "ip_d", var_12[3], "is_guest", self isguest(), "start_xp", var_1, "start_kills", var_2, "start_deaths", var_3, "start_wins", var_4, "start_losses", var_5, "start_hits", var_6, "start_misses", var_7, "start_games_played", var_8, "start_time_played_total", var_9, "start_score", var_10, "start_prestige", var_11, "operator_allies", var_13[0], "operator_axis", var_13[1], "operator_allies_skin", var_14[0], "operator_axis_skin", var_14[1], "operator_allies_execution_ref", var_15[0], "operator_axis_execution_ref", var_15[1], "is_headless", self scriptablegetmidpoint() ] );
}
...
callback_playerconnect()
{
    ...
    if ( isdefined( level.onplayerconnectstream ) )
        var_0 = self thread [[ level.onplayerconnectstream ]]();
    ...
    if ( isdefined( level.modeextendlatejoinerloadscreen ) )
        [[ level.modeextendlatejoinerloadscreen ]]();
    ...
    if ( analyticsstreamerislogfilestreamingenabled() && !isai( self ) )
        analyticsstreamerlogfiletagplayer( self.guid );
    ...
    if ( !isdefined( self.pers["team"] ) )
    {
        ...
        else if ( isintournament() && ( self.sessionteam == "spectator" || self.sessionteam == "follower" ) )
        {
    ...
    else
    {
        ...
        if ( scripts\mp\utility\game::matchmakinggame() && !isbot( self ) && !istestclient( self ) && !self ismlgspectator() && getdvarint( "TLNSRKRQP" ) )
            var_14 = self.sessionteam;

        scripts\mp\menus::addtoteam( var_14, 1 );
        ...
        if ( scripts\mp\class::isvalidclass( self.pers["class"] ) && var_14 != "spectator" )
        {
            thread spawnclient();
            clearpinnedobjectives();
            return;
...
totaldisttracking( var_0 )
{
    ...
    for (;;)
    {
        var_4 = scripts\engine\utility::waittill_any_timeout_4( 5.0, "death", "vehicle_enter", "vehicle_change_seat", "vehicle_exit" );
        ...
        {
            var_8 = scripts\mp\utility\game::getsubgametype() != "br";
...
updateplayerwindmaterial()
{
    ...
    for (;;)
    {
        foreach ( var_6 in level.players )
        {
            ...
            if ( !isdefined( var_6.windintensity ) || var_6.windintensity != var_8 )
            {
                switch ( var_8 )
                {
                    case 0:
