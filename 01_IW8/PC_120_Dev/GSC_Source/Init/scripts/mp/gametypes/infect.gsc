...
main()
{
    ...
    level.extratimeincreasecountcap = getdvarint( "scr_infect_time_increase_cap", 20 );
    ...
    level.infecteddisablenvg = getdvarint( "scr_infect_disable_nvg", 0 );
    ...
    if ( level.isgroundwarinfected )
    {
        ...
        level.infectparachuteheightoffset = var_2;
        level.alwaysdoskyspawnontacinsert = getdvarint( "scr_infect_alwaysTacInsertSkySpawn", 0 );
...
updategametypedvars()
{
    ...
    level.infectedsupertwo = getdvar( "scr_infect_superInfectTwo", scripts\engine\utility::ter_op( level.isgroundwarinfected, "super_tac_insert", "none" ) );
    ...
    level.infectbonussuperonspawn = getdvarint( "scr_infect_infectBonusSuperOnSpawn", 0 );
    level.infectbonussuperontacinsert = getdvarint( "scr_infect_infectBonusSuperOnTacInsert", 0 );
    level.finalsurvivorcount = getdvarint( "scr_infect_finalSuvivorCount", 1 );
    level.infectsetradaronnumsurvivors = getdvarint( "scr_infect_setRadarOnNumSurvivors", 6 );
...
{
    if ( istrue( var_1 ) )
        var_2 = scripts\engine\utility::ter_op( var_0, level.infectedsupertwo, level.survivorsupertwo );
    else
    ...
    if ( var_10 )
    {
        ...
        if ( scripts\mp\utility\dvars::getwatcheddvar( "timelimit" ) != 0 )
        {
            ...
            if ( scripts\mp\utility\game::matchmakinggame() )
            {
                ...
                var_17 = level.extratimeincreasecount <= level.extratimeincreasecountcap;
            }
...
delayedprocesskill( var_0, var_1 )
{
    ...
    if ( level.infect_teamscores["allies"] > 1 )
    {
        ...
        if ( !var_1 )
        {
            ...
            {
                var_2 = getdvarint( "scr_infect_survivorinitialscore", 50 );

                if ( var_2 > 0 )
                    level.survivorscoreevent = var_2;
                else
            ...
            {
                if ( !scripts\mp\utility\player::isreallyalive( var_4 ) || self.sessionstate == "spectator" )
                    continue;

                if ( var_4.team == "allies" && var_4 != self && distance( var_4.infect_spawnpos, var_4.origin ) > 32 )
                    var_4 thread scripts\mp\utility\points::giveunifiedpoints( "survivor", undefined, level.survivorscoreevent );

                if ( var_4.team == "axis" && var_4 != var_0 && var_4 != self )
                    var_4 thread scripts\mp\utility\points::giveunifiedpoints( "assist", undefined, level.infectbonusscore );
            }
        }
    }
...
monitordisconnect()
{
    ...
    if ( isdefined( self.infect_isbeingchosen ) || level.infect_chosefirstinfected )
    {
        if ( level.infect_teamscores["axis"] && level.infect_teamscores["allies"] )
        {
            if ( level.isgroundwarinfected )
            {
                if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.infectsetradaronnumsurvivors )
                    level thread startlastsurvivorsuavsweep();
            }

            if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.finalsurvivorcount )
                onfinalsurvivor();
...
setspecialloadouts()
{
    ...
    else if ( level.onlineprimaryoverride )
    {
        ...
        level.infect_loadouts["allies"]["loadoutPrimaryVariantID"] = -1;
        ...
        level.infect_loadouts["allies"]["loadoutSecondaryVariantID"] = -1;
    ...
    else if ( level.onlineprimaryoverride )
    {
        ...
        level.infect_loadouts["axis_initial"]["loadoutKillstreak2"] = "none";
        level.infect_loadouts["axis_initial"]["loadoutKillstreak3"] = "none";
        level.infect_loadouts["axis_initial"]["loadoutPerks"] = [];
    ...
    else
    {
        ...
        level.infect_loadouts["axis_initial"]["loadoutPrimaryVariantID"] = -1;
        level.infect_loadouts["axis_initial"]["loadoutSecondary"] = level.initialsecondaryweapon;
        level.infect_loadouts["axis_initial"]["loadoutSecondaryAttachment"] = level.attachmentinitialsecondary;
        level.infect_loadouts["axis_initial"]["loadoutSecondaryAttachment2"] = level.attachmentinitialsecondarytwo;
        level.infect_loadouts["axis_initial"]["loadoutSecondaryCamo"] = "none";
        level.infect_loadouts["axis_initial"]["loadoutSecondaryReticle"] = "none";
        level.infect_loadouts["axis_initial"]["loadoutSecondaryVariantID"] = -1;
        level.infect_loadouts["axis_initial"]["loadoutEquipmentPrimary"] = level.infectedlethal;
        level.infect_loadouts["axis_initial"]["loadoutEquipmentSecondary"] = level.infectedtactical;
        level.infect_loadouts["axis_initial"]["loadoutSuper"] = "none";
        level.infect_loadouts["axis_initial"]["loadoutStreakType"] = "assault";
    ...
    else
    {
        ...
        level.infect_loadouts["axis"]["loadoutPrimaryVariantID"] = -1;
        ...
        level.infect_loadouts["axis"]["loadoutKillstreak2"] = "none";
        level.infect_loadouts["axis"]["loadoutKillstreak3"] = "none";
        level.infect_loadouts["axis"]["loadoutPerks"] = [];
    ...
    for (;;)
    {
        if ( self isnightvisionon() )
        {
            ...
            if ( var_0 >= 2 )
            {
                ...
                scripts\common\utility::allow_nvgs( 0 );
                rechargenvg();
                scripts\common\utility::allow_nvgs( 1 );
                var_1 = 0;
...
clearandrestoreinfectedtacinsert()
{
    ...
    if ( isdefined( self ) && scripts\mp\utility\player::isreallyalive( self ) )
    {
        ...
        var_0 = "gamemode";
        var_1 = scripts\mp\class::loadout_getglobalclassstruct();
        var_1 = scripts\mp\class::loadout_updateglobalclass( var_1, var_0 );
...
init_infectgroundwarvehicles()
{
    ...
    var_18 = level.numnonrallyvehicles;
...
{
    level endon( "game_ended" );
    level.infectedairdroppositions = [];
    level thread setupinfectedairdroppositions();
    scripts\mp\flags::gameflagwait( "prematch_done" );
    wait( level.droptime );
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_0 ) && scripts\mp\utility\killstreak::currentactivevehiclecount() < scripts\mp\utility\killstreak::maxvehiclesallowed() && level.fauxvehiclecount + var_1 < scripts\mp\utility\killstreak::maxvehiclesallowed() && scripts\cp_mp\killstreaks\airdrop::getnumdroppedcrates() < 8 )
        {
            ...
            var_3 = getinfectedairdropposition();
...
getinfectedairdropposition()
{
    switch ( level.mapname )
    {
        case "mp_downtown_gw":
            var_0 = level.infectedairdroppositions[0];
            level.infectedairdroppositions[0] = undefined;
            level.infectedairdroppositions = scripts\engine\utility::array_removeundefined( level.infectedairdroppositions );
            break;
        case "mp_quarry2":
            var_0 = level.infectedairdroppositions[0];
            level.infectedairdroppositions[0] = undefined;
...
setupinfectedairdroppositions()
{
    switch ( level.mapname )
    {
        ...
        case "mp_boneyard_gw":
            ...
            break;
        case "mp_aniyah":
            level.infectedairdroppositions = [ ( 2716, -158, 708 ), ( 8215, 874, 342 ), ( 6524, 1561, 316 ), ( 5827, -860, 357 ), ( 4268, -47, 452 ), ( 2305, -1994, 386 ), ( 5172, 2587, 231 ), ( 2691, 3096, 302 ), ( -910, 3497, 318 ), ( -3579, -544, 320 ), ( 2591, 1659, 545 ), ( 1033, -514, 447 ), ( -93, -2789, 169 ), ( -1301, 924, 315 ), ( -2843, 2779, 316 ) ];
            break;
        default:
            break;
    }

    level.infectedairdroppositions = scripts\engine\utility::array_randomize( level.infectedairdroppositions );
}
