...
main()
{
    ...
    level.tankrespawntime = getdvarint( "scr_arm_tankRespawnTime", 120 );
    level.apcrespawntime = getdvarint( "scr_arm_apcRespawnTime", 120 );
    level.longdialoguecooldown = getdvarint( "scr_arm_longDialogueCooldown", 1 );
    level.mercymatchending_nuke = getdvarint( "scr_arm_mercyEndingNuke", 1 );
    ...
    level.disablepersonalnuke = getdvarint( "scr_arm_disablePersonalNuke", 0 );
    level.personalnukecostoverride = getdvarint( "scr_personalNukeCostOverride", 30 );
    level.showteamtanks = getdvarint( "scr_arm_showTeamTanks", 1 );
    level.showteamlittlebirds = getdvarint( "scr_arm_showTeamLittleBirds", 1 );
    level.spawnselectionafktime = getdvarint( "scr_arm_spawnSelectionAFKTime", 120 );
    level.manualadjustlittlebirdlocs = getdvarint( "scr_arm_manualAdjustLittleBirdLocs", 1 );
    level.dontspawnjeep = getdvarint( "scr_arm_dontSpawnJeep", 0 );
    level.dontspawncargotruck = getdvarint( "scr_arm_dontSpawnCargoTruck", 0 );
    level.usepingsystem = getdvarint( "scr_arm_usePingSystem", 0 );
    ...
    {
        level.trackedlittlebirds = [];
        level.trackedlittlebirds["axis"] = [];
        level.trackedlittlebirds["allies"] = [];
        level.trackedlittlebirds["untouched"] = [];
    }
...
updatescores()
{
    ...
    var_5 = scripts\mp\gamescore::calculatewinningteam();
    ...
    var_8 = scripts\mp\gamescore::calculatewinningteam();

    if ( var_5 != var_8 )
        scripts\mp\gamescore::playscorestatusdialog( var_8, 1, var_5 );
}
...
sethqmarkerobjective()
{
    ...
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.axishqloc.enemymarker = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
    ...
    scripts\mp\objidpoolmanager::update_objective_setbackground( var_2.objidnum, 7 );
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.allieshqloc.enemymarker = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
...
runobjflag( var_0, var_1 )
{
    ...
    var_2 scripts\mp\gameobjects::allowuse( "enemy" );
...
docratedropsmoke( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_0 ) )
        var_0 scripts\engine\utility::waittill_any_timeout_1( var_2, "crate_dropped" );
    else
        wait( var_2 );
...
nuke_startmercycountdown( var_0 )
{
    ...
    var_1 = 0;
    var_2 = gettime();
    var_3 = level.mercymatchending_time * 1000 + var_2;
    setomnvar( "ui_nuke_end_milliseconds", level.mercymatchending_time * 1000 + var_2 );

    while ( calculateobjectivesheld( var_0 ) == level.objectives.size )
    {
        waitframe();
...
nuke_triggermercywin( var_0 )
{
    ...
    foreach ( var_5 in level.players )
    {
        ...
        {
            if ( isdefined( var_5.operatorsfx ) )
            {
                var_5 clearsoundsubmix( "iw8_mp_spawn_camera" );
                var_5.operatorsfx stoploopsound( var_5.operatorsfxalias );
                var_5.operatorsfx delete();
                var_5.operatorsfx = undefined;
                var_5.operatorsfxalias = undefined;
            }
        }
    }
...
{
    thread scripts\mp\spawnselection::refreshsquadspawnareaomnvars( var_0.team, var_0.squadindex );
}
...
init_groundwarvehicles()
{
    ...
    foreach ( var_1 in level.vehicleinfo )
    {
        ...
        foreach ( var_9 in var_1.vehiclespawns )
        {
            if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_9.script_noteworthy ) && var_9.script_noteworthy == level.localeid )
            {
                ...
                if ( istrue( level.dontspawnjeep ) && var_1.refname == "jeep" || istrue( level.dontspawncargotruck ) && var_1.refname == "cargo_truck" )
                    continue;
                else
                {
    ...
    var_28 = level.numnonrallyvehicles;
...
vehiclespawn_tank( var_0, var_1 )
{
    ...
    if ( istrue( level.showteamtanks ) )
        vehicle_createspawnselectiontankmarker( var_3, var_3.team );

    return var_3;
}

vehicle_createspawnselectiontankmarker( var_0, var_1 )
{
    wait 1;
    ...
    var_0.spawnselectionmarker = var_2;
    thread vehicle_watchmarkedtankdeath( var_0 );
    level.trackedtanks[var_1][level.trackedtanks[var_1].size] = var_0;
}

vehicle_watchmarkedtankdeath( var_0 )
{
    var_1 = var_0.spawnselectionmarker.objidnum;
    var_2 = var_0.team;
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    level.trackedtanks[var_2] = scripts\engine\utility::array_remove( level.trackedtanks[var_2], var_0 );
}
...
vehicle_showteamtanks()
{
    ...
    foreach ( var_1 in level.trackedtanks[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1.spawnselectionmarker.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level.trackedtanks["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.spawnselectionmarker.objidnum, self );

    foreach ( var_1 in level.trackedtanks["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.spawnselectionmarker.objidnum, self );
}
...
vehicle_createspawnselectionlittlebirdmarker( var_0, var_1 )
{
    ...
    var_0.spawnselectionmarker = var_2;
    thread vehicle_watchmarkedlittlebirddeath( var_0 );
    level.trackedlittlebirds["untouched"][level.trackedlittlebirds["untouched"].size] = var_0;
}

vehicle_watchmarkedlittlebirddeath( var_0 )
{
    var_1 = var_0.spawnselectionmarker.objidnum;
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    var_2 = var_0.lastteamused;

    if ( !isdefined( var_0.lastteamused ) )
        var_2 = "untouched";

    level.trackedlittlebirds[var_2] = scripts\engine\utility::array_remove( level.trackedlittlebirds[var_2], var_0 );
}
...
vehicle_showvalidlittlebirds()
{
    ...
    foreach ( var_1 in level.trackedlittlebirds[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1.spawnselectionmarker.objidnum, self );

    foreach ( var_1 in level.trackedlittlebirds["untouched"] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1.spawnselectionmarker.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level.trackedlittlebirds["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.spawnselectionmarker.objidnum, self );

    foreach ( var_1 in level.trackedlittlebirds["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.spawnselectionmarker.objidnum, self );
...
{
    if ( isdefined( self.lastteamused ) )
        level.trackedlittlebirds[self.lastteamused] = scripts\engine\utility::array_remove( level.trackedlittlebirds[self.lastteamused], self );
    else
        level.trackedlittlebirds["untouched"] = scripts\engine\utility::array_remove( level.trackedlittlebirds["untouched"], self );

    self.lastteamused = var_0;
    level.trackedlittlebirds[var_0][level.trackedlittlebirds[var_0].size] = self;
    scripts\mp\objidpoolmanager::update_objective_icon( self.spawnselectionmarker.objidnum, "icon_minimap_littlebird_spawn_selection" );

    foreach ( var_2 in level.players )
    {
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( self.spawnselectionmarker.objidnum, var_2 );

        if ( isdefined( var_2 ) && isdefined( var_2.team ) && var_2.team == var_0 && istrue( var_2.inspawnselection ) )
        {
            scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( self.spawnselectionmarker.objidnum, var_2 );
            continue;
        }
...
init_rallyvehicles()
{
    ...
    {
        if ( level.localeid == "locale_6" && level.mapname == "mp_downtown_gw" && istrue( level.manualadjustlittlebirdlocs ) )
        {
            var_10 = [];
        ...
        foreach ( var_2 in var_0 )
        {
            ...
            level thread vehicle_createspawnselectionlittlebirdmarker( var_6, "neutral" );
        }
    }
    ...
    foreach ( var_6 in level.rallypointvehicles )
    {
        ...
        if ( var_6.team == "axis" )
        {
            ...
            else if ( var_6.vehiclename == "little_bird" )
            {
                var_22++;
