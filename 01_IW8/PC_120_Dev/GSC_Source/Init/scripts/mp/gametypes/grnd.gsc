...
initspawns()
{
    ...
    foreach ( var_22, var_3 in level.objectives )
    {
        var_3.spawnset = "dropzone_" + var_22;
        scripts\mp\spawnlogic::registerspawnset( var_3.spawnset, var_3.spawnpoints );
        var_3.fallbackspawnset = "dropzone_fallback_" + var_22;
...
calculatespawndisttozones( var_0 )
{
    ...
    foreach ( var_2 in level.objectives )
    {
        ...
        var_0.scriptdata.distsqtokothzones[var_2.trigger getentitynumber()] = var_3;
...
setupzones()
{
    ...
    if ( level.mapname == "mp_shipment" )
    {
        ...
        var_7 = scripts\engine\utility::array_remove_array( var_7, var_11 );
    }
    ...
    for ( var_19 = 0; var_19 < var_7.size; var_19++ )
    {
        ...
        {
            if ( var_13.scriptable istouching( var_0[var_21] ) )
            {
                if ( isdefined( var_2 ) )
                {
        ...
        var_18 = [];
        var_18[0] = var_13.scriptable;
        var_22 = scripts\mp\gametypes\obj_zonecapture::setupobjective( var_2, var_18 );
        var_22.origin = var_2.origin;
        level.objectives[var_22.objectivekey] = var_22;
    }
...
dzmainloop()
{
    ...
    level.zone thread scripts\common\utility::trycall( level.matchdata_loggameevent, "hill_moved", level.zone.trigger.origin );
    ...
    if ( level.firstzoneactivationdelay )
    {
        ...
        level.zone thread scripts\mp\gametypes\obj_zonecapture::lockedradialunfill( level.firstzoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
hqactivatenextzone( var_0, var_1 )
{
    ...
    else if ( level.zoneactivationdelay )
    {
        ...
        level.zoneislocked = 1;
        level thread scripts\mp\hud_message::updatematchstatushintforallplayers( undefined, 42, 42 );
        level.zone thread scripts\mp\gametypes\obj_zonecapture::lockedradialunfill( level.zoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
hpcaptureloop()
{
    ...
    for (;;)
    {
        ...
        var_0 = level scripts\engine\utility::waittill_any_return_2( "zone_captured", "zone_destroyed" );
...
give_capture_credit( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isplayer( var_5 ) )
    {
        ...
        {
            var_5 thread scripts\common\utility::trycall( level.matchdata_loggameevent, "capture", var_5.origin );
            var_5 thread scripts\mp\rank::scoreeventpopup( "dz_capture" );
...
randomdrops()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_0 ) && scripts\mp\utility\killstreak::currentactivevehiclecount() < scripts\mp\utility\killstreak::maxvehiclesallowed() && level.fauxvehiclecount + var_1 < scripts\mp\utility\killstreak::maxvehiclesallowed() && scripts\cp_mp\killstreaks\airdrop::getnumdroppedcrates() < 8 )
        {
            ...
            if ( shoulddonodedrop( var_2 ) && isdefined( var_3 ) && var_3.size > 0 )
            {
                var_4 = randomintrange( 0, var_3.size );
                var_5 = var_3[var_4];
                var_6 = getclosestpointonnavmesh( var_5.origin );
                var_7 = var_5.origin;
                var_8 = var_6;
                var_9 = scripts\engine\trace::create_contents( 0, 1, 1, 1, 0, 1, 0 );
                var_10 = [];
                var_11 = scripts\engine\trace::ray_trace( var_7, var_8, var_10, var_9 );
                var_5 = var_11["position"];
            }
            else
                var_5 = getdropposition() + ( randomintrange( -50, 50 ), randomintrange( -50, 50 ), 0 );

            if ( var_2 == "mega" )
            {
                var_12 = spawnstruct();
                var_12.cratetype = undefined;
                var_12.numcrates = undefined;
                var_12.usephysics = undefined;
                scripts\cp_mp\killstreaks\airdrop_multiple::airdrop_multiple_dropcrates( var_0, var_0.team, var_5, randomfloat( 360 ), var_5, var_12 );
            }
            else
            {
                scripts\cp_mp\vehicles\vehicle_tracking::reservevehicle();
                scripts\cp_mp\killstreaks\airdrop::dropkillstreakcratefromscriptedheli( var_0, var_0.team, var_2, var_5, randomfloat( 360 ), var_5, 1 );
            }

            var_13 = level.droptime;
        }
        else
            var_13 = 0.5;
