...
main()
{
    if ( scripts\mp\utility\game::getsubgametype() == "dmz" && scripts\mp\gametypes\br_public::isdmzbotpracticematch() )
...
bot_br_think()
{
    ...
    for (;;)
    {
        if ( scripts\mp\gametypes\br_public::isbotpracticematch() && !scripts\mp\flags::gameflag( "graceperiod_done" ) )
        {
            self.ignoreall = 0;
            wait 0.05;
            continue;
        ...
        {
            if ( scripts\mp\gametypes\br_public::isbotpracticematch() && !isdefined( self.infil_complete ) )
            {
                self.ignoreall = 1;
            ...
            if ( ( scripts\mp\gametypes\br_public::isplayerbrsquadleader() || var_0 ) && istrue( level.c130inbounds ) )
            {
        ...
        if ( isdefined( level.br_circle ) && isnavmeshloaded() )
        {
            ...
            {
                if ( ( istrue( self isskydiving() ) || istrue( self isparachuting() ) ) && istrue( self.jumped ) && istrue( scripts\mp\gametypes\br_public::isbotpracticematch() ) )
                {
...
get_player_enemy()
{
    ...
    if ( !isdefined( var_0 ) || istrue( var_0.inlaststand ) || !isalive( var_0 ) || var_0 scripts\mp\gametypes\br_public::isplayerinorgoingtogulag() )
        return undefined;
