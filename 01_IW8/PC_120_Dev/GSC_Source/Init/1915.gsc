...
onjoinedspectators( var_0 )
{
    ...
    if ( var_0 ismlgspectator() || isdefined( var_0.pers["mlgSpectator"] ) && var_0.pers["mlgSpectator"] )
    {
        ...
        var_0 thread saveendgamelocals();
    }
}
...
autoattachtoplayer()
{
    ...
    for (;;)
    {
        ...
        if ( !self ismlgspectator() || self isspectatingplayer() || self ismlgfreecamenabled() || self allowspectateallteams() )
            break;

        if ( level.players.size > 1 )
        {
            if ( level.currentround > 1 && isdefined( level.lastspectatedplayer ) && self.team != "follower" )
                var_0 = level.lastspectatedplayer;
            else if ( self.team == "follower" )
            {
                var_1 = self setmlgdominationpoint();
                var_2 = 0;
...
updatemlgspectator()
{
    ...
    for (;;)
    {
        if ( self ismlgspectator() && self isspectatingplayer() )
        {
            ...
            if ( isdefined( var_0 ) )
                level.lastspectatedplayer = var_0.clientid;
        }
