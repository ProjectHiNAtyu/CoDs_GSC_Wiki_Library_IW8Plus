...
findfirstpoiinlink()
{
    if ( sighttracepassed( self geteye(), self.poi_firstpoint.origin, 0, undefined ) )
    {
        ...
        if ( isdefined( var_0.target ) )
            self.nextpoi = scripts\engine\utility::getstruct( var_0.target, "_encstr_B8CE0BE43FC317DF690862BA48" );
...
findnextpoiinlink( var_0 )
{
    ...
    if ( var_0 )
    {
        if ( isdefined( self.currentpoi.target ) )
            self.nextpoi = scripts\engine\utility::getstruct( self.currentpoi.target, "_encstr_B8CE0BE43FC317DF690862BA48" );
        else
    ...
    else
    {
        if ( !iswithinfov( self.nextpoi ) )
        {
            if ( isdefined( self.nextpoi.target ) )
                self.nextpoi = scripts\engine\utility::getstruct( self.nextpoi.target, "_encstr_B8CE0BE43FC317DF690862BA48" );
            else
                scripts\common\ai::poi_enable( 0 );
