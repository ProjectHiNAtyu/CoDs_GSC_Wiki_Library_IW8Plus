...
{
    level._id_128AD = [];
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_128A7 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
        level thread _id_1289F();
}

_id_128A7( var_0 )
{
    foreach ( var_2 in level._id_128AD )
    {
        var_2 _id_128AC( var_0 );
        var_2 _id_128AB( var_0 );
    }
}
...
_id_128A6()
{
    ...
    for (;;)
    {
        ...
        scripts\engine\utility::_id_13141( var_2, "offhand_pullback" );
    }
}
...
tacinsert_used( var_0 )
{
    ...
    if ( !_id_128A0( var_0, var_3, var_4 ) )
    {
        scripts\mp\hud_message::showerrormessage( "MP/TAC_INSERT_CANNOT_PLACE" );
        thread _id_128A9();
        return 0;
    ...
    var_17 thread _id_128A1();
    var_17 setotherent( self );
    var_17.weapon_name = "tac_insert_trigger";
    var_18 = var_7["entity"];

    if ( isdefined( var_18 ) && ( scripts\mp\vehicles\vehicle_compass_mp::recondronetracecontents( var_18 ) || scripts\mp\gametypes\br_gondola::reader( var_18 ) ) )
    {
        var_17 linkto( var_18 );
        var_19 = getdvarint( "scr_br_tac_insert_moving_platform_valid", 1 );

        if ( var_19 )
            var_17 thread _id_128A5( self );
    }
    ...
    _id_1257F( var_1, var_16, var_17, 1 );
    var_21 = var_17 getentitynumber();
    level._id_128AD[var_21] = var_17;

    if ( var_17.issuper && isdefined( level._id_128A3 ) )
        self [[ level._id_128A3 ]]();
...
tacinsert_setupandwaitfordeath( var_0 )
{
    ...
    thread _id_128A2( var_0 );

    if ( !self.issuper )
        thread _id_128A8( var_0 );
}
...
_id_128A8( var_0 )
{
    ...
    level endon( "game_ended" );
    var_0 endon( "disconnect" );
    self.update_readings = spawn( "script_origin", self.origin );
    self.update_readings makeusable();
    self.update_readings setcursorhint( "HINT_NOICON" );
    self.update_readings sethintstring( &"MP_PATCH/PICKUP_TI" );
    self.update_readings linkto( self );
    ...
    for (;;)
    {
        self.update_readings waittill( "trigger", var_2 );
...
_id_128AC( var_0 )
{
    ...
    if ( !isdefined( self ) || !isdefined( self.update_readings ) )
        return;
    ...
    if ( var_1 )
        self.update_readings enableplayeruse( var_0 );
    else
        self.update_readings disableplayeruse( var_0 );
}
...
tacinsert_destroy( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.owner ) )
    {
        if ( istrue( var_2 ) )
        {
            ...
            self.owner scripts\mp\supers::fake_digit_pool( "super_tac_insert" );
            self.owner scripts\cp\helicopter\chopper_boss::total_successful_vehicle_spawns( "super_tac_insert", 1, var_0, var_1 );
            scripts\mp\analyticslog::logevent_fieldupgradeexpired( self.owner, self.superid, 1, istrue( var_1 ) );
        }
        else
        {
            self.owner scripts\cp\helicopter\chopper_boss::total_successful_vehicle_spawns( "super_tac_insert", 0, var_0, var_1 );
            scripts\mp\analyticslog::logevent_fieldupgradeexpired( self.owner, self.superid, 0, istrue( var_1 ) );
        }

        if ( !istrue( var_2 ) && !istrue( var_3 ) )
            self.owner thread _id_128AA();
...
_id_128A0( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_6 ) )
    {
        if ( isdefined( var_6["hittype"] ) && var_6["hittype"] != "hittype_none" )
        {
            ...
            if ( isdefined( var_8 ) )
            {
                if ( var_8 scripts\cp_mp\vehicles\vehicle::isvehicle() && !scripts\mp\vehicles\vehicle_compass_mp::recondronetracecontents( var_8 ) )
                    var_3 = 0;

                var_9 = getdvarint( "scr_br_tac_insert_moving_platform_valid", 1 );

                if ( !var_9 && ( scripts\mp\vehicles\vehicle_compass_mp::recondronetracecontents( var_8 ) || scripts\mp\gametypes\br_gondola::reader( var_8 ) ) )
                    var_3 = 0;
...
_id_128A9()
{
    ...
    if ( istrue( level.relic_amped_wait_till_revived ) )
    {
        ...
        var_1 = scripts\engine\utility::_id_13141( 5, "super_use_finished_lb" );
...
_id_1257F( var_0, var_1, var_2, var_3 )
{
    _id_1257C();
    var_4 = undefined;
