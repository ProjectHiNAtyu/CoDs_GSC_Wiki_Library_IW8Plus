...
initparachutedvars()
{
    ...
    level.trigger_elevator_spawners = getdvarint( "scr_parachute_autodeploy_cut", 1 );
    level.trigger_explosion_grenades = getdvarint( "scr_parachute_cut", 1 );
    level.parachuteinitfinished = 1;
    level.allow_momentum = [];
    ...
    if ( !isdefined( level.triggeraddobjectivetext ) )
        level.triggeraddobjectivetext = ::triggeravailablekillstreaks;

    level._id_12355 = spawnstruct();
    level._id_12355._id_127DC = getdvarint( "scr_parachute_hint_enabled", 1 );
    level._id_12355._id_133B7 = getdvarfloat( "scr_parachute_hint_xyvelscale_high", 0.0 );
    level._id_12355._id_133B8 = getdvarfloat( "scr_parachute_hint_xyvelscale_low", 0.0 );
    level._id_12355._id_133B9 = getdvarfloat( "scr_parachute_hint_xyvelscale_maxheight", 0.0 );
    level._id_12355._id_133B6 = getdvarfloat( "scr_parachute_hint_xylimit", 0.0 );
    level._id_12355._id_133CB = getdvarfloat( "scr_parachute_hint_zdrop", 0.0 );
    level._id_12355.zoffset = getdvarfloat( "scr_parachute_hint_zoffset", 0.0 );
    level._id_12355._id_13463 = getdvarfloat( "scr_parachute_hint_zvelscale", 0.0 );
    level._id_12355._id_133CC = getdvarfloat( "scr_parachute_hint_zlimit", 0.0 );
    level._id_12355.isplayerbrsquadleader = getdvarfloat( "scr_parachute_hint_falling_xyratio", 0.0 );
}
...
startfreefall( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    {
        thread last_molotov_throw_time( var_0, var_1, var_2, var_3, var_4, var_5 );
        return;
    }
    ...
    if ( level.gametype == "br" && ( !istrue( var_4 ) || ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) ) && getdvarint( "scr_infil_parachute_vfx", 1 ) )
        thread playerswithoutdismemberment( getdvarint( "scr_bmo_parachuteTouchdownVFX", 1 ) == 1 && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) );
...
playerswithoutdismemberment( var_0 )
{
    ...
    if ( isdefined( self.operatorcustomization ) && isdefined( self.operatorcustomization.col_circletick ) )
        var_2 = var_2 + self.operatorcustomization.col_circletick;
...
mine_launch_vfx()
{
    if ( isdefined( level.debug_showcardlocs ) )
        return level.debug_showcardlocs;
...
removepickup()
{
    return isdefined( self.isstunnedby ) && ( self.isstunnedby == 5 || self.isstunnedby == 6 );
}
...
startparachute()
{
    ...
    if ( isparachutegametype() )
    {
        if ( isdefined( level.totalcashongroundatstart ) )
            self [[ level.totalcashongroundatstart ]]( self );
    }
}
...
triggeravailablekillstreaks()
{
    ...
    {
        if ( isdefined( var_0.isstunnedby ) && ( var_0.isstunnedby == 1 || var_0.isstunnedby == 2 ) )
        {
            var_0 isspecialcaseweapon();
            var_0.isstunnedby = 3;
        }
    }
...
_id_12DEB()
{
    ...
    var_2 = clamp( var_0.origin[2], 0, level._id_12355._id_133B9 ) / level._id_12355._id_133B9;
    var_3 = level._id_12355._id_133B8 + ( level._id_12355._id_133B7 - level._id_12355._id_133B8 ) * var_2;
    var_4 = var_1 * ( 1, 1, 0 );
    var_5 = length( var_4 );
    var_6 = var_4 * var_3;
    var_7 = var_5 * var_3;

    if ( var_7 > level._id_12355._id_133B6 )
        var_6 = var_6 * ( level._id_12355._id_133B6 / var_7 );

    var_8 = 0;
    var_8 = var_8 - var_0.origin[2] * level._id_12355._id_133CB;
    var_8 = var_8 + level._id_12355.zoffset;
    var_8 = var_8 + var_1[2] * level._id_12355._id_13463;
    var_8 = clamp( var_8, -1 * level._id_12355._id_133CC, 0 );
    var_9 = 1;

    if ( var_5 < level._id_12355.isplayerbrsquadleader * var_1[2] )
        var_9 = 2;

    var_0 setadditionalstreampos( var_0.origin + ( var_6[0], var_6[1], var_8 ), 0, var_9 );
}

last_molotov_throw_time( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    self endon( "disconnect" );
    level endon( "game_ended" );
    ...
    var_6 = self;
    var_6.isstunnedby = 1;
    var_6.isspawningstopped = istrue( var_4 );
    var_6.isspecialistbonus = 0;
    var_6 issnipersemi( var_0, var_1, var_2, var_3, var_5 );
    var_6 isstandardsandbox();
    var_7 = gettime();

    for (;;)
    {
        if ( var_6 isskydiving() || var_7 + 2000 < gettime() || var_6 removepickup() )
            break;
    ...
    for (;;)
    {
        if ( level._id_12355._id_127DC && var_6.isspecialistbonus < gettime() )
        {
            var_6 _id_12DEB();
            var_6.isspecialistbonus = gettime() + 500;
        }

        if ( var_6 isinfreefall() && var_6.isstunnedby != 1 )
        {
            var_6 isstandardsandbox();
            var_6.isstunnedby = 1;
        }

        if ( var_6 isparachuting() && var_6.isstunnedby != 2 )
        {
            var_6 isspreadweapon();
            var_6.isstunnedby = 2;
        }

        if ( !var_6 isskydiving() && var_6.isstunnedby != 3 && var_6.isstunnedby != 4 && var_6.isstunnedby != 6 || var_6.isstunnedby == 5 )
        {
            var_6 isspecialcaseweapon();

            if ( var_6.isstunnedby != 5 )
                var_6.isstunnedby = 3;
            else
                var_6.isstunnedby = 6;
        }
        ...
        var_9 = var_6 isonground() && ( var_6.isstunnedby == 3 || var_6 removepickup() );
        var_10 = var_6 isonladder();
        var_11 = !scripts\mp\utility\player::isreallyalive( var_6 );

        if ( var_8 || var_9 || var_10 || var_11 )
        {
            var_6 isspecialiststreak();
            var_6.isstunnedby = undefined;
            var_6.isspawningstopped = undefined;
            var_6.isspecialistbonus = undefined;
...
issnipersemi( var_0, var_1, var_2, var_3, var_4 )
{
    self skydive_interrupt();
    ...
    if ( level.gametype == "br" && ( !istrue( self.isspawningstopped ) || getdvar( "scr_br_gametype", "" ) == "dmz" ) || getdvar( "scr_br_gametype", "" ) == "risk" )
        thread playerswithoutdismemberment( getdvarint( "scr_bmo_parachuteTouchdownVFX", 1 ) == 1 && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) );
...
isstandardsandbox()
{
    ...
    {
        self setclientomnvar( "ui_br_altimeter_state", 1 );
        level.allow_momentum[self getxuid()] = self;
    }

    if ( !self.isspawningstopped )
    {
        var_0 = "enabled";

        if ( isdefined( self.operatorcustomization ) && isdefined( self.operatorcustomization.col_circletick ) )
            var_0 = var_0 + self.operatorcustomization.col_circletick;
    }
}
...
isspreadweapon()
{
    ...
    if ( isdefined( self.riskspawn_flagcaptured ) && self.riskspawn_flagcaptured == 1 && var_0 != "truckwar" && var_0 != "reveal" && var_0 != "brdov" )
    {
    ...
    if ( isdefined( level.traceselectedmaplocation ) )
        self [[ level.traceselectedmaplocation ]]();
}

isspecialcaseweapon( var_0 )
{
    self.ignorefalldamagetime = gettime() + 5000;
    self.sat_wait_for_controller = gettime();
    ...
    level.allow_momentum[self getxuid()] = undefined;
    self notify( "parachute_complete" );
    self animscriptsetinputparamreplicationstatus( 0 );
}
