...
init()
{
    if ( !isdefined( level.handlefocusfire ) )
        level.handlefocusfire = getdvarint( "scr_game_disablespawncamera", 0 ) == 1;
...
startspawncamera( var_0, var_1, var_2 )
{
    ...
    scripts\mp\utility\player::_id_11965( "spawnCamera::startSpawnCamera() START" );
    scripts\mp\utility\player::hideminimap( 1 );
    gasmaskhealthperpip();

    if ( !isdefined( self.spawncameraent ) )
    {
        scripts\mp\utility\player::_id_11965( "spawnCamera::startSpawnCamera() UNDEFINED SPAWNCAMERAENT!!!" );
    ...
    scripts\engine\utility::_id_13147( 0.1, "force_spawn" );
    thread removecameraondisconnect( self.spawncameraent );
    self cameralinkto( self.spawncameraent, "tag_origin", 1, 1 );
    scripts\mp\utility\player::_id_11965( "spawnCamera::startSpawnCamera() CameraLinkTo()" );
...
getspawncamera()
{
    ...
    else if ( istrue( level.evaluatespawnforcameraselection ) && isdefined( level.scriptablenousestate ) && isdefined( level.scriptablenousestate[self.team] ) )
    {
        var_14 = level.scriptablenousestate[self.team];
        var_15 = undefined;
        var_16 = undefined;
...
snaptospawncamera()
{
    ...
    while ( !istrue( self.destroyaward ) )
        waitframe();
    ...
    scripts\mp\utility\player::_id_11965( "spawnCamera::snapToSpawnCamera()" );
...
{
    self endon( "disconnect" );
    scripts\mp\utility\player::_id_11965( "spawnCamera::runSlamZoomOnSpawn() START" );
    self waittill( "spawncamera_start" );
    scripts\mp\utility\player::_id_11965( "spawnCamera::runSlamZoomOnSpawn() spawncamera_start PASSED" );
    ...
    scripts\mp\utility\player::_id_11965( "spawnCamera::runSlamZoomOnSpawn() CameraLinkTo()" );
    handlemovetoblended();
    deletespawncamera();
    ...
    scripts\mp\utility\player::_id_11965( "spawnCamera::runSlamZoomOnSpawn() COMPLETE" );
}
...
handlemovetoblended( var_0 )
{
    ...
    scripts\mp\utility\player::_id_11965( "spawnCamera::handleMoveTo() START" );
    ...
    if ( !var_3 || !var_4 )
    {
        ...
        scripts\mp\utility\player::_id_11965( "spawnCamera::handleMoveTo() FULL Blend Set" );
    ...
    else
    {
        ...
        scripts\mp\utility\player::_id_11965( "spawnCamera::handleMoveTo() FIRST HALF Blend Set" );
        ...
        scripts\mp\utility\player::_id_11965( "spawnCamera::handleMoveTo() SECOND HALF Blend Set" );
        wait 0.3;
        wait 0.2;
    }

    scripts\mp\utility\player::_id_11965( "spawnCamera::handleMoveTo() COMPLETE" );
...
fadeblackforgeo( var_0 )
{
    ...
    if ( istrue( self.spawningintovehicle ) || scripts\cp_mp\utility\game_utility::isnightmap() )
    {
        ...
        if ( isdefined( self._id_1300B ) && self._id_1300B == "light_tank" )
        {
...
startoperatorsound()
{
    ...
    self.trap_toggle_logic = spawn( "script_origin", ( 0, 0, 0 ) );
    self.trap_toggle_logic showonlytoplayer( self );
    ...
    self.trap_trigger_logic = var_1;
    self.trap_toggle_logic playloopsound( var_1 );
    thread init_relic_ammo_drain();
}

init_relic_ammo_drain()
{
    self endon( "spawned_player" );
    var_0 = self.trap_toggle_logic;
    var_1 = self.trap_trigger_logic;
    self waittill( "disconnect" );
