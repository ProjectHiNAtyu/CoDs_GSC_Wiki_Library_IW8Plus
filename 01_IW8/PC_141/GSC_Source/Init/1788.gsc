...
_id_12190()
{
    level._id_122E3 = getdvarint( "MONKPPPQR", 0 );
    level.lb_impulse_dmg_factor_mid_high = ::getdismembermentlist;
    level.lb_impulse_dmg_threshold_low = ::getdropbagdelay;
    level.lb_impulse_dmg_factor_mid_low = ::getdomneutralizeomnvarvalue;
}
...
getdismembermentlist()
{
    wait 0.1;
    waittillframeend;
    level.keypadsetup = getent( "mp_lobby_floor_01", "targetname" );
    level.keypadsetup.extractquest_missionitem = getentarray( "floor_01_clutter", "targetname" );
    level.keypadsetup.lights = getentarray( "floor_01_lights", "script_noteworthy" );
    level.kickafkplayer = getent( "mp_lobby_floor_02", "targetname" );
    level.kickafkplayer.extractquest_missionitem = getentarray( "floor_02_clutter", "targetname" );
    level.kickafkplayer.lights = getentarray( "floor_02_lights", "script_noteworthy" );

    if ( level.keypadsetup.extractquest_missionitem.size )
        scripts\engine\utility::array_call( level.keypadsetup.extractquest_missionitem, ::linkto, level.keypadsetup );

    if ( level.kickafkplayer.extractquest_missionitem.size )
        scripts\engine\utility::array_call( level.kickafkplayer.extractquest_missionitem, ::linkto, level.kickafkplayer );

    var_0 = scripts\engine\utility::array_combine( level.keypadsetup.lights, level.kickafkplayer.lights );

    foreach ( var_2 in var_0 )
        var_2.originalpos = var_2.origin;

    var_4 = level.keypadsetup.origin;
    var_5 = level.kickafkplayer.origin;
    ...
    for (;;)
    {
        if ( var_11 )
        {
            var_12 = level.kickafkplayer;
            var_13 = level.keypadsetup;
        }
        else
        {
            var_12 = level.keypadsetup;
            var_13 = level.kickafkplayer;
        }
...
getdomneutralizeomnvarvalue()
{
    ...
    level.stay_on_roof_until_bothered_internal = [];
        ...
        foreach ( var_7 in var_5 )
        {
            ...
            if ( isdefined( var_7.script_noteworthy ) )
            {
                ...
                if ( scripts\engine\utility::array_contains_key( level._effect, var_9[0] ) )
                {
                    ...
                    var_10 = level.stay_on_roof_until_bothered_internal.size;
                    level.stay_on_roof_until_bothered_internal[var_10] = scripts\engine\utility::spawn_tag_origin();
                    level.stay_on_roof_until_bothered_internal[var_10].origin = var_7.origin;

                    if ( isdefined( var_7.angles ) )
                        level.stay_on_roof_until_bothered_internal[var_10].angles = var_7.angles;

                    level.stay_on_roof_until_bothered_internal[var_10].fxname = var_9[0];
                    level.stay_on_roof_until_bothered_internal[var_10] show();

                    if ( isdefined( var_7.script_label ) && var_7.script_label == "non_moving" )
                        continue;

                    var_11 = scripts\engine\utility::getclosest( var_7.origin, var_4 );
                    level.stay_on_roof_until_bothered_internal[var_10] linkto( var_11 );
                }
                else
    ...
    if ( level.stay_on_roof_until_bothered_internal.size )
    {
        foreach ( var_15 in level.stay_on_roof_until_bothered_internal )
...
{
    level thread [[ level.lb_impulse_dmg_factor_mid_high ]]();
}

laststand_dogtags()
{
    level thread [[ level.lb_impulse_dmg_threshold_low ]]();
}

laststand_dogtag_monitor()
{
    level thread [[ level.lb_impulse_dmg_factor_mid_low ]]();
}
...
_id_12DAA()
{
    if ( isdefined( level.active_scene_data._id_125FB ) )
        setdvar( "LTQMSPKRKO", level.active_scene_data._id_125FB );
    else
...
_id_12DAB()
{
    if ( isdefined( level.active_scene_data._id_125FC ) )
        setdvar( "LLNMKLQQP", level.active_scene_data._id_125FC );
    else
        setdvar( "LLNMKLQQP", 2 );
}
    ...
    switch ( var_0 )
    {
        case "small":
            foreach ( var_2 in level.ui_bg_images_2d )
                var_2.origin = level._id_11ABE.origin;

            break;
        case "medium":
            foreach ( var_2 in level.ui_bg_images_2d )
                var_2.origin = level._id_11ABD.origin;

            break;
        case "large":
            foreach ( var_2 in level.ui_bg_images_2d )
                var_2.origin = level._id_11ABC.origin;

            break;
        case "watch":
            foreach ( var_2 in level.ui_bg_images_2d )
                var_2.origin = level._id_11ABF.origin;
...
get_section_state( var_0 )
{
    ...
    switch ( var_0.name )
    {
        ...
        case "character_preview_select":
            var_1["scene"] = level.delete_ent;
            var_1["camera"] = level.delete_ent.basecam;
            break;
        case "character_preview_select_detail":
            var_1["scene"] = level.delete_entarray;
            var_1["camera"] = level.delete_entarray.basecam;
            break;
        ...
        case "character_gamebattles":
            var_1["scene"] = level.delete_enemies_if_reaching_max_ai;
            var_1["camera"] = level.delete_enemies_if_reaching_max_ai.basecam;
        ...
        case "loadout_showcase_preview_barrel_alt2":
            var_1["scene"] = level.delete_exfil_ai_structs;
            var_1["camera"] = level.delete_exfil_ai_structs.basecam;
            break;
        ...
        case "loadout_showcase_preview_charm_alt4":
            var_1["scene"] = level.delete_fan_blades;
            var_1["camera"] = level.delete_fan_blades.basecam;
            break;
        case "loadout_showcase_preview_charm_alt5":
            var_1["scene"] = level.delete_furthest_respawn_enemy;
            var_1["camera"] = level.delete_furthest_respawn_enemy.basecam;
            break;
        case "loadout_showcase_preview_charm_alt6":
            var_1["scene"] = level.delete_headicon;
            var_1["camera"] = level.delete_headicon.basecam;
            break;
        case "loadout_showcase_preview_small_charm_alt1":
            var_1["scene"] = level.delete_on_death_or_dissconnect;
            var_1["camera"] = level.delete_on_death_or_dissconnect.basecam;
            break;
        ...
        case "loadout_showcase_preview_sticker":
            var_1["scene"] = level.delete_pipe_ents;
            var_1["camera"] = level.delete_pipe_ents.basecam;
            break;
        case "loadout_showcase_preview_sticker_alt1":
            var_1["scene"] = level.delete_players_black_screen;
            var_1["camera"] = level.delete_players_black_screen.basecam;
            break;
        case "loadout_showcase_preview_sticker_alt2":
            var_1["scene"] = level.delete_race;
            var_1["camera"] = level.delete_race.basecam;
            break;
        case "loadout_showcase_preview_sticker_alt3":
            var_1["scene"] = level.delete_script_object;
            var_1["camera"] = level.delete_script_object.basecam;
            break;
        case "loadout_showcase_preview_sticker_alt4":
            var_1["scene"] = level.delete_silo_lights;
            var_1["camera"] = level.delete_silo_lights.basecam;
            break;
        ...
        case "loadout_showcase_preview_large_charm_alt3":
            var_1["scene"] = level.delete_headicon_on_death;
            var_1["camera"] = level.delete_headicon_on_death.basecam;
            break;
        case "loadout_showcase_preview_large_charm_alt4":
            var_1["scene"] = level.delete_intro_lights;
            var_1["camera"] = level.delete_intro_lights.basecam;
            break;
        case "loadout_showcase_preview_large_charm_alt5":
            var_1["scene"] = level.delete_intro_lights_after_delay;
            var_1["camera"] = level.delete_intro_lights_after_delay.basecam;
            break;
        case "loadout_showcase_preview_large_charm_alt6":
            var_1["scene"] = level.camera_loadout_showcase_preview_large_charm_alt6;
            var_1["camera"] = level.camera_loadout_showcase_preview_large_charm_alt6.basecam;
            break;
        ...
        case "loadout_showcase_preview_large_laser_alt1":
            var_1["scene"] = level.delete_keypad_display_models;
            var_1["camera"] = level.delete_keypad_display_models.basecam;
            break;
        ...
        case "loadout_showcase_preview_large_optic_alt1":
            var_1["scene"] = level.delete_laser_entities;
            var_1["camera"] = level.delete_laser_entities.basecam;
            break;
        case "loadout_showcase_preview_large_optic_alt2":
            var_1["scene"] = level.delete_light;
            var_1["camera"] = level.delete_light.basecam;
            break;
        ...
        case "loadout_showcase_preview_large_sticker":
            var_1["scene"] = level.delete_me;
            var_1["camera"] = level.delete_me.basecam;
            break;
        case "loadout_showcase_preview_large_sticker_alt1":
            var_1["scene"] = level.delete_name_fx;
            var_1["camera"] = level.delete_name_fx.basecam;
            break;
        case "loadout_showcase_preview_large_sticker_alt2":
            var_1["scene"] = level.delete_objective_on_death;
            var_1["camera"] = level.delete_objective_on_death.basecam;
            break;
        case "loadout_showcase_preview_large_sticker_alt3":
            var_1["scene"] = level.delete_objective_on_death_safe;
            var_1["camera"] = level.delete_objective_on_death_safe.basecam;
            break;
        ...
        case "loadout_showcase_preview_large_stock_alt1":
            var_1["scene"] = level.delete_old_gate;
            var_1["camera"] = level.delete_old_gate.basecam;
            break;
        ...
        case "loadout_showcase_preview_small_laser_alt1":
            var_1["scene"] = level.delete_on_exit_icon_trigger_pre_race;
            var_1["camera"] = level.delete_on_exit_icon_trigger_pre_race.basecam;
            break;
        ...
        case "loadout_showcase_preview_small_reargrip_alt1":
            var_1["scene"] = level.delete_on_track_delete;
            var_1["camera"] = level.delete_on_track_delete.basecam;
            break;
        case "loadout_showcase_preview_small_sticker":
            var_1["scene"] = level.delete_on_unloaded;
            var_1["camera"] = level.delete_on_unloaded.basecam;
            break;
        ...
        case "loadout_showcase_armory":
            var_1["scene"] = level.delete_ents_to_clean_up;
            var_1["camera"] = level.delete_ents_to_clean_up.basecam;
            break;
...
create_camera_position_list()
{
    ...
    level.camera_loadout_showcase._id_125FB = 6;
    ...
    level.camera_loadout_showcase_overview._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_watch._id_125FB = 6;
    level.delete_ents_to_clean_up = spawnstruct();
    level.delete_ents_to_clean_up.basecam = getent( "camera_mp_gunsmith_armory", "targetname" );
    level.delete_ents_to_clean_up.basecam.depthoffieldvalues = [ 7, 67 ];
    level.delete_ents_to_clean_up.myfov = 44;
    level.delete_ents_to_clean_up.weapon_locs[0] = getent( "weapon_loc_hq1", "targetname" );
    level.delete_ents_to_clean_up.weapon_locs[1] = getent( "weapon_loc_hq2", "targetname" );
    level.delete_ents_to_clean_up.weapon_locs[3] = getent( "weapon_loc_watch", "targetname" );
    level.delete_ents_to_clean_up._id_125FB = 6;
    ...
    level.camera_loadout_showcase_p._id_125FB = 6;
    ...
    level.camera_loadout_showcase_p_large._id_125FB = 6;
    ...
    level.camera_loadout_showcase_s._id_125FB = 6;
    ...
    level.camera_loadout_showcase_o._id_125FB = 6;
    ...
    level.camera_loadout_showcase_o_large._id_125FB = 6;
    ...
    level.camera_loadout_showcase_l._id_125FB = 6;
    ...
    level.camera_loadout_showcase_t._id_125FB = 6;
    ...
    level.camera_loadout_showcase_perks._id_125FB = 6;
    ...
    level.camera_loadout_showcase_x._id_125FB = 6;
    ...
    level.camera_loadout_showcase_y._id_125FB = 6;
    ...
    level.camera_loadout_showcase_z._id_125FB = 6;
    ...
    level.camera_loadout_showcase_specialist._id_125FB = 6;
    ...
    level.camera_loadout_showcase_watch._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_riot._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_barrel._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_barrel_alt1._id_125FB = 6;
    level.delete_exfil_ai_structs = spawnstruct();
    level.delete_exfil_ai_structs.basecam = getent( "camera_mp_gunsmith_preview_barrel_alt2", "targetname" );
    level.delete_exfil_ai_structs.basecam.depthoffieldvalues = [ 8, 54 ];
    level.delete_exfil_ai_structs.myfov = 36;
    level.delete_exfil_ai_structs.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_exfil_ai_structs._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_charm._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_charm_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_charm_alt2._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_charm_alt3._id_125FB = 6;
    level.delete_fan_blades = spawnstruct();
    level.delete_fan_blades.basecam = getent( "camera_mp_gunsmith_preview_charm_alt4", "targetname" );
    level.delete_fan_blades.basecam.depthoffieldvalues = [ 21, 18 ];
    level.delete_fan_blades.myfov = 36;
    level.delete_fan_blades.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_fan_blades._id_125FB = 6;
    level.delete_furthest_respawn_enemy = spawnstruct();
    level.delete_furthest_respawn_enemy.basecam = getent( "camera_mp_gunsmith_preview_charm_alt5", "targetname" );
    level.delete_furthest_respawn_enemy.basecam.depthoffieldvalues = [ 22, 18 ];
    level.delete_furthest_respawn_enemy.myfov = 36;
    level.delete_furthest_respawn_enemy.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_furthest_respawn_enemy._id_125FB = 6;
    level.delete_headicon = spawnstruct();
    level.delete_headicon.basecam = getent( "camera_mp_gunsmith_preview_charm_alt6", "targetname" );
    level.delete_headicon.basecam.depthoffieldvalues = [ 21, 16 ];
    level.delete_headicon.myfov = 36;
    level.delete_headicon.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_headicon._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_laser._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_laser_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_laser_alt2._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_magazine._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_magazine_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_magazine_alt2._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_muzzle._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_muzzle_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_optic._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_optic_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_reargrip._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_reargrip_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_reargrip_alt2._id_125FB = 6;
    level.delete_pipe_ents = spawnstruct();
    level.delete_pipe_ents.basecam = getent( "camera_mp_gunsmith_preview_sticker", "targetname" );
    level.delete_pipe_ents.basecam.depthoffieldvalues = [ 20, 32 ];
    level.delete_pipe_ents.myfov = 36;
    level.delete_pipe_ents.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_pipe_ents._id_125FB = 6;
    level.delete_players_black_screen = spawnstruct();
    level.delete_players_black_screen.basecam = getent( "camera_mp_gunsmith_preview_sticker_alt1", "targetname" );
    level.delete_players_black_screen.basecam.depthoffieldvalues = [ 20, 32 ];
    level.delete_players_black_screen.myfov = 36;
    level.delete_players_black_screen.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_players_black_screen._id_125FB = 6;
    level.delete_race = spawnstruct();
    level.delete_race.basecam = getent( "camera_mp_gunsmith_preview_sticker_alt2", "targetname" );
    level.delete_race.basecam.depthoffieldvalues = [ 20, 34 ];
    level.delete_race.myfov = 36;
    level.delete_race.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_race._id_125FB = 6;
    level.delete_script_object = spawnstruct();
    level.delete_script_object.basecam = getent( "camera_mp_gunsmith_preview_sticker_alt3", "targetname" );
    level.delete_script_object.basecam.depthoffieldvalues = [ 20, 34 ];
    level.delete_script_object.myfov = 36;
    level.delete_script_object.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_script_object._id_125FB = 6;
    level.delete_silo_lights = spawnstruct();
    level.delete_silo_lights.basecam = getent( "camera_mp_gunsmith_preview_sticker_alt4", "targetname" );
    level.delete_silo_lights.basecam.depthoffieldvalues = [ 20, 34 ];
    level.delete_silo_lights.myfov = 36;
    level.delete_silo_lights.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_silo_lights._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_stock._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_stock_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_stock_alt2._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_underbarrel._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_barrel._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_charm._id_125FB = 6;
    level.delete_on_death_or_dissconnect = spawnstruct();
    level.delete_on_death_or_dissconnect.basecam = getent( "camera_mp_gunsmith_preview_small_charm_alt1", "targetname" );
    level.delete_on_death_or_dissconnect.basecam.depthoffieldvalues = [ 25, 19 ];
    level.delete_on_death_or_dissconnect.myfov = 36;
    level.delete_on_death_or_dissconnect.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_on_death_or_dissconnect._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_laser._id_125FB = 6;
    level.delete_on_exit_icon_trigger_pre_race = spawnstruct();
    level.delete_on_exit_icon_trigger_pre_race.basecam = getent( "camera_mp_gunsmith_preview_small_laser_alt1", "targetname" );
    level.delete_on_exit_icon_trigger_pre_race.basecam.depthoffieldvalues = [ 8, 25 ];
    level.delete_on_exit_icon_trigger_pre_race.myfov = 36;
    level.delete_on_exit_icon_trigger_pre_race.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_on_exit_icon_trigger_pre_race._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_magazine._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_magazine_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_muzzle._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_optic._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_reargrip._id_125FB = 6;
    level.delete_on_track_delete = spawnstruct();
    level.delete_on_track_delete.basecam = getent( "camera_mp_gunsmith_preview_small_reargrip_alt1", "targetname" );
    level.delete_on_track_delete.basecam.depthoffieldvalues = [ 12, 20 ];
    level.delete_on_track_delete.myfov = 36;
    level.delete_on_track_delete.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_on_track_delete._id_125FB = 6;
    level.delete_on_unloaded = spawnstruct();
    level.delete_on_unloaded.basecam = getent( "camera_mp_gunsmith_preview_small_sticker", "targetname" );
    level.delete_on_unloaded.basecam.depthoffieldvalues = [ 16, 22 ];
    level.delete_on_unloaded.myfov = 36;
    level.delete_on_unloaded.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_on_unloaded._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_stock._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_small_trigger._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_barrel._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_charm._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_charm_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_charm_alt2._id_125FB = 6;
    level.delete_headicon_on_death = spawnstruct();
    level.delete_headicon_on_death.basecam = getent( "camera_mp_gunsmith_preview_large_charm_alt3", "targetname" );
    level.delete_headicon_on_death.basecam.depthoffieldvalues = [ 25, 19 ];
    level.delete_headicon_on_death.myfov = 36;
    level.delete_headicon_on_death.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_headicon_on_death._id_125FB = 6;
    level.delete_intro_lights = spawnstruct();
    level.delete_intro_lights.basecam = getent( "camera_mp_gunsmith_preview_large_charm_alt4", "targetname" );
    level.delete_intro_lights.basecam.depthoffieldvalues = [ 21, 16 ];
    level.delete_intro_lights.myfov = 36;
    level.delete_intro_lights.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_intro_lights._id_125FB = 6;
    level.delete_intro_lights_after_delay = spawnstruct();
    level.delete_intro_lights_after_delay.basecam = getent( "camera_mp_gunsmith_preview_large_charm_alt5", "targetname" );
    level.delete_intro_lights_after_delay.basecam.depthoffieldvalues = [ 21, 17 ];
    level.delete_intro_lights_after_delay.myfov = 36;
    level.delete_intro_lights_after_delay.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_intro_lights_after_delay._id_125FB = 6;
    level.camera_loadout_showcase_preview_large_charm_alt6 = spawnstruct();
    level.camera_loadout_showcase_preview_large_charm_alt6.basecam = getent( "camera_mp_gunsmith_preview_large_charm_alt6", "targetname" );
    level.camera_loadout_showcase_preview_large_charm_alt6.basecam.depthoffieldvalues = [ 21, 17 ];
    level.camera_loadout_showcase_preview_large_charm_alt6.myfov = 36;
    level.camera_loadout_showcase_preview_large_charm_alt6.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.camera_loadout_showcase_preview_large_charm_alt6._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_barrel_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_laser._id_125FB = 6;
    level.delete_keypad_display_models = spawnstruct();
    level.delete_keypad_display_models.basecam = getent( "camera_mp_gunsmith_preview_large_laser_alt1", "targetname" );
    level.delete_keypad_display_models.basecam.depthoffieldvalues = [ 12, 26 ];
    level.delete_keypad_display_models.myfov = 36;
    level.delete_keypad_display_models.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_keypad_display_models._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_magazine._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_magazine_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_magazine_alt2._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_muzzle._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_muzzle_alt1._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_optic._id_125FB = 6;
    level.delete_laser_entities = spawnstruct();
    level.delete_laser_entities.basecam = getent( "camera_mp_gunsmith_preview_large_optic_alt1", "targetname" );
    level.delete_laser_entities.basecam.depthoffieldvalues = [ 21.5, 25 ];
    level.delete_laser_entities.myfov = 36;
    level.delete_laser_entities.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_laser_entities._id_125FB = 6;
    level.delete_light = spawnstruct();
    level.delete_light.basecam = getent( "camera_mp_gunsmith_preview_large_optic_alt2", "targetname" );
    level.delete_light.basecam.depthoffieldvalues = [ 21.5, 25 ];
    level.delete_light.myfov = 36;
    level.delete_light.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_light._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_reargrip._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_reargrip_alt1._id_125FB = 6;
    level.delete_me = spawnstruct();
    level.delete_me.basecam = getent( "camera_mp_gunsmith_preview_large_sticker", "targetname" );
    level.delete_me.basecam.depthoffieldvalues = [ 20, 32 ];
    level.delete_me.myfov = 36;
    level.delete_me.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_me._id_125FB = 6;
    level.delete_name_fx = spawnstruct();
    level.delete_name_fx.basecam = getent( "camera_mp_gunsmith_preview_large_sticker_alt1", "targetname" );
    level.delete_name_fx.basecam.depthoffieldvalues = [ 20, 35 ];
    level.delete_name_fx.myfov = 36;
    level.delete_name_fx.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_name_fx._id_125FB = 6;
    level.delete_objective_on_death = spawnstruct();
    level.delete_objective_on_death.basecam = getent( "camera_mp_gunsmith_preview_large_sticker_alt2", "targetname" );
    level.delete_objective_on_death.basecam.depthoffieldvalues = [ 20, 32 ];
    level.delete_objective_on_death.myfov = 36;
    level.delete_objective_on_death.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_objective_on_death._id_125FB = 6;
    level.delete_objective_on_death_safe = spawnstruct();
    level.delete_objective_on_death_safe.basecam = getent( "camera_mp_gunsmith_preview_large_sticker_alt3", "targetname" );
    level.delete_objective_on_death_safe.basecam.depthoffieldvalues = [ 20, 35 ];
    level.delete_objective_on_death_safe.myfov = 36;
    level.delete_objective_on_death_safe.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_objective_on_death_safe._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_stock._id_125FB = 6;
    level.delete_old_gate = spawnstruct();
    level.delete_old_gate.basecam = getent( "camera_mp_gunsmith_preview_large_stock_alt1", "targetname" );
    level.delete_old_gate.basecam.depthoffieldvalues = [ 16, 43 ];
    level.delete_old_gate.myfov = 36;
    level.delete_old_gate.weapon_locs[2] = getent( "weapon_loc_preview", "targetname" );
    level.delete_old_gate._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_underbarrel._id_125FB = 6;
    ...
    level.camera_loadout_showcase_preview_large_underbarrel_alt1._id_125FB = 6;
    ...
    level.camera_character_tango._id_125FB = 6;
    ...
    level.camera_character_faction_select_l.update_char_loc = ::_id_12DA0;
    level.camera_character_faction_select_l._id_125FB = 6;
    ...
    level.camera_character_faction_select_l_detail.update_char_loc = ::_id_12DA0;
    level.camera_character_faction_select_l_detail._id_125FB = 6;
    ...
    level.camera_character_faction_select_r.update_char_loc = ::_id_12D9F;
    level.camera_character_faction_select_r.char_index = 1;
    level.camera_character_faction_select_r._id_125FB = 6;
    ...
    level.camera_character_faction_select_r_detail.update_char_loc = ::_id_12D9F;
    level.camera_character_faction_select_r_detail.char_index = 1;
    level.camera_character_faction_select_r_detail._id_125FB = 6;
    level.delete_ent = spawnstruct();
    level.delete_ent.basecam = getent( "char_preview_detail", "targetname" );
    level.delete_ent.basecam.depthoffieldvalues = [ 22, 60 ];
    level.delete_ent.myfov = 37;
    level.delete_ent.char_loc = getent( "charroom_char_preview", "targetname" );
    level.delete_ent._id_125FB = 6;
    level.delete_entarray = spawnstruct();
    level.delete_entarray.basecam = getent( "char_preview", "targetname" );
    level.delete_entarray.basecam.depthoffieldvalues = [ 22, 181 ];
    level.delete_entarray.myfov = 37;
    level.delete_entarray.char_loc = getent( "charroom_char_preview", "targetname" );
    level.delete_entarray._id_125FB = 6;
    ...
    if ( isdefined( level.lastmovingplatform ) && level.lastmovingplatform == 1 )
        level.camera_lobby._id_125FB = 5;

    if ( isdefined( level.lastpropchangetime ) && level.lastpropchangetime == 1 || isdefined( level.lastplundereventtype ) && level.lastplundereventtype == 1 )
        level.camera_lobby._id_125FB = 8;

    if ( isdefined( level.lastpropchangetime ) && level.lastpropchangetime == 1 || isdefined( level.lastplundereventtype ) && level.lastplundereventtype == 1 )
        level.camera_lobby._id_125FC = 6;

    if ( isdefined( level.lastseentime ) && level.lastseentime == 1 )
        level.camera_lobby._id_125FB = 8;

    if ( isdefined( level.lastseentime ) && level.lastseentime == 1 )
        level.camera_lobby._id_125FC = 6;

    if ( isdefined( level.lastspawnvocallouttimes ) && level.lastspawnvocallouttimes == 1 )
        level.camera_lobby._id_125FB = 8;
    ...
    if ( isdefined( level.lastmovingplatform ) && level.lastmovingplatform == 1 )
        level.camera_lobby_detail._id_125FB = 5;

    if ( isdefined( level.lastpropchangetime ) && level.lastpropchangetime == 1 || isdefined( level.lastplundereventtype ) && level.lastplundereventtype == 1 )
        level.camera_lobby_detail._id_125FB = 8;

    if ( isdefined( level.lastpropchangetime ) && level.lastpropchangetime == 1 || isdefined( level.lastplundereventtype ) && level.lastplundereventtype == 1 )
        level.camera_lobby_detail._id_125FC = 6;
    ...
    level.delete_enemies_if_reaching_max_ai = spawnstruct();
    level.delete_enemies_if_reaching_max_ai.basecam = getent( "char_tournament_overcam", "targetname" );
    level.delete_enemies_if_reaching_max_ai.basecam.depthoffieldvalues = [ 22, 256 ];
    level.delete_enemies_if_reaching_max_ai.myfov = 36;
    level.delete_enemies_if_reaching_max_ai.update_char_loc = ::_id_12D86;
}
...
setup_initial_entities()
{
    ...
    for ( var_3 = 0; var_3 < 4; var_3++ )
    {
        ...
        level.characters[var_0] = spawn( "script_character", var_1.origin, 0, 0, var_0, "MPClientCharacter" );
        level.characters[var_0].relic_nuketimer = 1;
    }
...
set_warning_levels()
{
    if ( !isdefined( level.player_give_loadout ) )
    {
        level.player_give_loadout = loadfx( "vfx/iw8_mp/watches/vfx_mp_holo_watch" );
        wait 1;
    }

    level._id_1323F = spawnfx( level.player_give_loadout, level.weapons[3].origin );
    level._id_1323F.angles = ( 221, -34.753, 181.997 );
    level._id_1323F.origin = level._id_1323F.origin + ( 1, -0.66, 0.98 );
    waitframe();
    triggerfx( level._id_1323F );
}
...
startjuggdelivery( var_0 )
{
    ...
    var_4 = level._id_1323F.angles;
    var_5 = level._id_1323F.origin + ( var_1, var_2, var_3 );
    level._id_1323F delete();
    level._id_1323F = spawnfx( level.player_give_loadout, var_5 );
    level._id_1323F.angles = var_4;
    triggerfx( level._id_1323F );
}
...
_id_11DEA( var_0 )
{
    ...
    var_4 = level._id_1323F.angles + ( var_1, var_2, var_3 );
    level._id_1323F delete();
    level._id_1323F = spawnfx( level.player_give_loadout, level.weapons[3].origin + ( 1, -0.66, 1.1 ) );
    level._id_1323F.angles = var_4;
    triggerfx( level._id_1323F );
}
...
_id_121AF()
{
    ...
    level._id_11ABE = scripts\engine\utility::getstruct( "weapRaritySmall", "targetname" );
    level._id_11ABD = scripts\engine\utility::getstruct( "weapRarityMedium", "targetname" );
    level._id_11ABC = scripts\engine\utility::getstruct( "weapRarityLarge", "targetname" );
    level._id_11ABF = scripts\engine\utility::getstruct( "weapRarityWatch", "targetname" );
    devui_bg_swap( 0 );
}
