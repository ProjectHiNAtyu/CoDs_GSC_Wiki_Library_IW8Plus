...
supportbox_init()
{
    level.blockclasschange = [];
    scripts\common\interactive::interactive_addusedcallback( ::supportbox_usedcallback, "equip_supportBox" );
    scripts\common\interactive::interactive_addusedcallback( ::bot_spawned_this_round, "equip_armorBox" );
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_12853 );
...
_id_12853( var_0 )
{
    foreach ( var_2 in level.blockclasschange )
    {
        if ( isdefined( var_2 ) )
            var_2 _id_12851( var_0 );
    }
}

choosedropbagmodel( var_0, var_1, var_2, var_3 )
{
    var_0 endon( "death" );
    var_0 thread supportbox_watchdisownedtimeout();
    var_0 thread supportbox_hideandshowaftertime();
    var_0 thread _id_12854( self );

    if ( !scripts\mp\flags::gameflag( "prematch_fade_done" ) )
        var_0 thread supportbox_watchprematchend();
    ...
    level.blockclasschange[var_0 getentitynumber()] = var_0;
...
_id_12854( var_0 )
{
    ...
    var_1 = scripts\engine\utility::_id_13141( 2, "touching_platform" );
    ...
    if ( var_8["fraction"] < 1.0 )
    {
        ...
        if ( isdefined( var_2 ) )
        {
            if ( recondronetracecontents( var_2 ) )
                self.origin = var_0.origin;
        }
    }
}

supportbox_watchprematchend()
{
    self endon( "death" );
    self endon( "missile_stuck" );
...
{
    var_0 endon( "death" );
    var_0._id_129E9 = "vfx/iw8_mp/equipment/vfx_offhand_wm_supportbox_timeout.vfx";
    choosedropbagmodel( var_0, "equip_supportBox", "super_ammo_drop", "hud_icon_equipment_support_box" );
...
{
    var_0 endon( "death" );
    var_0._id_129E9 = "vfx/iw8_mp/equipment/vfx_offhand_wm_armorbox_timeout.vfx";
    choosedropbagmodel( var_0, "equip_armorBox", "super_armor_drop", "ui_mp_br_loot_icon_health_armor_box" );
...
supportbox_destroy( var_0 )
{
    ...
    if ( isdefined( self._id_129E9 ) )
        var_1 = self._id_129E9;
...
supportbox_delete( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( self.equipmentref ) && self.equipmentref == "equip_armorBox" )
        self.owner scripts\cp\helicopter\chopper_boss::total_successful_vehicle_spawns( "super_armor_drop", self.usedcount, var_0, var_2 );
    else
        self.owner scripts\cp\helicopter\chopper_boss::total_successful_vehicle_spawns( "super_ammo_drop", self.usedcount, var_0, var_2 );
...
supportbox_playercanuse( var_0 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::namehud() != "x2" )
    {
        if ( isdefined( self.owner ) && !scripts\engine\utility::array_contains( scripts\mp\gametypes\br_public::munitions_override_time( self.owner.team, self.owner.squadindex ), var_0 ) )
            return 0;
...
{
    self.ph_endgame = scripts\cp_mp\entityheadicons::setheadicon_singleimage( [], var_0, 20, 1, 1000, 100, undefined, 1 );
...
supportbox_givexpforuse( var_0 )
{
    if ( isdefined( self.owner ) && !scripts\cp_mp\utility\player_utility::playersareenemies( self.owner, var_0 ) )
    {
        if ( self.owner != var_0 )
        {
            ...
            self.owner scripts\cp\helicopter\chopper_boss::onsupportboxammousedbyteammate( self );
        }

        self.owner scripts\mp\utility\stats::incpersstat( "munitionsBoxUsed", 1 );
        self.owner scripts\mp\supers::fake_digit_pool( "super_ammo_drop" );
        self.usedcount++;
    }
}
...
supportbox_handlemovingplatforms( var_0 )
{
    ...
    if ( isdefined( var_0 ) && scripts\mp\equipment\claymore::recondronetracecontents( var_0 ) )
    {
...
_id_12851( var_0 )
{
    ...
    var_1 = self.ph_endgame;

    if ( !isdefined( var_1 ) )
        return;

    var_2 = self.ph_loadouts;

    if ( !isdefined( var_2 ) )
        return;

    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\menus::_id_1261E() && isdefined( self.owner ) )
        var_3 = scripts\engine\utility::array_contains( scripts\mp\gametypes\br_public::munitions_override_time( self.owner.team, self.owner.squadindex ), var_0 );
    else
    {
        if ( isdefined( self.owner ) )
    ...
    if ( var_3 && !var_5 )
    {
        scripts\cp_mp\entityheadicons::_id_1211F( var_1, var_0 );
        scripts\cp_mp\entityheadicons::_id_12120( var_2, var_0 );
    }
    else if ( var_3 )
    {
        scripts\cp_mp\entityheadicons::_id_12120( var_1, var_0 );
        scripts\cp_mp\entityheadicons::_id_1211F( var_2, var_0 );
    }
    else
    {
        scripts\cp_mp\entityheadicons::_id_12120( var_1, var_0 );
...
supportbox_usedcallback( var_0, var_1 )
{
    ...
    {
        if ( isdefined( level.train_elements_enable ) )
            var_2 = var_0 [[ level.train_elements_enable ]]( var_1 );
        else
        ...
        if ( var_2 )
        {
            ...
            var_0 _id_12851( var_1 );
            var_0 supportbox_givexpforuse( var_1 );
        }
    }
}

bot_spawned_this_round( var_0, var_1 )
{
    ...
    if ( var_0 supportbox_playercanuse( var_1 ) )
    {
        if ( isdefined( level.to ) )
            var_2 = var_0 [[ level.to ]]( var_1 );
        else
            var_2 = 0;

        if ( var_2 && !istrue( var_0._id_12D17 ) )
        {
            var_0.playersused[var_1 getentitynumber()] = var_1;
            var_0 _id_12851( var_1 );
            var_0 supportbox_givexpforuse( var_1 );
