...
init()
{
    ...
    scripts\mp\utility\spawn_event_aggregator::registeronplayerspawncallback( ::reload_use_think );
}
...
getspawnpoint()
{
    ...
    if ( isdefined( self.selectedspawnarea ) )
    {
        if ( issubstr( self.selectedspawnarea, "squad" ) )
        {
            ...
            if ( isdefined( var_3.vehicle ) )
            {
                ...
                self._id_1300B = var_3.vehicle.vehiclename;
            }
    ...
    if ( istrue( level.usesquadspawn ) && istrue( self.squadspawnconfirmed ) )
    {
        ...
        if ( isdefined( var_14 ) && isdefined( self.squadindex ) && self.team == var_14.team && self.squadindex == var_14.squadindex )
        {
            ...
            var_14 thread scripts\mp\utility\points::obj_pregame( "squad_spawn" );
            thread scripts\mp\utility\points::obj_pregame( "squad_spawn_self" );
            self._id_131BF = 1;
        }
    }
    ...
    for (;;)
    {
        ...
        foreach ( var_3 in level.availablespawnlocations[self.team] )
        {
            ...
            if ( isdefined( var_6.objectivekey ) )
                var_8 = istrue( var_6.playersetkeypadcodelengthindex );
            else if ( issubstr( var_3, "vehicle" ) )
            {
                ...
                {
                    var_8 = istrue( var_11.playersetkeypadcodelengthindex );
                    var_9 = pavelow_boss_hit_by_emp( var_11, self );
...
{
    return level.objectives[var_0].ownerteam != var_1 || level.objectives[var_0].stalemate || level.objectives[var_0].captureblocked || istrue( level.objectives[var_0]._id_12581 ) || level.objectives[var_0].claimteam != "none" && level.objectives[var_0].claimteam != var_1;
}

resetbreakertostate( var_0 )
{
    var_1 = gettime();
...
{
    var_0 endon( "death" );
    var_0.playersetkeypadcodelengthindex = 0;
    var_1 = getdvarfloat( "scr_gw_apc_ignore_damage_health_pct", 0.15 );

    for (;;)
    {
        var_2 = gettime();
        var_3 = istrue( var_0.death_killstreak_watcher );
        var_4 = scripts\cp_mp\utility\weapon_utility::islockedonto( var_0 );
        var_5 = scripts\cp_mp\vehicles\vehicle_occupancy::_id_12F79( var_0 );
        ...
        var_14 = var_0.playersetkeypadcodelengthindex;

        if ( var_6 || var_8 || var_9 || var_3 || var_4 || !var_5 )
            var_0.playersetkeypadcodelengthindex = 1;
        else
            var_0.playersetkeypadcodelengthindex = 0;
...
pavelow_boss_hit_by_emp( var_0, var_1 )
{
...
spawnselectionthink( var_0 )
{
    ...
    if ( istrue( level._id_122E8 ) )
        thread scripts\mp\gametypes\arm::_id_12FBC();

    if ( istrue( level._id_122E7 ) )
        thread scripts\mp\gametypes\arm::_id_12FBD();
...
watchforafk()
{
    ...
    if ( isdefined( level._id_12597 ) )
        wait( level._id_12597 );
    else
        wait 60.0;
    ...
    for (;;)
    {
        ...
        if ( var_0 == "tac_ops_spawn_area_selected" )
        {
            ...
            foreach ( var_4 in level.spawnselectionlocations )
            {
                ...
                {
                    _id_12E01( var_4[self.team].typeref, self, self.team );

                    if ( isdefined( var_4[self.team].objectivekey ) && isobjectiveindanger( var_4[self.team].objectivekey, self.team ) )
                        break;

                    if ( isdefined( var_4[self.team].typeref ) && issubstr( var_4[self.team].typeref, "vehicle" ) && istrue( var_4[self.team].dynamicent.playersetkeypadcodelengthindex ) )
                        break;
            ...
            if ( !isdefined( var_2 ) )
            {
                var_6 = munitions_override_waittill( self.team, self.squadindex );
                ...
                foreach ( var_4 in var_6 )
                {
                    if ( isdefined( var_4.dynamicent ) && var_4.dynamicent getentitynumber() == var_1 )
                    {
                        ...
                        var_9 = spawnstruct();
                        var_9 = scripts\mp\spawnscoring::dialog_monitor_shieldraise( var_9, var_4.dynamicent );

                        if ( !isdefined( var_9._id_12585 ) )
                        {
                            var_4.dynamicent.syringe_out = gettime();
                            break;
                        }
...
_id_12DF1( var_0 )
{
    if ( !isdefined( self.onspawn_fastspeed ) )
        self.onspawn_fastspeed = [];

    var_1 = 65536;
    var_2 = 131072;
    var_3 = 262144;
    var_4 = 524288;
    self.onspawn_fastspeed["rightmouseup"] = istrue( self.onspawn_fastspeed["rightmouseup"] ) || ( var_0 & var_1 ) > 0;
    self.onspawn_fastspeed["leftmouseup"] = istrue( self.onspawn_fastspeed["leftmouseup"] ) || ( var_0 & var_2 ) > 0;
    self.onspawn_fastspeed["activate"] = istrue( self.onspawn_fastspeed["activate"] ) || ( var_0 & var_3 ) > 0;
    self.onspawn_fastspeed["gostand"] = istrue( self.onspawn_fastspeed["gostand"] ) || ( var_0 & var_4 ) > 0;
...
watchendconditions()
{
    ...
    var_0 = scripts\engine\utility::_id_13140( "tac_ops_map_selection_valid", "tac_ops_map_changed", "tac_ops_map_cleared", "spawned_player", "tac_ops_map_game_ended", "tac_ops_map_nuke" );
...
endcondition_focuschanged( var_0 )
{
    ...
    foreach ( var_3 in level.spawnselectionlocations )
    {
        ...
        {
            thread rocket_fuel( var_3[self.team].anchorentity.origin );
    ...
    if ( !isdefined( var_1 ) )
    {
        var_5 = munitions_override_waittill( self.team, self.squadindex );

        foreach ( var_3 in var_5 )
        {
            if ( isdefined( var_3.dynamicent ) && var_3.dynamicent getentitynumber() == var_0 )
            {
                thread rocket_fuel( var_3.dynamicent.origin );
                var_1 = var_3.typeref;
                break;
            }
        }
...
rocket_fuel( var_0 )
{
    ...
    if ( !isdefined( level._id_13320 ) )
        level._id_13320 = max( getdvarint( "NOLPSOTORP", 20000 ) * 0.5 / 1000, 1.0 );

    for (;;)
    {
        self predictstreampos( var_0, 1 );
        wait( level._id_13320 );
    }
}
...
evaluatesquadspawn()
{
    ...
    for ( var_4 = 0; var_4 < 4; var_4++ )
    {
        ...
        var_5.playerplundereventomnvars = undefined;
        var_3[var_4] = var_5;
    }
    ...
    for (;;)
    {
        ...
        var_11 = munitions_override_waittill( self.team, self.squadindex );
        ...
        for ( var_4 = 0; var_4 < 4; var_4++ )
        {
            ...
            {
                var_15 = istrue( var_9.playersetkeypadcodelengthindex );
                var_16 = var_1[var_4];
                ...
                if ( !isdefined( var_3[var_4].playerplundereventomnvars ) || var_3[var_4].playerplundereventomnvars != var_16 )
                {
                    var_3[var_4].playerplundereventomnvars = var_16;
            ...
            if ( isdefined( var_3[var_4].playerplundereventomnvars ) && var_3[var_4].playerplundereventomnvars != -1 )
            {
                var_3[var_4].playerplundereventomnvars = -1;
...
handlesquadspawncycle()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_13135( "cyclePos", "cycleNeg" );
...
reload_use_think()
{
    thread quickdropremoveselfrevivetokenfrominventory();

    if ( getdvarint( "scr_squad_spawn_debug", 0 ) == 1 )
        thread scripts\mp\spawnscoring::_id_1262D();
}
...
quickdropremoveselfrevivetokenfrominventory()
{
    ...
    for (;;)
    {
        if ( issquadspawnable( self ) )
            self.playersetkeypadcodelengthindex = issquadmateindanger( self );
...
issquadmateindanger( var_0 )
{
    ...
    if ( isdefined( var_0.secondwindthink ) && var_0.secondwindthink + 3000 > var_1 )
        return 1;
    ...
    {
        if ( istrue( var_0.vehicle.death_killstreak_watcher ) )
            return 1;
    ...
    if ( isdefined( var_0 ) && isdefined( var_0.syringe_out ) && var_0.syringe_out + 1000 > var_1 )
        return 1;
...
timeuntilspawnmessaging( var_0, var_1 )
{
    ...
    if ( var_0 > 0 )
    {
        ...
        scripts\engine\utility::_id_13147( var_0, "force_spawn" );
    }
}
...
getclosestavailablespawnlocation()
{
    ...
    else
    {
        ...
        {
            var_7 = munitions_override_waittill( self.team, self.squadindex );
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            foreach ( var_3 in level.availablespawnlocations[var_1] )
            {
                ...
                if ( isdefined( var_4.objectivekey ) )
                    var_4.playersetkeypadcodelengthindex = isobjectiveindanger( var_4.objectivekey, var_1 );
            }
        }
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            ...
            foreach ( var_4 in level.availablespawnlocations[var_1] )
            {
                if ( !issubstr( var_4, "HQ" ) && issubstr( var_4, "fob" ) )
                {
                    ...
                    level.objectives[var_17]._id_12581 = var_14;
                    level.objectives[var_17]._id_12582 = gettime();
                }
            }
        }
...
_id_12E01( var_0, var_1, var_2 )
{
    ...
    if ( !isdefined( var_3 ) || !isdefined( level.objectives[var_3] ) || !isdefined( level.objectives[var_3]._id_12582 ) || level.objectives[var_3]._id_12582 == gettime() )
        return;
    ...
    level.objectives[var_3]._id_12581 = var_13;
    level.objectives[var_3]._id_12582 = gettime();
}
