...
init()
{
    ...
    level._id_12CEC = getuavstrengthmin();
    level._id_12CE8 = getuavstrengthmax();
    level._id_12CE9 = getuavstrengthlevelshowenemydirectional();
    level._id_12CEB = getuavstrengthlevelneutral();
    level._id_12CEA = getuavstrengthlevelshowenemyfastsweep();

    if ( !isdefined( level.uavsquadasteamenabled ) && scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "squadAsTeamEnabled" ) )
        level.uavsquadasteamenabled = level [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "squadAsTeamEnabled" ) ]]() && getdvarint( "scr_uav_for_squad_only", 1 );

    if ( level.teambased && !istrue( level.uavsquadasteamenabled ) )
    {
    ...
    level.allow_br_loot_to_br_marked = 0;
...
onplayerconnect()
{
    ...
    var_1 = level.teambased && istrue( level.uavsquadasteamenabled );
...
useuav( var_0, var_1 )
{
    ...
    scripts\cp\helicopter\chopper_boss::_id_11CB0( "t9_ch_global_call_in_uav_for_operator_mission", 1 );
    scripts\cp\helicopter\chopper_boss::_id_11CB0( "t9_ch_global_call_in_uav_for_operator_mission_op2", 1 );

    if ( getdvarint( "LRTSSKLKPK", 1 ) >= 8 )
        scripts\cp\helicopter\chopper_boss::_id_11CB0( "t9_ch_global_call_in_uav_for_operator_mission_op3", 1 );
        ...
        case "directional_uav":
            ...
            if ( level.teambased )
            {
                foreach ( var_7 in level.players )
                {
                    ...
                    {
                        if ( istrue( level.uavsquadasteamenabled ) && var_7.squadindex != var_3 )
                            continue;
...
launchuav( var_0, var_1, var_2, var_3 )
{
    ...
    if ( level.gametype == "br" )
        var_0 _id_12CE4( var_4, 15000, var_1 );
    ...
    var_0 thread scripts\cp_mp\utility\killstreak_utility::playkillstreakoperatordialog( "use_" + var_2.streakname, 1, var_13 );
    ...
    if ( isent( var_7 ) )
    {
        ...
        var_14 = randomintrange( 6000, 6500 );

        if ( var_7.uavtype == "directional_uav" || var_7.uavtype == "harp" )
            var_14 = randomintrange( 30000, 31000 );

        if ( isdefined( level.spawnpoints ) )
            var_15 = level.spawnpoints;
        else
            var_15 = level.startspawnpoints;

        if ( !isdefined( var_15 ) )
        {
            var_16 = spawnstruct();
            var_16.origin = ( var_0.origin[0], var_0.origin[1], 6969 );
            var_15 = [ var_16 ];
        }
        ...
        {
            if ( var_16.origin[2] < var_17.origin[2] )
                var_17 = var_16;
        }

        var_20 = var_17.origin[2];
        var_21 = var_6.origin[2];

        if ( var_20 < 0 )
        {
            var_21 = var_21 + var_20 * -1;
            var_20 = 0;
        }

        var_22 = randomint( 360 );
        var_23 = randomint( 1000 );

        if ( var_7.uavtype == "directional_uav" || var_7.uavtype == "harp" )
            var_23 = randomintrange( 20000, 22000 );

        var_24 = var_23 + 4000;
        var_25 = cos( var_22 ) * var_24;
        var_26 = sin( var_22 ) * var_24;
        var_27 = vectornormalize( ( var_25, var_26, var_14 ) );
        var_27 = var_27 * var_14;

        if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "getGameType" ) )
        {
            var_28 = [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "getGameType" ) ]]();
        ...
        var_7 linkto( var_6, "tag_origin", var_27, ( 0, var_22 - 90, 0 ) );
    ...
    if ( isdefined( level.activeuavs[var_4] ) && level.activeuavs[var_4] > 0 )
    {
        if ( isent( var_7 ) )
        {
            foreach ( var_31 in level.uavmodels[var_4] )
            {
                if ( var_31 == var_7 )
                    continue;

                if ( isdefined( var_31.timetoadd ) )
                    var_31.timetoadd = var_31.timetoadd + 5;
            }
        }
        else
    ...
    if ( isdefined( var_7 ) && var_7.damagetaken < var_7.maxhealth )
    {
        ...
        {
            var_7 unlink();
            var_7.give_respawn_flare = var_7.origin + anglestoforward( var_7.angles ) * 50000;
            var_7 moveto( var_7.give_respawn_flare, 50 );
        ...
        if ( isdefined( var_7 ) && var_7.damagetaken < var_7.maxhealth )
        {
            ...
            if ( isent( var_7 ) )
                var_7 moveto( var_7.give_respawn_flare, 15 );
    ...
    if ( isdefined( var_7 ) )
    {
        ...
        var_7.streakinfo.is_in_gas = !istrue( var_7.leftplayspace );

        if ( !istrue( self._id_11B42 ) )
            var_7.owner scripts\cp_mp\utility\killstreak_utility::_id_11B45( var_7.streakinfo );
    ...
    if ( var_1 == "directional_uav" )
    {
        ...
        if ( level.teambased )
        {
            foreach ( var_34 in level.players )
            {
                if ( isdefined( var_34 ) && var_34.pers["team"] == var_4 )
                {
                    if ( istrue( level.uavsquadasteamenabled ) && var_34.squadindex != var_5 )
                        continue;

                    var_34.radarshowenemydirection = 0;
                }
            }
        }
    }
...
{
    self endon( "death" );
    self.owner scripts\engine\utility::_id_1312D( "disconnect", "joined_team" );
...
restorestrengthafterhostmigration()
{
    ...
    for (;;)
    {
        ...
        if ( level.teambased )
        {
            ...
            {
                if ( istrue( level.uavsquadasteamenabled ) )
                {
                    foreach ( var_5, var_3 in level.squaddata[var_1] )
                    {
                        var_4 = _getsquadradarstrength( var_5 );
                        _setsquadradarstrength( var_1, var_5, var_4 );
                    }

                    continue;
                }

                var_4 = _getteamradarstrength( var_1 );
                _setteamradarstrength( var_1, var_4 );
            }
        }
    }
}
                ...
                if ( istrue( level.uavsquadasteamenabled ) )
                {
                    if ( isdefined( level.squaddata ) )
                    {
                        foreach ( var_4, var_3 in level.squaddata[var_1] )
                            updatesquaduavstatus( var_1, var_4 );
                    }
...
_setsquadradarstrength( var_0, var_1, var_2 )
{
    updatesquaduavstatus( var_0, var_1, var_2 );
}
...
updateteamuavstatus( var_0, var_1 )
{
    if ( isdefined( var_1 ) )
        ...
        var_2 = _getteamradarstrength( var_0 );

    updategroupuavstatus( var_2, var_0 );
}
...
updatesquaduavstatus( var_0, var_1, var_2 )
{
    if ( isdefined( var_2 ) )
        ...
        var_3 = _getsquadradarstrength( var_1 );

    updategroupuavstatus( var_3, var_0, var_1 );
}

updategroupuavstatus( var_0, var_1, var_2 )
{
    var_0 = int( max( min( var_0, level._id_12CE8 ), level._id_12CEC ) );
    var_3 = var_0 == level._id_12CEC;
    var_4 = !var_3;
    var_5 = var_0 >= level._id_12CE9;
    var_6 = !var_3;

    if ( var_0 == level._id_12CEB )
    {
        var_7 = "normal_radar";
        var_4 = 0;
    }
    else if ( var_0 == level._id_12CE8 || var_5 )
        var_7 = "constant_radar";
    else if ( var_0 == level._id_12CEA )
        var_7 = "fast_radar";
    else
        var_7 = "normal_radar";
    ...
    foreach ( var_10 in var_8 )
    {
        ...
        if ( istrue( var_10.quest_secretdistmin ) )
            continue;

        if ( istrue( var_10._id_12345 ) )
            continue;
...
updateplayersuavstatus()
{
    foreach ( var_1 in level.players )
    {
        if ( istrue( var_1.quest_secretdistmin ) )
            continue;
        ...
        if ( var_3 > 0 )
            var_2 = level._id_12CEC;

        var_2 = int( max( min( var_2, level._id_12CE8 ), level._id_12CEC ) );
        var_1.radarstrength = var_2;
        var_4 = var_1.team == "spectator" || var_1.team == "follower" || var_1.team == "free";

        if ( var_2 <= level._id_12CEB || var_4 )
        {
            var_1.hasradar = 0;
        ...
        if ( var_2 >= level._id_12CEA )
            var_1.radarmode = "fast_radar";
        else
            var_1.radarmode = "normal_radar";

        var_1.radarshowenemydirection = var_2 >= level._id_12CE9;
...
adduavmodel()
{
    ...
    {
        if ( istrue( level.uavsquadasteamenabled ) )
            level.uavmodels[self.owner.squadindex][level.uavmodels[self.owner.squadindex].size] = self;
        else
...
addactiveuav()
{
    ...
    if ( level.teambased )
    {
        ...
        if ( self.uavtype == "directional_uav" || self.uavtype == "harp" )
        {
            ...
            level.allow_br_loot_to_br_marked++;
            return;
        }
    }
    else
...
addactivecounteruav()
{
    ...
    {
        if ( istrue( level.uavsquadasteamenabled ) )
            level.activecounteruavs[self.owner.squadindex]++;
        else
...
watchhighlightfadetime( var_0, var_1, var_2, var_3 )
{
    ...
    self endon( "disconnect" );
    level endon( "game_ended" );
    scripts\engine\utility::_id_13147( var_2, "leave" );
...
applyshutdownonspawn()
{
    ...
    for (;;)
    {
        ...
        if ( level.teambased )
        {
            if ( istrue( level.uavsquadasteamenabled ) )
            {
                if ( var_2.squadindex == var_0.squadindex )
                    continue;
            }
...
revealminimapforteam( var_0 )
{
    ...
    foreach ( var_3 in level.players )
    {
        ...
        if ( level.teambased )
        {
            if ( istrue( level.uavsquadasteamenabled ) )
            {
                if ( self.owner.squadindex != var_3.squadindex )
                    continue;
            }
            else if ( self.team != var_3.team )
                continue;
        }
...
applymapenableonspawn()
{
    ...
    for (;;)
    {
        ...
        if ( level.teambased )
        {
            if ( istrue( level.uavsquadasteamenabled ) )
            {
                if ( self.owner.squadindex != var_1.squadindex )
                    continue;
            }
...
setforceradars( var_0, var_1 )
{
    ...
    if ( level.teambased )
    {
        if ( istrue( level.uavsquadasteamenabled ) )
        {
            ...
            {
                foreach ( var_11, var_10 in level.squaddata[var_8] )
                {
                    level.radarmode[var_11] = var_3;
                    level.activeuavs[var_11] = var_4;
                    level.activeadvanceduavs[var_11] = var_5;
                    var_6++;
                    _setsquadradarstrength( var_11, var_2 );
                }
            }

            level.allow_br_loot_to_br_marked = var_6;
        }
        else
        {
            foreach ( var_14 in level.teamnamelist )
            {
                level.radarmode[var_14] = var_3;
                level.activeuavs[var_14] = var_4;
                level.activeadvanceduavs[var_14] = var_5;
                level.allow_br_loot_to_br_marked = level.teamnamelist.size;
                _setteamradarstrength( var_14, var_2 );
            }
        }
    }
    else
...
_id_12CE4( var_0, var_1, var_2 )
{
    if ( !isdefined( var_2 ) )
        return;

    var_3 = _id_12CE5( var_0, var_1 );
