...
initconfig()
{
    ...
    if ( isdefined( level.analytics_init ) )
        var_1 [[ level.analytics_init ]]();
}
...
watchmarkeractivate( var_0 )
{
    ...
    else
    {
        ...
        if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "juggernaut", "dropCrateFromScriptedHeli" ) && level.gametype != "br" )
        {
            ...
            var_2 thread scripts\cp_mp\killstreaks\airdrop::battle_tracks_shouldstartbattletracks( var_0, "airdrop" );
        ...
        {
            var_5 = 1;
            var_2 thread scripts\cp_mp\killstreaks\airdrop::_id_1256A( var_3, var_5 );
        }
    }
}
...
createjuggobjective()
{
    ...
    if ( istrue( level.risk_currentflagstier ) )
    {
        var_0 = 0;
        ...
        if ( istrue( var_0 ) && getdvarint( "scr_juggernaut_team_colored_icons", 1 ) )
        {
    ...
    else
    {
        ...
        if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "requestObjectiveID" ) )
            var_1 = scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "requestObjectiveID" );

        if ( isdefined( var_1 ) )
        {
            var_2 = [[ var_1 ]]( 99 );
            self.juggcontext.juggobjid = var_2;
            scripts\mp\objidpoolmanager::objective_add_objective( var_2, "active", self.origin, "icon_minimap_juggernaut" );
            ...
            if ( level.teambased )
                scripts\mp\objidpoolmanager::update_objective_ownerteam( var_2, self.team );
            else
                scripts\mp\objidpoolmanager::_id_12D9D( var_2, self );
        }
    }
}
...
watchjuggernautweaponswitch( var_0 )
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_1312D( "manual_switch_from_minigun", "weapon_pickup" );
