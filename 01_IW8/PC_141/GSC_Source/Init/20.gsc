...
{
    var_0 scripts\engine\utility::_id_1312D( "bots_human_spawned", "bots_human_disconnected" );
    ...
    for (;;)
    {
        ...
        if ( var_46 )
        {
            ...
            if ( var_45 > 0 && var_44 > 0 )
                level scripts\engine\utility::_id_13127( "spawned_enemies", "spawned_allies" );
            else if ( var_45 > 0 )
                level waittill( "spawned_enemies" );
            else if ( var_44 > 0 )
...
bot_think()
{
    ...
    if ( !isdefined( self.checkpointstruct ) )
    {
        var_1 = 1;
        self.checkpointstruct = 1;
    ...
    for (;;)
    {
        ...
        scripts\mp\bots\bots_loadout::checkpoint_edit_role_limiter();
        checkpoint_fx();
...
bot_think_seek_dropped_weapons()
{
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\bots\bots_util::checkforovertime( 0.33 ) )
        {
...
bot_think_revive()
{
    ...
    for (;;)
    {
        ...
        if ( !bot_can_revive() )
        {
            self.register_outer_room_spawners = 0;
            continue;
        }
        ...
        for ( var_4 = 0; var_4 < var_1.size; var_4++ )
        {
            ...
            if ( distancesquared( self.origin, var_8 ) < 4194304 )
            {
                ...
                self.register_outer_room_spawners = 1;
...
revive_watch_for_finished( var_0 )
{
    ...
    self endon( "bad_path" );
    self endon( "goal" );
    var_0 scripts\engine\utility::_id_1312D( "death", "revived" );
