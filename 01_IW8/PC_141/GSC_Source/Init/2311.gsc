...
molotov_init()
{
    var_0 = spawnstruct();
    level.molotov = var_0;
    var_0.solutions = getdvarint( "scr_molotovMaxPools", 100 );
    var_0.so_outcomenotify = getdvarint( "scr_molotovMaxCastsPerFrame", 8 );
    var_0.playerpowerssetupkeybindings = getdvarint( "scr_molotovInstantCleanup", 1 ) > 0;
    var_0._id_12D14 = 0;
    var_0.wire_think = [];
    var_0.scriptables = [];
    var_0.triggers = [];
...
molotov_init_cast_data()
{
    var_0 = spectatetestonprematchfadedone();
...
molotov_used( var_0 )
{
    ...
    if ( isdefined( level.shallowwaterfuncs ) && [[ level.shallowwaterfuncs.dofiresmoke ]]( var_0 ) )
    {
        thread scripts\mp\weapons::_id_1236D( var_0.origin );
        thread scripts\mp\weapons::smokegrenadeexplode( var_0.origin );
        thread scripts\mp\weapons::sfx_smoke_grenade_smoke( var_0.origin );
        var_0 delete();
    }
    else
    {
        if ( isdefined( level.lbravo_spawner_jammer4b ) )
            [[ level.lbravo_spawner_jammer4b ]]( var_0, self );
...
molotov_stuck_player( var_0, var_1, var_2, var_3 )
{
    ...
    if ( !isdefined( level.shallowwaterfuncs ) || ![[ level.shallowwaterfuncs.checkfireextinguishcrouched ]]( var_1 ) )
    ...
    if ( !isdefined( level.shallowwaterfuncs ) || ![[ level.shallowwaterfuncs.checkfireextinguishinwater ]]( var_1 ) )
    {
        var_3 = var_3 * ( 0, 0, 1 );
        var_4 = var_0.origin;
        var_5 = ( 0, 0, -1 );
...
{
    if ( !scripts\mp\bots\bots_util::bot_bots_enabled_or_added() && !scripts\mp\utility\game::cleanuparenamolotovs() )
        return;
...
spinning( var_0 )
{
    ...
    var_1 = spectatetestonprematchfadedone();
...
spinpropkey( var_0 )
{
    ...
    if ( var_1.scriptables.size > var_1.solutions )
        spectate3rdallowed();
}

splashtime_drops( var_0 )
{
    var_1 = spectatetestonprematchfadedone();
    var_1.triggers[self.id] = self;

    if ( var_1.triggers.size > var_1.solutions )
        spectatecommands();
}
...
spectatenumber()
{
    ...
    if ( !isdefined( var_1 ) )
        var_0.wire_think = scripts\engine\utility::array_remove( var_0.wire_think, self.id );
...
spectate3rdallowed( var_0 )
{
    var_1 = spectatetestonprematchfadedone();
    var_2 = undefined;
    var_3 = undefined;

    foreach ( var_3 in var_1.wire_think )
    {
    ...
    var_0 = istrue( var_0 ) || var_1.playerpowerssetupkeybindings;

    if ( !isdefined( var_2 ) )
        spectatekey( var_3, var_0 );
}

spectatecommands( var_0 )
{
    var_1 = spectatetestonprematchfadedone();
    var_2 = undefined;
    var_3 = undefined;
...
spectatekey( var_0, var_1 )
{
    var_2 = spectatetestonprematchfadedone();
    ...
    if ( isdefined( var_3 ) )
    {
        if ( !istrue( var_1 ) )
        {
            ...
            var_2.wire_think = scripts\engine\utility::array_remove( var_2.wire_think, var_0 );
            var_3 thread molotov_pool_end();
            return;
        }

        var_3 thread spectatenumber();
    }

    var_4 = var_2.triggers[var_0];

    if ( isdefined( var_4 ) )
        var_4 thread spectatepoint();
}
...
spawnvehicles( var_0 )
{
    ...
    if ( var_1.caststhisframe >= var_1.so_outcomenotify )
        return 0;
...
splashtime_helis()
{
    ...
    if ( !isdefined( var_0.timeout_consec_kills ) )
        var_0.timeout_consec_kills = [];
    ...
    foreach ( var_2 in self.branches )
    {
        ...
        {
            if ( isdefined( var_4 ) )
                var_0.timeout_consec_kills[var_0.timeout_consec_kills.size] = var_4;
        }
    }
}
...
molotov_start_branch()
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            if ( !spawnvehicles( self.shareddata ) )
            {
                waitframe();
                continue;
            }
...
molotov_branch_create_pool( var_0, var_1, var_2 )
{
    ...
    var_7.id = spin_fan_blades();
    var_7 spinpropkey( self.shareddata );
    return var_7;
}
...
molotov_pool_start()
{
    ...
    if ( isdefined( self ) )
        ...
        level thread spinafterdelay( var_1, var_0 );
}
...
molotov_pool_end( var_0 )
{
    ...
    if ( isdefined( self.trigger ) )
        self.trigger thread spectatepoint();

    if ( !istrue( var_0 ) )
        wait 3.5;

    thread spectatenumber();
}

spinafterdelay( var_0, var_1 )
{
    if ( isdefined( var_0 ) )
        var_0 thread spectatepoint();
    ...
    if ( !isdefined( var_0 ) )
        var_2.wire_think = scripts\engine\utility::array_remove( var_2.wire_think, var_1 );
}
...
molotov_watch_pool_trigger_enter()
{
    ...
    for (;;)
    {
        ...
        if ( !isplayer( var_0 ) && !istrue( var_0.blade_trigger_think ) )
            continue;
...
{
    var_0 endon( "death" );
    scripts\engine\utility::_id_13142( 7, "disconnect", "joined_team" );
    var_0 delete();
}

spectatetestonprematchfadedone()
{
    return level.molotov;
}

spectatingthisplayer()
{
    var_0 = spectatetestonprematchfadedone();
    var_1 = var_0.pooldata;
    return var_1;
...
{
    var_0 = spectatetestonprematchfadedone();
    var_1 = var_0._id_12D14;
    var_0._id_12D14++;
    var_0.wire_think = scripts\engine\utility::array_add( var_0.wire_think, var_1 );
    return var_1;
}
...
molotov_start_burning( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( level.shallowwaterfuncs ) && [[ level.shallowwaterfuncs.checkfireextinguishcrouched ]]( self ) )
        return;
...
molotov_begin_fx()
{
    ...
    self.watchweapondeathordisconnect = 1;
...
molotov_end_fx()
{
    ...
    if ( istrue( self.watchweapondeathordisconnect ) )
    {
        self setscriptablepartstate( "equipMtovFXWorld", "neutral", 0 );
        self setscriptablepartstate( "equipMtovFXView", "neutral", 0 );
    }

    self.watchweapondeathordisconnect = undefined;
}
    ...
    if ( isdefined( var_0.shareddata ) )
    {
        if ( isdefined( var_0.shareddata.branches ) )
        {
            foreach ( var_3 in var_0.shareddata.branches )
                spawnwalltriggers( var_3, var_1 );
        }

        if ( isdefined( var_0.shareddata.timeout_consec_kills ) )
        {
            foreach ( var_6 in var_0.shareddata.timeout_consec_kills )
            {
                if ( isdefined( var_6 ) )
