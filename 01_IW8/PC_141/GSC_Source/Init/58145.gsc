...
init()
{
    level.br_give_weapon_ammo = scripts\engine\utility::getstructarray( "ascend_solo_begin", "script_noteworthy" );
    level.give_player_grenade_on_respawn = scripts\engine\utility::getstructarray( "descend_solo_begin", "script_noteworthy" );
    ...
    foreach ( var_1 in level.br_give_weapon_ammo )
        scripts\cp_mp\auto_ascender::markupascenderstruct( var_1, 1 );

    foreach ( var_4 in level.give_player_grenade_on_respawn )
    ...
    foreach ( var_1 in level.br_give_weapon_ammo )
    {
        foreach ( var_4 in level.give_player_grenade_on_respawn )
        {
            if ( !isdefined( var_4._id_123F7 ) && distance2dsquared( var_1.origin, var_4.origin ) < 10 )
            {
                var_1._id_123F7 = var_4;
                var_4._id_123F7 = var_1;
                level._id_12A42--;
                break;
            }
        }
    ...
    initanimtree();
    scripts\engine\scriptable::_id_11F48( "ascender_solo", ::br_getrandomlootweapon );
}
...
br_getrandomlootweapon( var_0, var_1, var_2, var_3, var_4 )
{
    level._id_11F63 = var_0;
...
startascenderanim( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( var_4 > 0 )
        thread _id_125DD( self.angles + ( 0, var_4 * -1, 0 ) );
...
ascenderuse( var_0, var_1 )
{
    ...
    if ( !scripts\cp_mp\auto_ascender::descendsolostarts( var_0, var_1 ) )
        return;
    ...
    var_1.br_ending_debugmode = var_12;
    var_1.br_ending_delay = var_13;
    ...
    if ( !var_2.dir )
    {
        if ( scripts\common\utility::iscp() )
            var_15 = getdvarfloat( "scr_descender_speed_cp", scripts\cp_mp\auto_ascender::minarmordropondeath() );
        else
            var_15 = getdvarfloat( "scr_descender_speed", scripts\cp_mp\auto_ascender::min_z() );
    }
    else
        var_15 = getdvarfloat( "scr_ascender_speed", scripts\cp_mp\auto_ascender::min_x() );

    var_16 = var_14 / var_15;
    var_17 = scripts\cp_mp\auto_ascender::min_pt() * var_16;
    var_18 = scripts\cp_mp\auto_ascender::min_y() * var_16;
    ...
    if ( getdvarint( "scr_ascender_allowDisconnect", 1 ) > 0 )
        var_19 = var_1 watchforrelease( var_2, var_16, var_12, var_13 );
    else
        wait( var_16 );
...
watchforrelease( var_0, var_1, var_2, var_3 )
{
    var_4 = self;
    var_5 = gettime();
...
remove_launcher_xmags( var_0 )
{
    foreach ( var_2 in level.br_give_weapon_ammo )
    {
        if ( distance2dsquared( var_2.origin, var_0 ) < scripts\cp_mp\auto_ascender::mine_cave_turrets() )
        {
            if ( isdefined( var_2._id_123F7 ) )
            {
                if ( abs( var_0[2] - var_2.origin[2] ) < scripts\cp_mp\auto_ascender::mindia_exterior_sfx() || abs( var_0[2] - var_2._id_123F7.origin[2] ) < scripts\cp_mp\auto_ascender::mindia_exterior_sfx() )
                    return 1;
            }
        }
    }
