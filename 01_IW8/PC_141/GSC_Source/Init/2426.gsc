...
callback_vehicledamage_internal( var_0 )
{
    ...
    if ( var_0.meansofdeath == "MOD_MELEE" )
    {
        if ( isdefined( var_5 ) && var_5 == "radar_drone_recon" )
        {
            ...
            var_0.attacker scripts\cp_mp\pet_watch::_id_12A8E( self );
    }

    if ( isdefined( level.spawnpointdangertime ) )
        var_0.damage = self [[ level.spawnpointdangertime ]]( var_0 );
    ...
    if ( var_0.meansofdeath == "MOD_MELEE" )
    {
        ...
        else
        {
            ...
            if ( isdefined( level.spawnboardroom_gasmask ) )
                var_8 = self [[ level.spawnboardroom_gasmask ]]( var_0 );
    ...
    {
        if ( isdefined( level.spawnboardroom_specialist ) && self [[ level.spawnboardroom_specialist ]]( var_0 ) )
            var_0.damage = 0;
        else if ( scripts\mp\damage::removeplatepouch( var_0.objweapon ) )
            var_0.damage = 0;
    ...
    {
        if ( scripts\mp\damage::repair_grill_fixing_long_sfx( var_0.objweapon ) )
            var_0.damage = scripts\mp\damage::_id_12600( self, var_0.attacker, var_0.objweapon, var_0.damage, var_0.idflags );
        else if ( scripts\mp\damage::registerhint( var_0.objweapon ) )
        {
            if ( scripts\mp\damage::_id_12276( var_0.objweapon ) )
                var_0.damage = 1;
            else
                var_0.damage = 0;
        }
        else if ( scripts\mp\damage::resetstuckthermite( var_0.objweapon ) )
        {
            if ( !scripts\mp\damage::_id_122AA( var_0.objweapon ) )
                var_0.damage = 0;
        }
        else if ( scripts\mp\damage::redshirt_drop_off_behavior( var_0.objweapon, var_0.meansofdeath ) )
            var_0.damage = 0;
        else if ( isdefined( var_5 ) )
    ...
    if ( isdefined( var_5 ) )
    {
        var_9 = get_post_mod_damage_callback( var_5 );

        if ( isdefined( var_9 ) )
        {
            var_7 = self [[ var_9 ]]( var_0 );
    ...
    var_10 = scripts\cp_mp\vehicles\vehicle_damage::_id_12F34( var_0 );

    if ( !var_10 )
    {
        var_11 = scripts\cp_mp\vehicles\vehicle_damage::_id_12F0B();

        if ( isdefined( var_11 ) && var_11 == "heavy" )
        {
            var_12 = scripts\engine\utility::ter_op( isexplosivedamagemod( var_0.meansofdeath ) || var_0.meansofdeath == "MOD_FIRE", 0.15, 0.25 );
            var_0.damage = var_0.damage * var_12;
        }
    }

    if ( scripts\mp\utility\game::getgametype() == "br" )
        scripts\cp_mp\vehicles\vehicle_damage::_id_12F36( var_0, 1 );
    else
        scripts\cp_mp\vehicles\vehicle_damage::_id_12F35( var_0, var_10, 1 );
    ...
    if ( istrue( level._id_12FF0 ) )
        var_0.damage = 0;

    scripts\mp\utility\damage::non_player_log_attacker_data( var_0 );
    var_13 = var_0.damage;
    var_14 = 1 - max( var_4 - var_13, 0 ) / var_4;
    var_15 = "hitequip";
    var_16 = var_0.damage >= self.health;
    var_17 = istrue( var_0.remove_usability_crutch_on_death ) || istrue( var_0.removematchingents_bymodel );

    if ( var_17 && isdefined( var_0.objweapon.basename ) )
    {
        switch ( var_0.objweapon.basename )
        {
            ...
            case "lighttank_tur_mp":
                if ( isdefined( var_0.meansofdeath ) && var_0.meansofdeath != "MOD_RIFLE_BULLET" )
                    var_0.attacker thread scripts\mp\utility\points::obj_pregame( "critical_vehicle_damage" );
    ...
    if ( isdefined( var_0.attacker ) )
        var_0.attacker scripts\mp\damagefeedback::updatedamagefeedback( "hitequip", var_16, var_17, var_15, undefined, 1 );

    scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_updatedamagefeedback( var_0 );

    if ( isdefined( level.onvehicledamaged ) )
        self thread [[ level.onvehicledamaged ]]( var_0 );

    if ( var_16 )
    {
        if ( isdefined( var_0.attacker ) && isplayer( var_0.attacker ) )
            var_0.attacker scripts\cp_mp\pet_watch::door_anim( self );
    ...
    if ( self getscriptablehaspart( var_18 ) )
    {
        if ( self getscriptableparthasstate( var_18, "blowUp" ) )
            self setscriptablepartstate( var_18, "blowUp", 1 );
    }
    ...
    if ( level.teambased )
    {
        if ( !var_3 )
        {
            if ( isdefined( var_2 ) && !isdefined( var_1 ) )
                return scripts\cp_mp\vehicles\vehicle::_id_12F6E( var_0, var_2 );
            else
                return scripts\cp_mp\vehicles\vehicle::_id_12F6D( var_0, var_1 );
        }
