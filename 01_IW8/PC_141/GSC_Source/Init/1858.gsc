...
init()
{
    var_0 = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_healthregentime", "scr_player_healthregentime" );
...
healhregenthink( var_0 )
{
    ...
    if ( var_2 < 0.55 && !scripts\mp\utility\player::isusingremote() && !istrue( self.playerpostsetplunder ) )
        onenterdeathsdoor();
    ...
    for (;;)
    {
        ...
        if ( isdefined( level._id_11B66 ) )
            var_7 = [[ level._id_11B66 ]]( var_7 );
    ...
    if ( var_6 )
    {
        ...
        scripts\cp\helicopter\chopper_boss::train_attach_player_hurts( var_10 );
        scripts\mp\analyticslog::logevent_playerhealed( self, var_10, self );
    }
    else if ( !isai( self ) )
        scripts\mp\analyticslog::setup_lights_in_region( self, var_10 );
...
waitforhealthregendelay()
{
    ...
    while ( self.currentregendelay - self.regendelayreduction > 0 )
    {
        ...
        if ( isdefined( level._id_11B65 ) )
            var_5 = [[ level._id_11B65 ]]( var_5 );
...
applyshrapnelfxinternal()
{
    ...
    level endon( "game_ended" );
    var_0 = scripts\engine\utility::_id_13136( "shrapnel_ended_early", "shrapnel_ended", "force_regeneration" );
