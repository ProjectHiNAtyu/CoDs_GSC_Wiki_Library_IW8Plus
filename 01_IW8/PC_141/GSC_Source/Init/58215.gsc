...
lategame_buytimer_set()
{
    var_0 = scripts\mp\gametypes\br_plunder::weapon_xp_iw8_knife( "equip_mp_fulton", 1 );
    var_0.type = 0;
    var_0.usetime = getdvarfloat( "scr_fultonUseTime", 0.75 );
    var_0._id_12E52 = 2;
    var_0._id_12E50 = getdvarint( "scr_fultonUseAmount", 250 );
    var_0._id_12954 = 1;
    var_0._id_1292F = 1;
    var_0._id_12E53 = "MP/CANNOT_DEPOSIT_CASH_BALLOON_FULL";
    var_0._id_12E54 = "MP/CANNOT_DEPOSIT_CASH_BALLOON_LEAVING";
    var_0._id_11F68 = "fulton_use_cache";
    var_0._id_11F69 = "usable";
    var_0._id_11F62 = "unusable";
    var_0.is_wave_gametype = getdvarint( "scr_fultonCountdown", 20 );
    var_0.is_wave_hud_enabled = "MP/CASH_BALLOON_LEAVING_IN_N";
    var_0.detonatesound = getdvarint( "scr_fultonCapacity", 1500 );
    var_0.flip_target = ::lb_dmg_factor_driverless_collision;
    var_0.is_vandalize_attack_available = ::lb_dmg_factor_fuselage;
    var_0.branalytics_bonusxp_debugdata = ::launcherfired;
    var_0.isattachmentselectfire = "fulton";
    var_0.dropplunder = getdvarint( "scr_fultonDropPlunder", 1 ) > 0;
    var_0.determine_starting_breadcrumb = getdvarint( "scr_fultonCanTakeDamage", 1 ) > 0;
}
...
laststandweapondelay( var_0 )
{
    ...
    self.laststandkillteamifdowndisable = var_1;
    var_1 thread launch_smoke_mortar();
}

lastteamused( var_0 )
{
    var_1 = scripts\mp\gametypes\br_plunder::weapon_xp_iw8_knife( "equip_mp_fulton" );
    thread scripts\mp\gametypes\br::_id_1294F( "br_fulton_balloon_shot_down", self.owner, self.team );
    scripts\mp\gametypes\br_plunder::initchallengeandeventglobals( var_1.dropplunder );
...
laststandweaponobj()
{
    ...
    if ( isdefined( self.owner ) )
        self.owner.laststandkillteamifdowndisable = undefined;
    ...
    scripts\mp\gametypes\br_plunder::weapon_xp_iw8_ar_mcharlie( self );
...
lb_dmg_factor_tail_stabilizer( var_0 )
{
    ...
    else if ( 1 && isdefined( self.laststandkillteamifdowndisable ) )
    {
        self playlocalsound( "br_pickup_deny" );
    ...
    thread laststandweapondelay( var_0 );
...
laststandplayers( var_0, var_1 )
{
    if ( isdefined( var_1.set_relic_dogtags ) )
    {
        if ( isdefined( var_1.set_relic_dogtags.targetname ) )
        {
            if ( var_1.set_relic_dogtags.targetname == "train_wz" )
...
launch_smoke_mortar()
{
    ...
    thread scripts\mp\gametypes\br::_id_1294F( "br_fulton_device_placed", self.owner, self.team );
    var_0 = scripts\mp\utility\teams::getfriendlyplayers( self.team );
    scripts\mp\gametypes\br_plunder::weapon_xp_iw8_lm_mkilo3( self, "equip_mp_fulton" );
    thread scripts\mp\gametypes\br_plunder::weapon_xp_iw8_sm_beta( self, var_0 );
    scripts\mp\gametypes\br_plunder::weapon_xp_iw8_sm_uzulu( self, var_0 );
    var_1 = spawn( "script_model", self.origin );
    var_1 setmodel( "tag_origin" );
    var_1.angles = self.angles * ( 0, 1, 0 );
    var_1.laststandkillteamifdowndisable = self;
    ...
    var_2 = scripts\mp\gametypes\br_plunder::weapon_xp_iw8_knife( "equip_mp_fulton", 1 );

    if ( var_2.determine_starting_breadcrumb )
        thread scripts\mp\damage::monitordamage( 500, "", ::lastunruly, ::lasttuttxt );
}
...
lastunruly( var_0 )
{
    thread lastteamused( var_0 );
}

lb_dmg_factor_landing_gear( var_0, var_1, var_2 )
{
    var_3 = scripts\mp\gametypes\br_plunder::weapon_xp_iw8_knife( "equip_mp_fulton" );
    var_1 thread scripts\mp\gametypes\br_plunder::waitfornukecarriernearlz( var_2, var_3._id_12E52, var_0 );
}

lb_dmg_factor_driverless_collision( var_0, var_1 )
{
    if ( isdefined( var_0.owner ) )
        var_0.owner.laststandkillteamifdowndisable = undefined;

    scripts\mp\gametypes\br_plunder::weapon_xp_iw8_pi_papa320( var_0 );
}

lb_dmg_factor_fuselage( var_0 )
{
    var_0 endon( "death" );
    var_1 = scripts\mp\gametypes\br_plunder::weapon_xp_iw8_knife( "equip_mp_fulton" );
    var_2 = undefined;
    var_0 scripts\mp\damage::monitordamageend();
    var_2 = scripts\mp\gametypes\br_plunder::initcashtracking();
    thread scripts\mp\gametypes\br::_id_1294F( "br_fulton_balloon_successfully_away", self.owner, self.team );
...
launcherfired( var_0 )
{
    thread scripts\mp\gametypes\br::_id_1294F( "br_fulton_balloon_full", var_0.owner, var_0.team );
}

laststandoutlineid( var_0 )
{
...
launcher_second_techo()
{
    ...
    scripts\cp\helicopter\chopper_boss::train_elements_disable( self.super, 1 );
    var_0 = scripts\mp\supers::getcurrentsuper();
    scripts\mp\supers::_id_12179( 0 );
    scripts\mp\supers::_id_12178( 0 );
    var_0.wasrefunded = 1;
