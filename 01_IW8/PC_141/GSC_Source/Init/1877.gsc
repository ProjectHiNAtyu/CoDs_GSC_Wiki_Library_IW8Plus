...
relic_nobulletdamage_modifyplayerdamage( var_0 )
{
    return var_0 >= 100;
}
...
getclasschoice( var_0 )
{
    ...
    if ( isdefined( level.onping ) && level.onping > 0 )
    {
        if ( scripts\mp\utility\game::matchmakinggame() )
...
executeclasschange( var_0, var_1 )
{
    ...
    else
    {
        ...
        var_3 = 1;
        var_4 = scripts\mp\gamelogic::demo_debug_nuke( scripts\mp\utility\game::namehud() );
        ...
        {
            var_5 = scripts\mp\utility\game::relic_amped_play_beep() && self getclientomnvar( "ui_open_loadout_bag" );
...
handleclassedit( var_0 )
{
    ...
    var_3 = scripts\mp\class::setallclientomnvarot();
    var_4 = var_2 || var_3;
...
autoassign()
{
    if ( scripts\mp\utility\game::getgametype() == "infect" )
        ...
        thread setteam( "allies" );
    else if ( blockade_get_bomb_icon_on_cell_phone() )
        thread setteam( "allies" );
    else
    {
        ...
        if ( istrue( level.teammaxfill ) )
        {
            ...
            {
                if ( scripts\mp\teams::_id_1226E() && var_1 == "team_two_hundred" )
                    continue;
        ...
        if ( var_5.size > 0 )
        {
            ...
            if ( istrue( level.brjugg_initdialog ) )
                var_9 = 0;
...
setteam( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::namehud() != "dmz" && scripts\mp\utility\game::namehud() != "risk" && scripts\mp\utility\game::namehud() != "sandbox" && scripts\mp\utility\game::namehud() != "rumble" )
        {
            ...
            scripts\mp\utility\script::getweaponrandomvariantid( "setTeam() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_1 );
        }
    ...
    if ( ( self.sessionstate == "spectator" || wait_for_weapons_free() ) && !istrue( self.suicideswitched ) )
    {
...
{
    return isdefined( level.wait_for_weapons_free ) && self [[ level.wait_for_weapons_free ]]();
}
...
leavesquad( var_0, var_1 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
        thread scripts\mp\spawnselection::_id_11B60( var_0, var_1 );
...
waitforclassselect()
{
    ...
    for (;;)
    {
        var_0 = scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::allowclasschoice() && ( !scripts\mp\flags::gameflag( "prematch_done" ) || istrue( level._id_1233E ) || istrue( level.brlatejoin ) );
        ...
        if ( var_0 || var_1 )
        {
            ...
            if ( !isai( self ) )
            {
                ...
                if ( var_3 && !scripts\mp\utility\game::registercarryobjectpickupcheck() )
...
beginclasschoice( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::namehud() != "dmz" && scripts\mp\utility\game::namehud() != "risk" && scripts\mp\utility\game::namehud() != "sandbox" && scripts\mp\utility\game::namehud() != "rumble" )
        {
            ...
            scripts\mp\utility\script::getweaponrandomvariantid( "beginClassChoice() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_2 );
        }
...
bypassclasschoice()
{
    ...
    if ( remove_munitions_in_radius() && level.enforceantiboosting && !isbot( self ) )
    {
...
addtoteam( var_0, var_1, var_2 )
{
    ...
    if ( var_0 == "spectator" || var_0 == "follower" )
    {
        ...
        scripts\cp_mp\pet_watch::tower_ground_mortar();
    }
    else
