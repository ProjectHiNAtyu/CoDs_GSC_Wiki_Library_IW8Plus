...
{
    if ( var_0 == nostand() && ( isdefined( level._id_133C3 ) || cleanuparenamolotovs() ) )
        return 1;
...
{
    return getdvarint( "scr_br_use_agents", 0 ) > 0 || istrue( level.closenukecrate );
}
...
nextroundisfinalround()
{
    ...
    {
        if ( level resetunresolvedcollision( var_2 ) )
            continue;
...
waslastround()
{
    ...
    if ( istrue( level.get_ai_within_range ) )
        return 1;
...
isanymlgmatch()
{
    ...
    return getdvarint( "LOMTKQTRTM" ) || istrue( level.dmz_numteamswithplayers );
}

ismlgsystemlink()
{
    if ( level.systemlink && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.dmz_numteamswithplayers ) ) )
        return 1;
...
ismlgprivatematch()
{
    if ( privatematch() && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.dmz_numteamswithplayers ) ) )
        return 1;
...
reset_attack_next_available_time()
{
    if ( namehud() == "br" )
        return 1;
    else if ( namehud() == "jugg" )
        return 1;
...
monitor_bush_trig()
{
    level.trial_end_time = 5;
    return level.trial_end_time;
}

monitor_dropkit_marker_throw()
{
    if ( !isdefined( level.trial_enemy_dont_drop_weapon ) )
        level.trial_enemy_dont_drop_weapon = getdvarint( "scr_br_payload_retreat_time", 30 );

    return level.trial_enemy_dont_drop_weapon;
}

monitor_death_thread()
{
    if ( !isdefined( level.trial_enemies_killed ) )
        level.trial_enemies_killed = getdvarint( "scr_br_payload_redraw_time", 10 );

    return level.trial_enemies_killed;
}
...
setup_enemy_sentries( var_0, var_1, var_2, var_3, var_4 )
{
...
remove_outline()
{
    return istrue( level.remove_on_death );
}

relic_amped_test_explode_other_player()
{
    return istrue( level.relic_amped_wait_till_revived ) || istrue( level.relic_award_bullets ) || istrue( level.relic_amped_victim );
}
...
deregistergasmaskscriptableatframeend( var_0 )
{
    ...
    if ( ( getgametype() == "arm" || relic_amped_test_explode_other_player() ) && resetchallengetimer() || getdvarint( "scr_game_forceEnableParachuteCut", 0 ) == 1 )
    {
        if ( var_3 != "mp_aniyah" )
        {
            ...
            if ( istrue( level.trigger_explosion_grenades ) && !istrue( var_0.halfheight ) )
                var_2 = 1;
        }
    }
...
registercarryobjectpickupcheck()
{
...
enemy_left_monitor()
{
    if ( registercarryobjectpickupcheck() )
    {
    ...
    else if ( isdefined( level.onping ) && level.onping > 0 )
        return 1;
