...
{
    if ( istrue( level._id_1231B ) || var_0 == "MOD_EXECUTION" )
        return;
    ...
    else if ( istrue( self.iszombie ) || istrue( level.open_teleport_room_door ) && level.gametype == "infect" && self.team == "axis" )
...
{
    if ( !isarray( level.adjustlootleadermarkcount ) )
        level.adjustlootleadermarkcount = [];

    if ( !isent( level.adjusteventdistributionpadding ) )
        level.adjusteventdistributionpadding = spawn( "sound_transient_soundbanks", ( 0, 0, 0 ) );
}

announcedomplatespawns( var_0 )
{
    ai_ascender_getclosestascender();

    if ( isdefined( level.adjustlootleadermarkcount[var_0] ) )
        return;

    level.adjusteventdistributionpadding settransientsoundbank( var_0 + ".all", 1 );
    level.adjustlootleadermarkcount[var_0] = level.adjustlootleadermarkcount.size;
    return;
}
...
_id_11CA3( var_0 )
{
    ...
    if ( isdefined( level.adjustlootleadermarkcount[var_0] ) )
    {
        level.adjusteventdistributionpadding settransientsoundbank( var_0 + ".all", 0 );
        level.adjustlootleadermarkcount[var_0] = undefined;
    }
    else
    {
