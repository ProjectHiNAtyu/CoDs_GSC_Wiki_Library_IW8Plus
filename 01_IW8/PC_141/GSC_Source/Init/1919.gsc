...
init()
{
    ...
    level.multiteambased = level.teamnamelist.size > 2 || scripts\mp\utility\game::getgametype() == "br" && scripts\mp\menus::_id_1261E();
    level.teamdata = [];
    ...
    {
        scripts\mp\utility\teams::never_kill_off_old( var_9 );
    ...
    if ( isdefined( level.killstreak_aquired ) )
        level thread [[ level.killstreak_aquired ]]();
...
handleultraoperatorskins( var_0 )
{
    if ( !isdefined( level.ultraoperatordata ) )
    {
        var_1 = [];
        var_1["t9woods_western_v"] = "active_woods";
        var_1["t9weaver_western_b"] = "active_weaver";
        var_1["t9st3_western_r"] = "active_stone";
        var_2 = "operatorSkins.csv";
        var_3 = 1;
        var_4 = 0;
        level.ultraoperatordata = [];

        foreach ( var_9, var_6 in var_1 )
        {
            var_7 = tablelookup( "operatorSkins.csv", var_3, var_9, var_4 );
            var_8 = int( var_7 );
            level.ultraoperatordata[var_8] = var_6;
        }
    }
    ...
    var_6 = level.ultraoperatordata[var_0];

    if ( isdefined( var_6 ) )
        self setscriptablepartstate( var_10, var_6 );
    else
...
createoperatorcustomization()
{
    ...
    var_10.clothtype = monitorplayerimpact( var_9 );
    var_10.superfaction = getoperatorsuperfaction( var_8 );
    var_10.execution = getoperatorexecution( var_8 );
    var_10.intel_collect_vo_func = monitorplayerimpactend( var_8 );
    var_10.intel_collected = monitortacmapusage( var_8 );
    var_10.suit = var_3;
    var_10.rebuild = 0;
    var_10.playanim_aibegindismountturret = relic_steelballs_dash( var_1 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        var_11 = monitormounted( var_9 );

        if ( var_11 != "" )
            var_10.col_circletick = var_11;

        handleultraoperatorskins( var_9 );
    }

    var_12 = spawnstruct();
    var_12.apc = no_jugg_early_exit( "apc" );
    var_12.bot_custom_classes_allowed = nocrash( "apc" );
    var_12.break_window_glass = no_jugg_early_exit( "atv" );
    var_12.diode_light_think = no_jugg_early_exit( "cargo_truck" );
    var_12.respawndelay = no_jugg_early_exit( "jeep" );
    var_12.set_relic_nobulletdamage = no_jugg_early_exit( "little_bird" );
    var_12._id_12895 = no_jugg_early_exit( "tac_rover" );
    var_12._id_128E0 = no_jugg_early_exit( "tank_east" );
    var_12._id_128E1 = nocrash( "tank_east" );
    var_12._id_128EB = no_jugg_early_exit( "tank_west" );
    var_12._id_128EC = nocrash( "tank_west" );
    var_12.start_loc = no_jugg_early_exit( "motorcycle" );
    var_12.bot_choose_attack_role = no_more_wire_to_cut( "apc", 4 );
    var_12.breakable_gate_clip = no_more_wire_to_cut( "atv", 6 );
    var_12.directimpactkill = no_more_wire_to_cut( "cargo_truck", 8 );
    var_12.respawnfade = no_more_wire_to_cut( "jeep", 10 );
    var_12.set_relic_noks = no_more_wire_to_cut( "little_bird", 12 );
    var_12._id_12896 = no_more_wire_to_cut( "tac_rover", 14 );
    var_12.start_safehouse_gunshop = no_more_wire_to_cut( "motorcycle", 16 );
    var_12.brenableagents = no_previous_interaction_point( "atv" );
    var_12._id_1289A = no_previous_interaction_point( "tac_rover" );
    var_12.set_vehicle_anims_decho_civ = no_previous_interaction_point( "little_bird" );
    var_12.start_static_plane_lights = no_previous_interaction_point( "motorcycle" );
...
lookupcurrentoperator( var_0 )
{
    ...
    if ( getdvarint( "scr_forceHeadlessCustomization", 1 ) == 1 && !isagent( self ) && self isplayerheadless() )
    {
        ...
        if ( !isdefined( self.ph_setfinalkillcamwinner ) )
        {
            foreach ( var_9 in var_7 )
            {
                var_10 = getarraykeys( level.operatorcustomization[var_9] );

                if ( !isdefined( level.phase_zero_combat ) )
                    level.phase_zero_combat = [];

                if ( !isdefined( level.phase_zero_combat[var_9] ) || level.phase_zero_combat[var_9] > var_10.size )
                    level.phase_zero_combat[var_9] = 0;

                for ( var_11 = var_10[level.phase_zero_combat[var_9]]; isdefined( var_11 ) && ( var_11 == "default_western" || var_11 == "default_eastern" ); var_11 = var_10[level.phase_zero_combat[var_9]] )
                {
                    level.phase_zero_combat[var_9] = level.phase_zero_combat[var_9] + 1;

                    if ( level.phase_zero_combat[var_9] > var_10.size )
                        level.phase_zero_combat[var_9] = 0;
                }
            ...
            self.ph_setfinalkillcamwinner = 1;
        }
...
lookupcurrentoperatorskin( var_0 )
{
    ...
    else
    {
        if ( getdvarint( "scr_forceHeadlessCustomization", 1 ) == 1 && !isagent( self ) && self isplayerheadless() )
        {
            ...
            {
                if ( !isdefined( level.phase_two_combat ) )
                    propaddtocircle();

                var_5 = level.phase_two_combat[var_1]["curIndex"];
                level.playercustomizationdata[var_3][var_0].operatorskinindex = level.phase_two_combat[var_1]["lootIDs"][var_5];
                level.phase_two_combat[var_1]["curIndex"] = level.phase_two_combat[var_1]["curIndex"] + 1;

                if ( level.phase_two_combat[var_1]["curIndex"] >= level.phase_two_combat[var_1]["maxIndex"] )
                    level.phase_two_combat[var_1]["curIndex"] = 0;
            }
        }
...
getoperatorcustomization()
{
    ...
    if ( isdefined( level.spawnboardroom_miniguns ) )
        [var_0, var_1] = [[ level.spawnboardroom_miniguns ]]( self, var_0, var_1 );
...
checkforafk()
{
    ...
    if ( istrue( self.coop_handle_invuln_ac130 ) )
        return;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && istrue( self.comparescore ) )
    {
        self.comparescore = undefined;
        var_10 = 30;
    }
    else if ( var_11 > 0 )
        var_10 = var_11;
    ...
    if ( scripts\mp\utility\game::isroundbased() && scripts\mp\utility\game::getgametype() != "ctf" )
    {
        ...
        if ( var_0 && !isgamebattlematch() && !istrue( self.apply_track_settings ) )
        {
            self.apply_track_settings = 1;
            self.pers["roundsAFK"]++;

            if ( self.pers["roundsAFK"] > 1 )
                level thread round_enemies_fallback_logic( self );
        }
...
getjointeampermissions( var_0 )
{
    ...
    if ( level.maxteamsize == 0 || var_1 < level.maxteamsize )
        ...
        return 1;
    else if ( scripts\mp\menus::blockade_get_bomb_icon_on_cell_phone() )
        return 1;
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_1312D( "death_or_disconnect", "joined_team" );
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_1312E( "death_or_disconnect", "joined_team", "joined_squad" );
    scripts\mp\utility\outline::outlinedisable( var_0, self );
}
...
monitorplayerimpactend( var_0 )
{
    ...
    if ( var_1 == 0 )
        self.setgameendflagsandnotifies = tablelookup( "operators.csv", 1, var_0, 23 );
    else
        self.setgameendflagsandnotifies = tablelookup( "operatorquips.csv", 0, var_1, 6 );

    return self.setgameendflagsandnotifies;
}

monitortacmapusage( var_0 )
{
    self.setgun = tablelookup( "mp_cp/executiontable.csv", 1, self.loadoutexecution, 19 );
    return self.setgun;
}
...
propaddtocircle()
{
    ...
    for ( var_1 = 0; var_1 < var_0; var_1++ )
    {
        if ( tablelookupbyrow( "operatorskins.csv", var_1, 18 ) != "" )
        {
            ...
            if ( !isdefined( level.phase_two_combat[var_2] ) )
            {
                level.phase_two_combat[var_2]["lootIDs"] = [];
                level.phase_two_combat[var_2]["curIndex"] = 0;
                level.phase_two_combat[var_2]["maxIndex"] = 0;
            }

            level.phase_two_combat[var_2]["lootIDs"][level.phase_two_combat[var_2]["lootIDs"].size] = int( var_3 );
            level.phase_two_combat[var_2]["maxIndex"] = level.phase_two_combat[var_2]["maxIndex"] + 1;
        }
    }
}
...
initoperatorcustomization()
{
    ...
    for (;;)
    {
        ...
        var_5 = monitorweaponswitchbr( var_1 );
...
unset_relic_focus_fire()
{
    ...
    if ( istrue( level.onlinegame ) )
    {
        ...
        if ( isdefined( var_2 ) && var_2.size > 0 )
        {
            foreach ( var_4 in var_2 )
            {
                ...
                {
                    var_0 = var_4.team;
                    self._id_12951 = var_4._id_12951;
                    break;
                }
            }
        }
    ...
    if ( !isdefined( var_0 ) )
        var_0 = isweaponsilenced( var_1 );

    if ( !isdefined( self._id_12951 ) )
        self._id_12951 = gettime();
...
isweaponsilenced( var_0 )
{
    ...
    foreach ( var_3 in level.teamnamelist )
    {
        ...
        if ( var_4 > 0 )
        {
            ...
            if ( isdefined( var_6[0]._id_12951 ) && gettime() > var_6[0]._id_12951 + 300000 )
                continue;
        }
...
_id_1226E()
{
    return getdvarint( "scr_br_zombie_encounters", 0 ) > 0 || scripts\mp\utility\game::cleanuparenamolotovs();
}
