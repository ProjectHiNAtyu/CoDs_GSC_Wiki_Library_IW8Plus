...
claymore_use( var_0 )
{
    ...
    if ( vehicle_occupancy_monitorturretcontrols() )
    {
        var_1.plantoffsetz = var_1.plantoffsetz + 2;
        var_1.contents = fulton_destroy();
    }
...
claymore_plant()
{
    ...
    thread enemy_monitor_trialending();
    wait 1;
        ...
        foreach ( var_3 in var_1 )
        {
            if ( isdefined( var_3.classname ) )
            {
                ...
                if ( var_3.classname == "agent" || var_3.classname == "player" )
                {
                    ...
                    if ( var_3 scripts\mp\gametypes\br_public::wait_for_players_on_platform() )
                        continue;
...
claymore_explodeonnotify()
{
    ...
    self.global_relic_amped_func = var_1;
    thread claymore_explode( var_1 );
}
...
claymore_modifieddamage( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    {
        if ( isdefined( self.enemy_respawn_manager ) )
        {
            foreach ( var_6 in self.enemy_respawn_manager )
            {
    ...
    else if ( var_1 == getcompleteweaponname( "claymore_radial_mp" ) )
    {
        ...
        thread enemy_mine_damaged_think( var_2 );
    }
    else
        return var_4;
    ...
    if ( isdefined( var_2.global_relic_amped_func ) )
    {
        var_14 = isdefined( var_2.owner ) && var_2.global_relic_amped_func == var_2.owner;
        var_15 = var_2.global_relic_amped_func == var_0;
...
enemy_mine_damaged_think( var_0 )
{
    ...
    if ( !isdefined( self.enemy_respawn_manager ) )
        self.enemy_respawn_manager = [];

    self.enemy_respawn_manager[self.enemy_respawn_manager.size] = var_0;
    waittillframeend;
    self.enemy_respawn_manager = undefined;
}
...
enemy_push_players_logic( var_0 )
{
    self endon( "death" );
