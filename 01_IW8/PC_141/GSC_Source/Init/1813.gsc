...
setupvfxobjs( var_0 )
{
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( var_2.script_noteworthy ) && scripts\engine\utility::string_starts_with( var_2.script_noteworthy, "vfx_" ) )
        {
            ...
            if ( !reset_target_group( "setupVFXObjs(): obj", var_2 ) )
                continue;

            if ( !reset_target_group( "setupVFXObjs(): model", var_3 ) )
                continue;
...
delayfxcall( var_0, var_1, var_2 )
{
    ...
    if ( !reset_target_group( "delayFXCall()", var_1 ) )
        return;
...
animsuite_translation_pingpong()
{
    ...
    for (;;)
    {
        ...
        if ( !reset_target_group( "animSuite_Translation_PingPong()", self ) )
            return;
...
animsuite_playthreadedsound( var_0, var_1 )
{
    ...
    if ( !reset_target_group( "animSuite_playThreadedSound()", self ) )
        return;
...
animsuite_translation_once()
{
    ...
    for (;;)
    {
        ...
        if ( !reset_target_group( "animSuite_Translation_Once()", self ) )
            return;
    }
}
...
animsuite_rotation_pingpong()
{
    ...
    for (;;)
    {
        ...
        if ( !reset_target_group( "animSuite_Rotation_PingPong()", self ) )
            return;
...
animsuite_rotation_continuous()
{
    ...
    for (;;)
    {
        ...
        if ( !reset_target_group( "animSuite_Rotation_Continuous()", self ) )
            return;
    }
}

reset_target_group( var_0, var_1 )
{
    if ( !isdefined( var_1 ) )
        return 0;
