...
quarry_mortar_guys_internal()
{
    level._id_132E2 = loadweaponranktable( "mp/t9_weaponranktable.csv" );
...
needs_radar( var_0 )
{
    ...
    for ( var_1 = monitor_dropped_phones() - 1; var_1 >= 0; var_1-- )
    {
        if ( var_0 >= neurotoxin_damage_loop( var_1 ) )
            return var_1;
    }
...
{
    return level._id_132E2.maxrank;
}
...
neurotoxin_damage_loop( var_0 )
{
    return level._id_132E2.rankinfo[var_0].minxp;
}
...
nonbunkerdoors()
{
    if ( !isdefined( level.wave_ai_killed ) )
        level.wave_ai_killed = getdvarfloat( "scr_playlist_weaponxp_scalar", 1.0 );

    return level.wave_ai_killed;
}
...
getweaponrankxpmultipliertotal()
{
    ...
    var_2 = modeplayerkilledspawn();
    var_3 = mounttrig();
    var_4 = nonbunkerdoors();
    return var_0 * var_1 * var_2 * var_3 * var_4;
}

modeplayerkilledspawn()
{
    if ( !isdefined( level.lethal_crate_spawn ) )
        level.lethal_crate_spawn = float( tablelookup( "mp/gametypesTable.csv", 0, scripts\mp\utility\game::getgametype(), 19 ) );

    return level.lethal_crate_spawn;
}

modelpart()
{
    if ( !isdefined( level.leave_pool_behind_after_deactivation ) )
        level.leave_pool_behind_after_deactivation = getdvarint( "scr_KPH_override", int( tablelookup( "mp/gametypesTable.csv", 0, scripts\mp\utility\game::getgametype(), 20 ) ) );

    return level.leave_pool_behind_after_deactivation;
}

modemayconsiderplayerdead()
{
    if ( !isdefined( level.leaveforplayer ) )
    {
        ...
        level.leaveforplayer = modelpart() / 60 * ( var_0 / 60 );
    }

    return level.leaveforplayer;
}
