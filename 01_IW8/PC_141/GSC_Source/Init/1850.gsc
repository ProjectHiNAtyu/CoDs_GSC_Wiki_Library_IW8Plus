...
roundend_checkscorelimit( var_0, var_1 )
{
    ...
    else if ( level.roundscorelimit > 0 )
    {
        ...
        {
            if ( level scripts\mp\utility\game::resetunresolvedcollision( var_8 ) )
                continue;
...
forceend( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( level scripts\mp\utility\game::resetunresolvedcollision( var_2 ) )
                continue;
...
onscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        var_2 = scripts\mp\gamescore::define_as_level_infil_driver();
    ...
    else
    {
        ...
        {
            level.spawner_debug_model = var_2;
...
updategameevents()
{
    ...
    if ( level.teambased )
    {
        ...
        if ( !var_17 && !var_20 )
        {
            ...
            if ( _id_1228C() && game["roundsPlayed"] == 0 )
...
waitforplayers( var_0 )
{
    ...
    for (;;)
    {
        ...
        foreach ( var_12 in level.teamnamelist )
        {
            if ( !dropoff_sound_playerwm_handler( var_12 ) )
            {
...
dropoff_sound_playerwm_handler( var_0 )
{
    var_1 = 0;
...
prematchperiod()
{
    ...
    if ( isdefined( level.zombienumhitsheli ) && [[ level.zombienumhitsheli ]]() )
        return;
    ...
    if ( getdvarint( "scr_live_lobby", 0 ) == 1 && !istrue( level._id_1233E ) )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            level thread scripts\mp\gametypes\br_public::chopperexfil_sh020_start();
    ...
    else if ( !istrue( level._id_1233E ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            if ( var_2 != 0 )
            {
                level thread _id_13228();
                level thread create_vehicle_vehicledata();
                level waittill( "start_prematch" );
            }
...
freezeplayerforroundend( var_0 )
{
    ...
    if ( self isonground() || self isonladder() )
        ...
        thread lbravo_spawner_jammer2b();
}
...
givematchbonus( var_0, var_1 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::namehud() != "dmz" && scripts\mp\utility\game::namehud() != "risk" && scripts\mp\utility\game::namehud() != "kingslayer" && scripts\mp\utility\game::namehud() != "rumble" )
        return;
...
setxenonranks( var_0 )
{
    ...
    {
        var_3 = var_1[var_2];
        addwatch2v2topscore( var_3 );
    }
}

addwatch2v2topscore( var_0, var_1 )
{
    if ( !isdefined( var_0 ) )
        return;
    ...
    else if ( scripts\mp\utility\game::reset_attack_next_available_time() )
    {
        var_3 = 1000 - var_1;
...
checktimelimit( var_0 )
{
    ...
    if ( var_1 > 0 && !isdefined( level.laser_start_ent_thermal ) )
    {
        if ( var_2 )
        {
            if ( istrue( level._id_120EE ) )
    ...
    if ( var_2 )
    {
        ...
        if ( level.currenttimelimitdelay < level.ontimelimitgraceperiod )
        {
            if ( istrue( level._id_120EE ) )
            {
                if ( !isdefined( level.laser_shut_down_interact_monitor ) )
...
checkteamscorelimitsoon( var_0 )
{
    ...
    if ( !level.playedmatchendingsoon && scripts\mp\utility\game::canplayhalfwayvo() )
    {
        if ( getteamscore( var_0 ) >= int( level.scorelimit * level.currentround - level.scorelimit / 2 ) )
        {
            ...
            thread scripts\mp\music_and_dialog::spawn_fulton_ac130_mdl( var_0 );
...
checkscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( level scripts\mp\utility\game::resetunresolvedcollision( level.teamnamelist[var_2] ) )
                continue;
    ...
    else
    {
        ...
        if ( self.score < level.roundscorelimit && !istrue( level._id_11F20 ) )
            return 0;
    }
...
matchstarttimerperplayer_internal( var_0, var_1 )
{
    ...
    if ( var_1 )
        childthread snapshot_grenade_applysnapshot( var_2 ) childthread clearvisionsettimer( var_2 );

    wait( var_2 );
}

snapshot_grenade_applysnapshot( var_0 )
{
    level endon( "match_start_timer_beginning" );
    var_1 = spawn( "script_origin", ( 0, 0, 0 ) );
    var_1 hide();
...
teamstarttimer( var_0, var_1, var_2 )
{
    ...
    if ( !level.gameended )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            thread scripts\mp\gametypes\br_public::zombienumhitscar( var_0 );
            thread scripts\mp\gametypes\br_public::zombiehud( var_0 );
            thread scripts\mp\gametypes\br_public::bot_gametype_attacker_limit_for_team();
        }
...
{
    if ( isdefined( level._id_11E07 ) )
    {
        level [[ level._id_11E07 ]]( var_0, var_1 );
        return;
    }

    if ( !level.doeomcombat )
    {
        wait( level.x1airstrikemanager + 0.1 );
...
vehicle_collision_registerevent( var_0 )
{
    if ( isdefined( var_0 ) )
    {
        foreach ( var_2 in level.players )
        {
            ...
            {
                if ( scripts\mp\utility\game::getgametype() == "br" )
                    var_4 = scripts\mp\gametypes\br_public::munitions_override_time( var_2.team, var_2.squadindex );
                else
                    var_4 = scripts\mp\utility\teams::getteamdata( var_2.team, "players" );

                if ( isdefined( var_2.vehicle_collision_registereventinternal ) )
                    var_2.vehicle_collision_registereventinternal = !var_2.vehicle_collision_registereventinternal;
                else
                    var_2.vehicle_collision_registereventinternal = 1;
                ...
                if ( istrue( scripts\mp\utility\game::matchmakinggame() ) )
                {
                    if ( isdefined( var_2 ) && var_2 isfireteamleader() )
                    {
                        ...
                        foreach ( var_10 in var_8 )
                        {
                            ...
                            if ( var_11 != var_3 )
                            {
                                if ( isdefined( var_10.vehicle_collision_registereventinternal ) )
                                    var_10.vehicle_collision_registereventinternal = !var_10.vehicle_collision_registereventinternal;
                                else
                                    var_10.vehicle_collision_registereventinternal = 1;
...
overrideviewkickscalek98()
{
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        if ( !scripts\mp\utility\game::privatematch() && !scripts\mp\gametypes\br_public::register_player_character() )
        {
            var_0 = [];
            ...
            foreach ( var_2 in level.players )
            {
                ...
                if ( isdefined( var_2.vehicle_collision_registereventinternal ) && var_2.vehicle_collision_registereventinternal )
                {
                    var_3 = var_2.team;
                    var_4 = 0;
                    var_5 = var_0.size;
                    ...
                    {
                        var_0[var_5] = var_3;
                        var_7 = scripts\mp\gametypes\br_public::munitions_override_time( var_2.team, var_2.squadindex );
                        var_8 = [];
...
callback_startgametype()
{
    ...
    level.get_closest_visible_drone = 0;
    level.gameended = 0;
    level.forcedend = 0;
    ...
    level.dmz_numteamswithplayers = getdvarint( "scr_game_cdltuning", 0 );
    level.handledevcommand_publicevents = scripts\mp\utility\game::isanymlgmatch();
    ...
    level._id_1332C = getdvarint( "scr_game_wrapKillstreaks" );
    ...
    level.matchrules_vampirism = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_vampirism", "scr_game_vampirism" );

    if ( level.matchrules_vampirism )
        level.modifyplayerdamage = scripts\mp\damage::gamemodemodifyplayerdamage;

    level.finalkillcamtype = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_killcamType", "scr_game_killcamType" );

    if ( level.finalkillcamtype == 2 )
        level.skipfinalkillcam = 1;

    level.allowkillstreaks = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_allowKillstreaks", "scr_game_allowKillstreaks" );
    level.roundretainstreaks = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreaks", "scr_game_roundRetainStreaks" );
    level.roundretainstreakprog = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreakProg", "scr_game_roundRetainStreakProg" );
    level.deathretainstreaks = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_deathRetainStreaks", "scr_game_deathRetainStreaks" );
    level.allowperks = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_allowPerks", "scr_game_allowPerks" );
    level.allowsupers = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_allowSupers", "scr_game_allowSupers" );
    level.superfastchargerate = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_superFastChargeRate", "scr_game_superFastChargeRate" );
    level.superpointsmod = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_superPointsMod", "scr_game_superPointsMod" );
    level.spawnprotectiontimer = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_spawnProtectionTimer", "scr_game_spawnProtectionTimer" );
    level.lethaldelay = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_lethalDelay", "scr_game_lethalDelay" );
    level.equipmentmatchstartshieldms = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_equipmentMSProtect", "scr_game_equipmentMSProtect" ) * 1000;
    level.magcount = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_magcount", "scr_game_magcount" );

    if ( !isdefined( level.practiceround ) )
        level.practiceround = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_practiceRound", "scr_game_practiceRound" );

    level.postgameexfil = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_postGameExfil", "scr_game_postGameExfil" );
    level.exfilactivetimer = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilActiveTimer", "scr_game_exfilActiveTimer" );
    level.exfilextracttimer = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilExtractTimer", "scr_game_exfilExtractTimer" );
    level.useammorestocklocs = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_useAmmoRestockLocs", "scr_game_useAmmoRestockLocs" );
    level._id_125CB = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_allow3rdspectate", "scr_game_allow3rdspectate" );
    setdvar( "LKNNQKNTS", level._id_125CB );
    ...
    if ( scripts\mp\utility\game::replace_access_card_on_deathordisconnect() )
        downangles();
    ...
    level.radarhideshots = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + var_0 + "_radarHidePings", "scr_game_radarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_minimap", level.radarhideshots );
    level.navbarhideshots = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + var_0 + "_navBarHidePings", "scr_game_navBarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_navbar", level.navbarhideshots );
    level.navbarhideenemies = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + var_0 + "_navBarHideEnemy", "scr_game_navBarHideEnemy" );
    ...
    level.handlecommand_br = getdvarint( "scr_player_disableMount" );
    level.disablebattlechatter = getdvarint( "scr_game_disableBattleChatter" );
    level.hacking_lua_notify_func = getdvarint( "scr_game_disableAnnouncer" );
    level.scoremod = [];
    ...
    {
        if ( scripts\mp\utility\game_utility_mp::spawnedasspectator() )
        {
    ...
    scripts\cp_mp\utility\game_utility::processassist_regularcp();
    level.maxallowedteamkills = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_ffPunishLimit", "scr_game_ffPunishLimit" );
    thread scripts\mp\init_cp_mp::init();
    thread scripts\mp\teams::init();
    scripts\mp\vehicles\little_bird_mg_mp::_id_1292A();
    scripts\cp_mp\utility\player_utility::progression_speed();
    ...
    scripts\mp\utility\player_frame_update_aggregator::registerplayerframeupdatecallback( scripts\mp\weapons::on_min );

    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::privatematch() )
        level scripts\common\ui::lui_registercallback( "play_again_squad_request", ::vehicle_collision_registerevent );
    ...
    if ( getdvarint( "scr_allow_custom_loadouts", 0 ) == 0 || scripts\mp\utility\game::registercarryobjectpickupcheck() )
...
timelimitthread()
{
    ...
    while ( game["state"] == "playing" )
    {
        ...
        if ( isdefined( level.starttime ) )
        {
            if ( gettimeremaining() < 3000 || istrue( level.laser_start_ent_thermal ) )
            {
                waitframe();
                continue;
            }
...
timelimitclock()
{
    ...
    if ( isdefined( level._id_129E0 ) )
    {
        thread [[ level._id_129E0 ]]();
        return;
    }
    ...
    while ( game["state"] == "playing" )
    {
        if ( !level.timerstopped && scripts\mp\utility\game::gettimelimit() && !istrue( level.bombsplanted ) )
        {
            ...
            if ( var_6 <= 10 || var_6 <= 30 && var_6 % var_4 == var_7 )
            {
                ...
                if ( var_8 )
                {
                    var_9 = minigun_sweep_to_loc_safe( var_5 );
...
gametimer()
{
    ...
    if ( !isdefined( game["startTimeFromMatchStart"] ) )
    {
        ...
        scripts\common\utility::_id_12C2C( level.smoke_wheelson_chosen_spawn );
    }
...
startgame()
{
    ...
    if ( getdvarint( "scr_game_updated_prematch_allows", 1 ) == 1 )
    {
        foreach ( var_1 in level.players )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                var_1 scripts\mp\persistence::put_passenger_in_truck();
        }
    }
...
cleanpatchablecollision()
{
    ...
    foreach ( var_3, var_2 in var_0 )
    {
        if ( isdefined( var_2 ) )
        {
            if ( isdefined( level.trophy_watch_flight ) && istrue( level.trophy_watch_flight[var_3] ) )
            {
                level.trophy_watch_flight[var_3] = undefined;
                continue;
            }
    ...
    level.trophy_tryreflectsnapshot = 1;

    if ( isdefined( level.trophy_watch_flight ) )
    {
        foreach ( var_3, var_5 in level.trophy_watch_flight )
        {

        }
    }

    level.trophy_watch_flight = undefined;
}

_id_11CBB( var_0 )
{
    if ( istrue( level.trophy_tryreflectsnapshot ) )
        return;

    if ( !isdefined( level.trophy_watch_flight ) )
        level.trophy_watch_flight = [];

    level.trophy_watch_flight[var_0] = 1;
}
...
wavetimerwatcher()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_1312D( "wave_respawn_allies", "wave_respawn_axis" );
...
displayroundend( var_0, var_1, var_2 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() && scripts\mp\utility\game::iswinbytworulegametype() && scripts\mp\utility\game::_id_122A8() )
    {
        var_0 = "tie";
...
displayroundswitch()
{
    ...
    if ( isdefined( level.spawnboardroom_juggdrop ) )
        [[ level.spawnboardroom_juggdrop ]]();

    var_0 = modular_spawning_vehicles();
...
freezeallplayers( var_0, var_1, var_2, var_3 )
{
    ...
    var_4 = scripts\mp\utility\game::remove_munitions_globally();
...
setendofroundsoundtimescalefactor()
{
    if ( !isdefined( level.init_relic_dfa ) )
    {
        createheadiconatorigin( "end_of_round" );
        level.init_relic_dfa = 1;
        level._id_11CCF = undefined;
    }
}

resetsoundtimescalefactor()
{
    if ( !isdefined( level._id_11CCF ) )
    {
        createheadiconatorigin( "reset" );
        level.init_relic_dfa = undefined;
        level.taketeamplunder = undefined;
        level._id_11CCF = 1;
    }
}
...
restart()
{
    if ( isdefined( level.trace_to_origin_weight ) )
        [[ level.trace_to_origin_weight ]]();
...
endgame( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isdefined( level.spawner_debug_model ) )
        var_0 = level.spawner_debug_model;
...
endgame_regularmp( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\common\utility::_id_12C2C( level.smokeglvfx );
    var_5 = 0;
    ...
    level.get_ai_within_range = istrue( var_3 ) && scripts\mp\utility\game::getgametype() == "ctf";

    if ( isdefined( var_0 ) && level.get_ai_within_range )
    {
...
init_placed_equipment( var_0 )
{
    if ( isdefined( var_0 ) )
    {
        if ( var_0 == 1 || var_0 == 2 )
        {
            level.ai_dropgren_override_hide = 0;
            level.addallkillstreaksunlocked = var_0;
...
endgame_endround( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() )
    {
        if ( scripts\mp\utility\game::getgametype() == "arena" )
        {
            ...
            foreach ( var_7 in level.players )
            {
                ...
                {
                    if ( var_7 == var_4[0] )
                        var_7 scripts\cp_mp\pet_watch::any_enemy_nearby();
                }
            }
        }
    ...
    rankedmatchupdates( var_0 );
    setdvar( "SLLNLPRON", scripts\mp\utility\game::remove_munitions_globally() );
    ...
    else
    {
        if ( level.teambased )
        {
            ...
            {
                if ( level scripts\mp\utility\game::resetunresolvedcollision( var_25 ) )
                    continue;
            ...
            if ( !scripts\mp\utility\game::waslastround() )
            {
                ...
                foreach ( var_7 in level.players )
                {
                    ...
                    if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
                        var_7 scripts\common\utility::_id_12C2C( level.smoke_door, var_7.matchdatalifeindex, undefined, undefined, "MOD_ROUND_ENDED", "none", undefined, undefined );
                }
...
endgame_endgame( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\gametypes\br_public::register_player_character() && !level.forcedend )
    {
        level.ai_dropgren_override_hide = 1;
        level scripts\common\ui::lui_registercallback( "exit_squad_eliminated", ::init_placed_equipment );
    }

    if ( scripts\mp\utility\game::matchmakinggame() && isdefined( level._id_12283 ) && [[ level._id_12283 ]]() )
    ...
    updateleaderboardstats();
    level scripts\mp\accolades::initlocs_radio();
    level.doingbroshot = scripts\mp\broshot::initbroshot( var_0 );
    var_4 = scripts\mp\gametypes\br_gametypes::relic_amped_in_warning( "endGameIntermission" );

    if ( !level.doingbroshot && istrue( var_4 ) )
    {
        level notify( "spawning_intermission" );
        var_5 = modular_spawning_vehicles();

        foreach ( var_7 in level.players )
        {
            var_7 thread scripts\mp\utility\game::setuipostgamefade( 0.0 );

            if ( !isdefined( level.spawner_recently_used ) || !var_7 [[ level.spawner_recently_used ]]() )
    ...
    if ( !level.doingbroshot )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            scripts\mp\gametypes\br_circle::play_vo_on_intel_pickup();
            ...
            if ( getdvarint( "post_game_play_again_enabled", 1 ) == 1 )
            {
                ...
                {
                    if ( istrue( level.ai_dropgren_override_hide ) )
    ...
    if ( scripts\mp\utility\game::getgametype() != "br" || scripts\mp\utility\game::namehud() == "dmz" || scripts\mp\utility\game::namehud() == "risk" || scripts\mp\utility\game::namehud() == "kingslayer" || scripts\mp\utility\game::namehud() == "rumble" || scripts\mp\utility\game::remove_outline() )
        processlobbydata();
    ...
    foreach ( var_7 in level.players )
    {
        ...
        if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
            var_7 scripts\common\utility::_id_12C2C( level.smoke_door, var_7.matchdatalifeindex, undefined, undefined, "MOD_GAME_ENDED", "none", undefined, undefined );
        ...
        var_7 scripts\common\utility::_id_12C2C( level.smodelcollections );
    }

    scripts\common\utility::_id_12C2C( level.smokekill );
    ...
    if ( scripts\mp\gametypes\br_public::register_player_character() && isdefined( level.addallkillstreaksunlocked ) && level.addallkillstreaksunlocked == 2 )
    {
...
_id_12003()
{
    if ( !isdefined( level.init_relic_aggressive_melee ) )
    {
    ...
    level.init_relic_aggressive_melee = 1;
}
...
modular_spawning_vehicles()
{
    ...
    {
        var_0 = spawnstruct();
        var_0.origin = level.create_script_wait_for_flags._id_125CF.origin;
        var_0.angles = level.create_script_wait_for_flags._id_125CF.angles;
    }
...
br_pickupdenyalreadyhaverevive( var_0 )
{
    if ( isdefined( var_0 ) && !isdefined( var_0.clientmatchdataid ) )
    {
        var_0.clientmatchdataid = level.get_closest_visible_drone;
        level.get_closest_visible_drone++;
    }
}

_id_1197A( var_0 )
{
    if ( istrue( var_0._id_1197A ) )
        return;
    ...
    scripts\mp\matchdata::_id_12119( var_0 );
}
...
trackattackerleaderboarddeathstats( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( var_0 ) && isplayer( var_0 ) )
    {
        if ( var_3 != "MOD_FALLING" )
        {
            ...
            {
                var_0 scripts\common\utility::_id_12C2C( level.skydivestreamhintdvars, "tactical", "kills", 1, var_2 );
                var_0 scripts\common\utility::_id_12C2C( level.skydivestreamhintdvars, "tactical", "hits", 1, var_2 );
            ...
            if ( var_3 == "MOD_MELEE" && !scripts\mp\riotshield::isriotshield( var_2 ) && !scripts\mp\utility\weapon::isknifeonly( var_2 ) && !scripts\mp\utility\weapon::refreshsquadspawnareaomnvars( var_2 ) && !scripts\mp\utility\weapon::isaxeweapon( var_2 ) )
            {
                var_0 scripts\common\utility::_id_12C2C( level.skydivestreamhintdvars, "none", "kills", 1, var_2 );
                var_0 scripts\common\utility::_id_12C2C( level.skydivestreamhintdvars, "none", "hits", 1, var_2 );
...
setweaponstat( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_3.triggeredwincondition ) )
        var_3 = var_3.triggeredwincondition;

    if ( !isdefined( level.destorder ) )
        level.destorder = scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_track_picked_up_weapon_stats", 1 ) == 1;

    if ( scripts\mp\utility\weapon::ispickedupweapon( var_3 ) && !istrue( level.destorder ) )
        return;
    ...
    if ( var_5 == "weapon_grenade" || var_5 == "weapon_explosive" || var_4 == "trophy_mp" || var_4 == "forcepush_mp" )
    {
        ...
        scripts\common\utility::_id_12C2C( level.smoke_screen, var_4, var_2, var_1, var_6, var_3 );
        return;
    }
    ...
    if ( var_2 == "deaths" )
    {
        ...
        scripts\common\utility::_id_12C2C( level.smoke_screen, var_9, "deaths", var_1, var_6, var_3 );

        foreach ( var_12 in var_10 )
        {
            scripts\mp\persistence::incrementattachmentstat( var_12, var_2, var_1, var_3 );
            scripts\common\utility::_id_12C2C( level.skydivestreamhintdvars, var_12, var_2, var_1, var_3 );
        }
    }
}
...
_id_12DEF()
{
    ...
    if ( isdefined( level.cloudorigin ) && isdefined( level.cloudorigin.name ) )
    {
        switch ( level.cloudorigin.name )
        {
            ...
            case "kingslayer":
            case "dmz":
                var_0 = var_0 + ( "_" + level.cloudorigin.name );
                break;
            case "brz":
            case "":
                break;
    ...
    var_1 = scripts\engine\utility::ter_op( level.teambased, self.score, self.send_all_ai_to_players["gamemodeScore"] );
    ...
    incrementleaderboardstat( "kills", var_0, self.send_all_ai_to_players["kills"] );
    incrementleaderboardstat( "deaths", var_0, self.send_all_ai_to_players["deaths"] );

    switch ( scripts\mp\utility\game::getgametype() )
    {
        case "war":
            incrementleaderboardstat( "assists", var_0, self.send_all_ai_to_players["assists"] );
            break;
        case "arena":
            incrementleaderboardstat( "damage", var_0, self.send_all_ai_to_players["damage"] );
            incrementleaderboardstat( "assists", var_0, self.send_all_ai_to_players["assists"] );
            break;
        case "arm":
            incrementleaderboardstat( "captures", var_0, self.send_all_ai_to_players["captures"] );
            incrementleaderboardstat( "defends", var_0, self.send_all_ai_to_players["defends"] );
            break;
        case "cyber":
            incrementleaderboardstat( "revives", var_0, self.send_all_ai_to_players["rescues"] );
            incrementleaderboardstat( "plants", var_0, self.send_all_ai_to_players["plants"] );
            break;
        case "dm":
            updateleaderboardstatmaximum( "streak", var_0, self.send_all_ai_to_players["killChains"] );
            break;
        case "dom":
            incrementleaderboardstat( "captures", var_0, self.send_all_ai_to_players["captures"] );
            incrementleaderboardstat( "defends", var_0, self.send_all_ai_to_players["defends"] );
        ...
        case "sd":
            incrementleaderboardstat( "plants", var_0, self.send_all_ai_to_players["plants"] );
            incrementleaderboardstat( "defuses", var_0, self.send_all_ai_to_players["defuses"] );
            break;
        case "conf":
            incrementleaderboardstat( "confirms", var_0, self.send_all_ai_to_players["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.send_all_ai_to_players["denied"] );
            break;
        case "hq":
            incrementleaderboardstat( "captures", var_0, self.send_all_ai_to_players["captures"] );
            incrementleaderboardstat( "defends", var_0, self.send_all_ai_to_players["defends"] );
            break;
        case "koth":
            incrementleaderboardstat( "objTime", var_0, self.send_all_ai_to_players["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.send_all_ai_to_players["defends"] );
            break;
        case "ctf":
            incrementleaderboardstat( "captures", var_0, self.send_all_ai_to_players["captures"] );
            incrementleaderboardstat( "returns", var_0, self.send_all_ai_to_players["returns"] );
            break;
        case "sr":
            incrementleaderboardstat( "plants", var_0, self.send_all_ai_to_players["plants"] );
            incrementleaderboardstat( "rescues", var_0, self.send_all_ai_to_players["rescues"] );
            break;
        case "siege":
            incrementleaderboardstat( "captures", var_0, self.send_all_ai_to_players["captures"] );
            incrementleaderboardstat( "revives", var_0, self.send_all_ai_to_players["rescues"] );
            break;
        case "grind":
            incrementleaderboardstat( "banks", var_0, self.send_all_ai_to_players["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.send_all_ai_to_players["denied"] );
            break;
        case "infect":
            incrementleaderboardstat( "time", var_0, scripts\mp\utility\stats::getpersstat( "extrascore0" ) );
            incrementleaderboardstat( "infected", var_0, self.send_all_ai_to_players["killsAsInfected"] );
            break;
        case "gun":
            incrementleaderboardstat( "stabs", var_0, self.send_all_ai_to_players["stabs"] );
            incrementleaderboardstat( "setBacks", var_0, self.send_all_ai_to_players["setbacks"] );
            break;
        case "grnd":
            incrementleaderboardstat( "objTime", var_0, self.send_all_ai_to_players["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.send_all_ai_to_players["defends"] );
        ...
        case "pill":
            ...
            break;
        case "br":
            incrementleaderboardstat( "downs", var_0, self.send_all_ai_to_players["downs"] );
            incrementleaderboardstat( "contracts", var_0, self.send_all_ai_to_players["contracts"] );
            incrementleaderboardstat( "wins", var_0, self.send_all_ai_to_players["wins"] );
            incrementleaderboardstat( "topFive", var_0, self.send_all_ai_to_players["topFive"] );
            incrementleaderboardstat( "topTen", var_0, self.send_all_ai_to_players["topTen"] );
            incrementleaderboardstat( "topTwentyFive", var_0, self.send_all_ai_to_players["topTwentyFive"] );
            incrementleaderboardstat( "gamesPlayed", var_0, self.send_all_ai_to_players["gamesPlayed"] );
            incrementleaderboardstat( "revives", var_0, self.send_all_ai_to_players["rescues"] );
            incrementleaderboardstat( "cash", var_0, self.send_all_ai_to_players["cash"] );
            incrementleaderboardstat( "objTime", var_0, self.send_all_ai_to_players["objTime"] );
            break;
    }
}
...
checkforbestweapon()
{
    var_0 = scripts\common\utility::_id_12C2C( level.skydivehintnotify );
    var_1 = "";
    var_2 = -1;
...
_id_13228( var_0 )
{
    if ( !istrue( level.should_skip_info_loop ) )
        return;
    ...
    if ( var_6 <= var_8 )
    {
        ...
        if ( isdefined( level.tr_vis_radius_override_lod1 ) )
            [[ level.tr_vis_radius_override_lod1 ]]();
    }
...
_id_11C91()
{
    ...
    foreach ( var_2 in var_0 )
        scripts\cp_mp\utility\game_utility::_id_11C8D( var_2, "targetname" );
}

mine_caves_turret_support_behavior()
{
    return 9;
}
...
downangles()
{
    level.allowsupers = getdvarint( "scr_" + scripts\mp\utility\game::namehud() + "_allowSupers", level.allowsupers );
}
...
setup_functions( var_0 )
{
    ...
    if ( isdefined( var_0.get_player_aggro_score ) )
        var_6 = var_0.get_player_aggro_score;
