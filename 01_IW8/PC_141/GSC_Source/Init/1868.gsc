...
init()
{
    ...
    level.laststandhealth = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandHealth", "scr_player_lastStandHealth" );
    level.laststandrevivehealth = scripts\mp\utility\dvars::mortar_initmortars( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveHealth", "scr_player_lastStandReviveHealth" );
    ...
    level.laststandinvulntime = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandInvulnTime", "scr_player_lastStandInvulnTime" );
    level.search_target_think = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveDecayScale", "scr_player_lastStandReviveDecayScale" );
    level.laststandrevivetimer = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveTimer", "scr_player_lastStandReviveTimer" );
    level.laststandsuicidetimer = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandSuicideTimer", "scr_player_lastStandSuicideTimer" );
    level.laststandtimer = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandTimer", "scr_player_lastStandTimer" );
    level.search_turret_no_target_stop_delay = scripts\mp\utility\dvars::mortar_getimpactspot( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandWeapon", "scr_player_lastStandWeapon" );
    level.search_update_delay = scripts\mp\utility\dvars::mortar_init( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandWeaponDelay", "scr_player_lastStandWeaponDelay" );
...
laststandthink()
{
    ...
    onenter();
    var_0 = scripts\engine\utility::_id_13140( "last_stand_heal_success", "last_stand_revived", "last_stand_bleedout", "death", "disconnect", "last_stand_self_revive" );
...
onenter()
{
    ...
    if ( level.gametype == "br" )
        thread score_event_civilian_killed();
    ...
    self.inlaststand = 1;
    self.vo_use_computer = undefined;
    self.hasshownlaststandicon = 0;

    if ( level.gametype == "br" && !istrue( self.pavelow_boss_destroyed_watch ) )
    {
        if ( isdefined( level.dotooclosetominenags ) )
            [[ level.dotooclosetominenags ]]();
    }
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        {
            self.scriptedspawnpointarray = gettime();
            scripts\mp\gametypes\br_gametypes::_id_11E4F( "onLastStandEnter" );

            if ( !istrue( level.skiplaststandsplashes ) )
            {
                var_1 = scripts\mp\gametypes\br_public::multiplepubliceventsenabled( self.team, self.squadindex );
    ...
    var_5 = level.search_turret_no_target_stop_delay;
    ...
    else
    {
        ...
        if ( isdefined( level.set_corpse_detect_ranges ) )
            self thread [[ level.set_corpse_detect_ranges ]]();
        ...
        thread outro_enemy_spawning( var_5 );

        if ( isdefined( level.analytics_lui_mission_end_dlog ) )
            self [[ level.analytics_lui_mission_end_dlog ]]();
        else
            addoverheadicon();
        ...
        if ( isdefined( self.scripted_laser_func ) && self.scripted_laser_func scripts\mp\utility\perk::_hasperk( "specialty_reduce_regen_delay_on_kill" ) )
            self.scripted_laser_func scripts\mp\perks\perkfunctions::regendelayreduce_onkill();
        ...
        if ( isdefined( self.pers["squadMemberIndex"] ) )
        {
            ...
            self.search_nodes = scripts\mp\utility\outline::outlineenableforteam( self, self.team, var_7, "laststand" );
        }
...
{
    if ( !istrue( self.get_sight_alias ) && !istrue( self.disabledeathsdoorssfx ) )
    {
        self.get_sight_alias = 1;
...
outro_enemy_spawning( var_0 )
{
    ...
    {
        var_2 = level.search_update_delay;
...
disableweaponsovertime( var_0 )
{
    ...
    scripts\engine\utility::_id_13141( var_0, "death_or_disconnect" );
...
dodamagewhiledown()
{
    ...
    if ( !istrue( level.search_turret_fire_think ) )
        thread suicidesetup();
    ...
    while ( self.health > 0 )
    {
        ...
        if ( !istrue( scripts\mp\playerlogic::mine_caves_structs() ) )
        {
            var_5 = int( var_4 + var_2 ) - int( var_4 );
            var_4 = var_4 + var_2;
            self.relic_mythic_modifyplayerdamage = 1;
            self dodamage( var_5, self.origin, self, undefined, "MOD_TRIGGER_HURT", var_3, "none" );
            self.relic_mythic_modifyplayerdamage = undefined;
        }
...
_id_11FF8()
{
    ...
    for (;;)
    {
        if ( _id_11FF6( var_1 ) && !istrue( var_1.removematchingents_byclassname ) && var_1 isonground() )
        {
            var_0 notify( "self_revive_start" );
            var_1 setlaststandselfreviving( 1 );
            var_0 thread _id_11FF7();
        }
...
_id_11FF7()
{
    ...
    var_1._id_13113 = 0;
    var_1 thread _id_11FF5();

    while ( scripts\mp\utility\player::isreallyalive( var_0 ) && _id_11FF6( var_0 ) && var_1.curprogress < var_1.usetime )
    {
        while ( !var_0 isonground() && _id_11FF6( var_0 ) && !istrue( var_0.removematchingents_byclassname ) )
            waitframe();
        ...
        if ( !istrue( var_0.removematchingents_byclassname ) )
        {
            var_0 thread scripts\mp\gametypes\br_public::waitandunloadinfils( "iw8_ges_plyr_self_revive", 10 );

            if ( !istrue( var_0._id_1277D ) )
            {
                var_0 attach( "offhand_wm_stim", "tag_accessory_left" );
                var_0._id_1277D = 1;
            }

            var_0.removematchingents_byclassname = 1;
        ...
        if ( var_1.curprogress >= var_1.usetime )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                scripts\mp\gametypes\br_analytics::clear_up_objective_after_delay( var_0 );

            var_0 jeep( "self_revive_success", var_0 );

            if ( isdefined( level._id_11C99 ) )
                var_0 [[ level._id_11C99 ]]();
    ...
    if ( !istrue( var_0.beingrevived ) )
    {
        scripts\mp\gametypes\br::_id_12E03( var_0, var_0, var_1.curprogress, 1 );
...
_id_11FF5()
{
    ...
    var_0 scripts\engine\utility::_id_1313D( "last_stand_finished", "stopped_self_revive", "death_or_disconnect" );
    ...
    var_0.removematchingents_byclassname = 0;
}
...
onexitcommon( var_0 )
{
    ...
    var_1 = level.search_turret_no_target_stop_delay;
    ...
    if ( istrue( scripts\mp\playerlogic::mine_caves_structs() ) )
        scripts\mp\playerlogic::_id_120F3( 0 );

    if ( level.gametype == "br" )
    {
        scripts\mp\gametypes\br::_id_12E03( self, self, 0, 1 );
    ...
    if ( isdefined( level.spawndomplateflagtestmap ) )
        self [[ level.spawndomplateflagtestmap ]]( var_0 );
...
onrevive( var_0, var_1 )
{
    ...
    scripts\common\utility::binoculars_showtargetmarker( 1 );

    if ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" )
        scripts\mp\gametypes\br_plunder::weapon_xp_iw8_ar_asierra12( self, 1, 1 );
    ...
    var_3 = level.search_turret_no_target_stop_delay;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && !istrue( self.gulag ) )
    {
        ...
        if ( !istrue( level.skiplaststandsplashes ) )
        {
            var_5 = scripts\mp\gametypes\br_public::multiplepubliceventsenabled( self.team, self.squadindex );
    ...
    self.scriptedspawnpointsonmigration = undefined;
...
ondeath( var_0 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( self.scripted_laser_func ) && istrue( self.scripted_laser_func.inlaststand ) )
        self.scripted_laser_func thread tr_playerdisconnect( var_0, self.search_acceleration, self.searchcircleorigin );

    if ( istrue( self.removematchingents_byclassname ) )
...
revivesetup( var_0 )
{
    ...
    if ( var_0 scripts\mp\gametypes\br_public::pavelow_boss_destroyed_watch() )
        var_0.laststandreviveent _id_11FF8();
}
...
_id_11F9D()
{
    ...
    var_11 = var_2.scripted_laser_func;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
        scripts\mp\gametypes\br_analytics::clear_trap_flag( var_0, var_11, var_2, var_5, var_6, var_7, var_8, var_12, var_13, var_14, var_16 );

    var_0 jeep( "self_revive_on_kill_success", var_0 );
...
tr_playerdisconnect( var_0, var_1, var_2 )
{
    var_3 = var_0.victim;
...
{
    self endon( "death" );
    self.owner scripts\engine\utility::_id_1312E( "death_or_disconnect", "last_stand_finished", "last_stand_heal_active" );
    self delete();
}
...
revivetriggerthink( var_0 )
{
    ...
    for (;;)
    {
        ...
        if ( istrue( var_1.quest_scavdistmin ) )
        {
            ...
            while ( istrue( var_1.quest_scavdistmin ) && var_1 usebuttonpressed() )
                waitframe();
        }
        ...
        self.owner scripts\mp\playerlogic::_id_120F3( 1 );
        var_2 = 0;
        ...
        var_1 thread _id_12875( self.owner );

        if ( level.gametype == "br" )
            self.owner scripts\mp\gametypes\br_public::_id_12153( 1 );

        var_3 = scripts\engine\utility::_id_1313D( "use_hold_revive_success", "use_hold_revive_fail", "death_or_disconnect" );

        if ( var_3 == "use_hold_revive_success" )
            var_2 = 1;

        var_1 notify( "finish_buddy_reviving" );
        self.owner scripts\mp\playerlogic::_id_120F3( 0 );
        var_2 = self.owner jeep( var_3, var_1 );
...
_id_12875( var_0 )
{
    ...
    var_3._id_11D7D = var_2;

    for ( var_4 = 0; !var_4 || istrue( var_1.can_use_super_func ); var_1._id_13114 = 1 )
    {
        var_4 = var_1 scripts\cp_mp\killstreaks\killstreakdeploy::switchtodeployweapon( var_2, var_3, ::_id_1293D, undefined, ::train_handle_collide_mines, undefined, ::train_horn_sfx, 0 );
        waitframe();
    }

    var_1.can_use_super_func = 1;
    var_1._id_13114 = 0;
    var_1 thread _id_1321D();
    var_1 thread _id_13239( var_0 );
    var_1 thread _id_1323A( var_0 );
}
...
_id_1293D( var_0 )
{
    ...
    if ( !istrue( var_1._id_11D7E ) )
        return;

    var_1 scripts\engine\utility::_id_1313D( "death_or_disconnect", "finish_buddy_reviving", "last_stand_start" );
}
...
jeep( var_0, var_1 )
{
    ...
    var_2.removematchingents_byclassname = 0;
    ...
    if ( level.gametype == "br" )
    {
        ...
        var_2 scripts\mp\gametypes\br_public::_id_12153( 0 );
        var_2 scripts\mp\gametypes\br_public::_id_1215C( 0 );
    }
    ...
    if ( var_3 )
    {
        ...
        if ( level.gametype == "br" )
        {
            if ( !istrue( level.gameended ) )
            {
                var_2 scripts\mp\gametypes\br_public::_id_12155( 0 );
                scripts\mp\events::revivedplayer( var_1, var_2 );
                scripts\cp\helicopter\chopper_boss::tr_removelocaleinstance( var_1, var_2 );
                var_2 scripts\cp\helicopter\chopper_boss::_id_1271C( "alive_not_downed" );

                if ( var_1 != var_2 )
                    var_1 scripts\mp\utility\stats::incpersstat( "rescues", 1 );

                var_2 scripts\mp\gametypes\br_gametypes::_id_11E4F( "onLastStandRevive", var_1 );
                var_2.get_sight_alias = 0;
                var_2 stoplocalsound( "deaths_door_in" );

                if ( !istrue( var_2.br_skip_revive_sound ) )
                {
            ...
            if ( !scripts\mp\gametypes\br_public::reset_global_stealth_settings() && !istrue( var_2.br_skip_revive_sound ) )
    ...
    var_2 _id_128CE();
    var_2 thread entityplunderlosealldeposited();
    return var_3;
}
...
decayreviveprogress()
{
    ...
    self endon( "death" );
    level endon( "game_ended" );
    var_0 = level.search_target_think;
...
useholdthinkcleanup( var_0, var_1 )
{
    ...
    var_3 = self.owner;
    var_4 = var_3 scripts\engine\utility::_id_1313E( "death_or_disconnect", "use_hold_think_success", "use_hold_think_fail", "last_stand_finished" );
    self.inuse = 0;
    var_1 delete();
...
suicidesetup()
{
    ...
    for (;;)
    {
        ...
        if ( self stancebuttonpressed() && self isinexecutionvictim() == 0 && !scripts\mp\playerlogic::mine_caves_structs() )
        {
...
_id_12826()
{
    ...
    while ( !var_4 && !var_1 )
    {
        ...
        var_5 = scripts\engine\utility::_id_13145( 0.5, "stance_pressed_down", "stance_pressed_up", "last_stand_finished", "last_stand_self_revive", "last_stand_bleedout" );
...
handlerevivemessage()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::_id_1313C( "super_ready", "handle_revive_message" );
