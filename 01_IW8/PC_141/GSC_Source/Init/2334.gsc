...
tryuseautosentry( var_0, var_1 )
{
    ...
    if ( var_2 )
        scripts\common\utility::_id_12C2C( level.smartwatchinteract, level.sentrysettings["sentry_minigun"].streakname, self.origin );
...
tryusesam( var_0, var_1 )
{
    ...
    if ( var_2 )
        scripts\common\utility::_id_12C2C( level.smartwatchinteract, level.sentrysettings["sam_turret"].streakname, self.origin );
...
tryuseshocksentry( var_0 )
{
    ...
    if ( var_1 )
        scripts\common\utility::_id_12C2C( level.smartwatchinteract, var_0.streakname, self.origin );
    else
        waitframe();
...
setcarryingsentry( var_0, var_1, var_2, var_3 )
{
    ...
    for (;;)
    {
        var_4 = scripts\engine\utility::_id_13137( "place_sentry", "cancel_sentry", "force_cancel_placement", "emp_applied" );
...
sentry_oncarrierdeathoremp( var_0, var_1 )
{
    ...
    var_0 scripts\engine\utility::_id_1312D( "death", "emp_applied" );
...
sentry_oncarrierchangedteam( var_0 )
{
    self endon( "placed" );
    self endon( "death" );
    var_0 scripts\engine\utility::_id_1312D( "joined_team", "joined_spectators" );
    self delete();
}
    ...
    if ( !isdefined( self.samtargetent ) )
    {
        if ( level.teambased )
        {
            ...
            if ( istrue( var_2 ) && getdvarint( "scr_uav_for_squad_only", 1 ) )
            {
                ...
                {
                    foreach ( var_11, var_7 in level.squaddata[var_5] )
                    {
                        foreach ( var_9 in level.uavmodels[var_11] )
            ...
            else
            {
                ...
                {
                    foreach ( var_16 in level.uavmodels[var_14] )
            ...
            {
                if ( isdefined( var_16.isleaving ) && var_16.isleaving )
                    continue;

                if ( sighttracepassed( var_0, var_16.origin, 0, self ) )
                    return var_16;
            }

            foreach ( var_22 in level.littlebirds )
            {
                if ( isdefined( var_22.team ) && var_22.team == self.team )
                    continue;

                if ( sighttracepassed( var_0, var_22.origin, 0, self ) )
                    return var_22;
            }

            foreach ( var_25 in level.helis )
            {
                if ( isdefined( var_25.team ) && var_25.team == self.team )
                    continue;

                if ( sighttracepassed( var_0, var_25.origin, 0, self ) )
                    return var_25;
            }
            ...
            foreach ( var_16 in level.remote_uav )
            {
                ...
                if ( isdefined( var_16.team ) && var_16.team == self.team )
                    continue;

                if ( sighttracepassed( var_0, var_16.origin, 0, self, var_16 ) )
                    return var_16;
            }
        }
        else
        {
            foreach ( var_16 in level.uavmodels )
            {
                if ( isdefined( var_16.isleaving ) && var_16.isleaving )
                    continue;

                if ( isdefined( var_16.owner ) && isdefined( self.owner ) && var_16.owner == self.owner )
                    continue;
            ...
            foreach ( var_22 in level.littlebirds )
            {
                if ( isdefined( var_22.owner ) && isdefined( self.owner ) && var_22.owner == self.owner )
                    continue;
            ...
            foreach ( var_25 in level.helis )
            {
                if ( isdefined( var_25.owner ) && isdefined( self.owner ) && var_25.owner == self.owner )
                    continue;
...
sentryshocktargets()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_13141( 1, "turret_on_target" );
...
searchforshocksentryairtarget()
{
    if ( isdefined( level.uavmodels ) )
    {
        if ( level.teambased )
        {
            ...
            if ( istrue( var_0 ) && getdvarint( "scr_uav_for_squad_only", 1 ) )
            {
                ...
                {
                    foreach ( var_9, var_5 in level.squaddata[var_3] )
                    {
                        foreach ( var_7 in level.uavmodels[var_9] )
                        {
                            if ( targetvisibleinfront( var_7 ) )
                                return var_7;
                        }
                    }
                }
            ...
            else
            {
                foreach ( var_3 in var_1 )
                {
                    foreach ( var_7 in level.uavmodels[var_3] )
                    {
                        if ( targetvisibleinfront( var_7 ) )
                            return var_7;
                    }
        ...
        {
            foreach ( var_7 in level.uavmodels )
            {
                if ( var_7.owner == self.owner )
                    continue;
    ...
    {
        foreach ( var_18 in level.helis )
        {
            if ( var_18.streakname != "jackal" )
                continue;

            if ( level.teambased && var_18.team == self.owner.team )
                continue;

            if ( !level.teambased && var_18.owner == self.owner )
                continue;

            if ( targetvisibleinfront( var_18 ) )
                return var_18;
        }
    }
}
...
marktargetlaser( var_0 )
{
    ...
    self laseron();
    self.laser_on = 1;
    scripts\engine\utility::_id_1312D( "done_firing", "carried" );
    self laseroff();
    self.laser_on = 0;
}
