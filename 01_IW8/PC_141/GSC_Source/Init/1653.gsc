...
vehicle_killriders()
{
    ...
    foreach ( var_1 in self.riders )
    {
        ...
        if ( isdefined( var_1._blackboard ) && isdefined( var_1._blackboard.chosenvehicleanimpos ) && istrue( var_1._blackboard.chosenvehicleanimpos.get_russian_code_from_id ) )
        {
            var_2 = 100.0;
        ...
        if ( scripts\common\utility::iscp() && isdefined( self.get_health_stage ) )
        {
            if ( isdefined( self.get_health_stage.attacker ) && isplayer( self.get_health_stage.attacker ) )
                var_1 dodamage( var_1.maxhealth, self.get_health_stage.attacker.origin, self.get_health_stage.attacker, self.get_health_stage.attacker, self.get_health_stage.meansofdeath, self.get_health_stage.objweapon );
            else
                var_1 kill();
    ...
    if ( isdefined( var_5 ) )
    {
        ...
        foreach ( var_11 in self.riders )
        {
            if ( isalive( var_11 ) && isdefined( var_6[var_11.vehicle_position] ) )
            {
                ...
                if ( isdefined( var_8 ) && istrue( var_8.healthpack_health ) )
                    continue;
...
death_firesound( var_0 )
{
    ...
    scripts\engine\utility::_id_1312D( "fire_extinguish", "stop_crash_loop_sound" );
...
vehicle_damagelogic()
{
    ...
    while ( self.health > 0 )
    {
        ...
        if ( istrue( self.get_dropkit_price ) )
            return;
        ...
        if ( isdefined( var_3 ) )
        {
            ...
            if ( scripts\engine\utility::func_ref_exist( "vehicle_damage_modifier" ) )
            {
                ...
                if ( isdefined( level.kill_sentries ) )
                    var_15 = [[ level.kill_sentries ]]( var_3, self, var_2, var_11, var_6, undefined, var_5, var_4, var_7, var_9, var_8, var_10 );

                if ( isdefined( var_15 ) )
                    self.get_health_stage = var_15;
                else
                    self.get_health_stage = undefined;
...
helicopter_crash_move( var_0, var_1 )
{
    ...
    if ( var_5 )
    {
        ...
        scripts\engine\utility::_id_1312D( "goal", "near_goal" );
    ...
    else
    {
        ...
        while ( var_8 != "death" )
        {
            var_8 = scripts\engine\utility::_id_1312E( "goal", "near_goal", "death" );
