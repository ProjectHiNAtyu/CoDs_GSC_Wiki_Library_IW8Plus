...
tryuseremoteuav( var_0, var_1 )
{
    ...
    if ( var_3 )
    {
        scripts\common\utility::_id_12C2C( level.smartwatchinteract, var_1, self.origin );
...
createcarryremoteuav( var_0, var_1 )
{
    ...
    {
        var_4 = scripts\cp_mp\utility\killstreak_utility::molotov_crate_update_hint_logic_alt();
...
setcarryingremoteuav( var_0 )
{
    ...
    for (;;)
    {
        var_1 = setlocaledefaultvalue( "place_carryRemoteUAV", "cancel_carryRemoteUAV", "weapon_switch_started", "force_cancel_placement", "death_or_disconnect" );
...
setlocaledefaultvalue( var_0, var_1, var_2, var_3, var_4, var_5 )
{
...
carryremoteuav_handleexistence()
{
    ...
    self.owner scripts\engine\utility::_id_1312E( "death_or_disconnect", "joined_team", "joined_spectators" );
...
{
    level endon( "game_ended" );
    var_1 = scripts\engine\utility::_id_13135( "death_or_disconnect", "remoteuav_unlock" );
    ...
    for (;;)
    {
        ...
        if ( level.teambased )
        {
            ...
            if ( istrue( var_9 ) && getdvarint( "scr_uav_for_squad_only", 1 ) )
            {
                ...
                {
                    foreach ( var_19, var_15 in level.squaddata[var_13] )
                    {
                        foreach ( var_17 in level.uavmodels[var_19] )
            ...
            else
            {
                ...
                {
                    foreach ( var_17 in level.uavmodels[var_22] )
                        var_10[var_10.size] = var_17;
                }
            }
        ...
        if ( isdefined( var_26 ) )
        {
            if ( !isdefined( self.lockedtarget ) || isdefined( self.lockedtarget ) && self.lockedtarget != var_26 )
            {
                self weaponlockfinalize( var_26 );
                self.lockedtarget = var_26;
...
{
    level endon( "game_ended" );
    var_1 = scripts\engine\utility::_id_13136( "death_or_disconnect", "carried", "leaving" );
...
remoteuav_watch_distance()
{
    ...
    {
        var_0 = scripts\cp_mp\utility\killstreak_utility::molotov_crate_update_hint_logic_alt();
...
{
    self endon( "death" );
    self.owner scripts\engine\utility::_id_1312D( "joined_team", "joined_spectators" );
    self notify( "death" );
}
