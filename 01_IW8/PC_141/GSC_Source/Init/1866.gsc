...
init()
{
    ...
    level.sol_5_6_pool = getdvarint( "scr_ks_jugg_team_max", 0 );
}

jugg_makejuggernaut( var_0, var_1 )
{
    var_2 = retreatanddie( var_0 );
    ...
    if ( isdefined( level.exfill_chopper_dialogue ) )
        self [[ level.exfill_chopper_dialogue ]]();
    ...
    if ( istrue( self.update_hint_logic_juggernaut ) )
    {
        scripts\mp\playeractions::allowactionset( "fakeJugg", 1 );
        self.update_hint_logic_juggernaut = undefined;
    }
    ...
    else
    {
        ...
        foreach ( var_11 in var_9 )
        {
            ...
            if ( restartcircleelimination() )
            {
                self givemaxammo( var_11 );
                thread return_same_module_as_next_module( var_11 );
            }
        }
    }
    ...
    if ( isdefined( level.ampeddelta ) )
        self [[ level.ampeddelta ]]();
...
jugg_createconfig( var_0, var_1 )
{
    ...
    var_2.sound_events = 5;

    if ( level.gametype == "br" )
        var_2.sound_events = 3;
...
revive_wounded_out_handlerr()
{
    if ( respawntokenenabled() )
    {
        ...
        if ( var_0 )
        {
            ...
            if ( return_num_if_under_vehicle_cap( var_0 ) )
                var_1 = "flames";
...
jugg_getjuggmodels()
{
    ...
    if ( respawntokenenabled() )
    {
        ...
        if ( var_1 )
        {
            if ( return_num_if_under_vehicle_cap( var_1 ) )
            {
...
retreatanddie( var_0 )
{
    ...
    if ( var_2 != "stand" )
    {
        ...
        else if ( var_2 == "crouch" )
        {
            if ( !var_3 )
            {
                var_6 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::nag_player_remind_lore_vo( "stand" ), self, var_5 );
        ...
        else if ( var_2 == "prone" )
        {
            if ( !var_4 )
            {
                if ( var_3 )
                {
                    var_7 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::nag_player_remind_lore_vo( "crouch" ), self, var_5 );
...
jugg_watchoverlaydamagestates( var_0 )
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_1312D( "damage", "jugg_health_regen" );
...
{
    return istrue( self.juggcontext.juggconfig._id_12E6B );
}

restartwrapper()
{
    return istrue( self.juggcontext.juggconfig._id_12E7B );
}

return_same_module_as_next_module( var_0 )
{
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
    var_1 = self getweaponammostock( var_0 );
    thread rewardtotype( var_0, var_1 );
    thread revivingteammate( var_0, var_1 );
...
rewardscriptable()
{
    ...
    var_0.play_sparks_power = 0;

    for (;;)
    {
        self waittill( "perform_hero_drop" );

        if ( !istrue( var_0.play_sparks_power ) )
        {
            var_0.play_sparks_power = 1;
            self radiusdamage( self.origin, 500, 2000, 500, self, "MOD_CRUSH" );
            thread revivent_watchfordeath_safety( var_0 );
        }
    }
}
...
revive_or_disconnect_monitor( var_0, var_1 )
{
...
return_num_if_under_vehicle_cap( var_0 )
{
    return istrue( level.open_teleport_room_door ) && var_0 == 1;
}

brdoesloadoutoptiongivecustomweaponsimmediately()
{
    if ( istrue( level.sol_5_6_pool ) && level.activejuggernauts.size > 0 )
    {
        var_0 = 0;
