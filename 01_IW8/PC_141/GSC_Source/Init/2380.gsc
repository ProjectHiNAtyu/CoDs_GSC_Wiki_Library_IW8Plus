...
laststandheal_onset()
{
    ...
    for (;;)
    {
        if ( self.laststandheal_hassuperweapon && ( !istrue( self.inlaststand ) || istrue( self.stuckinlaststand ) || istrue( scripts\mp\playerlogic::mine_caves_structs() ) ) )
        {
        ...
        else if ( !self.laststandheal_hassuperweapon && istrue( self.inlaststand ) && !istrue( self.stuckinlaststand ) && !istrue( scripts\mp\playerlogic::mine_caves_structs() ) )
        {
            var_0 = scripts\mp\supers::getcurrentsuper().staticdata.weapon;
...
laststandheal_setinactivewhendone()
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    scripts\engine\utility::_id_1313C( "death", "last_stand_finished" );
