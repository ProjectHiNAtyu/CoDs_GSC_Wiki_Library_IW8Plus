...
main()
{
    ...
    else
    {
        ...
        level.smuggler_post_tele_kill = 1;
        level.smokinggunprogress = 1;
        level.snapplayertotoppos = 1;
    }
    ...
    if ( level.smuggler_post_tele_kill )
...
onstartgametype()
{
    ...
    level thread toppercentagetoadjusteconomy();
    level thread isvalidattachmentunlock();
...
updategametypedvars()
{
    ...
    level.smokinggunprogress = scripts\mp\utility\dvars::dvarintvalue( "initialAmmoCount", 1, 0, 15 );
    level.snapplayertotoppos = scripts\mp\utility\dvars::dvarintvalue( "killRewardAmmoCount", 1, 0, 15 );
    level.smuggler_post_tele_kill = scripts\mp\utility\dvars::dvarintvalue( "oneShotKill", 1, 0, 1 );
    level.smuggler_killed_early = spawnstruct();
    var_0 = getdvar( "scr_oic_weapon" );
    level.smuggler_killed_early.stopchallengetimers = var_0;
    level.smuggler_killed_early.weapon = scripts\mp\utility\weapon::getweaponrootname( var_0 );
    level.smuggler_killed_early.tiers = 0;
}
...
onplayerconnect()
{
    for (;;)
    {
        ...
        var_0.pers["gamemodeLoadout"] = level.thrust_fx_model["axis"];
        var_0 loadweaponsforplayer( [ level.smuggler_killed_early.stopchallengetimers ], 1 );
        var_0.throwingknifemelee = 1;
        var_0.thrownspecialcount = 0;
    }
}

getspawnpoint()
{
    if ( self.throwingknifemelee )
...
startmatchobjectiveicons()
{
    ...
    if ( !self.thrownspecialcount )
        scripts\mp\menus::addtoteam( "spectator", 1 );
}

onspawnplayer()
{
    if ( isdefined( self.tier ) && self.tier )
        numpropsperarea();
    else
        self.tier = 0;
    ...
    if ( scripts\mp\utility\game::getgametypenumlives() != 0 )
        thread wait_until_emp_drone_done();
}
...
waitloadoutdone()
{
    ...
    if ( level.smokinggunprogress > var_0 )
    {
        self setweaponammoclip( self.primaryweapon, var_0 );
        self setweaponammostock( self.primaryweapon, level.smokinggunprogress - var_0 );

        if ( self.primaryweapons[0] hasattachment( "akimbo", 1 ) )
            self setweaponammoclip( self.primaryweapon, level.smokinggunprogress - var_0, "left" );
    }
    else
    {
        self setweaponammoclip( self.primaryweapon, level.smokinggunprogress );
        self setweaponammostock( self.primaryweapon, 0 );

        if ( self.primaryweapons[0] hasattachment( "akimbo", 1 ) )
            self setweaponammoclip( self.primaryweapon, level.smokinggunprogress, "left" );
    }
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    if ( isdefined( var_1 ) && isplayer( var_1 ) && self != var_1 )
    {
        if ( isdefined( var_3 ) && var_3 == "MOD_EXECUTION" )
            var_1.tier = var_1.tier + ( level.snapplayertotoppos + 2 );
        else
            var_1.tier = var_1.tier + level.snapplayertotoppos;

        if ( var_1 attackbuttonpressed() )
            var_1 thread _id_1310C();
        else
            var_1 numpropsperarea();
...
numpropsperarea()
{
    ...
    if ( var_2 + self.tier > var_3 )
    {
        self setweaponammoclip( var_0, var_3 );
        self setweaponammostock( var_0, var_1 + var_2 + self.tier - var_3 );

        if ( isdefined( self.primaryweapons[0] ) && self.primaryweapons[0] hasattachment( "akimbo", 1 ) )
        {
            var_4 = self getweaponammoclip( var_0, "left" );
            self setweaponammoclip( var_0, var_1 + var_4 + self.tier - var_3, "left" );
        }
    }
    else
    {
        self setweaponammoclip( var_0, var_2 + self.tier );

        if ( isdefined( self.primaryweapons[0] ) && self.primaryweapons[0] hasattachment( "akimbo", 1 ) )
        {
            var_4 = self getweaponammoclip( var_0, "left" );
            self setweaponammoclip( var_0, var_4 + self.tier, "left" );
        }
    }
...
infil_name( var_0 )
{
    ...
    {
        if ( var_3.pers["deaths"] < scripts\mp\utility\game::getgametypenumlives() && var_3.thrownspecialcount )
        {
...
watch_for_players_activating_juggmaze_map( var_0 )
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_1 ) )
        {
            self._id_130A2 = var_1.name;
            self playlocalsound( "recondrone_tag_plr" );
        }
    }
}

isvalidattachmentunlock()
{
    level endon( "game_ended" );
    scripts\mp\flags::gameflagwait( "prematch_done" );
    scripts\mp\flags::gameflagwait( "graceperiod_done" );
...
toppercentagetoadjusteconomy()
{
    ...
    for (;;)
    {
        if ( gettime() > level._id_129F4 + var_2 )
        {
...
_id_1211C()
{
    if ( scripts\mp\utility\game::matchmakinggame() && getdvarint( "scr_oic_randomWeapon", 2 ) > 0 )
    {
        ...
        level.smuggler_killed_early.weapon = var_0["weapon"];
        level.smuggler_killed_early.tiers = var_0["variantID"];
    }

    if ( !isdefined( level.smuggler_killed_early.weapon ) || level.smuggler_killed_early.weapon == "none" )
        level.smuggler_killed_early.weapon = "iw8_pi_golf21";

    var_1 = register_trap_room_objectives();

    if ( var_1 > 1 )
    {
        level.smokinggunprogress = level.smokinggunprogress * var_1;
        level.snapplayertotoppos = level.snapplayertotoppos * var_1;
    }
}

register_trap_room_objectives()
{
    switch ( level.smuggler_killed_early.weapon )
    {
        case "iw8_ar_falpha":
            return 3;
        case "iw8_pi_mike9":
            if ( level.smuggler_killed_early.tiers == 1 )
                return 3;
        case "iw8_ar_falima":
            if ( level.smuggler_killed_early.tiers == 6 || level.smuggler_killed_early.tiers == 9 )
                return 3;
        case "iw8_sm_smgolf45":
            if ( level.smuggler_killed_early.tiers == 2 )
                return 2;
        default:
            return 1;
    }
...
{
    level.thrownoffhand = spawnstruct();
    level.thrownoffhand._id_132D2 = [];
    level.thrownoffhand._id_132D2["rand_pistol"] = 80;
    level.thrownoffhand._id_132D2["rand_smg"] = 40;
    level.thrownoffhand._id_132D2["rand_assault"] = 40;
    level.thrownoffhand._id_132D2["rand_lmg"] = 25;
    level.thrownoffhand._id_132D2["rand_sniper"] = 40;
    buildrandomweapontable();

    if ( var_0 == 2 )
    {
        level.thrownoffhand._id_132D2["rand_pistol"] = 100;
        level.thrownoffhand._id_132D2["rand_smg"] = 0;
        level.thrownoffhand._id_132D2["rand_assault"] = 0;
        level.thrownoffhand._id_132D2["rand_lmg"] = 0;
        level.thrownoffhand._id_132D2["rand_sniper"] = 0;
    }
    else if ( var_0 == 3 )
    {
        level.thrownoffhand._id_132D2["rand_pistol"] = 0;
        level.thrownoffhand._id_132D2["rand_smg"] = 0;
        level.thrownoffhand._id_132D2["rand_assault"] = 0;
        level.thrownoffhand._id_132D2["rand_lmg"] = 0;
        level.thrownoffhand._id_132D2["rand_sniper"] = 100;
    }
    else if ( var_0 == 4 )
    {
        level.thrownoffhand._id_132D2["rand_pistol"] = 0;
        level.thrownoffhand._id_132D2["rand_smg"] = 100;
        level.thrownoffhand._id_132D2["rand_assault"] = 0;
        level.thrownoffhand._id_132D2["rand_lmg"] = 0;
        level.thrownoffhand._id_132D2["rand_sniper"] = 0;
    }
    else if ( var_0 == 5 )
    {
        level.thrownoffhand._id_132D2["rand_pistol"] = 0;
        level.thrownoffhand._id_132D2["rand_smg"] = 0;
        level.thrownoffhand._id_132D2["rand_assault"] = 100;
        level.thrownoffhand._id_132D2["rand_lmg"] = 0;
        level.thrownoffhand._id_132D2["rand_sniper"] = 0;
    }
    else if ( var_0 == 6 )
    {
        ...
        level.thrownoffhand._id_132D2["rand_smg"] = 0;
        level.thrownoffhand._id_132D2["rand_assault"] = 0;
        level.thrownoffhand._id_132D2["rand_lmg"] = 100;
        level.thrownoffhand._id_132D2["rand_sniper"] = 0;
    }
    else if ( var_0 == 7 )
    {
        ...
        level.thrownoffhand._id_132D2["rand_assault"] = 0;
        level.thrownoffhand._id_132D2["rand_lmg"] = 100;
        level.thrownoffhand._id_132D2["rand_sniper"] = 50;
    }

    var_1 = mp_br_quarry_locations( level.thrownoffhand._id_132D2 );
...
buildrandomweapontable()
{
    ...
    for (;;)
    {
        ...
        var_3 = scripts\mp\gametypes\gun::modespawn( var_2["weapon"] );
        var_2["variantID"] = scripts\mp\class::_id_12886( var_2["weapon"], var_3 );
...
setspecialloadouts()
{
    level.thrust_fx_model["allies"]["loadoutPrimary"] = level.smuggler_killed_early.weapon;
    level.thrust_fx_model["allies"]["loadoutPrimaryAttachment"] = "none";
    level.thrust_fx_model["allies"]["loadoutPrimaryAttachment2"] = "none";
    level.thrust_fx_model["allies"]["loadoutPrimaryCamo"] = "none";
    level.thrust_fx_model["allies"]["loadoutPrimaryReticle"] = "none";
    level.thrust_fx_model["allies"]["loadoutPrimaryAddBlueprintAttachments"] = scripts\engine\utility::ter_op( level.smuggler_killed_early.tiers != 0, 1, 0 );
    level.thrust_fx_model["allies"]["loadoutPrimaryVariantID"] = level.smuggler_killed_early.tiers;
    level.thrust_fx_model["allies"]["loadoutSecondary"] = "none";
    level.thrust_fx_model["allies"]["loadoutSecondaryAttachment"] = "none";
    level.thrust_fx_model["allies"]["loadoutSecondaryAttachment2"] = "none";
    level.thrust_fx_model["allies"]["loadoutSecondaryCamo"] = "none";
    level.thrust_fx_model["allies"]["loadoutSecondaryReticle"] = "none";
    level.thrust_fx_model["allies"]["loadoutSecondaryVariantID"] = 0;
    level.thrust_fx_model["allies"]["loadoutEquipmentPrimary"] = "none";
    level.thrust_fx_model["allies"]["loadoutEquipmentSecondary"] = "none";
    level.thrust_fx_model["allies"]["loadoutStreakType"] = "assault";
    level.thrust_fx_model["allies"]["loadoutKillstreak1"] = "none";
    level.thrust_fx_model["allies"]["loadoutKillstreak2"] = "none";
    level.thrust_fx_model["allies"]["loadoutKillstreak3"] = "none";
    level.aon_loadouts["allies"]["loadoutPerks"] = [ "specialty_hustle" ];
    level.thrust_fx_model["allies"]["loadoutGesture"] = "playerData";
    level.thrust_fx_model["allies"]["loadoutFieldUpgrade1"] = "super_deadsilence";
    level.thrust_fx_model["allies"]["loadoutFieldUpgrade2"] = "none";
    level.thrust_fx_model["axis"] = level.thrust_fx_model["allies"];
}

mp_br_quarry_locations( var_0 )
{
    var_1 = [];
    var_2 = [];
    var_3 = 0;
