...
createspawnweaponatpos( var_0, var_1, var_2 )
{
    ...
    var_5 = scripts\mp\gametypes\br_pickups::module_set_goal_radius( var_0, var_1 );
...
createspawnweaponatposfromname( var_0, var_1 )
{
    ...
    var_4 = scripts\mp\gametypes\br_pickups::module_set_goal_radius( var_0, ( 0, 0, 90 ) );
...
playerdropweaponfrominventory( var_0 )
{
    ...
    if ( isdefined( var_8 ) )
    {
        ...
        var_14 = scripts\mp\gametypes\br_pickups::project_to_line();
...
weaponspawn( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    level.train_play_anim scripts\mp\trials\trial_target_utility::delay_end_common_combat( var_6, var_1, var_0 );
    return var_6;
}
...
takeweaponpickup( var_0 )
{
    ...
    if ( !isdefined( var_0.weapon ) )
    {
        ...
        {
            var_3 = scripts\mp\gametypes\br_pickups::model_parts( var_0 );
        ...
        if ( !isdefined( var_4 ) )
        {
            scripts\mp\utility\script::getweaponrandomvariantid( "takeWeaponPickup error - scriptable not setup for pickup: " + var_0.scriptablename );
            return;
        }
    ...
    if ( var_7 )
    {
        ...
        if ( var_8.basename != "none" )
        {
            ...
            if ( var_8.hasalternate )
            {
                ...
                if ( !chopper_boss_explosion( var_8, var_16 ) )
            ...
            var_21 = !scripts\mp\gametypes\br_extract_quest::is_using_stealth_debug( var_8 );

            if ( var_21 )
            {
                var_22 = scripts\mp\gametypes\br_pickups::module_set_goal_radius( var_0.origin, ( 0, var_20, 0 ) );
                ...
                if ( var_28["fraction"] < 1.0 )
                    var_22.oncontractend = var_28["entity"];

                var_29 = weaponspawn( var_8, self, var_22, 0 );

                if ( isdefined( var_29 ) )
                {
                    scripts\mp\gametypes\br_pickups::setupinfectedairdroppositions( var_29, var_13, var_14, var_15 );
                    var_30 = 1;
    ...
    else
    {
        ...
        {
            var_36 = var_0.flickerlights;
            var_37 = var_3 getaltweapon();

            if ( !chopper_boss_explosion( var_3, var_37 ) )
            {
    ...
    if ( isdefined( var_0.questbasetimer ) )
        level.train_play_anim_init scripts\mp\trials\trial_target_utility::delay_end_common_combat( var_0.questbasetimer, self, var_3 );
}
...
br_forcegiveweapon( var_0, var_1, var_2 )
{
    if ( !scripts\engine\utility::array_contains( level.br_pickups.br_lootguns, var_0 ) )
        var_0 = chopperexfil_skip_ascend1();
    ...
    var_3.count = 0;
    var_3.floor1lights = 0;
    var_3.flickerlights = 0;
...
br_forcegivecustomweapon( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    var_7.floor1lights = 0;
    var_7.flickerlights = 0;
    ...
    if ( scripts\mp\utility\weapon::reflectprojectile( var_1 ) )
        var_7.floor1lights = var_7.count;

    if ( var_1.hasalternate )
    {
        var_10 = var_1 getaltweapon();

        if ( !chopper_boss_explosion( var_1, var_10 ) )
        {
            var_11 = weaponclipsize( var_10 );
            var_7.flickerlights = var_11;

            if ( var_6 )
                var_7.flickerlights = 999;
        }
    }
    ...
    for (;;)
    {
        ...
        if ( !scripts\mp\flags::gameflag( "prematch_done" ) )
        {
            ...
            foreach ( var_2 in var_0 )
            {
                ...
                if ( isdefined( var_3 ) )
                {
                    if ( scripts\mp\utility\weapon::reflectprojectile( var_2 ) )
                    {
...
{
    if ( scripts\mp\gametypes\br_gametypes::relic_amped_explosion_time( "weapons" ) )
        return;
...
br_ammo_player_hud_monitor()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_13130( "ammo_update", "pickedupweapon", "weapon_switch_done", "weapon_change", "weapon_change_complete" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_919" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_12g" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_762" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_50cal" );
...
chopper_boss_explosion( var_0, var_1 )
{
...
br_ammo_player_reload_watch()
{
    ...
    for (;;)
    {
        ...
        if ( !getdvarint( "scr_prematch_infinite_ammo", istrue( level.zombiejumpbar ) ) || scripts\mp\flags::gameflag( "prematch_done" ) )
            self.br_ammo[var_1] = self getweaponammostock( var_0 );

        chopper_boss_loc( self, var_1 );
    }
}
...
chopperexif_fx_init( var_0, var_1 )
{
    var_2 = self;
...
{
    if ( istrue( level.chopper_boss_combat ) && !scripts\mp\gametypes\br_public::registerpublicevent() )
        return 1;
}

control_room_button( var_0 )
{
    foreach ( var_2 in var_0 )
        setburningdamage( var_2, istrue( var_2.patrolfunc ) );
}
