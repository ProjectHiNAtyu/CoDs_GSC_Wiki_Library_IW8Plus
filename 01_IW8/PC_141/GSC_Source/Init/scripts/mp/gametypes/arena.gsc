...
main()
{
    ...
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::touchedmovingplatform );
    ...
    level.bot_nag = molotov_damage_over_time();

    if ( level.bot_nag )
    {
        level.lbravo_spawner_safehouse2 = 0.5;
        level.leader_charge_dialogue = 0.05;
    }

    if ( scripts\cp_mp\utility\game_utility::getmapname() == "mp_m_overwinter" )
    {
        level._id_12385 = 1;
        level.blueprintextractchance = getdvarint( "scr_always_snowfight", 0 );

        if ( level.blueprintextractchance )
    ...
    {
        level.bot_pick_dd_zone_with_fewer_defenders = 1;
        level._id_125BF = 1;
    }
    else if ( getdvarint( "scr_arena_stpday" ) == 1 )
    {
        level.bot_on_player_death = 1;
        level._id_125BF = 1;
    }
...
onstartgametype()
{
    ...
    {
        if ( !level.blueprintextractchance && game["roundsPlayed"] == 0 && istrue( game["practiceRound"] ) )
        {
            level.extratime = 30;
            game["didSnowFight"] = 1;
        ...
        else if ( level.blueprintextractchance )
        {
            level.arenaloadouts = 3;
            var_5 = 0;
        }

        level._id_12388["vanish"] = loadfx( "vfx/core/impacts/small_snowhit" );
        level._id_12388["screen"] = loadfx( "vfx/iw8/weap/_impact/snowball/vfx_imp_snowball_scrn.vfx" );
    ...
    if ( var_5 )
    {
        if ( israndomloadouts() || remove_spawners_that_can_be_seen() || remove_roof_nodes() || remove_soldier_armor() )
        ...
        else if ( ispickuploadouts() )
        {
            ...
            if ( !isdefined( game["roundsPlayed"] ) || isdefined( game["roundsPlayed"] ) && game["roundsPlayed"] == 0 )
            {
                ...
                level.startweapon.variantid = mine_caves_turret_config( level.startweapon.weapon );
                level.arenaweapont1.variantid = mine_caves_turret_config( level.arenaweapont1.weapon );
                level.arenaweapont2.variantid = mine_caves_turret_config( level.arenaweapont2.weapon );
                level.arenaweapont3.variantid = mine_caves_turret_config( level.arenaweapont3.weapon );
                level.arenaweapont4.variantid = mine_caves_turret_config( level.arenaweapont4.weapon );
                level.arenaweapont5.variantid = mine_caves_turret_config( level.arenaweapont5.weapon );
                level.arenaweapont6.variantid = mine_caves_turret_config( level.arenaweapont6.weapon );
                level.arenaweapont7.variantid = mine_caves_turret_config( level.arenaweapont7.weapon );
                level.arenaweapont8.variantid = mine_caves_turret_config( level.arenaweapont8.weapon );
            }
...
updategametypedvars()
{
    ...
    level.bot_next_difficulty_type_index = getdvarint( "scr_arena_arenaAttachments", 0 );
    level.bot_parachute_into_map = getdvar( "scr_arena_arenaSuper", "none" );

    if ( level.bot_parachute_into_map == "none" )
        ...
        if ( !isdefined( var_0 ) )
        {
            ...
            if ( !isdefined( var_0 ) )
            {
                ...
                if ( var_4 > 3 )
                {
                    ...
                    foreach ( var_6 in scripts\mp\utility\teams::getteamdata( self.pers["team"], "players" ) )
                    {
                        if ( istrue( var_6.damagepercentlow ) )
                        {
                            var_9++;
                            continue;
                        }
                    ...
                    if ( isdefined( var_8 ) )
                    {
                        ...
                        if ( isdefined( var_8._id_12580 ) )
                            var_0.angles = var_8._id_12580;
                    }
                }
            }
    ...
    if ( !isnormalloadouts() )
    {
        ...
        foreach ( var_2 in level.teamnamelist )
        {
            if ( isdefined( var_0.pers["team"] ) && game["roundWinStreak"][var_2] > 0 && var_0.pers["team"] == var_2 )
            {
                ...
                var_0.pers["gamemodeLoadout"]["roundWinStreakPrimaryCamo"] = _id_1216F();
                var_0.pers["gamemodeLoadout"]["roundWinStreakecondaryCamoTeam"] = var_2;
                var_0.pers["gamemodeLoadout"]["roundWinStreakSecondaryCamo"] = _id_1216F();
            }
        }
    }
...
onjoinedteam()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_1312D( "joined_team", "joined_spectators" );
        _id_1217D();
    }
}
...
onspawnfinished()
{
    ...
    if ( istrue( game["practiceRound"] ) || istrue( level.blueprintextractchance ) )
        thread _id_12387();
    ...
    if ( !scripts\mp\flags::gameflag( "prematch_done" ) && game["roundsPlayed"] == 0 )
    {
        ...
        level scripts\engine\utility::_id_1312D( "prematch_done", "removeArenaOutlines" );
    }
...
onplayerdamaged( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13 )
{
    ...
    {
        if ( istrue( level._id_12385 ) && ( var_6.basename == "snowball_mp" || var_6.basename == "pball_mp" ) )
            playfxontagforclients( level._id_12388["screen"], var_2, "tag_eye", var_2 );
...
arena_endgame( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\cp_mp\pet_watch::_id_12D77();
...
runarenaloadoutrulesonplayer()
{
    if ( israndomloadouts() || israndompreviewloadouts() || remove_roof_nodes() || remove_soldier_armor() )
    {
        if ( israndompreviewloadouts() )
        {
...
buildloadoutsforweaponstreaming()
{
    ...
    if ( ispickuploadouts() )
    {
        ...
        else
        {
            ...
            else if ( istrue( level.open_teleport_room_door ) )
            {
                var_4 = game["arenaRandomLoadout"][0]["loadoutPrimaryCamo"];
                var_8 = game["arenaRandomLoadout"][0]["loadoutPrimaryCosmeticAttachment"];
                var_9[0] = game["arenaRandomLoadout"][0]["loadoutPrimarySticker"];
            ...
            if ( relic_vampire_globalfunc() )
                var_1 = scripts\mp\class::deactivate_hacked_console( var_2, var_4, var_5, level.startweapon.variantid, var_7, var_8, var_9 );
            else
...
buildsecondaries( var_0, var_1, var_2 )
{
    ...
    else if ( istrue( level.open_teleport_room_door ) && istrue( var_2 ) )
    {
        var_7 = mp_t_reflex_patch();
        game["arenaRandomLoadout"][game["arenaRandomLoadoutIndex"]]["loadoutSecondaryCamo"] = var_7;
...
_id_1220B()
{
    level.arenaweapont1 = deactivate_gas_trap_cloud_parent( level.arenaweapont1 );
    level.arenaweapont2 = deactivate_gas_trap_cloud_parent( level.arenaweapont2 );
    level.arenaweapont3 = deactivate_gas_trap_cloud_parent( level.arenaweapont3 );
    level.arenaweapont4 = deactivate_gas_trap_cloud_parent( level.arenaweapont4 );
    level.arenaweapont5 = deactivate_gas_trap_cloud_parent( level.arenaweapont5 );
    level.arenaweapont6 = deactivate_gas_trap_cloud_parent( level.arenaweapont6 );
    level.arenaweapont7 = deactivate_gas_trap_cloud_parent( level.arenaweapont7 );
    level.arenaweapont8 = deactivate_gas_trap_cloud_parent( level.arenaweapont8 );
}
...
deactivate_gas_trap_cloud_parent( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_2 = mp_t_reflex_patch();
        var_0.deleteallglass = var_2;

        if ( _id_12E9E( var_0.weapon ) )
        {
            var_6 = mp_vacant_patch();
            var_0.flashtheprops = var_6;
        }

        if ( _id_12E9F( var_0.weapon ) )
        {
            var_7 = mp_vacant_patch_thread();
            var_0._id_12779[0] = var_7[0];
            var_0._id_12779[1] = var_7[1];
            var_0._id_12779[2] = var_7[2];
            var_0._id_12779[3] = var_7[3];
        }
...
_id_121C9()
{
    level.arenaweapont1 = damagestate( level.arenaweapont1 );
    level.arenaweapont2 = damagestate( level.arenaweapont2 );
    level.arenaweapont3 = damagestate( level.arenaweapont3 );
    level.arenaweapont4 = damagestate( level.arenaweapont4 );
    level.arenaweapont5 = damagestate( level.arenaweapont5 );
    level.arenaweapont6 = damagestate( level.arenaweapont6 );
    level.arenaweapont7 = damagestate( level.arenaweapont7 );
    level.arenaweapont8 = damagestate( level.arenaweapont8 );
}
...
damagestate( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_7[3] = "none";
        var_8 = scripts\mp\utility\weapon::getweaponrootname( var_0.weapon );
        var_0.weaponobj = scripts\mp\class::deactivate_hacked_console( var_8, undefined, undefined, var_4, undefined, undefined, scripts\cp_mp\utility\game_utility::isnightmap() );
    }
...
_id_1220D()
{
    level.startweapon = _id_12218( level.startweapon );
    level.arenaweapont1 = _id_12218( level.arenaweapont1 );
    level.arenaweapont2 = _id_12218( level.arenaweapont2 );
    level.arenaweapont3 = _id_12218( level.arenaweapont3 );
    level.arenaweapont4 = _id_12218( level.arenaweapont4 );
    level.arenaweapont5 = _id_12218( level.arenaweapont5 );
    level.arenaweapont6 = _id_12218( level.arenaweapont6 );
    level.arenaweapont7 = _id_12218( level.arenaweapont7 );
    level.arenaweapont8 = _id_12218( level.arenaweapont8 );
}
...
_id_12218( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_1 = min_player_health( var_5 + "_mp" );
...
{
    var_1 = scripts\mp\utility\weapon::getweaponrootname( var_0 );
    var_2 = min_player_health( var_1 + "_mp" );
    return var_2;
}
...
defineplayerloadout( var_0 )
{
    ...
    if ( isdefined( level.startweapon.weapon ) && level.startweapon.weapon != "none" && !issubstr( level.startweapon.weapon, "equip" ) )
    {
        ...
        if ( istrue( level._id_125BF ) )
        {
            ...
            if ( _id_12E9E( level.startweapon.weapon ) )
            {
                var_6 = mp_vacant_patch();
                game["arenaRandomLoadout"][0]["loadoutPrimaryCosmeticAttachment"] = var_6;
            }

            if ( _id_12E9F( level.startweapon.weapon ) )
            {
                var_7 = mp_vacant_patch_thread();
        ...
        else if ( istrue( level.open_teleport_room_door ) )
        {
            ...
            var_1 = mine_caves_ambusher_internal( level.startweapon.weapon );
    }
    else
    {
        var_11 = "none";
        ...
        {
            if ( reset_button_handler( level.startweapon.weapon ) )
            {
                ...
                if ( level.bot_nag )
                {
                    ...
                    if ( level.bot_nag == 2 )
                    {
                        var_12 = scripts\engine\utility::ter_op( remove_roof_nodes(), "mp/classtable_arena_blueprints.csv", "mp/classTable_arena.csv" );
                        var_4 = int( scripts\mp\gametypes\gun::modespawn( "iw8_knife" ) );
    ...
    if ( level.allowsupers )
    {
        ...
        if ( issubstr( var_14, "mp_m_" ) && var_14 != "mp_m_speed" )
            level.bot_parachute_into_map = juggernaut_death_test();
    }
    else
    {
        level.bot_parachute_into_map = "none";
        setomnvar( "ui_disable_fieldupgrades", 1 );
    }
    ...
    level.arena_loadouts["default"]["loadoutFieldUpgrade1"] = level.bot_parachute_into_map;
    ...
    if ( !istrue( level._id_125BF ) && !istrue( level.open_teleport_room_door ) )
    {
        foreach ( var_16 in level.teamnamelist )
        {
            level.arena_loadouts[var_16]["loadoutPrimaryCamo"] = _id_1216F();
            level.arena_loadouts[var_16]["loadoutSecondaryCamo"] = _id_1216F();
        }
    }
}
...
cacherandomloadouts()
{
    ...
    var_0 = [];
    var_1 = 0;
    var_2 = scripts\engine\utility::ter_op( remove_roof_nodes(), "mp/classtable_arena_blueprints.csv", "mp/classTable_arena.csv" );
    ...
    if ( israndompreviewloadouts() )
        var_1 = 144;
    else if ( remove_reduce_recoil() )
        var_1 = 0;
    else if ( remove_steam_damage() )
        var_1 = 57;
    else if ( remove_specific_structs() )
        var_1 = 93;
    else if ( remove_stay_at_spawn_spawn_flag() )
        var_1 = 123;
    else if ( remove_tank_class() )
        var_1 = 146;
    else if ( remove_spawn_disable_struct() )
        var_1 = 188;
    ...
    if ( var_5 != "" && var_6 > 0.0 )
    {
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            if ( var_10.size == 2 )
            {
                var_11 = var_10[0];
                var_12 = int( var_10[1] );
                var_13 = scripts\mp\utility\weapon::_id_132DB( var_11, var_12 );
...
updateloadoutarray( var_0, var_1 )
{
    ...
    if ( remove_roof_nodes() || remove_soldier_armor() )
    ...
    if ( remove_roof_nodes() || remove_soldier_armor() )
        var_2["loadoutSecondaryVariantID"] = scripts\mp\class::_id_12885( var_0, var_1, 1, var_2["loadoutSecondary"] );
    else
    ...
    if ( level.allowsupers )
    {
        ...
        var_2["loadoutFieldUpgrade1"] = level.bot_parachute_into_map;
    ...
    if ( remove_spawners_that_can_be_seen() )
    {
        ...
        if ( isdefined( var_2 ) && isdefined( var_3 ) )
        {
            ...
            if ( var_10.size > 0 )
            {
                if ( randomfloat( 1.0 ) < var_3 )
                {
                    ...
                    var_7["loadoutPrimary"] = var_12._id_11DE7;
...
bot_nags( var_0, var_1 )
{
    var_2 = [];

    foreach ( var_4 in var_0 )
    {
        if ( arenaloadouts_getweapongroup( var_4._id_11DE7 ) == var_1 )
...
mine_caves_turret_config( var_0 )
{
    if ( relic_vampire_globalfunc() && !issubstr( var_0, "equip" ) && var_0 != "none" )
    {
        ...
        if ( var_4 != "" )
        {
            var_5 = scripts\mp\class::_id_12885( "mp/classtable_arena_blueprints.csv", var_2, 0, var_4 );
            return var_5;
        }
...
setupweapons()
{
    ...
    if ( istrue( level._id_125BF ) || istrue( level.open_teleport_room_door ) )
        _id_1220B();
    else if ( relic_vampire_globalfunc() )
        _id_121C9();
    else
        _id_1220D();
...
min_player_health( var_0 )
{
    var_1 = [];

    if ( level.bot_next_difficulty_type_index == 1 )
    {
...
spawnweapon( var_0, var_1, var_2 )
{
    ...
    var_4 = juggernaut_damage_override( var_0 );
    ...
    if ( var_9 != "" )
    {
        if ( istrue( level._id_12385 ) && var_3 == "equip_snowball" )
        {
            if ( isplayer( var_0 ) )
                var_8 = "single";
            ...
            if ( !isplayer( var_0 ) && var_0.script_label == "6" && !level.update_health_think )
            {
                if ( distance( var_0.origin, ( 0, 0, 138 ) ) < 10 )
                {
                    level.update_health_think = 1;
                    var_3 = "equip_pball";
                    var_8 = "single";
                }
            }
            else if ( !isplayer( var_0 ) && var_0.script_label == "8" && !level.update_health_think )
            {
        ...
        if ( isdefined( var_0.angles ) )
        {
            ...
            {
                if ( isdefined( var_0.script_noteworthy ) && var_0.script_noteworthy == "wall" || isdefined( var_0._id_11F40 ) && var_0._id_11F40 == "wall" )
        ...
        var_10._id_119EF = spawn( "trigger_radius", var_10.origin, 0, 32, 32 );
        var_10 thread respawn_enemies();

        if ( istrue( level._id_12385 ) )
        {
            var_10 thread _id_13252( var_0, var_8 );
    ...
    else
    {
        ...
        if ( istrue( level._id_125BF ) || istrue( level.open_teleport_room_door ) || relic_vampire_globalfunc() )
            var_17 = var_1.weaponobj;
        else
        {
            var_16 = min_player_health( var_15 + "_mp" );
        ...
        if ( isdefined( var_0.script_noteworthy ) && var_0.script_noteworthy == "wall" )
        {
            ...
            var_19.checkpoint_objective_id = var_22;
        }
...
respawn_enemies()
{
    ...
    {
        wait 0.05;
        self._id_119EF waittill( "trigger", var_1 );
...
spawngameendflagzone( var_0 )
{
    ...
    var_2.iconname = "_a";
    var_1 = scripts\mp\gametypes\obj_dom::setupobjective( var_2, "neutral", undefined, 1 );
    ...
    var_1.onuncontested = ::bot_koth_think;
    var_1.isarena = 1;
...
arenaflag_onusebegin( var_0 )
{
    ...
    self.watch_for_icbm_spawners = 1;
...
deleteotpreview()
{
    level scripts\engine\utility::_id_1312D( "prematch_done", "start_mode_setup" );
    disableotflag();
}
...
giveflagcapturexp( var_0, var_1 )
{
    ...
    if ( isplayer( var_2 ) )
    {
        ...
        var_2 thread scripts\common\utility::_id_12C2C( level.slopelocked, "capture", var_2.origin );
    }
...
_id_12293()
{
    ...
    if ( istrue( level.arenaflag.watch_for_icbm_spawners ) )
        return;
...
getoutlineasset( var_0, var_1 )
{
    ...
    if ( istrue( level._id_12385 ) && isdefined( var_1 ) && var_1 == "equip_pball" )
        var_2 = "outline_depth_yellow";
...
remove_specific_structs()
{
    return level.arenaloadouts == 9;
}

remove_stay_at_spawn_spawn_flag()
{
...
_id_12206()
{
    ...
    if ( !istrue( level.blueprintextractchance ) && !istrue( game["practiceRound"] ) )
    {
    ...
    level.update_health_think = 0;
    var_0 = scripts\engine\utility::getstructarray( "weapon_pickup", "targetname" );

    foreach ( var_2 in var_0 )
    {
        if ( istrue( level.blueprintextractchance ) || istrue( game["practiceRound"] ) || level.arenaloadouts != 3 )
        {
            spawnweapon( var_2, level.arenaweapont1 );
            continue;
        }
...
_id_13252( var_0, var_1 )
{
    ...
    else
    {
        ...
        playfx( level._id_12388["vanish"], self.origin );
        clearweaponoutlines();

        if ( !isplayer( var_0 ) )
            level thread _id_13120( var_0 );
...
_id_13120( var_0 )
{
    level endon( "game_ended" );
    wait 15;
    playfx( level._id_12388["vanish"], var_0.origin );
...
molotov_damage_over_time()
{
    ...
    else if ( reset_button_handler( level.arenaweapont1.weapon ) && reset_button_handler( level.arenaweapont2.weapon ) && reset_button_handler( level.arenaweapont3.weapon ) && reset_button_handler( level.arenaweapont4.weapon ) && reset_button_handler( level.arenaweapont5.weapon ) && reset_button_handler( level.arenaweapont6.weapon ) && reset_button_handler( level.arenaweapont7.weapon ) && reset_button_handler( level.arenaweapont8.weapon ) )
    {
        if ( reset_button_handler( level.startweapon.weapon ) )
            return 2;
        else
...
{
    switch ( level.bot_parachute_into_map )
    {
        case "super_weapon_drop":
        case "super_emp_drone":
        case "super_recon_drone":
            level.bot_parachute_into_map = "super_ammo_drop";
            break;
        default:
            break;
    }

    return level.bot_parachute_into_map;
}
