...
get_player_enemy( var_0, var_1, var_2, var_3 )
{
    ...
    foreach ( var_6 in var_4 )
    {
        if ( isdefined( var_6 ) && scripts\mp\utility\player::isreallyalive( var_6 ) && var_6.team != self.team )
            get_player_drone( var_6, var_2, var_3 );
    }
}

get_player_drone( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( level.get_player_cumulative_damage ) && isdefined( level.get_player_defusing_zone ) )
    {
        if ( isdefined( level.get_player_defusing_zone[var_1] ) && isdefined( level.get_player_defusing_zone[var_1][var_0.guid] ) && gettime() < level.get_player_defusing_zone[var_1][var_0.guid] )
            var_2 = 0;
        else
            level.get_player_defusing_zone[var_1][var_0.guid] = gettime() + level.get_player_cumulative_damage * 1000;
    }

    if ( istrue( var_2 ) && scripts\mp\utility\killstreak::getkillstreakenemyusedialogue( var_1 ) )
    {
        var_4 = scripts\mp\gametypes\br_public::codecorrectlyenteredbyanyone( self );
    ...
    if ( var_1 > 0 )
    {
        ...
        get_player_info_proc( var_0, var_3 );
    }

    var_6 = spawnstruct();
    var_6.vehicle_createspawnselectionlittlebirdmarker = istrue( var_2 );
    var_6.tacinsert_gamemode_callback = var_1;
    var_0 scripts\mp\gametypes\br_gametypes::_id_11E4F( "onKillstreakDanger", var_6 );
}
...
get_player_info_proc( var_0, var_1 )
{
    ...
    level thread _id_1320F( var_0, var_2, "death_or_disconnect" );
    level thread _id_1320F( var_0, var_2, undefined, var_1 );
}

_id_1320F( var_0, var_1, var_2, var_3 )
{
