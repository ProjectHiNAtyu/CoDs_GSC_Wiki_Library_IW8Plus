...
main()
{
    ...
    level.getheliflyheight = [];
    var_0 = getentarray( "cyber_emp_pickup_trig", "targetname" );

    foreach ( var_2 in var_0 )
        level.getheliflyheight[level.getheliflyheight.size] = var_2.origin;

    level.getgulagclosedcircleindex = ( 0, 0, 0 );
    ...
    foreach ( var_6 in var_4 )
    {
        ...
        {
            level.getgulagclosedcircleindex = var_6.origin;
            break;
        }
    }
    ...
    level.gethightestpriotiryactiveburnstate = 1;
    level.gethelispawns = [];
    level.scorefrozenuntil = 0;
    level.gethighestscore = 0;
...
updategametypedvars()
{
    ...
    level.x1timedivision = scripts\mp\utility\dvars::dvarintvalue( "ppkTeamNoFlag", 50, 0, 250 );
    level.x1timehandler = scripts\mp\utility\dvars::dvarintvalue( "ppkTeamWithFlag", 100, 0, 250 );
    level.x1stashlootcacheused = scripts\mp\utility\dvars::dvarintvalue( "ppkFlagCarrier", 250, 0, 250 );
    ...
    level.juggernautchallengedone = scripts\mp\utility\dvars::dvarfloatvalue( "flagActivationDelay", 10, 0, 30 );
...
initspawns()
{
    ...
    foreach ( var_3 in level.spawnpoints )
        defenderflagbase( var_3 );
}

defenderflagbase( var_0 )
{
    var_0.scriptdata.hasseendiscountsplash = undefined;
    var_1 = getpathdist( var_0.origin, level.getheliflyheight[0], 1000 );

    if ( var_1 < 0 )
        var_1 = scripts\engine\utility::distance_2d_squared( var_0.origin, level.getheliflyheight[0] );
    else
        var_1 = var_1 * var_1;

    var_0.scriptdata.hasseendiscountsplash = var_1;
}
...
getspawnpoint()
{
    ...
    {
        var_3 = [];
        var_3["defenderFlagPosition"] = level.getglobalbattlepassxpmultiplier.visuals[0].origin;

        if ( isdefined( level.getglobalbattlepassxpmultiplier.carrier ) )
            var_3["activeCarrierPosition"] = level.getglobalbattlepassxpmultiplier.carrier.origin;
        else
...
tdef()
{
    ...
    level.spawnaccesscards = "waypoint_mlg_empty_flag";
    level.spawnangle = "waypoint_mlg_full_flag";
...
gamemodeoverriderestockrechargeperupdate()
{
    level.getheliflyheight = scripts\engine\utility::array_randomize( level.getheliflyheight );
    var_0 = level.getheliflyheight[0] + ( 0, 0, 64 );
    var_1 = level.getheliflyheight[0] + ( 0, 0, -64 );
    var_2 = scripts\engine\trace::ray_trace( var_0, var_1, undefined, scripts\engine\trace::create_default_contents( 1 ) );
    level.getheliflyheight[0] = var_2["position"];
    level.getglobalbattlepassxpmultiplier = gasvfxstructs( "allies" );
    level thread juggheli_spawner_exfil();
}

juggheli_spawner_exfil()
{
    if ( !scripts\mp\flags::gameflag( "prematch_done" ) )
    {
        level.gethelinextgroupafterwait.test_br_ending scripts\mp\gameobjects::setvisibleteam( "none" );
        level scripts\engine\utility::_id_1312D( "prematch_done", "start_mode_setup" );
        level.gethelinextgroupafterwait.test_br_ending scripts\mp\gameobjects::setvisibleteam( "any" );
    }

    if ( level.juggernautchallengedone )
    {
        scripts\mp\flags::gameflagwait( "prematch_done" );
        level.gethelinextgroupafterwait.test_br_ending thread scripts\mp\gametypes\obj_zonecapture::setteamlastzombietime( level.juggernautchallengedone, level.getglobalbattlepassxpmultiplier.curorigin + level.getglobalbattlepassxpmultiplier.offset3d );
        wait( level.juggernautchallengedone );
        level.gethelinextgroupafterwait.test_br_ending scripts\mp\gameobjects::_id_1213C( level.iconcaptureflag, level.iconcaptureflag, level.spawnangle );
        level.getglobalbattlepassxpmultiplier.trigger scripts\engine\utility::trigger_on();
...
gasvfxstructs( var_0 )
{
    ...
    level.returntime = 0;
    var_1 = 32;
    var_2 = spawn( "trigger_radius", level.getheliflyheight[0], 0, var_1, 128 );
    var_3 = [];
    var_3[0] = spawn( "script_model", level.getheliflyheight[0] );
    ...
    var_5 scripts\mp\gameobjects::_id_1213C( level.iconescort, level.iconkill, level.spawnangle );
    ...
    var_5 scripts\mp\gameobjects::_id_11B99( ::juggheli_spawner_jammer5_3 );
    ...
    if ( isdefined( level.showenemycarrier ) )
    {
        ...
        var_5 scripts\mp\gameobjects::_id_1213C( level.iconescort, level.iconkill, level.spawnangle, var_6 );
    }
    else
    {
        var_5.objidpingfriendly = 1;
    ...
    level.gethelinextgroupafterwait = gate_closes( var_0, var_5 );
    return var_5;
}
...
gate_closes( var_0, var_1 )
{
    ...
    var_3.test_br_ending = scripts\mp\gameobjects::createobjidobject( var_2, "neutral", ( 0, 0, 85 ), undefined, "any", 0 );
    var_3.test_br_ending scripts\mp\gameobjects::setvisibleteam( "any" );

    if ( level.juggernautchallengedone )
    {
        var_1.trigger scripts\engine\utility::trigger_off();
        var_3.test_br_ending scripts\mp\gameobjects::_id_1213C( level.icontarget, level.icontarget, level.spawnangle );
    }
    else
        var_3.test_br_ending scripts\mp\gameobjects::_id_1213C( level.iconcaptureflag, level.iconcaptureflag, level.spawnangle );
...
onpickup( var_0, var_1, var_2 )
{
    ...
    level.gethelinextgroupafterwait.test_br_ending scripts\mp\gameobjects::setvisibleteam( "none" );
    level.getglobalbattlepassxpmultiplier.get_closest_unclaimed_destination = var_0;
    var_0 thread brloadoutupdateammo();
    var_0 thread juggernautoutsidegoalradius();
    ...
    if ( isdefined( level.showenemycarrier ) )
    {
        ...
        else
        {
            ...
            scripts\mp\objidpoolmanager::tell_player_to_pick_up_intel( self.pingobjidnum, 1 );
    ...
    scripts\mp\gameobjects::_id_1213C( level.iconescort, level.iconkill, level.spawnangle );
    ...
    var_0 thread scripts\common\utility::_id_12C2C( level.slopelocked, "pickup", var_0.origin );
...
ondrop( var_0 )
{
    ...
    level.getglobalbattlepassxpmultiplier.get_closest_unclaimed_destination = undefined;
    ...
    if ( level.returntime >= 0 )
        scripts\mp\gameobjects::_id_1213C( level.iconreturnflag, level.iconreturnflag, level.spawnangle );
    else
    {
        scripts\mp\gameobjects::_id_1213C( level.iconreturnflag, level.iconreturnflag, level.spawnangle );
        scripts\mp\objidpoolmanager::tell_player_to_pick_up_intel( self.objidnum, 1 );
    }
...
onreset()
{
    ...
    if ( isdefined( level.getglobalbattlepassxpmultiplier.woods_three_death_func ) )
    {
        level.getglobalbattlepassxpmultiplier.woods_three_death_func clearportableradar();
        level.getglobalbattlepassxpmultiplier.woods_three_death_func delete();
    }
...
brloadoutupdateammo( var_0 )
{
    ...
    level.getglobalbattlepassxpmultiplier endon( "dropped" );
    level.getglobalbattlepassxpmultiplier endon( "reset" );
    ...
    while ( !level.gameended )
    {
        ...
        if ( !level.gameended )
        {
            level.getglobalbattlepassxpmultiplier.carrier scripts\mp\utility\stats::incpersstat( "objTime", 1 );
            level.getglobalbattlepassxpmultiplier.carrier scripts\mp\persistence::statsetchild( "round", "objTime", level.getglobalbattlepassxpmultiplier.carrier.pers["objTime"] );
            level.getglobalbattlepassxpmultiplier.carrier scripts\mp\utility\stats::setextrascore0( level.getglobalbattlepassxpmultiplier.carrier.pers["objTime"] );
            level.getglobalbattlepassxpmultiplier.carrier scripts\mp\gamescore::giveplayerscore( "tdef_hold_obj", 10 );
        }
    }
}
...
juggernautoutsidegoalradius( var_0 )
{
    ...
    var_1 = model_ent( var_0 );
    var_2 = spawn( "script_model", level.getglobalbattlepassxpmultiplier.visuals[0].origin );
    var_2.team = scripts\mp\utility\game::getotherteam( var_0 )[0];
    var_2.owner = var_1;
    var_2 makeportableradar( var_1 );
    level.getglobalbattlepassxpmultiplier.woods_three_death_func = var_2;
    level.getglobalbattlepassxpmultiplier thread juggheli_spawner_jammer5_4();
    level.getglobalbattlepassxpmultiplier thread juggmazedogtagrevive();
}
...
juggheli_spawner_jammer5_4()
{
    level endon( "game_ended" );
    self endon( "dropped" );
    self.woods_three_death_func endon( "death" );
    level endon( "portableRadarRunning" );

    for (;;)
    {
        self.woods_three_death_func moveto( self.get_closest_unclaimed_destination.origin, 0.05 );
        wait 0.05;
    }
}
...
juggmazedogtagrevive()
{
    level endon( "game_ended" );
    self endon( "dropped" );
    var_0 = self.woods_three_death_func.team;
    var_0 = scripts\mp\utility\game::getotherteam( var_0 )[0];
    self.woods_three_death_func.owner scripts\engine\utility::_id_1312E( "disconnect", "joined_team", "joined_spectators" );
    self.woods_three_death_func clearportableradar();
...
getrespawndelay()
{
    var_0 = level.getglobalbattlepassxpmultiplier scripts\mp\gameobjects::getownerteam();
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    var_10 = level.x1timedivision;

    if ( isdefined( level.getglobalbattlepassxpmultiplier ) && level.getglobalbattlepassxpmultiplier scripts\mp\gameobjects::getownerteam() == var_1.pers["team"] )
    {
        if ( isdefined( level.getglobalbattlepassxpmultiplier.carrier ) && var_1 != level.getglobalbattlepassxpmultiplier.carrier )
        {
            level.getglobalbattlepassxpmultiplier.carrier thread scripts\mp\rank::scoreeventpopup( "carrier_bonus" );
            var_11 = scripts\mp\rank::getscoreinfovalue( "carrier_bonus" );
            scripts\mp\gamescore::giveplayerscore( "carrier_bonus", var_11, self );
            level.getglobalbattlepassxpmultiplier.carrier thread scripts\mp\rank::giverankxp( "carrier_bonus", var_11 );
        ...
        var_10 = level.x1timehandler;
    }
    else if ( isdefined( self.carryflag ) )
        var_10 = level.x1stashlootcacheused;
    ...
    if ( isdefined( var_1 ) && isplayer( var_1 ) && var_1.pers["team"] != self.pers["team"] )
    {
        ...
        if ( isdefined( self.carryflag ) )
        {
            ...
            thread scripts\common\utility::_id_12C2C( level.smoke_init, var_9, "carrying" );
        ...
        if ( !var_12 )
        {
            var_15 = 0;
            var_16 = 0;
            var_17 = distsquaredcheck( var_13, self.origin, level.getglobalbattlepassxpmultiplier.curorigin );

            if ( var_17 )
            {
                if ( level.getglobalbattlepassxpmultiplier.ownerteam == self.team )
                    var_15 = 1;
                else
                    var_16 = 1;
            }
            ...
            if ( var_15 )
            {
                ...
                thread scripts\common\utility::_id_12C2C( level.smoke_init, var_9, "defending" );
            ...
            else if ( var_16 )
            {
                ...
                var_1 scripts\mp\utility\stats::incpersstat( "defends", 1 );
                var_1 scripts\mp\persistence::statsetchild( "round", "defends", var_1.pers["defends"] );
                thread scripts\common\utility::_id_12C2C( level.smoke_init, var_9, "assaulting" );
            }
        }
    }
