...
main()
{
    ...
    if ( var_0 )
        level.relic_amped_wait_till_revived = getdvarint( "scr_infect_groundwarInfect", 0 );

    var_1[0] = scripts\mp\utility\game::getgametype();
    var_1[var_1.size] = "grnd";

    if ( istrue( level.relic_amped_wait_till_revived ) )
    ...
    updategametypedvars();
    level.isdonetsksubmap = 0;
    level.isdooropened = getdvarint( "scr_infect_time_increase_cap", 20 );
    ...
    level.analytics_init = ::playerskipkioskuse;
    level.leader_intro_dialogue = ::leader_intro_dialogue;
    ...
    level.playershowarenastartobjectivetext = getdvarint( "scr_infect_nvgMode", 0 );
    level.playershouldrespawn = getdvarint( "scr_infect_disable_nvg", 0 );
    level.tr_respawn = getdvarint( "scr_infect_onlineprimaryoverride", 0 );

    if ( level.relic_amped_wait_till_revived )
    {
        ...
        level.playerskiplootpickup = var_3;
        level.blueprintextract_trygetreward = getdvarint( "scr_infect_alwaysTacInsertSkySpawn", 0 );
        level._id_12354 = getdvarint( "scr_infect_skyDiveOnTacInsertPlacement", 1 );
        level.blend_movespeedscale_cpso = getdvarint( "scr_infect_allowSkyDiveCutParachute", 0 );
        level.unset_relic_dogtags = ::unset_relic_dogtags;
        level._id_128A3 = ::_id_128A3;
    }
...
onstartgametype()
{
    ...
    scripts\cp_mp\utility\script_utility::registersharedfunc( "vehicle_compass", "shouldBeVisibleToPlayer", ::_id_12EDE );

    if ( istrue( level.relic_amped_wait_till_revived ) )
    {
        scripts\mp\gametypes\arm::infilcoveroverlay();
        thread playlinkfx();
        thread setupboardroomcode();
    }
...
updategametypedvars()
{
    ...
    level._id_1285E = getdvar( "scr_infect_superSurvivorTwo", "none" );
    ...
    level.playershowskippromptcinematic = getdvar( "scr_infect_superInfectTwo", scripts\engine\utility::ter_op( level.relic_amped_wait_till_revived, "super_tac_insert", "none" ) );
    ...
    level.playersetomnvargulag = getdvarint( "scr_infect_infectBonusScore", 150 );
    level.playersetomnvarkeypad = getdvarint( "scr_infect_infectBonusSuperOnSpawn", 0 );
    level.playersetplunderomnvar = getdvarint( "scr_infect_infectBonusSuperOnTacInsert", 0 );
    level.isvalidanimsuiteentity = getdvarint( "scr_infect_finalSuvivorCount", 1 );
    level.playersleftloop = getdvarint( "scr_infect_setRadarOnNumSurvivors", 8 );
    level._id_1285D = [];
    var_0 = getdvar( "scr_infect_survivorStreakOverride", "" );

    if ( var_0 != "" )
        level._id_1285D = strtok( var_0, "," );

    var_1 = scripts\cp_mp\utility\game_utility::getmapname();

    if ( issubstr( var_1, "mp_m_" ) && var_1 != "mp_m_speed" )
    {
        level.survivorsuper = juggernaut_death_test( 0, 0 );
        level._id_1285E = juggernaut_death_test( 0, 1 );
        level.infectedsuper = juggernaut_death_test( 1, 0 );
        level.playershowskippromptcinematic = juggernaut_death_test( 1, 1 );
    }

    if ( level.survivorsuper == level._id_1285E )
        level._id_1285E = "none";
    else if ( level.survivorsuper == "none" && level._id_1285E != "none" )
    {
        level.survivorsuper = level._id_1285E;
        level._id_1285E = "none";
    }

    if ( level.infectedsuper == level.playershowskippromptcinematic )
        level.playershowskippromptcinematic = "none";
    else if ( level.infectedsuper == "none" && level.playershowskippromptcinematic != "none" )
    {
        level.infectedsuper = level.playershowskippromptcinematic;
        level.playershowskippromptcinematic = "none";
    }
...
buildandloadweapons()
{
    ...
    foreach ( var_1 in level.allweapons )
    {
        if ( var_1 != "none" )
        {
            ...
            if ( level.playershowarenastartobjectivetext )
                game["arenaRandomLoadout"][0]["loadoutPrimaryAttachment"] = next_drone_cd();
...
juggernaut_death_test( var_0, var_1 )
{
    if ( istrue( var_1 ) )
        var_2 = scripts\engine\utility::ter_op( var_0, level.playershowskippromptcinematic, level._id_1285E );
    else
...
onspawnfinished()
{
    ...
    if ( self.pers["team"] == "allies" )
    {
        ...
        if ( level.relic_amped_wait_till_revived )
            thread obj_leads_spawned_overall();

        spawnwithplayersecondary();

        if ( level.playershowarenastartobjectivetext && !level.playershouldrespawn && !isbot( self ) )
            thread teamanchoredentomnvars();
    }
    else if ( self.pers["team"] == "axis" )
    {
        if ( istrue( level.open_teleport_room_door ) )
        {
            ...
            self.relic_doubletap_helper = 1;
        }

        if ( istrue( level.blend_movespeedscale_cpso ) )
    ...
    if ( istrue( self.startingteamcount ) )
    {
        if ( !level.relic_amped_wait_till_revived || level.relic_amped_wait_till_revived && level.mapname == "mp_aniyah" )
    ...
    if ( istrue( self.relic_steelballs_slide ) )
    {
        self.relic_steelballs_slide = undefined;
        thread scripts\cp_mp\parachute::playerswithoutdismemberment( 1 );
    }

    thread scripts\mp\supers::givesuperpoints( level.playersetomnvarkeypad, undefined, 1 );
    thread _id_13121();
}
...
_id_13121()
{
    ...
    if ( istrue( level.open_teleport_room_door ) && self.team == "axis" )
...
setfirstinfected( var_0 )
{
    ...
    while ( !scripts\mp\utility\player::isreallyalive( self ) || scripts\mp\utility\player::isusingremote() || isdefined( self._id_13003 ) )
        waitframe();
    ...
    if ( isdefined( var_1 ) )
    {
        ...
        var_2.is_enemy_of_type = "INVOLUNTARY";
    ...
    if ( var_0 )
    {
        ...
        thread scripts\mp\music_and_dialog::playersetwasingulag();
    }

    self.isinitialinfected = 1;
    scripts\mp\utility\stats::incpersstat( "firstInfected", 1 );
    self.pers["gamemodeLoadout"] = level.infect_loadouts["axis_initial"];
    scripts\mp\equipment\tac_insert::_id_1257F( self.origin, self.angles );
    ...
    if ( isdefined( scripts\mp\supers::getcurrentsuper() ) && scripts\mp\supers::getcurrentsuperref() == "super_deadsilence" )
        thread _id_1271F();
    else if ( level.relic_amped_wait_till_revived )
        thread scripts\mp\supers::givesuperpoints( 350, undefined, 1 );

    if ( var_0 )
        level.infect_players[self.name] = 1;

    level.jugg_modifyfalldamage = 1;

    if ( !level.jugg_modifyfalldamage )
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    {
        if ( isdefined( level.tanks_killed ) && self == level.tanks_killed )
        {
            var_10 = 0;
            var_11 = 0;
        }
    ...
    if ( var_10 )
    {
        ...
        if ( scripts\mp\utility\dvars::getwatcheddvar( "timelimit" ) != 0 )
        {
            ...
            if ( scripts\mp\utility\game::matchmakinggame() )
            {
                level.isdonetsksubmap++;
                var_17 = level.isdonetsksubmap <= level.isdooropened;
            }
...
{
    self.startingteamcount = 1;

    if ( level.relic_amped_wait_till_revived && level.mapname != "mp_aniyah" )
        _id_12114();
    ...
    if ( level.infect_teamscores["allies"] > 1 )
    {
        if ( level.relic_amped_wait_till_revived )
        {
            level.watchsnowballpickup++;

            if ( level.watchsnowballpickup > 3 )
                level.watchsnowballpickup = 1;

            if ( level.infect_teamscores["allies"] == level.playersleftloop )
                level thread _id_1273B();
        }

        if ( level.watchsnowballpickup == 1 )
        {
        ...
        if ( !var_1 )
        {
            ...
            foreach ( var_5 in level.players )
            {
                ...
                if ( var_5.team == "axis" && var_5 != var_0 && var_5 != self )
                    var_5 thread scripts\mp\utility\points::giveunifiedpoints( "assist", undefined, level.playersetomnvargulag );
            }
        }
    }
    else if ( level.infect_teamscores["allies"] == level.isvalidanimsuiteentity )
        onfinalsurvivor();
...
setupboardroomcode()
{
    ...
    while ( level.infect_teamscores["allies"] > level.playersleftloop )
    {
        foreach ( var_1 in level.players )
        {
            if ( var_1.team == "axis" )
...
enduavonlatejoiner( var_0 )
{
    ...
    for (;;)
    {
        if ( level.infect_teamscores["allies"] > level.isvalidanimsuiteentity )
        {
...
monitordisconnect()
{
    ...
    if ( isdefined( self.infect_isbeingchosen ) || level.infect_chosefirstinfected )
    {
        if ( level.infect_teamscores["axis"] && level.infect_teamscores["allies"] )
        {
            if ( level.relic_amped_wait_till_revived )
            {
                if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.playersleftloop )
                    level thread _id_1273B();
            }

            if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.isvalidanimsuiteentity )
                onfinalsurvivor();
...
setspecialloadouts()
{
    ...
    var_0 = 0;
    var_1 = scripts\mp\gametypes\gun::modespawn( level.survivorprimaryweapon );
    var_0 = scripts\mp\class::_id_12886( level.survivorprimaryweapon, var_1 );
    var_2 = 0;
    var_3 = scripts\mp\gametypes\gun::modespawn( level.survivorsecondaryweapon );
    var_2 = scripts\mp\class::_id_12886( level.survivorsecondaryweapon, var_3 );
    ...
    else if ( level.tr_respawn )
    {
        ...
        if ( level._id_1285D.size > 0 )
        {
            level.infect_loadouts["allies"]["loadoutKillstreak1"] = level._id_1285D[0];
            level.infect_loadouts["allies"]["loadoutKillstreak2"] = level._id_1285D[1];
            level.infect_loadouts["allies"]["loadoutKillstreak3"] = level._id_1285D[2];
        ...
        level.infect_loadouts["allies"]["loadoutFieldUpgrade2"] = level._id_1285E;
    }
    else
    {
        level.infect_loadouts["allies"]["loadoutPrimary"] = level.survivorprimaryweapon;
    ...
    {
        var_6 = scripts\mp\gametypes\gun::modespawn( level.initialprimaryweapon );
        var_4 = scripts\mp\class::_id_12886( level.initialprimaryweapon, var_6 );
    }
    ...
    {
        var_7 = scripts\mp\gametypes\gun::modespawn( level.initialsecondaryweapon );
        var_5 = scripts\mp\class::_id_12886( level.initialsecondaryweapon, var_7 );
    }
    ...
    else if ( level.tr_respawn )
    {
        ...
        level.infect_loadouts["axis_initial"]["loadoutFieldUpgrade2"] = level.playershowskippromptcinematic;
    }
    else
    {
        level.infect_loadouts["axis_initial"]["loadoutPrimary"] = level.initialprimaryweapon;
    ...
    {
        var_9 = 0;
        var_10 = scripts\mp\gametypes\gun::modespawn( level.infectedprimaryweapon );
        var_9 = scripts\mp\class::_id_12886( level.infectedprimaryweapon, var_10 );
    }
    ...
    else
    {
        ...
        level.infect_loadouts["axis"]["loadoutFieldUpgrade2"] = level.playershowskippromptcinematic;
    ...
    for (;;)
    {
        if ( self isnightvisionon() )
        {
            ...
            if ( var_0 >= 10 )
            {
                ...
                scripts\common\utility::binoculars_used( 0 );
                _id_11B35();
                scripts\common\utility::binoculars_used( 1 );
                var_1 = 0;
...
exfilallyturning()
{
    ...
    self.loadoutfieldupgrade2 = level.playershowskippromptcinematic;
    wait 0.1;
    ...
    if ( isdefined( self ) && scripts\mp\utility\player::isreallyalive( self ) )
    {
        ...
        var_0 = "gamemode";
        var_1 = scripts\mp\class::setarenaomnvarplayertype();
        var_1 = scripts\mp\class::setcheckliststateforteam( var_1, var_0 );
        ...
        scripts\mp\equipment::giveequipment( level.infectedtactical, "secondary" );
        thread scripts\mp\supers::givesuperpoints( level.playersetplunderomnvar, undefined, 1 );
    }

    if ( istrue( level._id_12354 ) )
        thread _id_12824();
    else
...
{
    level endon( "game_ended" );
    level.playershooting = [];
    level thread _id_121F1();
    scripts\mp\flags::gameflagwait( "prematch_done" );
    wait( level.droptime );
...
modifyfalldamage()
{
    var_0 = level.playershooting[0];
    level.playershooting[level.playershooting.size] = level.playershooting[0];
    level.playershooting[0] = undefined;
    level.playershooting = scripts\engine\utility::array_removeundefined( level.playershooting );
    return var_0;
}
...
_id_121F1()
{
    switch ( level.mapname )
    {
        ...
        case "mp_boneyard_gw":
            ...
            break;
        case "mp_aniyah":
            level.playershooting = [ ( 2716, -158, 708 ), ( 8215, 874, 342 ), ( 6524, 1561, 316 ), ( 5827, -860, 357 ), ( 4268, -47, 452 ), ( 2305, -1994, 386 ), ( 5172, 2587, 231 ), ( 2691, 3096, 302 ), ( -910, 3497, 318 ), ( -3579, -544, 320 ), ( 2591, 1659, 545 ), ( 1033, -514, 447 ), ( -93, -2789, 169 ), ( -1301, 924, 315 ), ( -2843, 2779, 316 ) ];
            break;
        default:
            level.playershooting = minigunbackup();
            break;
    }

    level.playershooting = scripts\engine\utility::array_randomize( level.playershooting );
}
...
leader_intro_dialogue( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = 0;
