...
updategametypedvars()
{
    ...
    level.tryspawnscriptablereuse = scripts\mp\utility\dvars::dvarintvalue( "pauseTime", 1, 0, 1 );
    ...
    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::isanymlgmatch() && getdvarint( "scr_koth_playlistZoneActivationDelay", 15 ) != 0 )
        level.zoneactivationdelay = apc_rus_initdamage();
}
...
hardpointmainloop()
{
    ...
    scripts\mp\utility\game::setup_enemy_sentries( undefined, undefined, "New Hardpoint", level.zone.trigger.origin );
    level.zone thread scripts\common\utility::_id_12C2C( level.slopelocked, "hill_moved", level.zone.trigger.origin );
    ...
    if ( level.firstzoneactivationdelay )
    {
        ...
        level.zone thread scripts\mp\gametypes\obj_zonecapture::setteamlastzombietime( level.firstzoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
    ...
    for (;;)
    {
        if ( !isdefined( level.silo_door_runners ) )
...
_id_11C3E()
{
    switch ( level.mapname )
    {
        ...
        case "mp_hackney_am":
            level._id_11C40 = [ "1", "4", "3", "9", "8" ];
            break;
        case "mp_spear":
        case "mp_spear_pm":
            level._id_11C40 = [ "5", "4", "3", "2", "6" ];
            break;
        case "mp_cave":
        case "mp_cave_am":
            level._id_11C40 = [ "1", "5", "2", "3", "4" ];
            break;
        case "mp_petrograd":
            level._id_11C40 = [ "1", "9", "4", "5", "2" ];
            break;
        case "mp_deadzone":
            level._id_11C40 = [ "12", "11", "3", "1", "2" ];
            break;
        case "mp_raid":
            level._id_11C40 = [ "20", "2", "8", "9", "5" ];
            break;
        case "mp_piccadilly":
            level._id_11C40 = [ "1", "5", "7", "3", "8" ];
            break;
        case "mp_crash2_pm":
        case "mp_crash2":
            level._id_11C40 = [ "1", "20", "5", "4", "19" ];
            break;
        case "mp_emporium":
            level._id_11C40 = [ "1", "2", "3", "4", "5" ];
            break;
        case "mp_broadcast2":
            level._id_11C40 = [ "1", "2", "3", "4", "5" ];
            break;
...
getfirstzone()
{
    if ( isdefined( level._id_11C40 ) )
    {
        var_0 = level.objectives[level._id_11C40[0]];
        level.prevzoneindex = 0;
        level.lb_mg_dmg_factor_tail_stabilizer = level.objectives[level._id_11C40[1]];
    }
    else
    {
        var_0 = level.objectives["1"];
        level.prevzoneindex = 1;
        level.lb_mg_dmg_factor_tail_stabilizer = level.objectives["2"];
    }

    var_0 thread scripts\mp\gametypes\obj_zonecapture::_id_1323E();
    level.lb_mg_dmg_factor_tail_stabilizer thread scripts\mp\gametypes\obj_zonecapture::_id_1323E();
    return var_0;
}
...
getnextzone()
{
    ...
    else
    {
        if ( isdefined( level._id_11C40 ) )
        {
            ...
            if ( level.prevzoneindex > level._id_11C40.size - 1 )
                level.prevzoneindex = 0;

            var_23 = level.objectives[level._id_11C40[level.prevzoneindex]];
            var_30 = level.prevzoneindex + 1;

            if ( var_30 > level._id_11C40.size - 1 )
                var_30 = 1;

            level.lb_mg_dmg_factor_tail_stabilizer = level.objectives[level._id_11C40[var_30]];
        }
        else
        {
            level.prevzoneindex++;
            ...
            level.lb_mg_dmg_factor_tail_stabilizer = level.objectives[scripts\engine\utility::string( var_30 )];
        }
...
hqactivatenextzone( var_0, var_1 )
{
    ...
    else if ( level.zoneactivationdelay )
    {
        ...
        level._id_1345F = 1;
        level thread scripts\mp\hud_message::updatematchstatushintforallplayers( undefined, 36, 36 );
        level.zone thread scripts\mp\gametypes\obj_zonecapture::setteamlastzombietime( level.zoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
hpcaptureloop()
{
    ...
    for (;;)
    {
        ...
        var_0 = level scripts\engine\utility::_id_13135( "zone_captured", "zone_destroyed" );
...
initspawns()
{
    ...
    if ( !isdefined( level.spawnpoints ) || istrue( level.apc_rus_driverturretreload ) )
    {
    ...
    if ( istrue( level.apc_rus_driverturretreload ) )
        return;
...
give_capture_credit( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    level endon( "game_ended" );
    level.lastcaptime = gettime();
    scripts\mp\utility\game::setup_enemy_sentries( var_4, undefined, "Hardpoint Captured", var_4.origin );
    var_6 = var_4;
    ...
    if ( isplayer( var_6 ) )
    {
        var_6 thread scripts\common\utility::_id_12C2C( level.slopelocked, "capture", var_6.origin );

        if ( release_player_on_damage( var_6 ) && !scripts\mp\utility\game::isanymlgmatch() )
        {
    ...
    while ( !level.gameended )
    {
        ...
        if ( !level.zone.stalemate && !level.gameended )
        {
            ...
            foreach ( var_5 in level.zone.touchlist[var_3] )
            {
                ...
                if ( isdefined( var_5.player.player_rigs ) && var_5.player.player_rigs > 3 )
                {
                    var_5.player.player_rigs = 0;
                ...
                if ( !isdefined( var_5.player.player_rigs ) )
                    var_5.player.player_rigs = 0;
                else
                    var_5.player.player_rigs++;
...
release_player_on_damage( var_0 )
{
