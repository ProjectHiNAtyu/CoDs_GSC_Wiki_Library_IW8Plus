    ...
    if ( isdefined( var_21 ) )
    {
        for ( var_22 = 0; var_22 < var_21.size; var_22++ )
        {
            ...
            if ( isdefined( var_21[var_22].node ) )
            {
                ...
                var_11.yaw_updater = var_24.droplocations[var_25].origin;
                continue;
            }

            var_11.yaw_updater = var_21[var_22].origin;
        }

        level.z_below_check = 1;
        level thread _id_12544( var_1 );
    }

    level thread shownonspectatingwinnersplash();
}

shownonspectatingwinnersplash()
{
...
enter_laser_panel_anim_sequence( var_0, var_1 )
{
    var_2 = [];
...
_id_12543( var_0, var_1 )
{
    enter_laser_panel_anim_sequence( self.team, 1 );
    var_2 = 4096;
    var_3 = scripts\engine\utility::getstruct( "soa_tower_elevator_floor_3", "targetname" );

    if ( istrue( self.registeronplayerdisconnect ) || isdefined( var_3 ) && distance2d( var_0, var_3.origin ) < 5000 )
        var_2 = 10000;
    ...
    if ( !scripts\mp\gametypes\br_public::reset_global_stealth_settings() )
        scripts\mp\gametypes\br_public::connected_vandalize_node( "dropbag_incoming", self.team, 1 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        scripts\mp\gametypes\br_analytics::cleanupmolotovs( self, var_0, var_1, var_4 );
        thread oscope_freq_think( var_4 );
    }
}

oscope_freq_think( var_0 )
{
    var_0 endon( "death" );
    wait 1.0;
    ...
    while ( isdefined( var_0 ) )
    {
        ...
        if ( abs( var_2[2] ) < 0.01 )
        {
            ...
            if ( var_5["fraction"] == 1 )
                var_0 scripts\cp_mp\killstreaks\airdrop::force_remove_stim( var_0.origin, ( 0, 0, -10 ) );
            else
                break;
        }
...
_id_12544( var_0 )
{
    foreach ( var_2 in var_0 )
    {
        ...
        if ( isdefined( level.br_circle ) && isdefined( level.br_circle.safecircleent ) )
        {
            ...
            else if ( istrue( level._id_12E63 ) && isscriptabledefined() )
                var_3 = getclosestpointonnavmesh( var_3 );

            if ( istrue( level._id_12E4B ) )
                var_3 = move_and_delete_objects_cleararea( var_3 );

            if ( scripts\mp\outofbounds::ispointinoutofbounds( var_3 ) )
            {
                var_4 = scripts\mp\gametypes\br_circle::getsafecircleorigin();
                var_7 = scripts\mp\gametypes\br_c130::_id_12375( var_4, var_3 );
        ...
        var_8 = ml_p2_func( 1, 0, 0, 0, 0 );
        var_2 _id_12543( var_3, var_8 );
        waitframe();
    }
}
...
{
    if ( isdefined( level.hoverjet_watchgameend ) )
    {
        if ( !remove_hunter_class( var_0, level.hoverjet_watchgameend ) )
        {
            var_1 = minigun_tag( var_0, level.hoverjet_watchgameend );
...
remove_hunter_class( var_0, var_1 )
{
...
spawndropbagonlanding()
{
    ...
    if ( isdefined( var_0 ) )
    {
        var_3 = ml_p2_func( 0, 0, 0, 1, 0 );
        _id_12543( var_0.origin, var_3 );
        level thread shownonspectatingwinnersplash();
    }
}
...
_id_12A92( var_0 )
{
    level thread _id_12A93( var_0, self );
}

_id_12A93( var_0, var_1 )
{
    level endon( "game_ended" );
    var_0 endon( "death" );
    var_2 = var_1;
...
bleedout_logic()
{
    if ( scripts\mp\gametypes\br_gametypes::relic_amped_explosion_time( "dropbag" ) )
        return;
...
ml_p2_func( var_0, var_1, var_2, var_3, var_4 )
{
    if ( istrue( var_0 ) )
        return 0;
