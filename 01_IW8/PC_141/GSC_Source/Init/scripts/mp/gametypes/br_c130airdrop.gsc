...
init()
{
    scripts\cp_mp\utility\script_utility::registersharedfunc( "br_c130Airdrop", "c130Airdrop_onCrateUse", ::debug_spawn_crate_on_train );
    level.debug_spawning = [];
    level.debug_start_numbers_threaded = [];
    level.debug_showcardlocs = getdvarint( "scr_bmo_airdropCrateHeightOverride", 12000 );
}
...
debug_silo_thrust()
{
    ...
    for (;;)
    {
        ...
        if ( istrue( level._id_12E61 ) && istrue( var_0 ) )
        ...
        var_7 = level.debug_start_numbers_threaded.size;
        ...
        if ( var_7 < var_3 )
        {
            ...
            for ( var_12 = 0; var_12 < var_10; var_12++ )
            {
                ...
                var_14 = debug_reach_pipe_room( var_12 );
                var_15 = distance( var_14.startpt, var_14.endpt );
                var_16 = scripts\mp\gametypes\br_c130::getc130speed();
                var_17 = var_15 / var_16;
                var_18 = debug_spawnallaccesscards( var_14, var_15, var_16, var_17 );

                if ( var_12 == 0 )
                    scripts\mp\gametypes\br_gametype_dmz::_id_122E4( "br_c130airdrop_incoming" );

                var_18 debug_spawncover_badnodetest( var_13 );
...
debug_reach_pipe_room( var_0, var_1, var_2 )
{
    if ( isdefined( level.br_level.br_mapcenter ) && isdefined( level.br_level.br_mapsize ) )
        var_3 = debug_reach_wind_room( var_0, var_1, var_2 );
    else
...
debug_reach_wind_room( var_0, var_1, var_2 )
{
    var_3 = undefined;
    ...
    var_5 = scripts\mp\gametypes\br_c130::mounted();
    ...
    if ( !isdefined( var_0 ) || var_0 == 0 )
    {
        if ( !istrue( var_2 ) )
        {
            var_7 = debug_show2dvotext();
            var_8 = debug_shoot_offset( var_7 );
            var_6 = vectortoangles( var_4 - var_8 * ( 1, 1, 0 ) );
            level.debug_spawnrewardstest = var_6;
        }

        var_4 = var_4 + ( 0, 0, scripts\cp_mp\parachute::mine_launch_vfx() );
    }
    else
    {
        if ( isdefined( level.debug_spawnrewardstest ) )
        {
            var_6 = level.debug_spawnrewardstest;
            var_9 = 1;
        ...
        var_10 = var_10 + ( 0, 0, scripts\cp_mp\parachute::mine_launch_vfx() );
        var_4 = var_4 + var_10;
    }

    var_3 = scripts\mp\gametypes\br_c130::_id_12525( var_4, var_6 );
    return var_3;
}
...
{
    var_0 = undefined;
    var_1 = scripts\mp\gamescore::new_col_map();
    var_2 = [];
...
debug_spawnallaccesscards( var_0, var_1, var_2, var_3 )
{
    ...
    var_4.truckcollision = var_1;
    var_4.speed = var_2;
    var_4.lifetime = var_3;
    var_4.dohudplunderpulse = spawn( "script_model", var_4.startpt );
    var_4.dohudplunderpulse setmodel( "veh8_mil_air_acharlie130_magma_rigid" );
    var_4.dohudplunderpulse linkto( var_4, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    ...
    level.debug_spawning[level.debug_spawning.size] = var_4;
    return var_4;
}

debug_spawncover_badnodetest( var_0, var_1, var_2, var_3 )
{
    self setscriptablepartstate( "audio_lp_dmz", "on", 0 );
    self moveto( self.endpt, self.lifetime );
    thread debug_run_helicopter_boss();
    thread debug_safehouse_gunshop_start( var_0, var_1, var_2, var_3 );
}
...
debug_run_helicopter_boss()
{
    ...
    wait 1;
    level.debug_spawning = scripts\engine\utility::array_remove( level.debug_spawning, self );
    self setscriptablepartstate( "audio_lp_dmz", "off", 0 );

    if ( isdefined( self.minimapid ) )
        scripts\mp\objidpoolmanager::returnobjectiveid( self.minimapid );

    if ( isdefined( self.dohudplunderpulse ) )
        self.dohudplunderpulse delete();

    if ( isdefined( self ) )
        self delete();
}

debug_safehouse_gunshop_start( var_0, var_1, var_2, var_3 )
{
    self endon( "death" );

    if ( isdefined( self.hunters_killed_by_targets ) )
        self [[ self.hunters_killed_by_targets ]]( var_0, var_1, var_2, var_3 );
    else
    {
        var_4 = self.lifetime;
        var_5 = var_4 * 0.4;
        ...
        while ( var_12 < var_0 )
        {
            ...
            var_13 = debug_safehouse_regroup_start( self.origin + anglestoforward( self.angles ) * 500 );
            ...
            var_14 = scripts\cp_mp\killstreaks\airdrop::hud_racetrack_timer( var_13 + ( 0, 0, level.debug_showcardlocs - 100 ), var_13, self.angles, var_1, var_2 );
            ...
            var_15._id_12E76 = 5;
            level.debug_start_numbers_threaded[level.debug_start_numbers_threaded.size] = var_14;
        }
    }
}

debug_safehouse_regroup_start( var_0, var_1 )
{
    var_2 = undefined;
    var_3 = var_0 - ( 0, 0, 20000 );
    var_4 = [ self, self.dohudplunderpulse ];
    ...
    {
        var_7 = scripts\mp\gametypes\br_c130::ispointinbounds( var_2, 1, 0 ) && !debug_silo_jump( var_2 );
...
debug_silo_jump( var_0 )
{
    var_1 = 0;
    var_2 = level.debug_start_numbers_threaded;
...
debug_spawn_crate_on_train( var_0 )
{
    ...
    if ( !scripts\mp\gametypes\br_public::registerpublicevent() && getdvarint( "scr_dmz_airdrop_drop_tablet", 0 ) == 1 )
    {
    ...
    if ( isdefined( level.battle_tracks_stopbattletracksforplayer ) )
        var_5 = level.battle_tracks_stopbattletracksforplayer;

    if ( istrue( level.cansnapcamera ) )
        var_5 = int( var_5 * level.trial_spawn_vehicle );
    ...
    foreach ( var_8 in var_2 )
        var_8.should_damage_pavelow_boss = "c130_box";
