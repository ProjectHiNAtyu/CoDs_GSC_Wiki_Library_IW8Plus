...
main()
{
    ...
    level.init_fan_blades = getdvarint( "scr_arm_enableLittleBirdMG", 0 );
    ...
    level._id_128F1 = getdvarint( "scr_arm_tankRespawnTime", 120 );
    level.bot_cur_loadout_num = getdvarint( "scr_arm_apcRespawnTime", 120 );
    level.longdialoguecooldown = getdvarint( "scr_arm_longDialogueCooldown", 1 );
    level.spawn_exfil_juggs = getdvarint( "scr_arm_mercyEndingNuke", 1 );
    level.spawn_exfil_paratroopers = getdvarint( "scr_arm_mercyEndingTime", 30 );
    level.targetsite = getdvarint( "scr_arm_numNonRallyVehicles", 25 );
    level.handledevcommand_publicevents = getdvarint( "scr_arm_disablePersonalNuke", 0 );
    level.unset_relic_gas_martyr = getdvarint( "scr_personalNukeCostOverride", 30 );
    level._id_122E8 = getdvarint( "scr_arm_showTeamTanks", 1 );
    level._id_122E7 = getdvarint( "scr_arm_showTeamLittleBirds", 1 );
    level._id_12597 = getdvarint( "scr_arm_spawnSelectionAFKTime", 120 );
    level.sidekillcount = getdvarint( "scr_arm_manualAdjustLittleBirdLocs", 1 );
    level.helis_assault3_hangar = getdvarint( "scr_arm_dontSpawnJeep", 0 );
    level.helis_assault3_fob_check_size = getdvarint( "scr_arm_dontSpawnCargoTruck", 0 );
    level._id_12E65 = getdvarint( "scr_arm_usePingSystem", 1 );

    if ( istrue( level._id_122E8 ) )
    {
        level._id_12A95 = [];
        level._id_12A95["axis"] = [];
        level._id_12A95["allies"] = [];
    }

    if ( istrue( level._id_122E7 ) )
    {
        level._id_12A94 = [];
        level._id_12A94["axis"] = [];
        level._id_12A94["allies"] = [];
        level._id_12A94["untouched"] = [];
    }
...
onstartgametype()
{
    ...
    if ( istrue( level.spawn_exfil_juggs ) )
        level thread take_ai_weapon();

    infilcoveroverlay();

    if ( istrue( level._id_12E65 ) )
...
infilcoveroverlay()
{
    ...
    scripts\cp_mp\utility\game_utility::_id_11C8D( "vehicle_volume", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11C8D( "vehicle_volume_simplified", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11C8D( "super", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11C8D( "militarybase", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11C8D( "location_volume", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11C8D( "locale_area_trigger", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11C8D( "shadow_blocker", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11C8E( "door_prison_cell_metal_mp", 1 );
    scripts\cp_mp\utility\game_utility::_id_11C8E( "veh8_mil_air_acharlie130", 1 );
    ...
    scripts\cp_mp\utility\game_utility::_id_11C8C( "light" );
    scripts\cp_mp\utility\game_utility::_id_11C8C( "trigger_use_touch" );

    if ( isdefined( level.localeid ) && level.localeid == "locale_6" )
        scripts\cp_mp\utility\game_utility::_id_11C8D( "locale_8", "script_noteworthy" );
...
updatedomscores()
{
    ...
    level thread scripts\mp\spawnselection::_id_12DCC();
...
updatescores()
{
    ...
    var_5 = scripts\mp\gamescore::define_as_level_infil_driver();
    ...
    var_8 = scripts\mp\gamescore::define_as_level_infil_driver();

    if ( var_5 != var_8 )
        scripts\mp\gamescore::wavesv_finitemode( var_8, 1, var_5 );
}
...
runobjectives( var_0 )
{
    ...
    if ( istrue( level.useobjectives ) )
    {
        ...
        foreach ( var_2 in level.allfobs )
        {
            ...
            if ( istrue( level.leaderplunderstring ) )
                var_2._id_125C1 = scripts\mp\gametypes\obj_dom::_id_125C2;
    ...
    hackfixcameras();
    level thread bleedout_heartbeat_sfx_logic();
}
...
objective_manageobjectivesintrovisibility()
{
    ...
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.axishqloc.init_track.objidnum );
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.allieshqloc.init_track.objidnum );
...
sethqmarkerobjective()
{
    ...
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.axishqloc.init_track = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
    ...
    scripts\mp\objidpoolmanager::update_objective_setbackground( var_2.objidnum, 7 );
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.allieshqloc.init_track = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
...
spawnselection_showenemyhq()
{
    ...
    if ( self.team == "allies" )
    {
        ...
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.axishqloc.init_track.objidnum, self );
    }
    else
    {
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.allieshqloc.init_track.objidnum, self );
    ...
    if ( self.team == "axis" )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.allieshqloc.init_track.objidnum, self );
    else
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.axishqloc.init_track.objidnum, self );
}
...
docratedropsmoke( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_0 ) )
        var_0 scripts\engine\utility::_id_13141( var_2, "crate_dropped" );
    else
        wait( var_2 );
...
take_intro_items()
{
    ...
    for (;;)
    {
        if ( defenderflag_bflagstart( "axis" ) == level.objectives.size )
            level thread takerevivepickup( "axis" );
        else if ( defenderflag_bflagstart( "allies" ) == level.objectives.size )
            level thread takerevivepickup( "allies" );
...
takerevivepickup( var_0 )
{
    ...
    scripts\mp\trials\trial_utility::_id_1214A( "ui_nuke_data", 0, 9, level.spawn_exfil_paratroopers );
    scripts\mp\trials\trial_utility::_id_1214A( "ui_nuke_data", 11, 1, 1 );
    var_1 = 0;
    var_2 = gettime();
    var_3 = level.spawn_exfil_paratroopers * 1000 + var_2;
    setomnvar( "ui_nuke_end_milliseconds", level.spawn_exfil_paratroopers * 1000 + var_2 );

    while ( defenderflag_bflagstart( var_0 ) == level.objectives.size )
    {
        waitframe();
...
tango_infil_radio_idle( var_0 )
{
    ...
    foreach ( var_5 in level.players )
    {
        ...
        {
            if ( isdefined( var_5.trap_toggle_logic ) )
            {
                var_5 clearsoundsubmix( "iw8_mp_spawn_camera" );
                var_5.trap_toggle_logic stoploopsound( var_5.trap_trigger_logic );
                var_5.trap_toggle_logic delete();
                var_5.trap_toggle_logic = undefined;
                var_5.trap_trigger_logic = undefined;
            }
        }
    }
    ...
    if ( isdefined( level.teamdata[var_0]["alivePlayers"][0] ) )
    {
        ...
        var_7 scripts\mp\trials\trial_sniper::tryusenuke();
    }
    else
...
{
    thread scripts\mp\spawnselection::_id_11B60( var_0.team, var_0.squadindex );
}
...
parachute( var_0, var_1 )
{
    ...
    self playershow();
    self.plotarmor = 0;
    scripts\mp\utility\game::_id_1215A( self, 0 );
    ...
    foreach ( var_1 in level.vehicleinfo )
    {
        ...
        foreach ( var_9 in var_1.vehiclespawns )
        {
            if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_9.script_noteworthy ) && var_9.script_noteworthy == level.localeid )
            {
                ...
                if ( istrue( level.helis_assault3_hangar ) && var_1.refname == "jeep" || istrue( level.helis_assault3_fob_check_size ) && var_1.refname == "cargo_truck" )
                    continue;
                else
                {
...
_id_12EF5( var_0, var_1 )
{
    ...
    var_0._id_12598 = var_2;
    thread _id_12FD5( var_0 );
    level._id_12A95[var_1][level._id_12A95[var_1].size] = var_0;
}

_id_12FD5( var_0 )
{
    var_1 = var_0._id_12598.objidnum;
    var_2 = var_0.team;
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    level._id_12A95[var_2] = scripts\engine\utility::array_remove( level._id_12A95[var_2], var_0 );
}
...
_id_12FBC()
{
    ...
    foreach ( var_1 in level._id_12A95[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_12598.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_12A95["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_12598.objidnum, self );

    foreach ( var_1 in level._id_12A95["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_12598.objidnum, self );
}
...
_id_12EF4( var_0, var_1 )
{
    ...
    var_0._id_12598 = var_2;
    thread _id_12FD4( var_0 );
    level._id_12A94["untouched"][level._id_12A94["untouched"].size] = var_0;
}
...
_id_12FD4( var_0 )
{
    ...
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    var_2 = var_0.searchforcecirclecenter;

    if ( !isdefined( var_0.searchforcecirclecenter ) )
        var_2 = "untouched";

    level._id_12A94[var_2] = scripts\engine\utility::array_remove( level._id_12A94[var_2], var_0 );
}
...
_id_12FBD()
{
    ...
    foreach ( var_1 in level._id_12A94[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_12598.objidnum, self );

    foreach ( var_1 in level._id_12A94["untouched"] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_12598.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_12A94["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_12598.objidnum, self );

    foreach ( var_1 in level._id_12A94["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_12598.objidnum, self );
...
_id_12FAD( var_0 )
{
    if ( isdefined( self.searchforcecirclecenter ) )
        level._id_12A94[self.searchforcecirclecenter] = scripts\engine\utility::array_remove( level._id_12A94[self.searchforcecirclecenter], self );
    else
        level._id_12A94["untouched"] = scripts\engine\utility::array_remove( level._id_12A94["untouched"], self );

    self.searchforcecirclecenter = var_0;
    level._id_12A94[var_0][level._id_12A94[var_0].size] = self;
    scripts\mp\objidpoolmanager::update_objective_icon( self._id_12598.objidnum, "icon_minimap_littlebird_spawn_selection" );

    foreach ( var_2 in level.players )
    {
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( self._id_12598.objidnum, var_2 );

        if ( isdefined( var_2 ) && isdefined( var_2.team ) && var_2.team == var_0 && istrue( var_2.inspawnselection ) )
        {
            scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( self._id_12598.objidnum, var_2 );
            continue;
        }
...
init_rallyvehicles()
{
    ...
    if ( !istrue( level.disablelittlebirdrally ) )
    {
        foreach ( var_2 in var_0 )
        {
            ...
            if ( level.init_fan_blades )
                var_6 = _getrandomlocations::set_up_coop_push( var_4, var_5 );
            else
    ...
    {
        if ( level.localeid == "locale_6" && level.mapname == "mp_downtown_gw" && istrue( level.sidekillcount ) )
        {
            var_10 = [];
        ...
        foreach ( var_2 in var_0 )
        {
            ...
            level thread _id_12EF4( var_6, "neutral" );
        }
    }
...
arm_initoutofbounds()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_2.script_noteworthy ) && var_2.script_noteworthy == level.localeid && scripts\mp\utility\game_utility_mp::spawnfakepistol( var_2 ) )
        {
