...
{
    return istrue( var_0.oninstanceremoved );
}

removechevronsfromarray( var_0 )
{
...
{
    var_0 = self;
    return isdefined( var_0 ) && istrue( var_0._id_131BC );
}

regroup_process_started()
{
    var_0 = self;
    return ( istrue( var_0.chopperoccupied ) && !isalive( var_0 ) || var_0 wait_juggernaut_announce() ) && !istrue( var_0.gulag );
}

wait_juggernaut_announce()
{
    return istrue( self.iszombie );
}

wait_for_players_on_platform()
{
    return istrue( self.relic_amped_reset_deathshield_on_revived );
}
...
heal_removeonplayernotifies()
{
    ...
    scripts\engine\utility::_id_1312D( "death", "try_heal_cancel" );
    healend();
}
...
{
    if ( self isswitchingweapon() || self isreloading() || self ismantling() || self isthrowinggrenade() || self israisingweapon() || self ismeleeing() || self [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "player", "isPlayerADS" ) ]]() || istrue( self.quest_scavdistmin ) )
        return 0;
...
waitandunloadinfils( var_0, var_1 )
{
    self endon( "death_or_disconnect" );
...
pavelow_boss_destroyed_watch()
{
    var_0 = self;
    return istrue( var_0.pavelow_boss_destroyed_watch );
}

pause_combat_logic()
{
    var_0 = self;
    return istrue( var_0.pause_combat_logic );
}

pavelow_boss_health_bar()
{
    var_0 = self;
    return istrue( var_0.path_timeout );
}

patchable_collision( var_0 )
{
    var_1 = self;

    if ( !isdefined( var_0 ) || !isdefined( var_1.ai_semtex_swapp ) )
        return isdefined( var_1.ai_semtex_swapp );
    else
        return var_0 == var_1.ai_semtex_swapp;
}
...
cleanac130struct( var_0 )
{
    ...
    if ( isdefined( var_0.delete_subway_car ) )
        var_0.delete_subway_car delete();
    ...
    if ( isdefined( var_0.extractmissionhelipadscriptable ) )
        var_0.extractmissionhelipadscriptable delete();
...
{
    var_0 = self;
    return istrue( var_0.register_seat_data_for_vehicle ) && var_0.game_extrainfo & 64;
}
...
connectedplayercount( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    foreach ( var_7 in level.teamdata[var_1]["players"] )
    {
        ...
        if ( var_7 != var_2 )
            connected_search_node( var_0, var_7, var_3, var_4, var_5 );
    }
}

connected_vandalize_node( var_0, var_1, var_2, var_3, var_4, var_5 )
{
...
brleaderdialog( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    {
        var_8 = var_6[var_7];
        thread connected_search_node( var_0, var_8, var_1, var_3, var_4, var_5 );
    }
}

spawndropbagsovertime( var_0, var_1 )
{
    if ( !isdefined( var_1 ) )
        var_1 = 1;
    ...
    if ( istrue( self.watchvehicleingas ) )
        return 1;

    if ( register_player_character() )
    {
        if ( istrue( self.waterentityarray ) )
            return 1;

        if ( var_0 == "deploy_squad_leader" || var_0 == "prematch_enter" )
            return 1;

        if ( ( var_0 == "circle_closing" || var_0 == "first_circle" ) && !istrue( level.watch_for_molotov_ambush_and_spawners ) )
            return 1;
    }
...
codecorrectlyenteredbyanyone( var_0 )
{
    ...
    if ( reset_global_stealth_settings() || register_player_character() )
        var_1 = "allies";
...
connected_search_node( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( var_1 spawndropbagsovertime( var_0, var_2 ) )
        return;
    ...
    var_6 = codecorrectlyenteredbyanyone( var_1 );
...
register_bank_cut_modules()
{
    return reset_global_stealth_settings() || registerpublicevent() || register_player_character() || registerpreviousprop();
}
...
watch_for_players_ledgespawners()
{
    if ( istrue( self.zombievehiclelaststand ) )
        self waittill( "playerPrestreamComplete" );
}

wasinlaststand( var_0, var_1 )
{
    if ( !isdefined( var_1 ) )
...
wassquadspawned( var_0, var_1, var_2, var_3, var_4 )
{
    thread watch_and_open_scriptable_doors_in_radius( var_0, var_1, var_2, var_3, var_4 );
}

watch_and_open_scriptable_doors_in_radius( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( !isdefined( var_1 ) )
        var_1 = missionparticipation();

    var_6 = gettime() + var_1;
    self.zombievehiclelaststand = 1;
    ...
    self.zombievehiclelaststand = undefined;
...
vehicle_travel_player_disconnect_monitor()
{
    self notify( "playerPrestreamLocationWait" );
    self clearpredictedstreampos();
}

waittill_any_return_4()
{
    self cancelmantle();
    ...
    if ( scripts\cp_mp\utility\player_utility::isinvehicle( 1 ) )
    {
        ...
        var_0.is_enemy_of_type = "INVOLUNTARY";
...
damagearmor( var_0, var_1 )
{
    ...
    scripts\cp\helicopter\chopper_boss::toggle_ambient_vehicles_on_module( var_2 );
    ...
    if ( isplayer( self ) )
    {
        ...
        var_6 = spawnstruct();
        var_6.damageabsorbed = var_2;
        var_6.damagepassed = var_3;
        var_6.ignoreaudiobreak = var_1;
        scripts\mp\gametypes\br_gametypes::_id_11E4F( "onPlayerArmorDamaged", var_6 );
    }
...
damagehelmet( var_0, var_1, var_2 )
{
    ...
    if ( var_1 )
    {
        ...
        if ( isdefined( level.totalstructs ) )
            [[ level.totalstructs ]]( self, var_2 );
    }

    return var_3;
}

zombienumhitscar( var_0 )
{
    ...
    level thread scripts\mp\gametypes\br_infils::playerswapteam();
...
zombiehud( var_0 )
{
    if ( reset_global_stealth_settings() )
        return;
    ...
    if ( level.matchcountdowntime > 3 )
    {
        ...
        wait( var_1 );
        connected_vandalize_node( "prematch_end", var_0 );
    }
}
...
idflags_penetration_player_only( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = idflags_source_left_hand( var_0, var_1, var_2, var_3, var_4 );
    return var_5["position"];
}

idle_sfx( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = idflags_source_left_hand( var_0, var_1, var_2, var_3, var_4 );
    return var_5;
}

idflags_source_left_hand( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = -1200;

    if ( isdefined( level.br_level ) && isdefined( level.br_level.should_show_icon ) )
        var_5 = level.br_level.should_show_icon;
    ...
    var_14 = monitor_player_opens_box( var_0, 100 );

    if ( isdefined( var_14 ) && var_14.size > 0 )
        var_4 = scripts\engine\utility::array_combine( var_4, var_14 );

    var_15 = _id_11AC6( var_0, 0, 0, var_1, var_2, var_12, var_4 );

    if ( _id_12A7E( var_15, var_10 ) )
        return var_15;

    var_15 = _id_11AC6( var_0, var_9, 0, var_1, var_2, var_12, var_4 );

    if ( _id_12A7E( var_15, var_10 ) )
        return var_15;

    var_15 = _id_11AC6( var_0, 0, var_9, var_1, var_2, var_12, var_4 );

    if ( _id_12A7E( var_15, var_10 ) )
        return var_15;

    var_15 = _id_11AC6( var_0, -1 * var_9, 0, var_1, var_2, var_12, var_4 );

    if ( _id_12A7E( var_15, var_10 ) )
        return var_15;

    var_15 = _id_11AC6( var_0, 0, -1 * var_9, var_1, var_2, var_12, var_4 );
...
waittill_juggs_alive( var_0, var_1 )
{
...
munitions_override_time( var_0, var_1 )
{
    ...
    {
        if ( !isdefined( var_1 ) && scripts\mp\menus::blockade_get_bomb_icon_on_cell_phone() )
            return [];
        else
...
multiplepubliceventsenabled( var_0, var_1 )
{
    if ( scripts\mp\menus::_id_1261E() )
    {
        var_2 = [];
...
multieventdisabled( var_0, var_1 )
{
    if ( scripts\mp\menus::_id_1261E() )
    {
        var_2 = 0;
...
munition_slot_gunship_emptied_message( var_0 )
{
    if ( scripts\mp\menus::_id_1261E() )
...
_id_12175( var_0, var_1, var_2, var_3 )
{
    if ( scripts\mp\menus::_id_1261E() )
    {
        if ( !isdefined( level.squaddata[var_0][var_1].cloned_collision ) )
            level.squaddata[var_0][var_1].cloned_collision = [];

        level.squaddata[var_0][var_1].cloned_collision[var_2] = var_3;
    }
    else
        level.teamdata[var_0][var_2] = var_3;
}

munition_remover( var_0, var_1, var_2 )
{
    if ( scripts\mp\menus::_id_1261E() )
    {
        if ( !isdefined( level.squaddata[var_0][var_1].cloned_collision ) || !isdefined( level.squaddata[var_0][var_1].cloned_collision[var_2] ) )
            return;

        return level.squaddata[var_0][var_1].cloned_collision[var_2];
    }
    else
        return level.teamdata[var_0][var_2];
}

waypoint_endzone_vfx( var_0, var_1, var_2, var_3, var_4 )
{
    if ( scripts\mp\menus::_id_1261E() )
    {
        var_5 = munitions_override_time( var_2.team, var_2.squadindex );
...
packreinforcedperksquaddataintoomnvar()
{
    while ( !isdefined( self.pers ) || !isdefined( self.pers["squadMemberIndex"] ) )
        waitframe();

    var_0 = munitions_override_time( self.team, self.squadindex );
    var_1 = 0;
    var_2 = "ui_squad_reinforced_perk";
    var_3 = 0;
    ...
    for ( var_4 = 0; var_4 < var_0.size; var_4++ )
    {
        if ( var_0[var_4] == self )
        {
            var_5 = self.pers["squadMemberIndex"] - 1;
            var_6 = 1;
            var_7 = 0;
            ...
            var_8 = int( pow( 2, var_6 ) ) - 1;
            var_9 = ( var_7 & var_8 ) << var_5;
            var_10 = ~( var_8 << var_5 );
            var_11 = munition_remover( self.team, self.squadindex, var_2 );
            ...
            {
                var_1 = 1;
                _id_12175( self.team, self.squadindex, var_2, var_3 );
            }
        }

        if ( var_4 == var_0.size - 1 && var_1 == 1 )
        {
            foreach ( var_14 in var_0 )
                var_14 setclientomnvar( var_2, var_3 );
        }
    }
}
