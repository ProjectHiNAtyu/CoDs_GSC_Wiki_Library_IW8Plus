...
remove_map_hint( var_0, var_1, var_2 )
{
...
set_pressure_stability_reading( var_0, var_1, var_2, var_3 )
{
...
mine_caves_turret_1_support_internal( var_0, var_1, var_2 )
{
    var_3 = set_pressure_stability_reading( var_0, var_1, var_2.origin, var_2.radius );

    if ( !isdefined( var_3 ) )
        return;
    else if ( !isarray( var_3 ) )
    {
        if ( remove_map_hint( var_3, var_0, var_1 ) )
            return var_3;
    }
    else
    {
        var_4 = remove_map_hint( var_3[0], var_0, var_1 );
        var_5 = remove_map_hint( var_3[1], var_0, var_1 );
...
_id_11AC5( var_0, var_1, var_2 )
{
    ...
    {
        var_6 = mine_caves_turret_1_support_internal( var_0, var_1, var_5 );
...
_id_12375( var_0, var_1 )
{
    if ( !isdefined( level.outofboundstriggers ) || level.outofboundstriggers.size == 0 )
        return snappointtomapbounds2d( var_1 );

    if ( !isdefined( level.trial_enemy_quota ) )
        level.trial_enemy_quota = level.outofboundstriggers;

    var_2 = physics_createcontents( [ "physicscontents_playertrigger" ] );
    var_3 = scripts\engine\trace::ray_trace_ents( var_0, var_1, level.trial_enemy_quota, var_2 );

    if ( var_3["fraction"] < 1.0 )
        var_1 = var_3["position"];

    if ( isdefined( level.trial_explosive_clear ) && level.trial_explosive_clear.size > 0 && getdvarint( "scr_br_spawnOOBKillswitch", 0 ) == 0 )
    {
        var_4 = _id_11AC5( var_0, var_1, level.trial_explosive_clear );
...
spawnc130pathstructnew( var_0, var_1 )
{
    ...
    {
        if ( isdefined( level.playertakeextractionplunder ) )
            var_4 = level.playertakeextractionplunder;
        else
            var_4 = 75;
    }
    ...
    var_5 = var_5 + ( 0, 0, mine_light_vfx() );

    if ( !scripts\mp\gametypes\br_gametypes::relic_amped_explosion_time( "randomizeCircleCenter" ) )
        var_2 = var_2 + var_5;
    ...
    if ( isdefined( level.chuckerlogic ) )
        var_2 = level.chuckerlogic;

    if ( scripts\mp\gametypes\br_gametypes::relic_amped_in_warning( "planeConstrainedFromMapCenter" ) )
        var_2 = fire_rpg_to_target( var_2 );
    ...
    if ( scripts\mp\gametypes\br_gametypes::relic_amped_in_warning( "planeUseCircleRadius" ) )
        var_12 = var_11;

    var_13 = _id_12525( var_2, var_6, var_12 );
...
fire_rpg_to_target( var_0 )
{
    var_1 = physics_createcontents( [ "physicscontents_playertrigger" ] );
    var_2 = scripts\engine\trace::ray_trace_ents( level.br_level.br_mapcenter, var_0, level.trial_enemy_quota, var_1 );
...
_id_12525( var_0, var_1, var_2 )
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes::relic_amped_explosion_time( "planeSnapToOOB" ) )
    {
        var_4 = _id_12375( var_0, var_4 );
        var_5 = _id_12375( var_0, var_5 );
    }
    ...
    if ( scripts\mp\gametypes\br_gametypes::relic_amped_explosion_time( "planeSnapToOOB" ) )
    {
        if ( !abandonedtimeoutcallback( var_4 ) )
            var_6 = _id_12375( var_0, var_4 );

        if ( !abandonedtimeoutcallback( var_5 ) )
            var_7 = _id_12375( var_0, var_5 );
    }

    var_3 = vectornormalize( var_5 - var_4 );
    var_8 = 10;

    if ( scripts\mp\utility\game::namehud() == "mini" || scripts\mp\utility\game::namehud() == "zxp" || getdvarint( "scr_br_alt_mode_escape_skip_initial_circle", 0 ) || scripts\mp\utility\game::namehud() == "reveal" || isdefined( level.close_gunshop_door ) && level.close_gunshop_door._id_13032 == 3 )
    {
    ...
    var_9.startpt = var_4;
    var_9.endpt = var_5;
    var_9._id_12747 = var_6;
    var_9.init_relic_bang_and_boom = var_7;
    var_9.angle = var_1;
    var_9.pathdir = var_3;
...
updatec130pathomnvars( var_0 )
{
    ...
    if ( scripts\mp\gametypes\br_gametypes::relic_amped_explosion_time( "c130PlaneLine" ) )
        return;

    setomnvar( "ui_br_c130_path_start_x", int( var_0._id_12747[0] ) );
    setomnvar( "ui_br_c130_path_start_y", int( var_0._id_12747[1] ) );
    setomnvar( "ui_br_c130_path_end_x", int( var_0.init_relic_bang_and_boom[0] ) );
    setomnvar( "ui_br_c130_path_end_y", int( var_0.init_relic_bang_and_boom[1] ) );
}
...
createtestc130path( var_0, var_1 )
{
    ...
    if ( isdefined( level.br_level.br_mapcenter ) && isdefined( level.br_level.br_mapsize ) )
    {
        ...
        level thread scripts\mp\gametypes\br_analytics::clear_legacy_pickup_munitions( var_2.centerpt, var_2.angle[1], var_2._id_12747, var_2.init_relic_bang_and_boom );
    }
    else
...
spawnc130( var_0, var_1, var_2 )
{
    ...
    level.br_ac130.try_add_targetmarkergroup = var_0;
    return var_4;
}
...
spawnplayertoc130()
{
    ...
    scripts\mp\gametypes\br_infils::init_relic_amped();
    thread playerputinc130( level.br_ac130 );
    self.coop_handle_invuln_ac130 = 1;

    if ( istrue( self.register_seat_data_for_vehicle ) && getdvar( "scr_br_gametype", "" ) != "dmz" && getdvar( "scr_br_gametype", "" ) != "risk" && getdvar( "scr_br_gametype", "" ) != "bodycount" )
        level scripts\mp\gametypes\br_public::connected_search_node( "deploy_squad_leader", self, 1, 0 );
}
...
orbitcam( var_0 )
{
    ...
    if ( isdefined( level._id_13062 ) )
        scripts\mp\utility\player::_visionsetnaked( level._id_13062, 0 );
    else
        scripts\mp\utility\player::_visionsetnaked( "", 0 );

    self setplayerangles( var_0.angles );
    var_1 = scripts\mp\gametypes\br_infils::modifyvehicletoplayerdamage( self );
...
gunship_spawn( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( isdefined( level.chopper_death_callback ) )
        var_7 = level.chopper_death_callback;
...
gunship_spawnvfx()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        if ( !isdefined( level.chopper_glow_sticks ) )
            level.chopper_glow_sticks = "br_fx";

        self setscriptablepartstate( level.chopper_glow_sticks, "clouds" );
    }
    else
    {
...
ispointinbounds( var_0, var_1, var_2 )
{
    ...
    var_3 = scripts\mp\utility\game_utility_mp::quarry_wave_spawn_scoring( var_0, var_2 );

    if ( !var_3 )
        return 0;

    var_4 = scripts\mp\gametypes\br_public::registerpreviousprop() || scripts\mp\gametypes\br_public::register_player_character();

    if ( var_4 && isdefined( level.br_ac130 ) )
        return var_3;

    if ( resetplayermovespeedscale( var_0 ) )
        return 1;
    ...
    if ( istrue( var_1 ) )
    {
        if ( scripts\mp\outofbounds::removestuckenemyondeathordisconnect( var_0 ) )
            return 0;
    }
...
updatetriggerforcodcaster()
{
    ...
    for (;;)
    {
        var_1 = vectornormalize( self.try_add_targetmarkergroup._id_12747 - self.origin );
...
kickwhenoutofbounds( var_0 )
{
    ...
    waitframe();
    updatetriggerforcodcaster();
    var_1 = mine_light_vfx();

    while ( !abandonedtimeoutcallback( ( self.origin[0], self.origin[1], var_1 ) ) )
        waitframe();
    ...
    level.choppersupport_watchleashrange = 1;
    ...
    for (;;)
    {
        ...
        var_7 = vectornormalize( self.try_add_targetmarkergroup.centerpt - self.origin );
    ...
    for (;;)
    {
        ...
        if ( !abandonedtimeoutcallback( var_10 ) )
        {
            level.chopper_gunner = 1;
...
killaftertime( var_0, var_1 )
{
    ...
    self stoploopsound();
    scripts\mp\gametypes\br_infils::playerstreamhintlocation( self );
    wait 0.1;
    ...
    if ( isdefined( self.animstruct ) )
    {
        ...
        scripts\mp\gametypes\br_infils::enemygunship_updatedebugflashlight( self.animstruct );
    }
    else
    {
        if ( isdefined( self.innards ) )
...
listenjump( var_0, var_1 )
{
    ...
    var_0 updatetriggerforcodcaster();
    ...
    for (;;)
    {
        ...
        var_5 = scripts\mp\gametypes\br_public::munitions_override_time( self.team, self.squadindex );

        if ( !var_3 )
        {
            var_2 = var_0 calctrailpoint();

            if ( abandonedtimeoutcallback( var_2 ) )
            {
                self.jumptype = "solo";
        ...
        var_6 = scripts\mp\gametypes\br_public::reload_currentsequence_button_handler() || isdefined( self.pers["squadMemberIndex"] ) && self.pers["squadMemberIndex"] == 0;
...
leaveplane( var_0, var_1, var_2, var_3 )
{
    ...
    thread scripts\mp\gametypes\br_gametypes::_id_11E4F( "onLeaveAC130" );

    if ( scripts\mp\utility\game::namehud() == "truckwar" )
    {
        scripts\mp\gametypes\br_gametype_truckwar::getspawnpoint( 1 );
        self setorigin( self._id_11B4B.origin, 1 );
        self setplayerangles( self._id_11B4B.angles );
    }
}
...
{
    var_3 = scripts\engine\utility::ter_op( var_0 scripts\mp\gametypes\br_public::reload_currentsequence_button_handler(), 2, var_2 );
...
pushplayertoplayeroffset( var_0, var_1 )
{
    ...
    var_2 thread waitandstartparachuteoverheadmonitoring();
    var_3 = length( var_1 );
...
infilfreefallparachuteaudio( var_0 )
{
    ...
    if ( scripts\mp\utility\game::namehud() != "truckwar" && scripts\mp\utility\game::namehud() != "reveal" && scripts\mp\utility\game::namehud() != "brdov" )
    {
        ...
        var_1.riskspawn_flagcaptured = 1;
    }

    wait 1.0;
    var_1 clearclienttriggeraudiozone( 3.0 );

    if ( scripts\mp\utility\game::namehud() == "reveal" || scripts\mp\utility\game::namehud() == "brdov" )
    {
        wait 3.0;
...
waittoplayinfildialog()
{
    ...
    level.br_ac130 updatetriggerforcodcaster();

    for (;;)
    {
        var_0 = level.br_ac130 calctrailpoint();

        if ( abandonedtimeoutcallback( var_0 ) )
        {
