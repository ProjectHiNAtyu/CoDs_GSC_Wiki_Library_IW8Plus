...
setupobjective( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    if ( !isdefined( var_5 ) )
        var_5 = 1;
    ...
    if ( isdefined( var_0.target ) )
    {
        if ( !isdefined( var_0.visuals ) )
            var_6[0] = getent( var_0.target, "targetname" );
        else
            var_6 = var_0.visuals;
    }
    else
    {
        var_6[0] = spawn( "script_model", var_0.origin );
        var_6[0].angles = var_0.angles;
    }
    ...
    var_10 = scripts\mp\gameobjects::createuseobject( var_1, var_0, var_6, ( 0, 0, 100 ), var_9, var_3, var_4 );
    var_10 scripts\mp\gameobjects::allowuse( "enemy" );
    var_10 scripts\mp\gameobjects::cancontestclaim( 1 );
    var_10 scripts\mp\gameobjects::setusetime( level.flagcapturetime );

    if ( isdefined( level.capturetype ) )
        var_10 scripts\mp\gameobjects::setcapturebehavior( getcapturetype() );

    var_10.objectivekey = var_7;
    var_10.iconname = var_8;

    if ( !istrue( var_3 ) )
    {
        var_10 scripts\mp\gameobjects::setvisibleteam( "any", undefined, 1 );
        var_10.onuse = ::dompoint_onuse;
        var_10.onbeginuse = ::dompoint_onusebegin;
        var_10.onuseupdate = ::dompoint_onuseupdate;
        var_10.onenduse = ::dompoint_onuseend;
        var_10.oncontested = ::dompoint_oncontested;
        var_10.onuncontested = ::dompoint_onuncontested;
        var_10.onunoccupied = ::dompoint_onunoccupied;
        var_10.onpinnedstate = ::dompoint_onpinnedstate;
        var_10.onunpinnedstate = ::dompoint_onunpinnedstate;
        var_10.stompprogressreward = ::dompoint_stompprogressreward;
    }

    var_10.nousebar = 1;
    var_10.id = "domFlag";
    var_10.claimgracetime = level.flagcapturetime * 1000;
    var_10.firstcapture = 1;
    var_10 scripts\mp\gameobjects::pinobjiconontriggertouch();

    if ( istrue( level.leaderplunderstring ) )
        var_10._id_125C1 = ::_id_125C2;
    else if ( istrue( level.open_teleport_room_door ) && scripts\mp\utility\game::getgametype() != "br" )
        var_10._id_125C1 = ::_id_125C3;

    var_11 = var_6[0].origin + ( 0, 0, 32 );
    var_12 = var_6[0].origin + ( 0, 0, -32 );
    var_13 = scripts\engine\trace::create_contents( var_5, 1, 1, 1, 0, 1, 1 );
    var_14 = [];
    var_15 = scripts\engine\trace::ray_trace( var_11, var_12, var_14, var_13 );
    var_16 = checkmapoffsets( var_10 );
    var_10.baseeffectpos = var_15["position"] + var_16;
    var_17 = vectortoangles( var_15["normal"] );
    var_18 = checkmapfxangles( var_10, var_17 );
    var_10.baseeffectforward = anglestoforward( var_18 );
    var_19 = spawn( "script_model", var_10.baseeffectpos );
    var_19 setmodel( "dom_flag_scriptable" );
    var_19.angles = generateaxisanglesfromforwardvector( var_10.baseeffectforward, var_19.angles );
    var_10.scriptable = var_19;
    var_10.vfxnamemod = "";
    var_10.noscriptable = 1;

    if ( istrue( level.multiteambased ) )
        var_10.noscriptable = 1;

    var_10.flagmodel = spawn( "script_model", var_10.baseeffectpos );

    if ( istrue( level.open_teleport_room_door ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_20 = "decor_halloween_scarecrow";
        var_10.flagmodel.removeincoming = 1;
    }
    else
        var_20 = "military_dom_flag_neutral";

    var_10.flagmodel setmodel( var_20 );
    var_10.flagmodel.angles = drone_instant_killed_monitor( var_10 );
    var_10.outlineent = var_10.flagmodel;

    if ( istrue( level.open_teleport_room_door ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_10 thread open_starting_safehouse_door();
        var_10 thread helihint_gotopad();
    }

    var_10 initializematchrecording();

    if ( !istrue( var_3 ) )
        var_10 domflag_setneutral( undefined, 1 );
...
drone_instant_killed_monitor( var_0 )
{
...
dompoint_setcaptured( var_0, var_1 )
{
    ...
    if ( isdefined( self.assisttouchlist[var_0] ) )
    {
        ...
        foreach ( var_8 in var_6 )
        {
            ...
            var_9 scripts\cp\helicopter\chopper_boss::toggle_farah_lights();
        }
    }
...
dompoint_onuse( var_0 )
{
    ...
    if ( !self.neutralized )
    {
        ...
        if ( isdefined( level.onobjectivecomplete ) )
        {
            ...
            var_5 = "Flag " + monitorhvt_gooddroppos() + " Captured";

            if ( var_2 == "neutral" || var_2 == "none" )
                scripts\mp\utility\game::setup_enemy_sentries( var_0, undefined, var_5, var_0.origin, "neutral_flag" );
            else
                scripts\mp\utility\game::setup_enemy_sentries( var_0, undefined, var_5, var_0.origin );
        }
    }
...
{
    self.first_interaction = gettime();
...
dompoint_onuncontested( var_0 )
{
    ...
    var_2 = ( gettime() - self.first_interaction ) * 0.001;
    var_3 = "Flag " + monitorhvt_gooddroppos() + " Contested";
    scripts\mp\utility\game::setup_enemy_sentries( undefined, undefined, var_3, undefined, var_2 + " seconds" );
    self.first_interaction = undefined;
...
_id_125C2()
{
    ...
    if ( var_7["fraction"] != 1 )
    {
        ...
        if ( var_7["fraction"] != 1 )
        {
            playfx( level.oil_puddles["gos_fireworks"], var_7["position"] );
            playfx( level.oil_puddles["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
            playfx( level.oil_puddles["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
            level thread scripts\mp\gametypes\common::_id_1311F( var_7["position"] );
        }
    }
    else
    {
        playfx( level.oil_puddles["gos_fireworks"], var_2 );
        playfx( level.oil_puddles["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
        playfx( level.oil_puddles["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
        level thread scripts\mp\gametypes\common::_id_1311F( var_2 );
    }
}

_id_125C3()
{
    if ( isdefined( self.playerhumanhitground ) )
        return;
    else
    {
        self.playerhumanhitground = 1;
        var_0 = 20;
        var_1 = 600;
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.playerhumanconcusspush["blood_ceil_hw"], var_7["position"] - ( 0, 0, 5 ), ( -180, -180, 0 ), ( -180, -180, -180 ) );

        playfx( level.playerhumanconcusspush["blood_floor_hw"], self.flagmodel.origin + ( 0, 0, 2 ) );
        var_0 = 4000;
        var_1 = 20;
        var_2 = self.flagmodel.origin;
        var_3 = var_2 + ( 0, 0, var_0 );
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.playerhumanconcusspush["bats_fly_hw"], var_7["position"] + ( 0, 0, 50 ) );
        else
            playfx( level.playerhumanconcusspush["bats_fly_hw"], self.flagmodel.origin + ( 0, 0, 50 ) );
    }
}
...
open_starting_safehouse_door()
{
    ...
    if ( var_7["fraction"] != 1 )
        playfx( level.playerhumanconcusspush["bats_hw"], var_7["position"] );
    else
        playfx( level.playerhumanconcusspush["bats_hw"], self.flagmodel.origin + ( 0, 0, 200 ) );
}
...
updateflagstate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.open_teleport_room_door ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        if ( istrue( var_3 ) )
            ...
            self.flagmodel thread _id_11EE6();
        else if ( isdefined( var_2 ) )
            playfx( level.playerhumanconcusspush["vanish_hw_fr"], self.flagmodel.origin + ( 0, 0, 60 ) );
    }
    else if ( isdefined( var_2 ) )
    {
        if ( var_2 == "allies" )
...
giveflagcapturexp( var_0, var_1, var_2 )
{
    ...
    if ( isplayer( var_3 ) )
    {
        ...
        var_3 thread scripts\common\utility::_id_12C2C( level.slopelocked, "capture", var_3.origin );
    }
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( var_0 ) )
    {
        ...
        {
            if ( istrue( var_0.removeincoming ) )
                return;
        }
    }
            ...
            if ( var_20 || var_21 )
            {
                if ( var_18.claimteam == var_13.team )
                {
                    if ( !var_11 )
                    {
                        ...
                        var_18 notify( "assault", var_1 );
                        thread scripts\common\utility::_id_12C2C( level.slamcamoverlay, var_9, "assaulting" );
                        continue;
                    }
                }
                else if ( var_18.claimteam == var_1.team )
                {
                    if ( !var_12 )
                    {
                        ...
                        thread scripts\common\utility::_id_12C2C( level.slamcamoverlay, var_9, "defending" );
                        continue;
                    }
                }
            }
...
_id_11EE6()
{
    ...
    wait 0.55;
    playfx( level.playerhumanconcusspush["vanish_hw_fr"], self.origin + ( 0, 0, 60 ) );
...
helihint_gotopad()
{
    ...
    for (;;)
    {
        ...
        if ( isplayer( var_0 ) )
        {
            if ( !isdefined( self._id_12650 ) )
            {
                self._id_12650 = var_0;
                thread _id_13257( self.flagmodel, var_0 );
            }
        }
...
_id_13257( var_0, var_1 )
{
    ...
    while ( isdefined( self._id_12650 ) )
    {
        if ( !_id_12E9B( var_0, var_1 ) )
        {
            self._id_12650 = undefined;
            self._id_1264F = 0;
            self.iseligibleforteamrevive = undefined;
            self.bomber_spawn_origins = undefined;
            self._id_11B5D = undefined;
            self.flagmodel rotateto( ( 0, self.flagmodel.angles[1], self.flagmodel.angles[2] ), 0.2, 0.1, 0.1 );
        }
        else if ( isdefined( self._id_12650 ) && scripts\mp\utility\player::isreallyalive( self._id_12650 ) )
        {
...
_id_1324F()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_1 ) && isdefined( self._id_12650 ) && self._id_12650 == var_1 )
            self._id_1264F = self._id_1264F + var_0;

        if ( isdefined( self._id_1264F ) )
        {
            if ( self._id_1264F > 100 && !isdefined( self.iseligibleforteamrevive ) )
                self.iseligibleforteamrevive = 1;

            if ( self._id_1264F > 200 && !isdefined( self.bomber_spawn_origins ) )
            {
                self.bomber_spawn_origins = 1;
                playfx( level.playerhumanconcusspush["vanish_hw_en"], self.flagmodel.origin + ( 0, 0, 80 ) );
            }

            if ( self._id_1264F > 300 && !isdefined( self._id_11B5D ) )
            {
                self._id_11B5D = 1;
                thread _id_11D2D( var_1 );
            }
        }
...
_id_12E9B( var_0, var_1 )
{
    var_2 = 0.05;
    var_3 = var_1 musictriggerthink();
    var_4 = var_0 mp_m_cornfield_patch();
