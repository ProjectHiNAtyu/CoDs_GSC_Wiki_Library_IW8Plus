...
main()
{
    ...
    level.spawnextractsmoke = ::detonatedripfx;
...
initrules()
{
    ...
    updategametypedvars();
    level.ontimelimitgraceperiod = module_wait_for_level_flag_set_and_clear();
...
initrugbyents()
{
    level.race_ui_show_record_end = scripts\engine\utility::array_combine( level.outofboundstriggers, level.minetriggers, level.hurttriggers, level.radtriggers );
    initendzoneents();
...
{
    if ( isdefined( level._id_11E23 ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level._id_11E23.team, 0, 1 ) );
}
...
onplayerspawned( var_0 )
{
    ...
    if ( isdefined( level._id_11E23 ) && isdefined( level._id_11E23.team ) && isdefined( var_0.team ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level._id_11E23.team, 0, 1 ) );
}
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( level._id_11E23 ) )
    {
        ...
        {
            if ( var_1 == level._id_11E23 )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_as_juggernaut" );
            else if ( var_12 == level._id_11E23 )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_juggernaut" );
            else if ( var_1.team == level._id_11E23.team && var_1 != level._id_11E23 )
            {
                var_16 = distancesquared( level._id_11E23.origin, var_14 );
            ...
            else if ( var_1.team != level._id_11E23.team && var_12.team == level._id_11E23.team )
            {
                var_16 = distancesquared( level._id_11E23.origin, var_13 );
...
ontimelimitot()
{
    if ( level.rugby.activejuggernauts.size > 0 )
    {
        ...
        {
            setomnvar( "ui_overtime_time", gettime() + module_wait_for_level_flag_set_and_clear() * 1000 );
            level.rugby.inot = 1;
            level.canprocessot = 1;
            _id_12128();
            level thread rewardtovalue();
        }
    }
    else
    {
...
_id_121D4( var_0 )
{
    ...
    var_0 thread br_prematchspawnlocations( var_1, var_0.trigger.targetname );
    var_0 thread updatechevrons( "idle" );
}

br_prematchspawnlocations( var_0, var_1 )
{
    wait 1.0;
    var_2 = getentitylessscriptablearrayinradius( var_0, "targetname" );
    var_2 = x1fin_think( var_2, var_1 );
    var_3 = [];
...
gate_swings_open( var_0, var_1, var_2 )
{
    var_0.origin = var_1;
    var_0.angles = var_2;
    return var_0;
...
activatenewjuggernaut( var_0 )
{
    ...
    level thread vehicle_compass_init( var_0.team, var_6 );
...
vehicle_compass_init( var_0, var_1 )
{
    wait 3;
...
deactivatejuggernaut( var_0 )
{
    ...
    level.rugby.hidedeathicon = undefined;
    level.rugby._id_11989 = undefined;
    level.rugby.risk_flagspawnshiftingpercent = undefined;
    level.rugby.risk_flagspawnshiftingcenter = undefined;
...
watchjuggprogress()
{
    ...
    for (;;)
    {
        ...
        if ( var_9 > 0.8 )
            level thread dropoff( var_9, var_0, var_2 );
        ...
        if ( isdefined( level._id_11E23 ) )
        {
            ...
            {
                var_7 = var_9;
                level._id_11E23 thread scripts\mp\utility\points::giveunifiedpoints( "rugby_obj_push" );
            }
        }
...
dropoff( var_0, var_1, var_2 )
{
    if ( !isdefined( level.rugby.hidedeathicon ) )
    {
        level.rugby.hidedeathicon = 1;
        level.rugby._id_11989 = gettime();
        level.rugby.risk_flagspawnshiftingpercent = var_0;
        level.rugby.risk_flagspawnshiftingcenter = var_0;
    }
    else if ( isdefined( level.rugby._id_11989 ) && level.rugby._id_11989 + 30000 < gettime() )
    {
        level.rugby.hidedeathicon = 1;
        level.rugby._id_11989 = gettime();
        level.rugby.risk_flagspawnshiftingpercent = var_0;
    ...
    if ( istrue( level.rugby.hidedeathicon ) )
    {
...
handlejuggjumpspam()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::_id_13141( 1, "jugg_jumped" );
...
{
    var_0 = 0;
    level.risk_modifyflagstieronrespawn = spawnstruct();
    level.risk_modifyflagstieronrespawn.objidnum = scripts\mp\objidpoolmanager::requestreservedid( var_0 );
}
...
activatejuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( !reset_use_puzzle_effects( var_0 ) )
    {
        var_0 = level.force_teleport_player_into_plane;
        var_1 = level.force_teleport_downedplayer;
    }
...
reset_use_puzzle_effects( var_0 )
{
    ...
    foreach ( var_3 in level.race_ui_show_record_end )
    {
...
createjuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( istrue( var_2 ) )
    {
        level.force_teleport_player_into_plane = var_0;
        level.force_teleport_downedplayer = var_1;
...
juggcratemanageuse( var_0 )
{
    ...
    var_1.reset_nearby_spawn_times = 1;
    self._id_12E4F = scripts\mp\gameobjects::createholduseobject( "neutral", var_1, self.visuals, ( 0, 0, 64 ) );

    foreach ( var_3 in level.teamnamelist )
        self._id_12E4F.teamprogress = undefined;

    self._id_12E4F.trigger._id_12E64 = 16900;
    self._id_12E4F.trigger setuserange( 130 );
    self._id_12E4F.trigger sethintstring( &"MP_MODE_RUGBY/CRATE_USE" );
    self._id_12E4F.trigger makeusable();
    self._id_12E4F.trigger setcursorhint( "HINT_BUTTON" );
    self._id_12E4F.trigger setuseholdduration( "duration_none" );
    self._id_12E4F.trigger setusehideprogressbar( 1 );
    self._id_12E4F.trigger setusepriority( -3 );
    self._id_12E4F.trigger sethintonobstruction( "hide" );
    self._id_12E4F scripts\mp\gameobjects::setusetime( module_set_skip_basic_combat() );
    self._id_12E4F.interactteam = "any";
    self._id_12E4F.curprogress = 0;
    self._id_12E4F.defaultusetime = self._id_12E4F._id_12E74;
    self._id_12E4F.userate = 1;
    self._id_12E4F.id = "rugby_jugg";
    self._id_12E4F.exclusiveuse = 0;
    self._id_12E4F.exclusiveclaim = 0;
    self._id_12E4F.skiptouching = 1;
    self._id_12E4F.onbeginuse = ::_id_11E20;
    self._id_12E4F.onuse = ::_id_11E1F;
    self._id_12E4F.onenduse = ::_id_11E21;
    self._id_12E4F.oncantuse = ::_id_11E22;
    self._id_12E4F.inuse = 0;

    foreach ( var_6 in level.teamnamelist )
    {
        self._id_12E4F.numtouching[var_6] = 0;
        self._id_12E4F.touchlist[var_6] = [];
    }

    self._id_12E4F.cancontestclaim = 0;
    self._id_12E4F.stalemate = 0;
    self._id_12E4F.wasstalemate = 0;
    self._id_12E4F.cancontestclaim = 0;
    self._id_12E4F.majoritycapprogress = 1;
    self._id_12E4F.wasmajoritycapprogress = 0;
    self._id_12E4F.resetprogress = 1;

    if ( !scripts\mp\flags::gameflag( "prematch_done" ) )
        level scripts\engine\utility::_id_1312D( "prematch_done", "start_mode_setup" );

    self._id_12E4F.type = "useObject";
    self._id_12E4F.offset3d = ( 0, 0, 32 );
    self._id_12E4F scripts\mp\gameobjects::requestid( 1, 1 );
    self._id_12E4F scripts\mp\gameobjects::setobjectivestatusicons( "icon_waypoint_jugg_crate" );
    self._id_12E4F scripts\mp\gameobjects::setvisibleteam( "any" );
    scripts\mp\objidpoolmanager::objective_set_play_intro( self._id_12E4F.objidnum, 0 );
    scripts\mp\objidpoolmanager::objective_set_play_outro( self._id_12E4F.objidnum, 0 );
}
...
_id_11E1F( var_0 )
{
    ...
    if ( isdefined( self._id_12E4F ) )
        self._id_12E4F delete();
...
detonatedripfx( var_0 )
{
    ...
    var_2 = var_0 scripts\mp\juggernaut::retreatanddie( var_1 );
...
updateoverheadcamerapos( var_0 )
{
    ...
    var_10 = scripts\cp_mp\utility\killstreak_utility::molotov_crate_update_hint_logic_alt();
