...
setupobjective( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.open_teleport_room_door ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
            var_4 thread _id_1311E( var_4 );
        else if ( scripts\mp\utility\game::getgametype() == "grnd" )
            var_4 thread _id_1311D( var_4 );
    }
...
_id_1311E( var_0 )
{
    ...
    playfx( level.playerhumanconcusspush["ghostcat_hw"], var_3 + var_2 );
}

_id_1311D( var_0 )
{
    scripts\mp\flags::gameflagwait( "prematch_done" );
    playfx( level.playerhumanconcusspush["blood_floor_hw"], getgroundposition( var_0.trigger.origin, 4 ) + ( 0, 0, 2 ) );
}
...
assignchevrons( var_0, var_1 )
{
    ...
    var_2 = _id_11C7D( var_2, var_1 );
    var_2 = x1givelaststandoverride( var_2, var_1 );
    var_3 = [];
...
activatezone()
{
    ...
    if ( isdefined( level.modifybrvehicledamage ) )
        var_0 = [[ level.modifybrvehicledamage ]]();

    var_1 = 1024;

    if ( isdefined( level.modifycrushdamage ) )
        var_1 = [[ level.modifycrushdamage ]]();
    ...
    self.get_player_munition_currency = var_2;
...
deactivatezone()
{
    ...
    foreach ( var_1 in self.get_player_munition_currency )
        scripts\mp\spawnlogic::removespawndangerzone( var_1 );

    self.get_player_munition_currency = undefined;
...
setteamlastzombietime( var_0, var_1 )
{
    level endon( "game_ended" );
...
cleanupgametypevips()
{
    level scripts\engine\utility::_id_1312D( "game_ended", "zone_moved" );
...
zone_onuse( var_0 )
{
    ...
    if ( level.usehqrules && !level.usehprules && self.ownerteam != "neutral" )
    {
        ...
        if ( isdefined( var_3 ) )
        {
            ...
            foreach ( var_11 in var_9 )
            {
                ...
                var_7 scripts\cp\helicopter\chopper_boss::toggle_farah_lights();
            }
        }
    ...
    else
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "hq" && !level.usehprules )
        {
            ...
            apc_rus_adjustdriverturretammo( var_1 );
        }
        else if ( level.tryspawnscriptablereuse )
    ...
    if ( istrue( level.open_teleport_room_door ) )
        playfx( level.playerhumanconcusspush["vanish_hw_fr"], level.zone.trigger.origin + ( 0, 0, 40 ) );
}
...
zone_onusebegin( var_0 )
{
    if ( scripts\mp\utility\game::getgametype() == "hq" && !level.usehprules )
    {
        ...
        if ( var_1 != "neutral" && self.claimteam != var_1 )
            apc_rus_adjustdriverturretammo( var_1, 1 );
...
zone_onuseupdate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( var_4 == "neutral" && self.claimteam != var_4 )
        apc_rus_adjustdriverturretammo( self.claimteam );
    else if ( var_4 != "neutral" && self.claimteam != var_4 )
        apc_rus_adjustdriverturretammo( var_4, 1 );
    else if ( level.tryspawnscriptablereuse )
        level scripts\mp\gamelogic::pausetimer();
...
zone_onuseend( var_0, var_1, var_2 )
{
    ...
    if ( !var_2 )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
        {
            if ( level.usehprules )
            {
                if ( level.tryspawnscriptablereuse )
                    level scripts\mp\gamelogic::resumetimer();
            }
            else if ( var_3 != "neutral" )
                apc_rus_adjustdriverturretammo( var_3 );
        }
        else if ( level.tryspawnscriptablereuse )
            level scripts\mp\gamelogic::resumetimer();
    }
...
zone_onunoccupied()
{
    ...
    {
        level.zone scripts\mp\gameobjects::setobjectivestatusicons( level.icondefend, level.iconcapture );
        apc_rus_adjustdriverturretammo( self.ownerteam );
    }
    else
    {
        level notify( "zone_destroyed" );
...
zone_oncontested()
{
    ...
    level.zone thread scripts\common\utility::_id_12C2C( level.slopelocked, "hill_contested", level.zone.trigger.origin );
    self.didstatusnotify = 1;
}
...
zone_onuncontested( var_0 )
{
    ...
    if ( var_0 == "none" || var_1 == "neutral" )
    {
        ...
        level.zone thread scripts\common\utility::_id_12C2C( level.slopelocked, "hill_empty", level.zone.trigger.origin );
    }
    else
    {
        if ( scripts\mp\utility\game::getgametype() == "koth" )
        ...
        level.zone thread scripts\common\utility::_id_12C2C( level.slopelocked, "hill_uncontested", level.zone.trigger.origin );
    }

    var_7 = ( gettime() - self.first_interaction ) * 0.001;
    scripts\mp\utility\game::setup_enemy_sentries( undefined, undefined, "Zone Contested", level.zone.trigger.origin, var_7 + " seconds" );
...
x1givelaststandoverride( var_0, var_1 )
{
    var_2 = [];
    var_3 = [];
...
_id_1323E()
{
    ...
    foreach ( var_2 in level.teamnamelist )
    {
        self.lb_mg_dmg_factor_landing_gear[var_2] = 0;
        self.lb_mg_dmg_factor_main_rotor[var_2] = [];
    }

    self.lb_mg_dmg_factor_landing_gear["neutral"] = 0;
    self.lb_mg_dmg_factor_main_rotor["neutral"] = [];
    self.lb_mg_dmg_factor_main_rotor["none"] = [];
    self.downed_reviver_watch_for_return_to_usability = 1;
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
        {
            ...
            self.lb_mg_dmg_factor_landing_gear[var_2]++;
            var_6 = var_4.guid;
            var_7 = spawnstruct();
            var_7.player = var_4;
            var_7.starttime = gettime();
            self.lb_mg_dmg_factor_main_rotor[var_2][var_6] = var_7;
            var_4.player_rigs = 0;
        }

        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
            var_4 thread lb_mg_dmg_factor_tail_rotor( self );
    }
}

lb_mg_dmg_factor_tail_rotor( var_0 )
{
    level endon( "stop_watching_trigger" );
    ...
    if ( isdefined( self ) )
    {
        var_0.lb_mg_dmg_factor_main_rotor[var_1][self.guid] = undefined;
        self.player_rigs = 0;
    }
    else
    {
        ...
        foreach ( var_5, var_4 in var_0.lb_mg_dmg_factor_main_rotor[var_1] )
        {
        ...
        foreach ( var_5 in var_2 )
            var_0.lb_mg_dmg_factor_main_rotor[var_1][var_5] = undefined;
    }

    var_0.lb_mg_dmg_factor_landing_gear[var_1]--;
}
