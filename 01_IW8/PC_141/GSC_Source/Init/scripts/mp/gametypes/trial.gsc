...
main()
{
    ...
    level.birds_in_square_monitor = var_0;
    ...
    while ( !isdefined( level._id_12B81 ) )
        waitframe();
    ...
    switch ( level.trial["missionScript"] )
    {
        case "gun_nonlinear":
        case "gun":
            thread [[ level._id_12B81["gun"] ]]();
            break;
        case "clear":
            thread [[ level._id_12B81["clear"] ]]();
            break;
        case "sniper":
            thread [[ level._id_12B81["sniper"] ]]();
            break;
        case "jugg":
            thread [[ level._id_12B81["jugg"] ]]();
            break;
        case "lava":
            thread [[ level._id_12B81["lava"] ]]();
            break;
        case "pitcher":
            thread [[ level._id_12B81["pitcher"] ]]();
            break;
        case "gunslinger":
            thread [[ level._id_12B81["gunslinger"] ]]();
            break;
        case "race":
            thread [[ level._id_12B81["race"] ]]();
            break;
        case "arm_course":
            thread [[ level._id_12B81["arm_course"] ]]();
            break;
        default:
            break;
    }
...
_id_12BC8( var_0, var_1 )
{
    level notify( "exitLevel_called" );
    processlobbydata();

    if ( isdefined( level._id_12B62 ) )
        [[ level._id_12B62 ]]();
...
trial_mission_data_init()
{
    var_0 = _getactualcost::_id_12B72();
...
getspawnpoint()
{
    ...
    if ( isdefined( level._id_12B99 ) )
    {
        var_3 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_getavailablevehicleseats( level._id_12B99, 1 );
        var_4 = spawnstruct();
        var_4.useonspawn = 1;
        var_4.enterstartwaitmsg = "spawned_player";
        thread scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_enter( level._id_12B99, var_3[0], self, var_4 );
...
onplayerconnect( var_0 )
{
    ...
    level.player scripts\mp\gametypes\br::vehomn_clearcontrols();
    wait 1;
...
weapon_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( self.spawned_weapon ) && !var_14 && !var_15 )
        {
            ...
            if ( istrue( level.vehicle_mp_create ) )
...
trial_end_score_dialogue()
{
    ...
    _getactualcost::trial_ui_waittill_retry();

    if ( !istrue( level._id_12B90 ) )
...
_id_12B8F()
{
    for (;;)
    {
        ...
        {
            if ( !isdefined( level.relic_grounded_reload_monitor ) || !level.relic_grounded_reload_monitor )
                _getactualcost::_id_12B8E();
        }
    }
}
