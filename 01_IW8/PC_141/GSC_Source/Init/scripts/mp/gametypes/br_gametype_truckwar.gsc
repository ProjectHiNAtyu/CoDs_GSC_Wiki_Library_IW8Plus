...
init()
{
    level thread create_smoke_occluder();
    level thread create_solution_digit_mark();
    level thread create_silencer_pick_up();
    level thread create_silencer_pick_ups();
    ...
    level.clearsplashqueue = 0;
    level._id_12CDD = getdvarint( "scr_truckwar_circleMoving", 1 ) == 1;
    level._id_12CE1 = getdvarint( "scr_truckwar_useDefaultCircleSizes", 1 ) == 1;
    level._id_12CD3 = 15;
    ...
    if ( var_0 != "" )
    {
        ...
        foreach ( var_3 in var_1 )
            level._id_12CDF[level._id_12CDF.size] = float( var_3 );
    }
    ...
    if ( var_5 != "" )
    {
        ...
        foreach ( var_3 in var_6 )
            level._id_12CD2[level._id_12CD2.size] = float( var_3 );
    }
    ...
    if ( var_9 != "" )
    {
        ...
        foreach ( var_3 in var_10 )
            level._id_12CD7[level._id_12CD7.size] = float( var_3 );
    }

    level._id_12CE0 = getdvarfloat( "scr_truckwar_circleStartRadius", 50000 );
    level._id_12CDA = getdvarfloat( "scr_truckwar_circleInnerSize", 40000 );
    level._id_12CDB = getdvarfloat( "scr_truckwar_circleMinSize", 10000 );
    level._id_12CCF = getdvarfloat( "scr_truckwar_circleCloseMinTime", 30 );
    level._id_12CD0 = getdvarfloat( "scr_truckwar_circleCloseTime", 220 );
    level._id_12CD1 = getdvarfloat( "scr_truckwar_circleCloseTimeChange", 40 );
    level._id_12CD5 = getdvarfloat( "scr_truckwar_circleDelayTime", 60 );
    level._id_12CD6 = getdvarfloat( "scr_truckwar_circleDelayTimeChange", 10 );
    level._id_12CD4 = getdvarfloat( "scr_truckwar_circleDelayMinTime", 10 );
    level._id_12CDE = getdvarint( "scr_truckwar_circleOutOfBoundsPadding", 1 );
    level._id_12CD9 = getdvarfloat( "scr_truckwar_circleInitialDelayTime", 60 );
    level._id_12CD8 = getdvarfloat( "scr_truckwar_circleInitialCloseTime", 330 );
    level._id_12CDC = getdvarfloat( "scr_truckwar_circleMoveDist", 1 );
...
create_smoke_occluder()
{
    ...
    scripts\mp\gametypes\br_gametypes::handle_train_veh_collision( "c130PlaneLine" );
    scripts\mp\gametypes\br_gametypes::init_exit_doors( "tabletReplace" );
}

create_solution_digit_mark()
{
    scripts\mp\gametypes\br_gametypes::_id_11B97( "playerWelcomeSplashes", ::create_traversal_node_and_link );
    ...
    scripts\mp\gametypes\br_gametypes::_id_11B97( "isTeamEliminated", ::requiredplayercountoveride );
    scripts\mp\gametypes\br_gametypes::_id_11B97( "infilSequence", ::helicleanup );
    waittillframeend;
    scripts\mp\flags::gameflaginit( "use_truck_respawn", 0 );
    level.ontimelimit = ::create_thrust_fire;
...
onstartgametype()
{
    if ( !isdefined( level.chopper_carepackage_pilot_selected ) )
        level.chopper_carepackage_pilot_selected = [];

    level.chopper_carepackage_pilot_selected[level.chopper_carepackage_pilot_selected.size] = scripts\mp\gametypes\br_circle::garbage_bin_clip( ( -32000, 53750, 2266 ), 16000 );
    initspawns();
    level thread quarry_hacks_count();
    level thread _id_122F5();
}

create_traversal_node_and_link( var_0 )
{
    ...
    scripts\mp\gametypes\br_public::connected_search_node( "mode_intro", self, 0 );
...
quarry_hacks_count()
{
    ...
    if ( getdvarint( "scr_allow_vehicle_cargo_truck_mg", 1 ) == 1 )
        registervehicletype( "cargo_truck_mg", scripts\mp\vehicles\vehicle_damage_mp::disable_outline_on_atv_use, ::_id_13007 );
    ...
    if ( getdvarint( "scr_allow_vehicle_motorcycle", 1 ) == 1 )
        registervehicletype( "motorcycle", __ending_origin::start_reach_wind_room, ::_id_1300A );

    level waittill( "prematch_fade_done" );
    wait 2.0;
}

br_squadrevivestatus()
{
    ...
    foreach ( var_1 in level.vehicleinfo )
    {
        ...
        {
            case "cargo_truck_mg":
                var_1.vehiclespawns = nextscore( "cargo_truck_mg", "mkilo_physics_mg" );
                break;
        }
    ...
    scripts\mp\gametypes\br_vehicles::_id_12C2E( level.vehiclespawnlocs );
...
_id_125A1()
{
    ...
    foreach ( var_5, var_2 in level._id_12957 )
    {
...
propent( var_0, var_1, var_2 )
{
    level._id_12956[var_1] = var_0;
    level._id_12931[var_1] = 1;
    level thread _id_13261( var_0, var_1 );
    level thread _id_13262( var_0, var_1 );
    level thread tacinsert_deleteontoofar( var_0, var_1 );
    level thread tacinsert_destroyongameended( var_0, var_1 );
    level thread tacinsert_destroyuselistener( var_0, var_1 );
    ...
    {
        var_5 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_pockets", var_0.health );
        var_5 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_banked", var_0.maxhealth );

        if ( istrue( var_2 ) && getdvarint( "scr_truckwar_truck_replacement_respawn", 1 ) == 1 && !istrue( level._id_12C18 ) )
        {
...
_id_13261( var_0, var_1 )
{
    ...
    while ( isdefined( var_0 ) )
    {
        ...
        {
            if ( !var_0._id_131A4 && doorstate( var_0.origin, var_2 ) )
            {
            ...
            else if ( !var_0._id_131A6 && doorstate( var_0.origin, var_3 ) )
            {
                showsplashtoteam( var_1, "br_gametype_truckwar_gas_is_approaching_medium" );
                var_0._id_131A6 = 1;
                var_0._id_131A5 = 1;
            }
            else if ( !var_0._id_131A5 && doorstate( var_0.origin, var_4 ) )
            {
        ...
        {
            var_0 thread highlighttoteam();

            if ( level._id_12931[var_1] == 1 )
            {
                level._id_12931[var_1] = 0;
                var_5 = scripts\mp\utility\teams::getteamdata( var_1, "players" );
        ...
        else if ( level._id_12931[var_1] == 0 )
        {
...
_id_13262( var_0, var_1 )
{
    var_0 endon( "death" );
    level endon( "game_ended" );
    var_0.train_stopper = 0;
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_6 ) && gettime() < var_6 || var_5 )
        {
            ...
            foreach ( var_10 in level.outofboundstriggers )
            {
                ...
                {
                    var_0.train_stopper = var_0.train_stopper + level.framedurationseconds;
                    var_7 = 1;
                    var_8 = var_10;
                    break;
                }
            ...
            var_0.train_stopper = clamp( var_0.train_stopper, 0, var_3 );

            if ( var_0.train_stopper == var_3 )
...
tacinsert_deleteontoofar( var_0, var_1 )
{
    var_0 endon( "death" );
    level endon( "game_ended" );
    ...
    while ( isdefined( var_0 ) )
    {
        ...
        var_0 thread can_station_be_selected( var_1 );
        var_5 = scripts\mp\utility\teams::getteamdata( var_1, "players" );

        foreach ( var_7 in var_5 )
        {
            var_7 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_pockets", var_0.health );
            var_7 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_banked", var_0.maxhealth );
        }
...
tacinsert_destroyongameended( var_0, var_1 )
{
    level endon( "game_ended" );
    ...
    foreach ( var_4 in var_2 )
    {
        var_4 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_pockets", 0 );
        var_4 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_banked", 1 );
...
nextscore( var_0, var_1 )
{
    if ( level.mapname != "mp_br_mechanics" )
        return move_door_to_pos( var_0, var_1 );
...
modifyakimboburstrenettidamagehack( var_0, var_1 )
{
    var_2 = [];
    var_3 = 3;
    var_4 = 360 / var_3;
...
requiredplayercountoveride( var_0 )
{
    if ( !isdefined( level._id_12931 ) )
        return 0;

    return scripts\mp\utility\teams::getteamdata( var_0, "aliveCount" ) == 0 && ( !istrue( level._id_12931[var_0] ) || istrue( level._id_12C18 ) );
}
...
playerrespawn( var_0, var_1, var_2 )
{
    ...
    if ( istrue( level._id_12C18 ) || requiredplayercountoveride( self.team ) || scripts\engine\utility::ent_flag_exist( "respawn_vehicle_death" ) && scripts\engine\utility::ent_flag( "respawn_vehicle_death" ) )
    {
    ...
    var_3 = level.brking_managecircles;
    ...
    thread getovertimelength( var_3 );
    var_5 = var_3 - var_4;
    var_6 = var_3 - var_5;
    var_7 = var_3 - getdvarfloat( "scr_bmo_respawn_intermission_time", 5.0 );
    thread iskillstreakvehicle( var_7 );
    ...
    if ( scripts\engine\utility::ent_flag_exist( "respawn_vehicle_death" ) && scripts\engine\utility::ent_flag( "respawn_vehicle_death" ) )
        thread scripts\mp\gametypes\br_spectate::_id_120FA();
    else if ( scripts\engine\utility::ent_flag( "respawn_vehicle_in_gas" ) )
        scripts\mp\utility\lower_message::setlowermessageomnvar( 79 );

    while ( scripts\engine\utility::ent_flag_exist( "respawn_vehicle_death" ) && !scripts\engine\utility::ent_flag( "respawn_vehicle_death" ) && !level._id_12931[self.team] || scripts\cp_mp\utility\player_utility::isusingremote() )
        waitframe();

    if ( scripts\engine\utility::ent_flag_exist( "respawn_vehicle_death" ) && scripts\engine\utility::ent_flag( "respawn_vehicle_death" ) )
        thread scripts\mp\gametypes\br_spectate::_id_120FA();

    if ( requiredplayercountoveride( self.team ) || scripts\engine\utility::ent_flag_exist( "respawn_vehicle_death" ) && scripts\engine\utility::ent_flag( "respawn_vehicle_death" ) )
    {
    ...
    var_9 = getspawnpoint();
    var_10 = scripts\mp\gametypes\br_gulag::waittill_any_2( var_9 );
    var_11 = level._id_12956[self.team].origin;
    ...
    if ( scripts\mp\gametypes\br_public::registerpublicevent() && isdefined( level.vehicle_occupancy_instanceisregistered ) )
        var_9 = scripts\engine\utility::getstruct( level.vehicle_occupancy_instanceisregistered, "targetname" );
    ...
    if ( isdefined( level._id_12956[self.team] ) )
    {
        var_11 = level._id_12956[self.team].origin;
        scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_pockets", level._id_12956[self.team].health );
        scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_banked", level._id_12956[self.team].maxhealth );
    }

    level thread scripts\mp\gametypes\br_quest_util::_id_12E81( var_11, "revive" );
    scripts\mp\trials\trial_utility::_id_120FB( "ui_br_transition_type", 0 );
    _id_12E0F( self.team );
}

getovertimelength( var_0 )
{
    self endon( "disconnect" );
    waitframe();
...
iskillstreakvehicle( var_0 )
{
    ...
    if ( requiredplayercountoveride( self.team ) || !level._id_12931[self.team] )
        return;
    ...
    if ( getdvarint( "scr_bmo_use_spawn_intermission_fix", 1 ) == 1 )
    {
        ...
        self.quickdropremoveplunderfrominventory = var_2.origin;
...
fadeoutin()
{
    ...
    var_0 = scripts\engine\utility::_id_1313D( "spawned_player", "respawn_vehicle_in_gas", "gulag_auto_win" );
...
initspawns()
{
    level._id_1255F = getdvarfloat( "scr_spawn_height", 2500 );
}
...
getspawnpoint( var_0 )
{
    if ( !isdefined( self._id_11B4B ) )
    {
        ...
        return little_bird_mg_cp_onentervehicle( istrue( var_0 ) );
    }
    ...
    else if ( self._id_11B4B.team != self.team || self._id_11B4B.lifeid != self.lifeid || istrue( self._id_11B4B.relicsquadlink ) )
        return little_bird_mg_cp_onentervehicle( istrue( var_0 ) );

    return self._id_11B4B;
}

little_bird_mg_cp_onentervehicle( var_0 )
{
    var_1 = level._id_12956[self.team].origin;
    ...
    var_1 = ( var_1[0], var_1[1], var_3[2] + level._id_1255F );

    if ( istrue( level._id_12CDD ) && getdvarint( "scr_truckwar_respawn_lookahead", 1 ) == 1 )
    {
        if ( istrue( level.ending_zplanes ) )
        {
            ...
            if ( var_7 > var_5 )
            {
                ...
                {
                    var_12 = level.br_level.chopper_sound_fade_and_delete[level.br_circle.circleindex];
    ...
    self._id_11B4B.origin = var_1;
    self._id_11B4B.angles = level._id_12956[self.team].angles;
    self._id_11B4B.time = gettime();
    self._id_11B4B.team = self.team;
    self._id_11B4B.index = -1;
    self._id_11B4B.lifeid = self.lifeid;
    self._id_11B4B.relicsquadlink = var_0;
    return self._id_11B4B;
}
...
_id_13253()
{
    ...
    for (;;)
    {
        ...
        {
            var_1 = 1000;
            radiusdamage( level._id_12956["axis"].origin, 1000, var_1, var_1 );
        }
    }
}
...
animatedprop_setup()
{
    if ( !isdefined( level._id_12C1D ) )
        level._id_12C1D = [];

    level._id_12C1D[level._id_12C1D.size] = level.players[0].origin;

    foreach ( var_1 in level._id_12C1D )
    {
...
playerusehealslot()
{
    ...
    scripts\common\anim::addnotetrack_customfunction( "parachute", "parachute_detach_sfx", ::trial_white_phosphorus, "truck_drop" );
    ...
    scripts\common\anim::addnotetrack_customfunction( "parachute", "parachute_detach_sfx", ::trial_white_phosphorus, "truck_drop_trimmed" );
...
_id_12C09( var_0, var_1, var_2, var_3 )
{
    ...
    while ( gettime() < var_0 )
    {
        ...
        else
        {
            ...
            if ( isdefined( var_2 ) )
            {
                if ( var_2 - var_4 >= 300 )
                {
                    _id_12C0C( self.origin, self.angles );
                    break;
                }
            }
...
helicleanup( var_0 )
{
    thread _id_12C1B( var_0 );

    if ( getdvarint( "scr_truckwar_repair_station", 1 ) == 1 )
        level thread proximity_explode();
    ...
    level thread br_squadrevivestatus();
    ...
    foreach ( var_2 in level.players )
    {
        ...
        var_2 thread train_collision_item_valid();
    }

    level thread _id_1323B();
...
{
    var_0 = spawnstruct();
    var_0.origin = level._id_12957[self.team].origin;
...
train_collision_item_valid()
{
    ...
    var_0 = level._id_12956[self.team];

    if ( !istrue( var_0.patchfix ) )
        level thread _id_12C09( var_0, var_0.origin, var_0.angles, 0 );
    ...
    var_4 = self._id_11B4B.origin + var_1 * var_2 + ( 0, 0, 1 ) * var_3;
    var_5 = vectortoangles( self._id_11B4B.origin - var_4 );
...
proximity_explode()
{
    ...
    if ( getdvarint( "scr_truckwar_repair_disable_fire", 0 ) != 1 )
    {
        ...
        var_2 = modeignorevehicleexplosivedamage();

        foreach ( var_4 in var_2 )
        {
            var_5 = gas_tryplaycoughaudio( var_4, sqrt( var_1 ) );
            var_5._id_11CA9 = var_1;
            var_0[var_0.size] = var_5;
        }
    }

    var_1 = getdvarint( "scr_truckwar_repair_distsq_gas", 90000 );
    var_2 = moderestrictsarenakillstreaks();
    ...
    var_2 = monitoraveragevelocityandupdate();
...
gas_tryplaycoughaudio( var_0, var_1 )
{
    var_2 = undefined;
    ...
    var_2.inuse = 0;
    var_2.race_ui_remove_critical_message = 0;
    return var_2;
}
...
_id_1323B()
{
    ...
    for (;;)
    {
        foreach ( var_3 in level._id_11CAA )
            var_3._id_12E4E = 0;

        foreach ( var_17, var_6 in level._id_12956 )
        {
            if ( !isdefined( var_6 ) )
                continue;

            var_7 = istrue( var_6.phonemorsesinglescriptableused );
            var_8 = 0;
            ...
            foreach ( var_3 in level._id_11CAA )
            {
                ...
                if ( var_10 < var_3._id_11CA9 )
                {
                    var_3 skipinfil();
                    var_3._id_12E4E = 1;
                    var_8 = 1;

                    if ( istrue( var_6.phoneplayring ) )
                        continue;

                    if ( var_6.health == var_6.maxhealth )
                        continue;

                    var_6.phonemorsesinglescriptableused = 1;

                    if ( !var_7 )
                        _id_12E0F( var_17 );

                    var_11 = int( var_0 * var_6.maxhealth );
                    var_6 scripts\cp_mp\vehicles\vehicle_damage::_id_12EF6( var_11 );
                    ...
                    foreach ( var_14 in var_12 )
                    {
                        ...
                        var_14 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_pockets", var_6.health );
                        var_14 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_cash_banked", var_6.maxhealth );
                    }
            ...
            if ( var_7 && !var_8 )
            {
                var_6.phonemorsesinglescriptableused = 0;
                _id_12E0F( var_17 );
            }
        }
...
skipinfil()
{
    self.race_ui_remove_critical_message = self.inuse;
    self.inuse = 1;

    if ( self.inuse != self.race_ui_remove_critical_message )
    {
        if ( getdvarint( "scr_truckwar_repair_station_objective", 1 ) == 1 )
...
_id_12E0F( var_0 )
{
    var_1 = level._id_12956[var_0];
    var_2 = 0;

    if ( isdefined( var_1 ) && !istrue( var_1.isdestroyed ) )
    {
        if ( scripts\cp_mp\vehicles\vehicle_interact::_id_12F62( var_1, "armor" ) )
            var_2 = 1;

        if ( scripts\cp_mp\vehicles\vehicle_interact::_id_12F62( var_1, "uav" ) )
            var_2 = var_2 + 2;

        if ( scripts\cp_mp\vehicles\vehicle_interact::_id_12F62( var_1, "barrel" ) )
            var_2 = var_2 + 4;

        if ( scripts\cp_mp\vehicles\vehicle_interact::_id_12F62( var_1, "trophy" ) )
        {
            var_2 = var_2 + 8;
            var_2 = var_2 + ( var_1._id_12C04 << 4 );
        }
    }

    if ( !isdefined( var_1 ) || istrue( var_1.isdestroyed ) )
        var_2 = var_2 + 64;
    else if ( istrue( var_1.phoneplayring ) )
        var_2 = var_2 + 128;
    else if ( istrue( var_1.phonemorsesinglescriptableused ) )
        var_2 = var_2 + 256;

    var_3 = scripts\mp\utility\teams::getteamdata( var_0, "players" );

    foreach ( var_5 in var_3 )
        var_5 scripts\mp\trials\trial_utility::_id_120FB( "ui_br_team_placement", var_2 );
}

blueprintextract_beforepickupspawned()
{
    var_0 = level._id_12CE0;
    level.br_level.br_circleclosetimes = [ level._id_12CD8 ];
    level.br_level.br_circledelaytimes = [ level._id_12CD9 ];
    level.br_level.chopperexfil_introsound = [ 1 ];
    var_1 = level.br_level.br_circleminimapradii;
    var_2 = scripts\engine\utility::ter_op( isdefined( level._id_12CDF ), level._id_12CDF, level.br_level.br_circleradii );
    ...
    setdvar( "scr_br_moving_circle_count", level._id_12CD3 );
    var_3 = [];
    var_4 = [];
    var_5 = [];
    var_6 = [];
    var_7 = level._id_12CD0;
    var_8 = level._id_12CD5;

    for ( var_9 = 1; var_9 < level._id_12CD3; var_9++ )
    {
        if ( level._id_12CE1 )
        {
        ...
        if ( isdefined( level._id_12CD2 ) )
        {
            var_12 = scripts\engine\utility::ter_op( level._id_12CD2.size <= var_9, level._id_12CD2.size - 1, var_9 );
            var_7 = level._id_12CD2[var_12];
        }
        else
            var_7 = max( level._id_12CCF, var_7 - level._id_12CD1 );

        if ( isdefined( level._id_12CD7 ) )
        {
            var_12 = scripts\engine\utility::ter_op( level._id_12CD7.size <= var_9, level._id_12CD7.size - 1, var_9 );
            var_8 = level._id_12CD7[var_12];
        }
        else
            var_8 = max( level._id_12CD4, var_8 - level._id_12CD6 );

        var_4[var_4.size] = var_7;
        var_5[var_5.size] = var_8;
    }

    scripts\mp\gametypes\br_circle::is_relic_swat_active( var_3, var_4, var_5, var_6 );
}
...
create_struct_at()
{
    ...
    level.br_level.chopper_sound_fade_and_delete[0] = level.br_level.br_mapcenter + ( var_1, var_2, 0 );
    level.br_level.chopper_sound_fade_and_delete[1] = level.br_level.br_mapcenter + ( var_1, var_2, 0 );

    if ( getdvarint( "scr_truckwar_circle_precalc", 1 ) == 1 )
    {
        for ( var_3 = 2; var_3 < level.br_level.chopper_sound_fade_and_delete.size - 1; var_3++ )
            create_temp_infil_structs( var_3 );
    }
    else
    {
        for (;;)
...
create_temp_infil_structs( var_0 )
{
    ...
    if ( level.br_level.chopper_sound_fade_and_delete.size <= var_0 || !istrue( level._id_12CDD ) )
        return;

    var_1 = level.br_level.chopper_sound_fade_and_delete[var_0];
    var_2 = level.br_level.br_circleradii[var_0 - 1] * level._id_12CDC;
    var_3 = scripts\mp\gametypes\br_circle::mp_crash2( var_1, var_2, 0.8, 1.0, 1, 0, 1 );
    var_4 = vectornormalize( var_3 - var_1 ) * var_2;
    var_5 = var_1 + var_4;
    var_6 = getdvarint( "scr_truckwar_circle_padding", 10000 );

    for ( var_7 = vectornormalize( level.br_level.br_mapcenter - var_5 ); !scripts\mp\gametypes\br_circle::reset_vandalize_nodes( var_5, 1 ); var_5 = var_5 + var_7 * var_6 )
    {

    }

    level.br_level.chopper_sound_fade_and_delete[var_0 + 1] = var_5;
}

create_struct( var_0 )
{
    var_1 = level.br_level.br_mapbounds;
    var_2 = level._id_12CDE;
    var_3 = var_0[0] < var_1[0][0] * var_2 && var_0[0] > var_1[1][0] * var_2 && var_0[1] < var_1[0][1] * var_2 && var_0[1] > var_1[1][1] * var_2;
    return var_3;
}

doorstate( var_0, var_1 )
{
    ...
    if ( level.br_circle.circleindex > 0 )
    {
        var_13 = level.br_level.chopper_sound_fade_and_delete[level.br_circle.circleindex];
    ...
    return scripts\mp\vehicles\vehicle_compass_mp::wind_triggers( var_15 );
}

_id_12C1C()
{
    wait 5;
    scripts\mp\utility\sound::announcedomplatespawns( "br_truck_wars" );
}
