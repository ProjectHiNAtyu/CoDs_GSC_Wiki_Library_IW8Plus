...
check_for_solo()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( self.current_bombzone ) && !carriable_throw_watch( self.current_bombzone ) )
        {
            self.current_bombzone = undefined;
        ...
        if ( var_0 )
        {
            checkpoint_objective( "attack" );

            if ( !isdefined( self.current_bombzone ) )
                continue;

            checkshouldplaymusic();
            change_yaw_angle();
...
notify_enemy_team_bomb_used( var_0 )
{
    ...
    var_2 = scripts\mp\bots\bots_gametype_common::little_bird_mg_getspawnstructscallback( var_0 );

    foreach ( var_4 in var_2 )
    {
        if ( isdefined( var_4.current_bombzone ) && var_1 == var_4.current_bombzone )
            var_4 check_for_moody_traversal( "investigate_someone_using_bomb" );
    }
}
...
plant_bomb()
{
    ...
    var_0 = scripts\mp\bots\bots_gametype_common::loadandplayholoeffect( self.current_bombzone, 0 );
    ...
    if ( var_1 == "goal" )
    {
        ...
        var_6 = scripts\mp\bots\bots_gametype_common::carriable_weapon_change_watch( level.planttime + 2, "bomb_planted", var_5 );
...
defuse_bomb()
{
    ...
    var_0 = scripts\mp\bots\bots_gametype_common::load_sequence_4_vfx( self.current_bombzone ).origin;
    ...
    if ( var_1 == "goal" )
    {
        ...
        var_10 = scripts\mp\bots\bots_gametype_common::carriable_weapon_change_watch( level.defusetime + 2, "bomb_defused", var_9 );
...
{
    var_1 = make_intel_model_usable( var_0, self.team );
...
little_bird_mg_enterend()
{
    ...
    foreach ( var_2 in level.objectives )
    {
        if ( carriable_throw_watch( var_2 ) )
...
make_intel_model_usable( var_0, var_1 )
{
    var_2 = [];
...
checkpoint_next_fx_play( var_0, var_1 )
{
    var_2[0] = make_intel_model_usable( var_0[0], game["defenders"] ).size;
    var_2[1] = make_intel_model_usable( var_0[1], game["defenders"] ).size;
...
checkpoint_objective( var_0 )
{
    ...
    if ( var_0 == "attack" )
        var_1 = changecirclestateatlowtime();
...
changesleft()
{
    ...
    else if ( var_0.size == 2 )
    {
        ...
        if ( var_3[0] && var_3[1] || !var_3[0] && !var_3[1] )
        {
            ...
            var_1 = checkpoint_next_fx_play( var_0, var_4 );
...
changecirclestateatlowtime()
{
    ...
    if ( !isdefined( level.current_zone_target ) || !carriable_throw_watch( level.current_zone_target ) || gettime() > level.next_target_switch_time )
    {
        level.next_target_switch_time = gettime() + 1000 * randomintrange( 30, 45 );
        level.current_zone_target = scripts\engine\utility::random( little_bird_mg_enterend() );
    }
    ...
    var_1 = make_all_doors_solid( var_0 );
    self.current_bombzone = undefined;
...
checkshouldplaymusic()
{
    var_0 = make_all_doors_solid( self.current_bombzone );
...
change_yaw_angle()
{
    ...
    {
        var_1 = make_fly_struct( self.current_bombzone );
        ...
        else if ( isai( var_1 ) )
        {
            ...
            {
                var_0 = "atk_bomber";
                var_1 check_for_execution_allows();
            }
        }
    }
    ...
    check_for_moody_traversal( var_0 );
}
...
changepropkey()
{
    ...
    if ( is_bomb_planted_on( self.current_bombzone ) )
    {
        var_1 = make_control_station_interaction( self.current_bombzone );
    ...
    for (;;)
    {
        foreach ( var_1 in level.objectives )
        {
            foreach ( var_3 in level.players )
            {
                ...
                {
                    if ( !carriable_throw_watch( var_1 ) )
                    {
                        if ( var_3.role == "atk_bomber" || var_3.role == "defuser" )
                            var_3 check_for_execution_allows();
...
check_if_player_is_downed()
{
    switch ( level.mapname )
    {
        case "mp_m_speed":
            var_0 = ( 67, 1916, 22 );
            thread respawn_locations( var_0 );
            var_0 = ( 7, 1877, 22 );
            thread respawn_locations( var_0 );
            var_0 = ( -35, 1925, 22 );
            thread respawn_locations( var_0 );
