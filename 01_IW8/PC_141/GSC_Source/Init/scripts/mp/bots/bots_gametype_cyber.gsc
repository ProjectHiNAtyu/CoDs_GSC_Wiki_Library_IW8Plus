...
setup_callbacks()
{
    ...
    level.bot_funcs["emp_picked_up_cancel"] = ::cdlgametuning;
    level.bot_funcs["tactical_revive_override"] = ::cellspawns;
}
...
setup_bot_cyber()
{
    ...
    var_0 = scripts\mp\bots\bots_gametype_common::checkunscoredspawnpoint( [ "_allies", "_axis" ] );
...
bot_cyber_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( level._id_12927 ) )
        {
            if ( !isdefined( level.carepackage_id ) )
            {
                if ( scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "seek_cyber_emp", undefined ) == 0 )
                {
                    var_0 = ::get_friendly_convoy_vehicle;
                    var_1 = spawnstruct();
            ...
            else if ( self.isbombcarrier )
            {
                ...
                var_4 = scripts\mp\bots\bots_gametype_common::loadandplayholoeffect( var_3, 1 );
                ...
                if ( var_5 == "goal" )
                {
                    ...
                    level scripts\engine\utility::_id_13141( level.planttime + 2, "bomb_planted" );
                }
            }
            else if ( level.carepackage_id.team == self.team )
            {
                if ( !scripts\mp\bots\bots_util::bot_is_defending() )
                    scripts\mp\bots\bots_strategy::bot_guard_player( level.carepackage_id, 400 );
            }
            else if ( gettime() > self.next_time_hunt_carrier || sighttracepassed( self.origin + ( 0, 0, 77 ), level.carepackage_id.origin + ( 0, 0, 77 ), 0, self ) )
            {
                self botsetscriptgoal( level.carepackage_id.origin, 16, "hunt" );
        ...
        var_3 = level.objectives[scripts\engine\utility::get_enemy_team( level._id_12927 )];

        if ( self.team == level._id_12927 )
        {
            if ( !randomly_play_tape_reel_anims() )
        ...
        {
            var_6 = scripts\mp\bots\bots_gametype_common::load_sequence_4_vfx( var_3 ).origin;
...
get_friendly_convoy_vehicle( var_0 )
{
...
_id_12D84()
{
    if ( !level.bombplanted )
    {
        if ( isdefined( level._id_12927 ) )
        {
            level._id_12927 = undefined;
            level.carepackage_id = undefined;
        }

        var_0 = level.carepackage_id;
        level.carepackage_id = undefined;

        foreach ( var_2 in level.participants )
        {
            if ( isalive( var_2 ) && var_2.isbombcarrier )
                level.carepackage_id = var_2;
        }

        var_4 = 0;

        if ( !isdefined( var_0 ) && isdefined( level.carepackage_id ) )
        {
            var_4 = 1;

            if ( isai( level.carepackage_id ) )
                level.carepackage_id thread carriable_physics_launch_drop();
        }
        else if ( isdefined( var_0 ) && !isdefined( level.carepackage_id ) )
            var_4 = 1;
    ...
    {
        if ( isdefined( level.bombowner ) && !isdefined( level._id_12927 ) )
        {
            level._id_12927 = level.bombowner.team;
            level.score_event_headshot = gettime();
        }

        if ( !isdefined( level.bomb_defuser ) || !isalive( level.bomb_defuser ) || gettime() > level.score_event_headshot + 1000 )
        {
            var_7 = [];

            foreach ( var_2 in level.participants )
            {
                if ( isalive( var_2 ) && scripts\mp\utility\entity::isaiteamparticipant( var_2 ) && var_2.team != level._id_12927 )
                    var_7[var_7.size] = var_2;
            }

            if ( var_7.size > 0 )
            {
                var_10 = level.objectives[scripts\engine\utility::get_enemy_team( level._id_12927 )];
...
cellspawns()
{
    ...
    if ( istrue( self.register_outer_room_spawners ) )
        return 0;
