...
checkamorused( var_0, var_1, var_2 )
{
    var_3 = checkbluemassacre( "attacker", var_0 );

    foreach ( var_5 in level.players )
    {
        if ( !isai( var_5 ) && isdefined( var_5.team ) && var_5.team == var_0 )
        {
            if ( var_5 checkcarpuncherprogressgeneric() || distancesquared( var_1, var_5.origin ) > squared( var_2 ) )
...
checkandreportchallengetimer( var_0, var_1, var_2 )
{
    var_3 = checkbluemassacre( "defender", var_0 );

    foreach ( var_5 in level.players )
    {
        if ( !isai( var_5 ) && isdefined( var_5.team ) && var_5.team == var_0 )
        {
            if ( var_5 checkcarryobjectpickupcheck() || distancesquared( var_1, var_5.origin ) <= squared( var_2 ) )
                var_3 = scripts\engine\utility::array_add( var_3, var_5 );
        }
    }
...
{
    if ( isdefined( level.checkcarpuncherprogress ) )
        return self [[ level.checkcarpuncherprogress ]]();
...
{
    if ( isdefined( level.checkcodeentered ) )
        return self [[ level.checkcodeentered ]]();
...
{
    var_0 = [[ level.check_lbravo_hover_retreat ]]( self.team );
    var_1 = [[ level.check_maze_ai_state ]]( self.team );
    var_2 = [[ level.check_trigger_spawnflags ]]( self.team );
    var_3 = [[ level.checkammopickup ]]( self.team );
    ...
    if ( var_4 == "active" )
    {
        if ( var_0.size >= var_2 )
        {
            ...
            {
                if ( isai( var_7 ) && level.bot_personality_type[var_7.personality] == "stationary" && var_7 change_goal_radius_weapons_free() )
                {
            ...
            if ( var_5 )
            {
                checkfinalkillachievements( "attacker" );
                return;
            }

            checkfinalkillachievements( "defender" );
            return;
        }
        else
            checkfinalkillachievements( "attacker" );
    ...
    else if ( var_4 == "stationary" )
    {
        if ( var_1.size >= var_3 )
        {
            ...
            {
                if ( isai( var_10 ) && level.bot_personality_type[var_10.personality] == "active" && var_10 change_fronttruck_label() )
                {
...
check_player_used_tacmap()
{
    ...
    for (;;)
    {
        if ( gettime() > var_1 )
        {
            ...
            {
                var_4 = [[ level.check_lbravo_hover_retreat ]]( var_3 );
                var_5 = [[ level.check_maze_ai_state ]]( var_3 );
                var_6 = [[ level.check_trigger_spawnflags ]]( var_3 );
                var_7 = [[ level.checkammopickup ]]( var_3 );
                ...
                if ( var_4.size > var_6 )
                {
                    ...
                    {
                        if ( isai( var_11 ) && var_11 change_goal_radius_weapons_free() )
                        {
                            if ( level.bot_personality_type[var_11.personality] == "stationary" )
                            {
                                var_11 checkfinalkillachievements( "defender" );
                                var_9 = 1;
                                break;
                            }
                            else
                    ...
                    if ( !var_9 && var_8.size > 0 )
                        scripts\engine\utility::random( var_8 ) checkfinalkillachievements( "defender" );
                }
                ...
                if ( var_5.size > var_7 )
                {
                    ...
                    {
                        if ( isai( var_16 ) && var_16 change_fronttruck_label() )
                        {
                            if ( level.bot_personality_type[var_16.personality] == "active" )
                            {
                                var_16 checkfinalkillachievements( "attacker" );
                                var_14 = 1;
                                break;
                            }
                            else
                    ...
                    if ( !var_14 && var_13.size > 0 )
                        scripts\engine\utility::random( var_13 ) checkfinalkillachievements( "attacker" );
                }
            }
        }
...
{
    if ( isdefined( level.check_missile_fire_vo ) )
        return self [[ level.check_missile_fire_vo ]]();

    return 1;
}

change_goal_radius_weapons_free()
{
    if ( isdefined( level.check_missiles_reloaded_vo ) )
        return self [[ level.check_missiles_reloaded_vo ]]();
...
change_apc_label( var_0 )
{
    ...
    foreach ( var_5 in var_0 )
    {
        ...
        var_5._id_1344D = defaultclassindex( var_5 );
        var_5.dmzlootleaderupdateonpickup = _id_13451( var_5, 0, 0 );
        var_7 = [ ( 0, 0, 0 ), ( 1, 1, 0 ), ( 1, -1, 0 ), ( -1, 1, 0 ), ( -1, -1, 0 ) ];

        foreach ( var_9 in var_7 )
        {
            var_10 = _id_13451( var_5, var_9[0], var_9[1] );
...
defaultclassindex( var_0 )
{
    var_1 = spawnstruct();
    var_1.spawn_in_fake_skyhookmodels = ( 999999, 999999, 999999 );
    var_1.sniper_laser_vfx = ( -999999, -999999, -999999 );

    foreach ( var_3 in var_0.nodes )
    {
        var_1.spawn_in_fake_skyhookmodels = ( min( var_3.origin[0], var_1.spawn_in_fake_skyhookmodels[0] ), min( var_3.origin[1], var_1.spawn_in_fake_skyhookmodels[1] ), min( var_3.origin[2], var_1.spawn_in_fake_skyhookmodels[2] ) );
        var_1.sniper_laser_vfx = ( max( var_3.origin[0], var_1.sniper_laser_vfx[0] ), max( var_3.origin[1], var_1.sniper_laser_vfx[1] ), max( var_3.origin[2], var_1.sniper_laser_vfx[2] ) );
    }

    var_1.center = ( ( var_1.spawn_in_fake_skyhookmodels[0] + var_1.sniper_laser_vfx[0] ) / 2, ( var_1.spawn_in_fake_skyhookmodels[1] + var_1.sniper_laser_vfx[1] ) / 2, ( var_1.spawn_in_fake_skyhookmodels[2] + var_1.sniper_laser_vfx[2] ) / 2 );
    var_1.open_exit_doors = ( var_1.sniper_laser_vfx[0] - var_1.center[0], var_1.sniper_laser_vfx[1] - var_1.center[1], var_1.sniper_laser_vfx[2] - var_1.center[2] );
    var_1.radius = max( var_1.open_exit_doors[0], var_1.open_exit_doors[1] );
    return var_1;
}

_id_13451( var_0, var_1, var_2 )
{
    var_3 = ( var_0._id_1344D.center[0] + var_1 * var_0._id_1344D.open_exit_doors[0], var_0._id_1344D.center[1] + var_2 * var_0._id_1344D.open_exit_doors[1], 0 );
    var_4 = undefined;
    var_5 = 9999999;
...
loadandplayholoeffect( var_0, var_1 )
{
...
carriable_weapon_change_watch( var_0, var_1, var_2, var_3 )
{
    var_4 = 0;
    ...
    {
        childthread tac_rover_trail();
        childthread tac_rover_initomnvars();
    }
