...
setup_callbacks()
{
    ...
    {
        level.bot_funcs["dropped_weapon_think"] = ::bot_hp_allow_predictive_capping;
        level.bot_funcs["dropped_weapon_cancel"] = ::bot_is_capturing_hq_zone;
    }

    level.bot_funcs["gametype_think"] = ::challenges_init;
}
...
challenges_init()
{
    ...
    if ( !scripts\mp\gametypes\arena::isnormalloadouts() )
    {
        if ( self botgetdifficultysetting( "advancedPersonality" ) && self botgetdifficultysetting( "strategyLevel" ) > 0 )
            scripts\mp\bots\bots_gametype_gun::checklinktotraininternal( self getcurrentweapon().basename, self.pers["bot_original_personality"] );
    }
    ...
    for (;;)
    {
        if ( isdefined( level.arenaflag ) && level.arenaflag.visibleteam == "any" )
        {
            ...
            if ( !scripts\mp\bots\bots_util::bot_is_capturing() || !self.old_accuracy )
...
bot_hp_allow_predictive_capping()
{
    ...
    for (;;)
    {
        ...
        if ( bot_gun_pick_personality_from_weapon() || scripts\mp\bots\bots_util::checkforovertime( 0.33 ) )
        {
            if ( self [[ level.bot_funcs["should_pickup_weapons"] ]]() && !scripts\mp\bots\bots_util::bot_is_remote_or_linked() )
            {
                ...
                {
                    var_4 = var_3[0];
                    level.hvt_key_picked_up = var_4;
                    bot_has_streak_in_crate( var_4 );
                }
            }
        }
            ...
            if ( var_6 )
            {
                ...
                foreach ( var_10 in var_8 )
                {
                    ...
                    if ( isdefined( var_10.closest_point_on_grid ) )
                    {
                        ...
                        bot_has_streak_in_crate( var_10 );
                    }
                }
            }
            else if ( var_5 )
...
bot_has_streak_in_crate( var_0 )
{
    if ( scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "seek_dropped_weapon", var_0 ) == 0 )
    {
        ...
        if ( isdefined( var_0.checkpoint_objective_id ) )
            var_13 = var_0.checkpoint_objective_id;

        scripts\mp\bots\bots_strategy::bot_new_tactical_goal( "seek_dropped_weapon", var_13, 100, var_11 );
    }
}

bot_has_player_enemy( var_0 )
{
