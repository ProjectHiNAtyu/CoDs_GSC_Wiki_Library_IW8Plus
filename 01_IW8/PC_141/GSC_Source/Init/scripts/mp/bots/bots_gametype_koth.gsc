...
setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::checkpoint_create_carepackage_munitions;
    level.bot_funcs["should_start_cautious_approach"] = ::_id_12268;
}
...
setup_bot_koth()
{
    ...
    if ( !var_0 )
    {
        ...
        level.checkforactiveobjicon[var_5.trigger getentitynumber()] = 1;
        level.bot_gametype_precaching_done = 1;
        thread challengetrackerforjuggkills( var_5 );
    }
}

challengetrackerforjuggkills( var_0 )
{
    for ( var_1 = scripts\engine\utility::array_remove( level.objectives, var_0 ); var_1.size > 0; var_1 = scripts\engine\utility::array_remove( var_1, var_2 ) )
    {
        ...
        level.checkforactiveobjicon[var_2.trigger getentitynumber()] = 1;
    }
}

checkpoint_create_carepackage_munitions()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( level.zone ) || !isdefined( level.checkforactiveobjicon[level.zone.trigger getentitynumber()] ) )
        {
        ...
        if ( !istrue( level.zone.trigger.trigger_off ) && isdefined( level.zoneendtime ) && !isdefined( var_0 ) && !level.zonerandomlocationorder && level.checkpoint_add_carepackage )
        {
            ...
            if ( var_2 > 0 && var_2 < 10000 )
            {
                ...
                if ( !var_3 )
                {
                    var_4 = level.zone._id_1344D.radius * 6;

                    if ( var_2 < 5000 )
                        var_4 = level.zone._id_1344D.radius * 3;

                    var_5 = distance( level.zone._id_1344D.center, self.origin );

                    if ( var_5 > var_4 )
                        var_0 = checkpoints_count();
                }
                else
                {
                    ...
                    var_8 = checkforsubgametypeoverrides( level.zone );
        ...
        if ( !checkpoint_carepackage_munitions_role_think( var_9 ) )
            change_stealth_state_to_combat_warpper( var_9 );
    }
}
...
checkforsubgametypeoverrides( var_0 )
{
    return checkifvalidpropspectate( var_0 ).size;
}
...
checkifvalidpropspectate( var_0 )
{
    ...
    foreach ( var_3 in level.participants )
    {
        if ( var_3 != self && scripts\mp\utility\entity::isteamparticipant( var_3 ) && istestclient( self, var_3 ) )
        {
            ...
            {
                if ( !isai( var_3 ) || var_3 checkpoint_carepackage_munitions_role_think( var_0 ) )
...
checkpoint_carepackage_munitions_role_think( var_0 )
{
    ...
    return self.get_closest_enemy_near_turret == var_0;
}

change_stealth_state_to_combat_warpper( var_0 )
{
