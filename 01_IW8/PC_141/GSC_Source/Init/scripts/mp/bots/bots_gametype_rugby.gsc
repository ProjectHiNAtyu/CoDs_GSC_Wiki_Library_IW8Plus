...
setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::checkpoint_player_spawns;
    level.bot_funcs["crate_can_use"] = ::crate_can_use;
    level.bot_funcs["know_enemies_on_start"] = undefined;
    level.bot_funcs["jugg_picked_up_cancel"] = ::celebration_end;
}
...
_id_1218E()
{
    ...
    var_0 = checkuseconditioninthink( [ "_allies", "_axis" ] );
...
checkpoint_player_spawns()
{
    ...
    for (;;)
    {
        ...
        if ( self == level._id_11E23 )
        {
            ...
            var_4 = logevent_spawnviaautorevive( var_3.trigger );
            self botsetscriptgoal( var_4.origin, 0, "critical", var_4.angles[1] );
            var_5 = scripts\mp\bots\bots_util::bot_waittill_goal_or_fail();
            continue;
        }

        if ( level._id_11E23.team == self.team )
        {
            if ( !scripts\mp\bots\bots_util::bot_is_defending() )
                scripts\mp\bots\bots_strategy::bot_guard_player( level._id_11E23, 400 );

            continue;
        }

        if ( gettime() > self.next_time_hunt_carrier || sighttracepassed( self.origin + ( 0, 0, 77 ), level._id_11E23.origin + ( 0, 0, 77 ), 0, self ) )
        {
            self botsetscriptgoal( level._id_11E23.origin, 16, "hunt" );
            self.next_time_hunt_carrier = gettime() + randomintrange( 4500, 5500 );
        }
    }
}

logevent_spawnviaautorevive( var_0 )
{
...
_id_11E1E( var_0 )
{
    var_1 = rifle_lights( scripts\mp\gametypes\rugby::module_set_skip_basic_combat() + 2, "jugg_captured", randomint( 100 ) > 50 );
...
rifle_lights( var_0, var_1, var_2, var_3 )
{
    var_4 = 0;
...
{
    wait 1.0;
    checkpoint_release_spawnpoint( level.rugby.endzones["allies"][0].trigger );
    checkpoint_release_spawnpoint( level.rugby.endzones["axis"][0].trigger );
    level.bot_set_objective_bottargets = 1;
}

checkpoint_release_spawnpoint( var_0 )
{
