...
main()
{
    setup_callbacks();
    _id_1218C();
    thread spotlight_tag();
}

setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::checkreload;
}
...
_id_1218C()
{
    ...
    thread check_if_players_are_not_in_plane();
    level.bot_gametype_precaching_done = 1;
}

checkifteamrewardsareenabled()
{
    return level.getglobalbattlepassxpmultiplier;
}

checkforlaststandfinish()
{
    if ( isdefined( level.getglobalbattlepassxpmultiplier.carrier ) )
        return level.getglobalbattlepassxpmultiplier.carrier;
...
checkreload()
{
    ...
    for (;;)
    {
        ...
        if ( self.role == "carrier" )
        {
            ...
            {
                var_3 = checkifteamrewardsareenabled();

                if ( !isdefined( var_3 ) )
                {
                    var_4 = checkforlaststandfinish();
        ...
        {
            var_6 = level.getglobalbattlepassxpmultiplier.carrier;
            var_7 = var_6.origin;
...
propclonepower()
{
    ...
    var_2 = juggernaut_kills_tracker( self.team );
    var_3 = juggernaut_logic( self.team );
    ...
    if ( var_4 == "active" )
    {
        if ( var_0.size >= var_2 )
        {
            ...
            if ( var_5 )
            {
                juggernaut_state( "attacker" );
                return;
            }

            juggernaut_state( "defender" );
            return;
        }
        else
            juggernaut_state( "attacker" );
...
check_if_players_are_not_in_plane()
{
    ...
    for (;;)
    {
        ...
        {
            var_4 = juggernaut_kills_tracker( var_3 );
            var_5 = juggernaut_logic( var_3 );
            ...
            if ( var_6.size > var_4 )
            {
                ...
                foreach ( var_11 in var_6 )
                {
                    if ( isai( var_11 ) )
                    {
                        ...
                        {
                            var_11 juggernaut_state( "defender" );
                            var_9 = 1;
                            break;
                        }
                        else
                ...
                if ( !var_9 && var_8.size > 0 )
                    scripts\engine\utility::random( var_8 ) juggernaut_state( "defender" );
            }
            ...
            if ( var_7.size > var_5 )
            {
                ...
                foreach ( var_16 in var_7 )
                {
                    if ( isai( var_16 ) )
                    {
                        ...
                        {
                            var_16 juggernaut_state( "attacker" );
                            var_14 = 1;
                            break;
                        }
                        else
                ...
                if ( !var_14 && var_13.size > 0 )
                    scripts\engine\utility::random( var_13 ) juggernaut_state( "attacker" );
            }

            var_18 = checkifteamrewardsareenabled();

            if ( isdefined( var_18 ) )
            {
                var_19 = update_bomb_vest_lua( var_3, var_18 );
                ...
                if ( isdefined( var_19 ) && isdefined( var_19.role ) && var_19.role != "carrier" )
                {
                    if ( !isdefined( var_19.carryobject ) )
                    {
                        ...
                        if ( isdefined( var_20 ) )
                            var_20 juggernaut_state( undefined );

                        var_19 juggernaut_state( "carrier" );
...
juggernaut_kills_tracker( var_0 )
{
    var_1 = juggernaut_pincer( var_0 );
    return var_1;
}
