...
setup_callbacks()
{
    ...
    level.bot_funcs["get_watch_node_chance"] = ::check_cannot_spawn_tank;
}

setup_bot_ctf()
{
    level.check_trigger_spawnflags = ::get_ai_hearing_bomb_plant_sound;
    level.checkammopickup = ::get_ai_spawner;
    level.check_lbravo_hover_retreat = ::get_allied_attackers_for_team;
    level.check_maze_ai_state = ::get_allied_defenders_for_team;
    level.check_missiles_reloaded_vo = ::chase;
    level.checkcarpuncherprogress = ::check_crate_unreachable;
...
bot_ctf_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( self.role ) )
            scripts\mp\bots\bots_gametype_common::checkdamagesourcerelicswat();

        var_2 = scripts\mp\utility\game::getotherteam( self.team )[0];

        if ( chassis( self.team ) )
        {
            if ( isdefined( level.checkpoint_create_carepackage ) && isdefined( level.checkpoint_create_carepackage[var_2] ) )
                level.checkpoint_create_carepackage[var_2] = undefined;
        }
        ...
        if ( check_digit_models_to_create() )
        {
            ...
            else if ( scripts\mp\bots\bots_gametype_common::checkarmorpickup( self.team ) == 1 )
...
check_dropped_locations_and_offset()
{
    ...
    else if ( self botcanseeentity( var_2 ) )
    {
        ...
        if ( self botgetdifficultysetting( "strategyLevel" ) > 0 && !chassis( self.team ) )
        {
            if ( !isdefined( level.checkpoint_create_carepackage ) )
                level.checkpoint_create_carepackage = [];

            if ( !isdefined( level.checkpoint_create_carepackage[var_2.team] ) )
                level.checkpoint_create_carepackage[var_2.team] = [];

            level.checkpoint_create_carepackage[var_2.team]["origin"] = var_0;
            level.checkpoint_create_carepackage[var_2.team]["time"] = gettime();
        }
    }
    ...
    else
    {
        ...
        {
            if ( isdefined( level.checkpoint_create_carepackage ) && isdefined( level.checkpoint_create_carepackage[var_2.team] ) )
            {
                var_4 = gettime() - level.checkpoint_create_carepackage[var_2.team]["time"];

                if ( var_4 < 10000 )
                    var_3 = level.checkpoint_create_carepackage[var_2.team]["origin"];
            }
        }
...
{
    var_1 = scripts\mp\bots\bots_gametype_common::checkarmorpickup( var_0 );
...
get_ai_spawner( var_0 )
{
    var_1 = scripts\mp\bots\bots_gametype_common::checkarmorpickup( var_0 );
    return var_1 - get_ai_hearing_bomb_plant_sound( var_0 );
}

get_allied_attackers_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common::checkamorused( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}

get_allied_defenders_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common::checkandreportchallengetimer( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}
...
changing_loadout()
{
    ...
    level endon( "game_ended" );
    thread scripts\mp\bots\bots_gametype_common::check_player_used_tacmap();
}
...
{
    return check_bonus_leads_xp( self.team );
}

chase_hvt_vo()
{
    var_0 = scripts\mp\utility\game::getotherteam( self.team )[0];
    return check_bonus_leads_xp( var_0 );
}
