...
init()
{
    ...
    if ( !isdefined( level.end_wave_spawn_ahead_of_completion ) )
        level.end_wave_spawn_ahead_of_completion = ::choppersupport_movetolocation;
    ...
    level.heli_structs_entrances = scripts\cp_mp\utility\game_utility::molotov_get_level_data( "ks_heli_entrance" );
    level.heli_structs_goals = scripts\cp_mp\utility\game_utility::molotov_get_level_data( "ks_heli_goal" );
    level.heli_structs_paths = scripts\cp_mp\utility\game_utility::molotov_get_level_data( "ks_heli_path" );
...
spawnchopper( var_0, var_1 )
{
    ...
    var_22.focus_fire_outline_enabled = var_2[2] - 750;
    var_22._id_1264C = 50;
    var_22._id_1264D = 25;
    var_22._id_1264E = undefined;
    var_22.spawn_soldiers_in_convoy_truck = 7;
    var_22.spawn_spawners_multi = 12;
...
choppersupport_monitorowner()
{
    ...
    self.owner scripts\engine\utility::_id_1312D( "joined_team", "disconnect" );
    thread choppersupport_leave();
}
...
choppersupport_engageturrettarget( var_0 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0.turreton ) || istrue( var_0._id_12C98 ) )
        {
            waitframe();
            continue;
        }
        ...
        if ( isdefined( var_1 ) && var_1.size > 0 )
        {
            ...
            {
                if ( istrue( self.brmini_playerwelcomesplashes ) && isdefined( self.get_focus_fire_damage_multiplier ) )
                    self thread [[ self.get_focus_fire_damage_multiplier ]]();
                else if ( isdefined( self.try_to_play_custom_death_animation ) )
                    self thread [[ self.try_to_play_custom_death_animation ]]();
                else
...
choppersupport_acquireturrettarget( var_0, var_1 )
{
    ...
    if ( isdefined( var_4 ) )
    {
        ...
        if ( istrue( var_5 ) && self.currenttarget == var_4 && !istrue( self.brmini_playerwelcomesplashes ) )
            self thread [[ level.end_wave_spawn_ahead_of_completion ]]( var_4, 1 );
...
choppersupport_clearcurrenttarget( var_0 )
{
    ...
    if ( istrue( var_0._id_12908 ) )
        var_0._id_12908 = undefined;
...
choppersupport_fireonturrettarget( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    thread end_trans_1_obj( var_0 );
    thread choppersupport_watchtargetlos( var_0, var_2 );
    thread choppersupport_watchtargettimeout( var_0 );
    thread end_swivelroom_obj( var_0 );
    var_5 = weaponfiretime( "chopper_support_turret_mp" );
    var_6 = 0;
    var_7 = 100;

    if ( isdefined( self._id_1264C ) )
        var_7 = self._id_1264C;

    var_8 = 20;

    if ( isdefined( self.spawn_soldiers_in_convoy_truck ) )
        var_8 = self.spawn_soldiers_in_convoy_truck;

    var_9 = 40;

    if ( isdefined( self.spawn_spawners_multi ) )
        var_9 = self.spawn_spawners_multi;

    if ( istrue( var_4 ) )
    {
        var_10 = 3750;

        while ( istrue( end_silo_elevator( var_0, var_1 ) ) )
        {
    ...
    thread end_unlock_silo( var_0 );

    while ( istrue( end_silo_elevator( var_0, var_1 ) ) )
    {
        if ( istrue( var_0._id_12908 ) )
        {
            if ( isdefined( self.currenttarget ) && var_1 == self.currenttarget && !istrue( self.brmini_playerwelcomesplashes ) )
                self [[ level.end_wave_spawn_ahead_of_completion ]]( self.currenttarget, 1 );
            else
        ...
        else if ( var_0 choppersupport_turretlookingattarget() )
        {
            ...
            if ( var_6 == var_8 )
            {
                ...
                if ( isdefined( self._id_1264D ) )
                    var_7 = self._id_1264D;
            }
            else if ( var_6 == var_9 )
            {
                var_7 = undefined;

                if ( isdefined( self._id_1264E ) )
                    var_7 = self._id_1264E;
            }
...
end_trans_1_obj( var_0 )
{
    ...
    if ( !scripts\cp_mp\utility\game_utility::relic_mythic_do_pain() )
        return;
    ...
    for (;;)
    {
        if ( isdefined( var_0.attackingtarget ) )
        {
            ...
            {
                if ( !istrue( self.player_get_sniper_weapon_object ) )
                    self.player_get_sniper_weapon_object = 1;
...
choppersupport_watchtargetlos( var_0, var_1 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0._id_12908 ) && isdefined( var_0.attackingtarget ) )
        {
...
choppersupport_patrolfield( var_0 )
{
    ...
    if ( isdefined( self.try_to_play_custom_death_animation ) )
        self [[ self.try_to_play_custom_death_animation ]]( var_0 );
    else
    {
        if ( self.currentaction != "patrol" )
        ...
        for (;;)
        {
            ...
            if ( isdefined( var_3 ) && !istrue( self.brmini_playerwelcomesplashes ) )
                self [[ level.end_wave_spawn_ahead_of_completion ]]( var_3, 1 );
            else
            {
                var_5 = [];
                var_6 = ( 0, 0, 0 );
                ...
                if ( isdefined( var_6 ) && var_5.size > 0 && !istrue( self.brmini_playerwelcomesplashes ) )
                {
                    var_11 = var_6 / var_5.size;
                    self [[ level.end_wave_spawn_ahead_of_completion ]]( var_11 );
                }
            }
...
choppersupport_movetolocation( var_0, var_1 )
{
    ...
    for (;;)
    {
        ...
        scripts\engine\utility::_id_1312D( "near_goal", "begin_evasive_maneuvers" );
...
chopper_kill_vehicle( var_0, var_1 )
{
    self endon( "death" );
    self endon( "leaving" );
    self endon( "crashing" );
    ...
    for (;;)
    {
        ...
        if ( istrue( self.brmini_playerwelcomesplashes ) && isdefined( self.get_farthest_living_player_not_in_laststand ) )
            break;
        else
        {
            var_7 = 20;
...
choppersupport_cleanup( var_0 )
{
    ...
    if ( isdefined( self.entisvehicle ) )
        self [[ self.entisvehicle ]]();

    if ( isdefined( self.streakinfo ) )
        self.streakinfo.is_in_gas = istrue( var_0 );
    ...
    for (;;)
    {
        if ( istrue( self.player_get_sniper_weapon_object ) )
        {
            ...
            for (;;)
            {
                ...
                {
                    self.player_get_sniper_weapon_object = undefined;
...
choppersupport_gettargets( var_0, var_1, var_2, var_3 )
{
    ...
    for ( var_7 = 0; var_7 < var_5.size; var_7++ )
    {
        ...
        if ( istrue( var_10 ) )
        {
            var_13 = spawnstruct();
            var_13.player = var_8;
            var_13._id_11F8B = var_11;
            var_13._id_12921 = var_12;
...
choppersupport_istarget( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.required_tank_capacity ) && ![[ self.required_tank_capacity ]]( var_1 ) )
        return [ 0, 0, 0 ];

    if ( !end_silo_jump( var_1 ) )
        return [ 0, 0, 0 ];
...
choppersupport_getbesttarget( var_0, var_1 )
{
    ...
    {
        if ( !end_silo_jump( var_7.player ) )
            continue;

        if ( choppersupport_isactivetarget( var_7.player ) && !istrue( var_7._id_12921 ) )
            continue;
        ...
        if ( istrue( var_7._id_11F8B ) )
        {
            var_8 = 1;
            var_10 = var_10 + 40;
        }

        if ( istrue( var_7._id_12921 ) )
        {
            var_9 = 1;
            var_10 = var_10 + 20;
        }
...
choppersupport_explode()
{
    ...
    if ( isdefined( self.get_evade_start_structs_in_front ) )
        [[ self.get_evade_start_structs_in_front ]]();
...
choppersupport_crash( var_0 )
{
    ...
    if ( istrue( self.brmini_playerwelcomesplashes ) && isdefined( self.get_drop_location ) )
    {
        self setneargoalnotifydist( 300 );
        var_1 = 1;
        var_2 = [[ self.get_drop_location ]]();
    }
    else
...
{
    var_3 = self.origin;
    var_4 = self.focus_fire_outline_enabled;
    var_5 = undefined;
...
beginevasivemaneuvers()
{
    ...
    var_0 = scripts\engine\utility::_id_13141( 3.0, "death" );
...
choppersupport_handledeathdamage( var_0 )
{
    ...
    if ( istrue( self.removeselfrevivetoken ) && isdefined( level._id_1239C ) )
    {
        if ( isdefined( var_0.objweapon ) )
            [[ level._id_1239C._id_123AF ]]( var_0.objweapon.basename );
        else
            [[ level._id_1239C._id_123AF ]]( "none" );
    }
