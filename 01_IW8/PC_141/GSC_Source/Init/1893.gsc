...
timeuntilspawn( var_0 )
{
    ...
    if ( !isdefined( self.prophaschangesleft ) )
        self.prophaschangesleft = var_1;
...
mayspawn()
{
    ...
    if ( isdefined( level.handleheadshotkillrewardbullets ) && [[ level.handleheadshotkillrewardbullets ]]( self ) )
        return 0;
...
spawnclient( var_0 )
{
    ...
    if ( isdefined( level.spawner_poison_structs ) )
    {
        self [[ level.spawner_poison_structs ]]( var_0 );
        return;
    }
...
waitandspawnclient( var_0 )
{
    ...
    if ( isdefined( level._id_11957 ) )
        [[ level._id_11957 ]]( var_0 );
    ...
    if ( isdefined( self.setspawnpoint ) && !tivalidationcheck() )
        scripts\mp\equipment\tac_insert::_id_1257C( 1 );

    var_3 = timeuntilspawn( 0 );
    var_4 = 0.0;

    if ( !istrue( level.handlefocusfire ) )
    ...
    {
        if ( isdefined( self.prophaschangesleft ) )
            var_3 = max( self.prophaschangesleft - self.killcamwatchtime, 0 );
        else
    ...
    if ( !istrue( level.handlefocusfire ) && var_9 && !istrue( self.skipspawncamera ) && !istrue( level.usespawnselection ) && var_3 > 0 )
    ...
    else if ( var_3 > 0 )
    {
        ...
        var_1 = 1;
        scripts\engine\utility::_id_13147( var_3, "force_spawn" );
    ...
    if ( !istrue( level.handlefocusfire ) || scripts\mp\utility\game::getgametype() == "arm" )
...
tivalidationcheck()
{
    ...
    if ( isdefined( level.destroyscoreevent ) && ![[ level.destroyscoreevent ]]( self ) )
        return 0;
    ...
    foreach ( var_2 in var_0 )
    {
        ...
        scripts\mp\equipment\tac_insert::_id_1257C();
        return 0;
    }
...
revivespawnvalidationcheck()
{
    ...
    var_0 = spawnstruct();
    var_0._id_12585 = self.forcespawnorigin;
    var_0.reset_wave_loadout = 1;
    var_1 = [];
    var_1[0] = self;
...
getspawnpoint()
{
    ...
    else if ( isdefined( self.forcespawnorigin ) )
    {
        ...
        if ( !var_4.reset_wave_loadout && isdefined( self.prevrevivepos ) && !isdefined( self.rallypoint ) )
        ...
        {
            if ( positionwouldtelefrag( var_1 ) || !var_4.reset_wave_loadout )
            {
    ...
    else if ( isdefined( self.setspawnpoint ) )
    {
        ...
        {
            if ( istrue( level.relic_amped_wait_till_revived ) && level.mapname != "mp_aniyah" )
            {
                self.startingteamcount = undefined;
                var_5 = scripts\engine\utility::ter_op( istrue( level.blueprintextract_trygetreward ), 1, istrue( self.setspawnpoint.issuper ) );

                if ( var_5 )
                {
                    var_6 = min( level.playerskiplootpickup - self.setspawnpoint.playerspawnpos[2], level.playerskiplootpickup );

                    if ( var_6 < level.playerskiplootpickup - 950 )
                        var_6 = var_6 + 950;

                    self.setspawnpoint.playerspawnpos = self.setspawnpoint.playerspawnpos + ( 0, 0, var_6 );
                    self.relic_steelballs_slide = 1;
                }
            }
...
spawnplayer( var_0, var_1 )
{
    ...
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::namehud() != "dmz" && scripts\mp\utility\game::namehud() != "risk" && scripts\mp\utility\game::namehud() != "sandbox" && scripts\mp\utility\game::namehud() != "rumble" )
        {
            ...
            scripts\mp\utility\script::getweaponrandomvariantid( "spawnPlayer()" + self.name + " ui_options_menu = 2, hasInfil = " + var_2 + " infil_started = " + var_3 + "bypassClassChoiceFunc = " + var_4 );
        }
    ...
    else
    {
        ...
        scripts\mp\class::setarenaomnvarhealthtype( self.pers["copiedClass"], var_7 );

        if ( isdefined( self.pers["lastKiller"] ) )
        {
            setup_techo_lmgs( self.pers["lastKiller"] );
    ...
    if ( !var_0 )
    {
        ...
        if ( istrue( self.lastdirty ) )
        {
            self.lastdirty = undefined;
        ...
        self.prophaschangesleft = undefined;
    ...
    if ( !var_0 )
    {
        ...
        if ( !istrue( self.gulag ) && !istrue( self.respawningfromtoken ) && !istrue( self._id_11D0F ) )
            addtoalivecount();
    ...
    scripts\mp\utility\player::_id_11965( "playerlogic::spawnPlayer() !!!CODE SPAWN!!! @" + self.spawndata.spawnorigin );

    if ( !isdefined( level.scriptablenousestate ) )
        level.scriptablenousestate = [];

    level.scriptablenousestate[self.team] = self.spawndata.spawnorigin;
    ...
    scripts\mp\utility\game::deregistergasmaskscriptableatframeend( self );
    [[ level.onspawnplayer ]]();
    ...
    if ( !var_0 )
    {
        self.matchdatalifeindex = scripts\common\utility::_id_12C2C( level.smoke_emitter );
    ...
    if ( isdefined( self.selectedspawnarea ) )
        scripts\mp\analyticslog::setup_next_advance( self, self.selectedspawnarea );
    ...
    if ( !istrue( level.handlefocusfire ) && isdefined( self.spawncameraent ) || istrue( level.handlefocusfire ) && isdefined( self.spawncameraent ) && scripts\mp\utility\game::getgametype() == "arm" )
    {
...
logstartingloadout()
{
    ...
    if ( isdefined( self.curclass ) && self.curclass != "juggernaut" )
        scripts\mp\class::loadout_lognewlygivenloadout( self.obj_running_exfil_wave, self.classstruct, self.curclass );
}
...
spawnspectatormapcam( var_0 )
{
    ...
    scripts\mp\utility\player::_id_11965( "playerlogic::spawnSpectatorMapCam() !!!CODE SPAWN!!! @" + var_6.origin );
    ...
    if ( isdefined( var_0 ) && var_0 == 99 )
        ...
        var_15 = scripts\engine\utility::_id_13142( 2, "versus_menu_done", "prematch_over" );
...
onspawnspectator( var_0, var_1, var_2 )
{
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
    {
        ...
        scripts\mp\utility\player::_id_11965( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_0 );
        return;
    }
    ...
    if ( scripts\mp\utility\game::relic_amped_test_explode_other_player() && !self.hasspawned )
        self predictstreampos( var_3.origin );

    self spawn( var_3.origin, var_3.angles );
    scripts\mp\utility\player::_id_11965( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_3.origin );
}
...
getspectatepoint()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "brtdm" )
        return level.create_script_wait_for_flags._id_125CF;
...
spawnintermission( var_0, var_1, var_2 )
{
    ...
    if ( !isdefined( level.custom_ending ) )
    {
        ...
        scripts\mp\utility\player::_id_11965( "playerlogic::spawnIntermission() !!!CODE SPAWN!!! @" + var_0.origin );
        scripts\mp\utility\player::setdof_spectator();
    }
...
spawnendofgame()
{
    if ( isdefined( level.spawner_recently_used ) && self [[ level.spawner_recently_used ]]() )
        return;
...
callback_playerdisconnect( var_0 )
{
    ...
    self.handleplunderendgamesplash = 1;

    if ( scripts\mp\utility\game::getgametype() == "br" && self isinexecutionvictim() && isalive( self ) )
    {
        var_1 = self getexecutionpartner();

        if ( !isbot( self ) )
        {
            var_2 = self.health;

            if ( scripts\mp\damage::_id_1229C( self ) )
            {
                var_1 scripts\mp\utility\stats::incpersstat( "damage", var_2 );

                if ( !isdefined( var_1.get_player_aggro_score ) )
                    var_1.get_player_aggro_score = var_2;
                else
                    var_1.get_player_aggro_score = var_1.get_player_aggro_score + var_2;
            }
            else if ( isdefined( level.toggle_apc_objective ) )
                var_1 [[ level.toggle_apc_objective ]]( var_2 );
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        if ( scripts\mp\utility\game::namehud() == "br" )
        {
            var_5 = [];
            ...
            var_9 = var_5.size + 1;
            scripts\cp_mp\utility\game_utility::_id_1212A( var_9 );
        }
    }
    ...
    scripts\cp_mp\utility\player_utility::_id_11C80();
    ...
    else if ( self.sessionstate != "playing" && scripts\mp\utility\game::getgametype() == "br" && ( scripts\mp\gametypes\br_public::iswaitingtoentergulag( self ) || scripts\mp\gametypes\br_public::relic_mythic_should_do_pain( self ) || scripts\mp\gametypes\br_public::removechevronsfromarray( self ) ) )
        removefromalivecount( 1 );
    ...
    if ( isdefined( self.team ) )
        scripts\mp\utility\teams::_id_12E92( "disconnect", self.team, self );
    ...
    scripts\common\utility::_id_12C2C( level.smodelcollections, var_0 );
...
endemptygamewatcher()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_data_force_send_matchdata_for_no_players_left", 1 ) == 1 && istrue( level.br_prematchstarted ) )
        scripts\mp\gamelogic::_id_12003();
...
recordplayersegmentdata()
{
    ...
    for (;;)
    {
        ...
        {
            if ( istrue( var_1.destroy_intro_tank ) )
...
callback_playerconnect()
{
    ...
    if ( isdefined( level.track_get_teleport_target ) )
        var_0 = self thread [[ level.track_get_teleport_target ]]();
    ...
    if ( level.uniqueplayersconnected <= monitor_balloons() )
    {
    ...
    self.perkpackage_checkifready_callback = undefined;
    self.periodic_sound_events = undefined;
    self.lifeid = 0;
    ...
    scripts\cp_mp\utility\player_utility::animscripted_single_arrive_at();
    ...
    thread _hidesafecircleui::buildweaponfromrandomcategory( self );
    ...
    if ( !isdefined( self.pers["team"] ) )
    {
        ...
        else if ( ( scripts\mp\utility\game::matchmakinggame() || scripts\mp\utility\game::lobbyteamselectenabled() || isgamebattlematch() ) && self.sessionteam != "none" )
        {
            ...
            if ( ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) && getdvarint( "scr_br_manual_team_assignment", 0 ) == 1 )
                thread scripts\mp\teams::unset_relic_focus_fire();
            else
            {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::namehud() != "dmz" && scripts\mp\utility\game::namehud() != "risk" && scripts\mp\utility\game::namehud() != "rumble" && scripts\mp\utility\game::namehud() != "sandbox" )
                {
                    ...
                    scripts\mp\utility\script::getweaponrandomvariantid( "Callback_PlayerConnect() elseif MMG " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
        ...
        else
        {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                ...
                {
                    var_14 = isdefined( level.allowclasschoicefunc ) && istrue( self [[ level.allowclasschoicefunc ]]() );
                    scripts\mp\utility\script::getweaponrandomvariantid( "Callback_PlayerConnect() else " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
    ...
    else
    {
        ...
        if ( isdefined( level.track_get_teleport_target ) )
            self thread [[ level.track_get_teleport_target ]]();
...
kickifdontspawn()
{
    ...
    if ( istrue( self._id_12546 ) && !mayspawn() )
        return;
...
totaldisttracking( var_0 )
{
    ...
    for (;;)
    {
        var_4 = scripts\engine\utility::_id_13145( 5.0, "death", "vehicle_enter", "vehicle_change_seat", "vehicle_exit", "afk_tracking_resume" );

        if ( var_4 == "vehicle_exit" && !isdefined( self.lastvehicleseatchangetime ) )
            continue;

        if ( istrue( self.playerplunderpickupcallback ) )
            continue;
        ...
        {
            var_8 = scripts\mp\utility\game::namehud() != "br";
...
initleanplayerstats()
{
    ...
    scripts\mp\utility\stats::proplockunlock( "deaths" );
    ...
    scripts\mp\utility\stats::proplockunlock( "score" );
    ...
    scripts\mp\utility\stats::proplockunlock( "kills" );
}
...
initplayerstats()
{
    ...
    if ( !isdefined( self.send_all_ai_to_players ) )
        self.send_all_ai_to_players = [];
    ...
    scripts\mp\utility\stats::proplockunlock( "suicides" );

    if ( !isdefined( self.pers["headshots"] ) )
        scripts\mp\utility\stats::initpersstat( "headshots" );

    scripts\mp\utility\stats::proplockunlock( "headshots" );
    ...
    scripts\mp\utility\stats::proplockunlock( "assists" );
    ...
    scripts\mp\utility\stats::proplockunlock( "captures" );
    ...
    scripts\mp\utility\stats::proplockunlock( "returns" );
    ...
    scripts\mp\utility\stats::proplockunlock( "defends" );
    ...
    scripts\mp\utility\stats::proplockunlock( "plants" );
    ...
    scripts\mp\utility\stats::proplockunlock( "defuses" );
    ...
    scripts\mp\utility\stats::proplockunlock( "destructions" );
    ...
    scripts\mp\utility\stats::proplockunlock( "confirmed" );
    ...
    scripts\mp\utility\stats::proplockunlock( "denied" );
    ...
    scripts\mp\utility\stats::proplockunlock( "rescues" );
    ...
    scripts\mp\utility\stats::proplockunlock( "touchdowns" );
    ...
    scripts\mp\utility\stats::proplockunlock( "fieldgoals" );
    ...
    scripts\mp\utility\stats::proplockunlock( "killChains" );
    ...
    scripts\mp\utility\stats::proplockunlock( "killsAsSurvivor" );
    ...
    scripts\mp\utility\stats::proplockunlock( "stabs" );
    ...
    scripts\mp\utility\stats::proplockunlock( "setbacks" );
    ...
    scripts\mp\utility\stats::proplockunlock( "objTime" );
    ...
    scripts\mp\utility\stats::proplockunlock( "gamemodeScore" );
    ...
    scripts\mp\utility\stats::proplockunlock( "damage" );

    if ( !isdefined( self.pers["supersEarned"] ) )
        scripts\mp\utility\stats::initpersstat( "supersEarned" );

    scripts\mp\utility\stats::proplockunlock( "supersEarned" );
    ...
    scripts\mp\utility\stats::proplockunlock( "downs" );
    ...
    scripts\mp\utility\stats::proplockunlock( "killStreak" );
    scripts\mp\utility\stats::proplockunlock( "loss" );
    scripts\mp\utility\stats::proplockunlock( "win" );
}
...
removefromalivecount( var_0 )
{
    ...
    var_2 = scripts\mp\utility\game::getgametypenumlives() != 0 && var_1 == 0 && !istrue( level._id_1233D );
...
updateplayerwindmaterial()
{
    ...
    for (;;)
    {
        foreach ( var_6 in level.players )
        {
            ...
            if ( !isdefined( var_6._id_13308 ) || var_6._id_13308 != var_8 )
            {
                switch ( var_8 )
                {
                    case 0:
...
mine_caves_structs()
{
    return istrue( self.beingrevived ) || istrue( self.removematchingents_byclassname );
}
